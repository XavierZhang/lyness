...
native/      @lyness/node-addon-system source of record (see native/README.md)
...
- Every npm package is `@lyness/lyn-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@lyness/cordis` is a peerDependency (+ dev) of every harness package.
