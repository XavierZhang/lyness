...
**Remove at the first tagged release.** Until then, prefer correct foundations to compatibility shims: rename or repackage freely and update every reference. Backends reject old on-disk formats. SQLite uses monotonic `SCHEMA_VERSION`; `lyn-session` keeps `SESSION_FORMAT_VERSION` at `0` with no compatibility promise.
...
packages/    @lyness/<pkg> workspaces at packages/<group>/<pkg>/
  ...
  bundle/      installable lyn --profile patch-layer bundles
...
native/      @lyness/node-addon-landlock-run source of record (see native/README.md)
...
Run checks before pushes via [lyn-pre-push-checks](.agents/skills/lyn-pre-push-checks/SKILL.md); report only commands run. After `gh stack sync`, validate immediately; do not merge before checks pass.
...
- Every npm package is `@lyness/<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@lyness/cordis` is a peerDependency (+ dev) of every harness package.
