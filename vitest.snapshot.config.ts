...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'LYNESS_SNAPSHOT_MAX_CONCURRENCY',
  Math.min(DEFAULT_SNAPSHOT_MAX_CONCURRENCY, availableParallelism()),
)

const snapshotMaxWorkers = process.env.LYNESS_SNAPSHOT_MAX_WORKERS
  ? positiveIntFromEnv('LYNESS_SNAPSHOT_MAX_WORKERS', availableParallelism())
  : undefined
...
if (process.env.LYNESS_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.LYNESS_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.LYNESS_SNAPSHOT || 'replay') === 'replay'
