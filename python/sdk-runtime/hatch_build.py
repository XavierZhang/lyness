...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported lyness-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "lyness-runtime-bin is wheel-only; build and publish platform wheels only."
            )

        platform_tag = os.environ.get("LYNESS_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
