...
The `advanced` and `restart` comparisons match native delivery qualifiers against each input's own Session generation, including SDK events and notifications. Captured and other delivery generations retain their numeric values. Each side must use one generation across its Session roles, and fresh logs must identify the current writer. Comparison never rewrites committed recordings.
...
Alternatively export a non-empty `LYNESS_HOME`. The SDK rejects a launch that would silently use `~/.lyn`.

## Run against Node source

Repository contributors can select either development route; both execute the normal `lyn --profile sdk` launcher:

- Set `LYNESS_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
- Set `lyn_bin` to the absolute built `apps/cli/lib/bin.js` path to exercise the checkout's CLI directly. Supply an explicit `lyn_home`, plus `profile` and ordered `patches` as needed.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `lyness-runtime-bin` version.
