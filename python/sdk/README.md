...
`profile` may select another existing profile, but that composition must retain `@lyness/sdk-app` or another `@lyness/sdk-jsonrpc-server` row. Misconfiguration fails during CLI boot or SDK initialization; there is no complete-config fallback. `lyn_bin` may select another `lyn` executable while preserving the same profile grammar. Arbitrary argv replacement remains an internal fake-runtime test adapter, not public API.
