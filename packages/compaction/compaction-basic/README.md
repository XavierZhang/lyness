...
All settings are optional. With context window `W`, effective request output cap `O`, and headroom `B`, the default trigger is `floor(min(W × 0.8, W − O − B))`, where `B = 65,536` tokens. Retention keeps the newest 16% of `W − O` verbatim. The table below lists every setting; the generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-compaction-basic) also includes their types.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-compaction-basic) — every accepted config field and its source declaration.
