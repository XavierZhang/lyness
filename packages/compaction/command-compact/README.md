...
Type `/compact` in a chat UI when the conversation has grown long and you want to condense it immediately. The shipped `lyn` base mounts the command next to the default backend, so it is usually already available.
