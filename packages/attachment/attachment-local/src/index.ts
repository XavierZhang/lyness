...
import { lynCachePath, resolveLynHome } from '@lyness/lyn-home-paths'
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    const lynHome = resolveLynHome(config.lynHome)
    this.root = join(lynHome, 'attachments', 'v1')
    this.cacheRoot = lynCachePath({ lynHome }, 'attachments')
