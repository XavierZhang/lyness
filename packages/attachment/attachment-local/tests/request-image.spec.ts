...
async function home(): Promise<string> {
  const lynHome = await mkdtemp(join(tmpdir(), 'lyn-request-image-'))
  homes.push(lynHome)
  return lynHome
}

async function store(): Promise<LocalAttachmentStore> {
  return new LocalAttachmentStore(new Context(), { lynHome: await home() })
}
...
describe('local request-image cache', () => {
  it('rebuilds a cleared cache without moving or losing durable attachments', async () => {
    ...
    vi.stubEnv('LYNESS_HOME', fallbackHome)
    try {
      const lynHome = await home()
      const attachments = new LocalAttachmentStore(new Context(), { lynHome })
      ...
      const cacheRoot = join(lynHome, 'cache')
      const path = join(cacheRoot, 'attachments', 'request-images', hash.slice(0, 2), hash)

      expect(attachments.root).toBe(join(lynHome, 'attachments', 'v1'))
      ...
      const reopened = new LocalAttachmentStore(new Context(), { lynHome })
  ...
  it('regenerates invalid, oversized, incompatible, or mismatched cached variants', async () => {
    ...
    const initial = await attachments.readImageRequest(attachment, policy)
    const hash = String(initial.variantId).slice('sha256:'.length)
    const path = join(lynHome, 'cache', 'attachments', 'request-images', hash.slice(0, 2), hash)
