...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const lynHome = await mkdtemp(join(tmpdir(), 'lyn-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { lynHome })
      ...
      await rm(lynHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const lynHome = await mkdtemp(join(tmpdir(), 'lyn-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { lynHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const lynHome = await mkdtemp(join(tmpdir(), 'lyn-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { lynHome })
  ...
  it('prepares every batch member before any write', async () => {
    const lynHome = await mkdtemp(join(tmpdir(), 'lyn-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { lynHome })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const lynHome = await mkdtemp(join(tmpdir(), 'lyn-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { lynHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { lynHome, maxImageBytes: 1 })
