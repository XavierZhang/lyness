...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-api-job-controller) is the exhaustive source for accepted fields and their JSDoc.
...
None, as job observation is browser and Host control state; it registers no prompt, tool, or session event. The model's own view of the same work stays with [`lyn-tool-jobs`](../../jobs/tool-jobs/README.md).
...
**Runtime invariant:** No companion is published. The controller is a stateless projection of `ctx.jobs` reads; the registry's own `@lyness/lyn-jobs/invariant` owns the event protocol and event-versus-read relations these streams forward.
