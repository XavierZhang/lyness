 ...
 * `lyn-resource://file/session/<sessionId>/<path>`, carries an absolute path or one
 ...
 * An `absolute` address, `lyn-resource://file/absolute/<path>`, carries no
...
function unsupportedAddress(address: string): RemoteError<'workspace-file/unsupported-address'> {
  return new RemoteError(
    ...
    `${address} is not a lyn-resource://file/session/<sessionId>/<path> or lyn-resource://file/absolute/<path> address`,
    { address },
  )
}
...
function unknownWorkspace(address: string): RemoteError<'workspace-file/unknown-workspace'> {
  return new RemoteError(
    ...
    `${address} requires a lyn-resource://file/session/<sessionId>/<path> address`,
    { address },
  )
}
