...
import { RemoteError, typertOwnedValue } from '@lyness/lyn-typert-protocol'
...
describe('Client Remote transport readiness', () => {
  ...
  it.each([false, true])('replaces a stalled carrier and restores events (socket opened: %s)', async (autoOpen) => {
    await withFakeWebSocket('https://harness.example', async () => {
      vi.useFakeTimers()
      vi.stubGlobal('__LYNESS_CONNECTION_RECOVERY__', {
...
describe('Client Typert API', () => {
  ...
  it('publishes injected Host facts after Remote events report ready', async () => {
    ...
    const transportDescriptor = Object.getOwnPropertyDescriptor(globalThis, '__LYNESS_TRANSPORT__')
    const mock = RemoteMock.create({ host: { home: '/home/mock' } })
    Object.defineProperty(globalThis, 'location', {
      configurable: true,
      value: { hostname: '127.0.0.1', search: '' },
    })
    Object.defineProperty(globalThis, '__LYNESS_TRANSPORT__', {
      configurable: true,
    ...
    try {
      ...
      if (transportDescriptor === undefined) Reflect.deleteProperty(globalThis, '__LYNESS_TRANSPORT__')
      else Object.defineProperty(globalThis, '__LYNESS_TRANSPORT__', transportDescriptor)
    }
  })
...
describe('Remote stream client carrier lifecycle', () => {
  ...
  it('connects to the shell-owned Host while the document uses a local asset origin', async () => {
    await withFakeWebSocket('lyn-app://app', async () => {
      vi.stubGlobal('__LYNESS_TRANSPORT__', { streamBaseUrl: 'http://127.0.0.1:43210' })
      const client = new RemoteStreamMuxClient()
      try {
        client.start()
