...
import type { SessionSeqCursor } from '@lyness/lyn-session/types'
...
// (`/types`, zero imports), never the package root: the root's lyn-agent →
// lyn-session chain would drag the host `Context.sessions` merge into the
...
export type { SessionProjectionMap } from '@lyness/lyn-session-projection/types'
