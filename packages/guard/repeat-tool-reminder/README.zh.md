...
本包帮助模型跳出以相同参数反复调用同一工具却没有进展的循环。达到配置的重复次数时，它会要求模型检查上一次结果并改变方法或结束任务。提醒只是建议，绝不会阻止或延迟合理的重复调用。每个 agent 的重复分别跟踪，新的用户消息会清除计数。`lyn` 基础组合包默认启用本包，并在重复 3、5、8 次时提醒。
...
当模型应当自行发现自己在相同工具调用上循环时，挂载此插件。无需学习或接线：`lyn` 基础组合包已经运行它，默认值适用于大多数会话——想更早、更晚或在更少的工具上收到提醒时，调优下面的阈值与工具范围即可。
...
无效配置会在启动时以清晰错误失败——空的 `thresholds` 列表、小于 2 的重复次数或重复值——绝不会静默改变行为。生成的[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-repeat-tool-reminder)记录每个受支持的值。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#lynesslyn-repeat-tool-reminder)——每个受支持配置字段及其源声明。
