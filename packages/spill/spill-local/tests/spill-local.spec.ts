...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'lyn-spill-test-'))
})
afterEach(() => {
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'lyn-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'lyn-spill-'))
  ...
  it('covers the configured root AND discovered default roots (real gatherRoots)', async () => {
    ...
    const fakeTmp = mkdtempSync(join(tmpdir(), 'lyn-faketmp-'))
...
describe('discoverDefaultRoots', () => {
  it('returns only real lyn-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'lyn-disc-'))
    try {
      // A real backend-shaped root (lyn-spill-<6>) via mkdtemp — the only match.
