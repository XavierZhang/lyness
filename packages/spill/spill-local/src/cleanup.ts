...
/**
 * A backend-generated default root name: `lyn-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `lyn-spill-test-*` fixture
 * or a foreign tool's differently-shaped `lyn-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `lyn-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `lyn-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `lyn-spill-test-*` fixture or a foreign
