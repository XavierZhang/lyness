...
Mount this backend in a composition that spills text to the local filesystem. It registers as the `ctx.spillStore` service that the `lyn-spill-policy` plugin and other callers use.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-spill-local) is the exhaustive source for every accepted field.
