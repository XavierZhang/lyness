...
import type { ContentBlock, GenerateOptions, ImageBlock, StreamChunk, UserMessage } from '@lyness/lyn-llm'
import { deepSeekImageRequestPricing, resolveAdapterOptions } from '@lyness/lyn-llm-deepseek'
import { prepareImages } from '@lyness/lyn-llm-deepseek/src/images.ts'
import { serialize } from '@lyness/lyn-llm-deepseek/src/serialize.ts'
import { createMcpToolDefinition } from '@lyness/lyn-mcp-client'
import NodeRuntime from '@lyness/lyn-ptc-runtime-node'
...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), 'lyn-image-recovery-'))
  ...
  await ctx.plugin(LocalAttachments, { lynHome: join(root, 'home') })
