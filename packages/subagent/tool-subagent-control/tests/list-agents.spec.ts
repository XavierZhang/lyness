...
import type { SubagentCatalogEntry } from '@lyness/lyn-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'lyn-tool-list-agents-'))
  roots.push(root)
...
describe('lyn-tool-subagent-control/list-agents', () => {
