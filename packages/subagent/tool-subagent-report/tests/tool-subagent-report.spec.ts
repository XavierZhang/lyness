...
import * as control from '@lyness/tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'lyn-tool-subagent-report-'))
...
describe('lyn-tool-subagent-report', () => {
...
describe('lyn-tool-subagent-report result independence', () => {
