...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesssubagent-acp) is the exhaustive source for every accepted field and its JSDoc.

A lyness child uses the product launcher and an explicit absolute `LYNESS_HOME`. The isolated home prevents a nested runtime from discovering the launching person's profiles or credentials; the generic ACP provider does not impose this requirement on non-LYN agents.

```yaml
- id: subagent-acp
  name: '@lyness/subagent-acp'
  config:
    providerName: acp
    command: lyn
    ...
    permission: reject
    env:
      LYNESS_HOME: /absolute/path/to/isolated-child-home
...
- [lyn-subprocess seam](../../subprocess/subprocess/README.md) — the process-spawn and teardown machinery behind each run.
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesssubagent-acp) — every accepted config field and its source declaration.
