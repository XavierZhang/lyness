...
export interface AcpRunSpec {
   ...
   * Explicit `LYNESS_*` entries are deployment-owned facts for the child harness
   * (e.g. `LYNESS_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and LYNESS_* names
  ...
  const clientApp = createAcpClientApp({ name: 'lyness-subagent-acp' })
