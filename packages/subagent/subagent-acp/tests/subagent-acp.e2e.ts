...
/**
 * With-key cross-process boundary proof: the backend spawns the real lyn ACP profile, speaks ACP over
...
// The real ACP profile: lyn plus the example's live DeepSeek patch.
...
// How to launch the child ACP profile (src via tsx / lib via plain node, per LYNESS_EXAMPLE_MODE).
...
function resolveChildLaunch(lynHome: string) {
