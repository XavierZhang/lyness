...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-subagent-acp)是每个受支持字段及其 JSDoc 的穷尽式真源。

lyness 子进程使用产品启动器和一个显式的绝对路径 `LYNESS_HOME`。隔离的 home 可防止嵌套运行时发现启动者个人的 profile 或凭据；通用 ACP 提供方不会把这一要求强加给非 LYN agent。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#lynesslyn-subagent-acp)——每个受支持配置字段及其源声明。
