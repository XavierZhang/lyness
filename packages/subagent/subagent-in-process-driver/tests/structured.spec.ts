...
import { createUserMessage, ToolCallId, type ContentBlock, type GenerateOptions } from '@lyness/lyn-llm'
...
import type { Config as ToolConfig, ObjectJsonSchema } from '@lyness/lyn-tools'
import { defineContentToolFixture, RUN_CODE_NAME } from '@lyness/lyn-tools'
...
async function setup(script: Script, options: SetupOptions = {}) {
  ...
  if (options.toolMode === 'ptc' || options.toolMode === 'both') {
    ctx.provide('ptcRuntime', {
      language: 'typescript',
      isolation: 'test',
      resolve: (request: import('@lyness/lyn-ptc-runtime').PtcRunRequest) => ({ ...request, cwd: request.cwd ?? process.cwd(), timeoutMs: 120_000 }),
