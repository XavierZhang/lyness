...
Mount this backend when delegated work must build on the parent's conversation. The common path mirrors spawn: load the subagent service and this backend, then point a delegation tool such as `lyn-tool-subagent` at the `fork` provider.
...
- name: '@lyness/lyn-subagent-fork-in-process'
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-subagent-fork-in-process) is the exhaustive source for every accepted field and its JSDoc.
...
- [lyn-subagent-spawn-in-process](../subagent-spawn-in-process/README.md) — the fresh-child sibling backend.
- [lyn-tool-subagent](../tool-subagent/README.md) — the model-facing delegation tool that reaches this provider.
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-subagent-fork-in-process) — every accepted config field and its source declaration.
...
The parent receives only the child's own final output through `lyn-tool-subagent`, not the inherited prefix or intermediate work.
