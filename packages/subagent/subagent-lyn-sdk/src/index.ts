...
export const name = 'subagent-lyn-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('lyn-sdk'),
  lynBin: z.string(),
  profile: z.string().default('sdk'),
  patches: z.array(z.string()).default([]),
  lynHome: z.string().required(),
  cwd: z.string(),
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'lynBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'lynBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-lyn-sdk ${field} must name an existing file: ${path}`)
}

/** LYN SDK can apply Agent route options while the other start features remain child-owned. */
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-lyn-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-lyn-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.lynBin === undefined ? {} : { lynBin: this.config.lynBin },
      profile: this.config.profile,
      patches: this.config.patches,
      lynHome: this.config.lynHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-lyn-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-lyn-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-lyn-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-lyn-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-lyn-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.lynHome)) throw new TypeError('subagent-lyn-sdk lynHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.lynBin === undefined ? {} : { lynBin: resolveConfiguredFile('lynBin', resolved.lynBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-lyn-sdk', resolved.cwd)
