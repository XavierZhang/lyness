...
export interface SdkRunSpec {
  /** Explicit lyn CLI module; omission resolves the SDK client's same-version dependency. */
  lynBin?: string
  /** Named child profile. */
  profile: string
   ...
   * current `LYNESS_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-lyn-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: LynessOptions): Lyness } = {
  createHarness: options => new Lyness(options),
}
 ...
 * @returns an Error whose message contains only the fixed LYN SDK failure line.
 */
