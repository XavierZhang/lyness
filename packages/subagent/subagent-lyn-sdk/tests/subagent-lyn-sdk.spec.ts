...
let createdHarnessOptions: LynessOptions[] = []
...
beforeEach(() => {
  ...
  runInternals.createHarness = (options) => {
    ...
    return createProcessLyness(runtime, options)
  }
})
...
function expectedFailure(fields: string): string {
  return `Subagent failure (provider: LYN SDK; ${fields})`
}
...
describe('lyn-subagent-lyn-sdk provider', () => {
  ...
  it('runs a child turn end to end with a parent-unique run id', async () => {
    ...
    const run = await ctx.subagents.start('lyn-sdk', request('do X'))
    ...
    const nextRun = await ctx.subagents.start('lyn-sdk', request('again'))
  ...
  it('resolves relative launch files at load and forwards absolute paths', async () => {
    const ctx = await setup({ FAKE_TEXT: 'explicit lyn child' }, {
      lynBin: relative(process.cwd(), fakeRuntime),
      patches: [relative(process.cwd(), existingPatch)],
    })
    const run = await ctx.subagents.start('lyn-sdk', request())
    expect(text((await run.result).output)).toBe('explicit lyn child')
  ...
  it('initializes the child with the configured provider/model/maxTokens and the parent cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-init-'))
  ...
  it('preserves instance defaults around a partial request override', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-partial-route-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const run = await ctx.subagents.start('lyn-sdk', request('partial', new AbortController().signal, {
  ...
  it('isolates complete per-run route overrides on concurrent children', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-routes-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const runs = await Promise.all([
        ctx.subagents.start('lyn-sdk', request('first', new AbortController().signal, {
          provider: 'provider-a',
          model: 'model-a',
          reasoningEffort: ReasoningEffortId('high'),
          maxTokens: 111,
        })),
        ctx.subagents.start('lyn-sdk', request('second', new AbortController().signal, {
          provider: 'provider-b',
          model: 'model-b',
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.LYNESS_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'LYNESS_TEST_AMBIENT_SECRET_KEY,DEEPSEEK_API_KEY',
      ...
      expect(answer).toContain('LYNESS_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('DEEPSEEK_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.LYNESS_TEST_AMBIENT_SECRET_KEY
    }
  })
  ...
  it('aggregates safe initialize and shutdown facts when startup rollback fails', async () => {
    ...
    try {
      ...
      const error = await ctx.subagents.start('lyn-sdk', request()).catch((cause: unknown) => cause)
      expect(error).toBeInstanceOf(AggregateError)
      expect((error as Error).message).toBe(
        `subagent-lyn-sdk: ${expectedFailure('stage: initialize; category: protocol')}; `
        + `subagent-lyn-sdk: ${expectedFailure('stage: shutdown; category: unknown')}`,
      )
  ...
  it('reports only safe shutdown facts when cancelled startup rollback fails', async () => {
    ...
    const spy = vi.spyOn(Lyness.prototype, 'close').mockImplementation(async function (this: Lyness) {
      spy.mockRestore()
      await this.close()
      throw new Error(rawCleanup)
    })
  ...
  it('keeps an initialize failure authoritative when a later abort flag is already set', async () => {
    ...
    const start = vi.spyOn(Lyness.prototype, 'start').mockRejectedValue(rawFailure)
    const close = vi.spyOn(Lyness.prototype, 'close').mockResolvedValue()
    try {
      const controller = new AbortController()
      const pending = startSdkRun(request('p', controller.signal), {
        profile: 'sdk',
  ...
  it('aborting the required signal settles a hung child as aborted', async () => {
    ...
    const controller = new AbortController()
    const run = await ctx.subagents.start('lyn-sdk', request('p', controller.signal))
  ...
  it('cancelling between handshake and publish rejects start after reap', async () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-midcancel-'))
  ...
  it('rejects WITHOUT spawning when the signal is already aborted', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-preabort-'))
  ...
  it('rejects a pre-aborted request through the registered provider before cwd resolution', async () => {
    ...
    await expect(ctx.subagents.start('lyn-sdk', {
      label: 'p',
  ...
  it('rejects after reaping when the child dies before the handshake', async () => {
    ...
    const failure = await ctx.subagents.start('lyn-sdk', request()).then(
      () => { throw new Error('start unexpectedly succeeded') },
      (error: unknown) => error,
    )
    expect(String(failure)).toBe(
      `SdkRunFailure: subagent-lyn-sdk: ${expectedFailure('stage: initialize; category: transport')}`,
    )
  ...
  it.each([
    ...
    await expect(ctx.subagents.start('lyn-sdk', request())).rejects.toThrow(
      `subagent-lyn-sdk: ${expectedFailure(`stage: initialize; category: ${category}`)}`,
    )
    await ctx.fiber.dispose()
  })
  ...
  it('routes provider-level onError through ctx.logger.warn', async () => {
    ...
    expect(warnings[0]).toContain('subagent-lyn-sdk "lyn-sdk": child run failed (error)')
    await run.dispose()
    await ctx.fiber.dispose()
  })
  ...
  it('rejects non-positive timing bounds at load', async () => {
    const ctx = new Context()
    await ctx.plugin(SubagentRuntime)
    const base = { providerName: 'sdk', profile: 'sdk', patches: [], lynHome: process.cwd(), provider: 'p', model: 'm', env: {} }
  ...
  it('requires an explicit absolute Harness home for nested lyn runtimes', async () => {
    const ctx = new Context()
    await ctx.plugin(SubagentRuntime)
    await expect(ctx.plugin(sdk, {
      ...
      profile: 'sdk',
      patches: [],
      lynHome: './personal-home',
      provider: 'p',
      model: 'm',
      env: {},
    })).rejects.toThrow('lynHome must be an absolute path')
    await ctx.fiber.dispose()
  })

  it.each([
    { field: 'lynBin', override: { lynBin: './missing-lyn-bin' } },
    { field: 'lynBin', override: { lynBin: '.' } },
  ...
  it('uses a validated config cwd override instead of the parent session cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-lyn-sdk-cwd-'))
    try {
  ...
  it('fails loud when neither config cwd nor parent session cwd exists', async () => {
    ...
    await expect(ctx.subagents.start('lyn-sdk', {
      ...
      .rejects.toThrow(
        `subagent-lyn-sdk: ${expectedFailure('stage: initialize; category: configuration')}`,
      )
    await ctx.fiber.dispose()
  })

  it('keeps named plugin exports with no default export (loader shape)', () => {
    expect(sdk.name).toBe('subagent-lyn-sdk')
