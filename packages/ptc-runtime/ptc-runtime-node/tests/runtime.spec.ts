...
import type { PtcBindingFunction, PtcBindingNamespace, PtcRunRequest } from '@lyness/lyn-ptc-runtime'
...
async function setup(config: Config = {}, mode: 'read-only' | 'workspace-write' | 'danger-full-access' = 'danger-full-access') {
  const root = await mkdtemp(join(homedir(), '.lyn-node-runtime-test-'))
  ...
  it.skipIf(process.platform !== 'win32' || !sandboxUsable)('uses the common Windows grant lock and private native temp without exposing ambient values', async () => {
    ...
    const result = await run({
      ...
      bindings: bindings({ inspect: async (args) => {
        ...
        expect(path.startsWith(`${temp}\\lyn-`)).toBe(true)
    ...
    expect(value.env).toEqual([])
    expect(value.native.env).not.toContain('LYNESS_TEST_RUNTIME_SECRET')
    expect(value.native.temp.startsWith(`${temp}\\lyn-`)).toBe(true)
    expect(value.observed).toBe('native-temp')
    expect((await readdir(join(tmp, 'lyn-acl-locks'))).some(name => name.endsWith('.lock'))).toBe(true)
  })
