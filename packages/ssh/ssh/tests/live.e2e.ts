...
import * as LspStdio from '@lyness/lyn-lsp-stdio'
import { SessionProjectionRegistry } from '@lyness/lyn-session-projection'
import { SandboxPolicyService } from '@lyness/lyn-sandbox-policy'
import { SshFileSystem } from '@lyness/lyn-fs-ssh'
import { SshSubprocessRuntime } from '@lyness/lyn-subprocess-ssh'
import { SshSandboxProvider } from '@lyness/lyn-sandbox-ssh'
...
const configPath = process.env.LYNESS_SSH_TEST_CONFIG
const bootstrap = process.env.LYNESS_SSH_TEST_BOOTSTRAP
const languageServer = process.env.LYNESS_SSH_TEST_LSP
...
describe.skipIf(!enabled)('POSIX SSH runtime acceptance', () => {
  ...
  it('preserves binary fd 7 through native ownership and read-only confinement', async () => {
    ...
    try {
      ...
      if (test.hello.platform === 'linux') {
        ...
        expect(facts.cgroup).toMatch(/lyn-subprocess-[^\n]+\.scope/)
      }
    } finally { await removeOwned(test) }
  }, 60_000)
  ...
  it('reports unknown outcomes on disconnect and stops the remote managed descendants without replay', async () => {
    ...
    try {
      ...
      expect(scope).toMatch(/lyn-subprocess-[^\n]+\.scope$/)
