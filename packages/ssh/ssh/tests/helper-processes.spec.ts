...
import type { SubprocessHandle, SubprocessOutcome, SubprocessTerminalHandle } from '@lyness/lyn-subprocess'
...
async function harness() {
  const root = await mkdtemp('/tmp/lyn-ssh-owner-')
...
describe.skipIf(process.platform === 'win32')('SSH helper process settlement', () => {
  ...
  it('rolls back a listener path that exceeds the operating system socket limit', async () => {
    const root = await mkdtemp('/tmp/lyn-ssh-long-')
