...
import type { SubprocessHandle, SubprocessSpawnSpec, SubprocessTerminalHandle } from '@lyness/lyn-subprocess'
import { logSpillFailure, OutputCollector, prepareManagedProcessBinding, type SpillFailureReporter } from '@lyness/lyn-subprocess-local/output'
...
export class RemoteProcesses {
  ...
  private async endpoint(path: string): Promise<Endpoint> {
    ...
    const server = createServer({
      ...
      pskCallback: (_socket, identity) => identity === 'lyn-stream' ? capability : null,
    })
