...
`lyn-subprocess-e2b` runs the agent's shell commands and interactive terminals inside an E2B remote sandbox instead of the host. Existing command, terminal, and language-server workflows continue without E2B-specific tools. Host environment variables and secrets are excluded; only explicitly requested environment entries enter the sandbox. Use it with `lyn-e2b` and `lyn-fs-e2b` so commands, terminals, and files share one sandbox. Remote execution adds latency because each command requires asynchronous setup.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-subprocess-e2b) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-subprocess-e2b) — every accepted config field and its source declaration.
