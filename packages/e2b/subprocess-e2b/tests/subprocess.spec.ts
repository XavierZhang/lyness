...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0LYNESS_STALE=old\0BROKEN\0=bad\0'
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.lyn-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.lyn-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$lyn_e2b_env_bin" -i'))!
    expect(command).toContain('"$lyn_e2b_setsid" --wait -- "$lyn_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('LYNESS_MODE')
    ...
    expect(command).not.toContain('${!lyn_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$lyn_e2b_home" -a -d "$lyn_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('lyn_e2b_node="$(command -v node)"')
    expect(command).toContain('"$lyn_e2b_env_bin" -i "$lyn_e2b_node" -e')
    expect(command).toContain('"$lyn_e2b_env_bin" -i -- "${lyn_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$lyn_e2b_env_bin" -i -- "${lyn_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.lyn-e2b/processes/one/pid',
      '/workspace/.lyn-e2b/processes/one/exit-code',
      '/workspace/.lyn-e2b/processes/one/environment',
      '/workspace/.lyn-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.lyn-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0LYNESS_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.lyn-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('lyn_e2b_tee='))!
    expect(command).toContain('"$lyn_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$lyn_e2b_tee" --output-error=warn-nopipe')
