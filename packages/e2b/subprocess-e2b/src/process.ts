...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$lyn_e2b_env_bin" -i "$lyn_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$lyn_e2b_tee" --output-error=warn-nopipe >("$lyn_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$lyn_e2b_tee" --output-error=warn-nopipe >("$lyn_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'lyn_e2b_rm=$7',
    'shift 7',
    'lyn_e2b_pgid="$("$lyn_e2b_ps" -o pgid= -p "$$" | "$lyn_e2b_tr" -d " ")"',
    `printf '%s\\n' "$lyn_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t lyn_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$lyn_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$lyn_e2b_env_bin" -i -- "\${lyn_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'lyn_e2b_status=$?',
    `printf '%s\\n' "$lyn_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$lyn_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t lyn_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'lyn_e2b_env_bin="$(command -v env)"',
    'lyn_e2b_setsid="$(command -v setsid)"',
    'lyn_e2b_bash="$(command -v bash)"',
    'lyn_e2b_node="$(command -v node)"',
    'lyn_e2b_ps="$(command -v ps)"',
    'lyn_e2b_tr="$(command -v tr)"',
    'lyn_e2b_tee="$(command -v tee)"',
    'lyn_e2b_head="$(command -v head)"',
    'lyn_e2b_rm="$(command -v rm)"',
    'for lyn_e2b_tool in "$lyn_e2b_env_bin" "$lyn_e2b_setsid" "$lyn_e2b_bash" "$lyn_e2b_node" "$lyn_e2b_ps" "$lyn_e2b_tr" "$lyn_e2b_tee" "$lyn_e2b_head" "$lyn_e2b_rm"; do',
    '  [[ "$lyn_e2b_tool" == /* && -x "$lyn_e2b_tool" ]] || exit 125',
    'done',
    `exec "$lyn_e2b_env_bin" -i -- "\${lyn_e2b_env[@]}" "$lyn_e2b_setsid" --wait -- "$lyn_e2b_bash" -c ${quoteE2BShellArg(inner)} lyn-e2b "$lyn_e2b_env_bin" "$lyn_e2b_node" "$lyn_e2b_ps" "$lyn_e2b_tr" "$lyn_e2b_tee" "$lyn_e2b_head" "$lyn_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
