...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'lyn_state=$1',
  'mapfile -d \'\' -t lyn_env < "$lyn_state/environment"',
  'mapfile -d \'\' -t lyn_argv < "$lyn_state/argv"',
  'lyn_output_marker=$(<"$lyn_state/output-marker")',
  'rm -f -- "$lyn_state/environment" "$lyn_state/argv" "$lyn_state/output-marker" "$lyn_state/runner.bash"',
  'if (( ${#lyn_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$lyn_output_marker"',
  'exec env -i -- "${lyn_env[@]}" "${lyn_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`lyn-e2b-bootstrap:${randomUUID()}`)
