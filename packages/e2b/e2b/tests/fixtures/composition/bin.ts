...
try {
  ...
  const outputDrainHandle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "bash -c 'exec -a lyn-output-drain-descendant sleep 30' & printf 'leader-done\\n'"],
    cwd: process.cwd(),
  ...
  const outputDrainClean = !outputDrainProcesses.some(processInfo =>
    JSON.stringify([processInfo.cmd, processInfo.args]).includes('lyn-output-drain-descendant'),
  )
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'LYNESS_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('LYNESS_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('LYNESS_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'LYNESS_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /LYNESS_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
