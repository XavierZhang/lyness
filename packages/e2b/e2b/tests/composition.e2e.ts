...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', LYNESS_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/lyn-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'lyn_leak=0',
          'for lyn_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$lyn_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$lyn_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then lyn_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> LYN=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$LYNESS_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> LYN=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.lyn-e2b/terminals/')
