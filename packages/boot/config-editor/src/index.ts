...
import { Context, FiberState, Service, resolveConfig } from '@lyness/cordis'
import { entryListSchema, type PatchOptions } from '@lyness/cordis-plugin-include'
import yaml from 'js-yaml'
import type { Entry, EntryOptions } from '@lyness/cordis-plugin-loader'
import type {} from '@lyness/lyn-hmr'
import { composeEntries, loadProfileDirectory, readProfilePatches, reconcileProfilePatches } from '@lyness/lyn-app-boot'
...
export class ConfigEditor extends Service {
  ...
  configuration(): Array<{ entry: Entry; inherited: Record<string, unknown>; override: Record<string, unknown> }> {
    ...
    const loaded = loadProfileDirectory('lyn', profile.dir, profile.installAnchor)
  ...
  async edit(
    ...
    const run = async (): Promise<void> => {
      ...
      await withFileLock(join(this.ownerContext.profileContext.dir, 'package.json'), async () => {
        ...
        const beforePatches = readProfilePatches('lyn', this.ownerContext.profileContext)
        await reconcileProfilePatches(this.ownerContext.root, beforePatches, 'lyn')
        ...
        const inherited = this.inherited(entry, loadProfileDirectory('lyn', this.ownerContext.profileContext.dir, this.ownerContext.profileContext.installAnchor))
        ...
        const profile = this.ownerContext.profileContext
        const loaded = loadProfileDirectory('lyn', profile.dir, profile.installAnchor)
        const patches = readProfilePatches('lyn', profile, { ...loaded, patches: yaml.load(String(document), { schema: entryListSchema }) as PatchOptions[] })
        ...
        try {
          await reconcileProfilePatches(this.ownerContext.root, patches, 'lyn', [entry.options.id])
        } catch (error) {
