...
import { getLynRuntimeVersion } from './plugin-compatibility.ts'
...
/** Whether a decoded record accepts only exact LYN versions. */
 ...
 * @param runtimeVersion Exact LYN version, including prerelease and build metadata.
...
export function validatePluginVersionExemption(packageVersion: string, runtimeVersion: string): void {
  ...
  if (separator <= 0 || !PACKAGE_NAME.test(packageVersion.slice(0, separator))
    ...
    throw new Error('Version exemptions require an exact npm package-name@version and an exact LYN runtime version')
  }
}
...
export interface ProfileCompatibility {
  /** Accepted exact package-name@version keys mapped to their allowed LYN versions. */
...
export function readProfileCompatibility(profileDir: string): ProfileCompatibility {
  ...
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    return unreadable('must map exact package@version keys to LYN version lists')
  }
  ...
  for (const [key, versions] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (!isVersionList(versions)) {
      warnings.push(`${filename}: ${key} must contain a list of exact LYN versions; the record is ignored`)
      continue
    }
 ...
 * @param profileDir Absolute profile directory.
 * @returns Exact package-name@version keys mapped to their allowed LYN versions.
 */
 ...
 * @param runtimeVersion Exact LYN version; grants must name the current runtime, revocations may name historical ones.
...
export async function setProfileVersionExemption(
  ...
  const current = getLynRuntimeVersion()
  if (enabled && runtimeVersion !== current) {
    throw new Error(`Cannot approve LYN ${runtimeVersion}: this application runs LYN ${current}. Use --lyn-version ${current}.`)
  }
