 ...
 * `lyn --profile` launcher family.
 *
 * A profile is a directory under `$LYNESS_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `lyn.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"lyn": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `lyn.profile.bundles` order over
 ...
 * first from the lyn installation (the launcher's own package), then from the
 * profile directory. Pnpm-managed entries in the profile's `node_modules`
 * resolve first. Lyn-owned links add packages carried only by selected
 * bundles, while `$LYNESS_HOME/profiles/node_modules` supplies the installation
...
const PROFILE_MODULE_FALLBACK_DIR = '.lyn-module-fallback'

/** The bundle half of the `lyn` manifest section: what a bundle package exports. */
export interface LynBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `lyn` manifest section: what a profile directory composes. */
...
/**
 * The profile-launcher slice of the `lyn`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface LynManifestSection {
...
export interface ProfileManifest {
  ...
  lyn?: LynManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `lyn.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `lyn.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`lyn --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveLynHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveLynHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`lyn: invalid profile name ${JSON.stringify(name)}`)
  }
...
const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this lyn profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `lyn.profile.bundles` layer list.
...
export function initProfile(
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `lyn-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      lyn: { profile: { bundles: [...bundles], patchReload } },
    }
...
/** Ensure `link` is a symlink to `target`, replacing a wrong link or a lyn-managed packaged proxy. */
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      ...
      if (existing?.lyn?.moduleFallback?.targets === undefined) {
        throw new Error(`lyn: ${link} exists and is not a symlink or lyn-managed module proxy; remove it so lyn can manage the installation fallback`)
      }
...
interface ModuleProxyManifest {
  ...
  lyn: { moduleFallback: { targets: Record<string, string> } }
}

interface ModuleProxyRecord {
  version?: unknown
  lyn?: { moduleFallback?: { targets?: unknown } }
}
...
function packageEntryFromPackage(
  ...
  try {
    ...
    throw new Error(`lyn: cannot resolve ESM export ${specifier} from installed package ${packageName}`, { cause: error })
  }
  for (const candidate of candidates ?? []) {
    ...
    if (!target.startsWith('./') || /^\.\.(?:[\\/]|$)/u.test(relativeEntry)) {
      throw new Error(`lyn: installed package ${packageName} export ${subpath} resolves outside its package: ${target}`)
    }
...
function packageProxySource(
  ...
  if (typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`lyn: installed package ${packageName} must declare a non-empty version`)
  }
  ...
  if (declared === undefined) {
    ...
    try {
      ...
      throw new Error(`lyn: installed package ${packageName} main entry is missing at ${entry}`, { cause: error })
    }
  }
...
function ensureModuleProxy(
  ...
  if (stat !== undefined) {
    ...
    if (existing?.lyn?.moduleFallback?.targets === undefined) {
      throw new Error(`lyn: ${link} exists and is not a lyn-managed module proxy; remove it so lyn can manage the installation fallback`)
    }
    if (existing.version === version
      && JSON.stringify(existing.lyn.moduleFallback.targets) === JSON.stringify(targets)
...
function resolveModuleFallbackEntries(
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (lyn-subprocess,
    // lyn-compaction, ...) are peers of their implementations, never plain
...
function moduleFallbackEntryCurrent(modulesDir: string, entry: ModuleFallbackEntry): boolean {
  ...
  try {
    ...
    return existing?.version === entry.version
      && JSON.stringify(existing.lyn?.moduleFallback?.targets) === JSON.stringify(entry.targets)
...
export interface ProfileModuleFallbackOptions {
  /** Absolute package.json path of the running lyn installation. */
  installAnchor: string
  /** Loaded profile whose selected bundles may carry profile-local plugins. */
  profile?: Profile
  /** Harness home; defaults to {@link resolveLynHome}. */
  home?: string
}

/**
 * Maintain module fallbacks for one profile launch. The shared
 * `$LYNESS_HOME/profiles/node_modules` mirrors the lyn installation dependency
...
export async function healProfilesModuleFallback(options: ProfileModuleFallbackOptions): Promise<void> {
  const { installAnchor, profile, home = resolveLynHome() } = options
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.lyn?.profile?.bundles
  ...
  const needsReloadDefault = manifest.lyn?.profile?.patchReload === undefined && isCurrentTuple
  ...
  const normalized: ProfileManifest = {
    ...
    lyn: {
      ...manifest.lyn,
      profile: {
        ...manifest.lyn?.profile,
        bundles: [...template.bundles],
        patchReload: manifest.lyn?.profile?.patchReload ?? template.patchReload,
      },
    },
  }
 ...
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `lyn.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the lyn app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the lyn installation or ${profileDir}; `
    + `run 'lyn plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `lyn.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `lyn.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the lyn app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveLynHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveLynHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'lyn plugin --profile ${name} add <package>'`,
      )
    }
  ...
  // A hand-written profile manifest may omit the lyn section entirely.
  const bundles = manifest.lyn?.profile?.bundles ?? []
  const rawPatchReload: unknown = manifest.lyn?.profile?.patchReload
  if (rawPatchReload !== undefined && rawPatchReload !== 'live' && rawPatchReload !== 'startup') {
    throw new Error(
      `${binName}: profile manifest ${join(dir, 'package.json')} lyn.profile.patchReload must be "live" or "startup"`,
    )
  }
  ...
  const layers = bundles.map((packageName): ProfileLayer => {
    ...
    const declared = bundleManifest.lyn?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no lyn.bundle in its package.json`)
    }
