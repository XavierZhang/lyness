/** Compatibility checks at the composition entry points LYN owns; no Loader instrumentation. */
...
/** Prepare profile or preset rows before the owning LYN caller passes them to Loader.
 ...
 * @param binName Diagnostic prefix for a denied row; defaults to `lyn`.
...
export function prepareProfileEntries(
  ctx: Context, entries: readonly EntryOptions[], parentURL: string | undefined, binName = 'lyn',
): EntryOptions[] {
...
function preflight(
  ...
  const check = (rows: EntryOptions[], base: string): boolean => {
    ...
    for (const row of rows) {
      ...
      if ((row.group === true || row.name === 'cordis:group' || row.name === '@lyness/cordis-plugin-group')
        && Array.isArray(row.config) && check(row.config as EntryOptions[], base)) blocked = true
      if (row.name !== 'cordis:include' && row.name !== '@lyness/cordis-plugin-include') continue
...
export function prepareProfilePatches(
  ctx: Context, patches: PatchOptions[], parentURL: string, binName = 'lyn',
): PatchOptions[] {
