...
import { EntryGroup, ModuleLoader, type EntryOptions } from '@lyness/cordis-plugin-loader'
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'lyn-config-schema-'))
...
describe('collectConfigSchemas', () => {
  ...
  it('recognizes external canonical Include aliases and caches them per tree base', async () => {
    ...
    modules.set('@lyness/cordis-plugin-include', { default: ExternalInclude })
    ...
    expect(importModule.mock.calls.filter(([name]) => name === '@lyness/cordis-plugin-include')).toHaveLength(1)
    expect(importModule.mock.calls.filter(([name]) => name === '@lyness/cordis-plugin-group')).toHaveLength(1)
  })
  ...
  it('recognizes an external canonical Group even when Include cannot resolve', async () => {
    ...
    modules.set('@lyness/cordis-plugin-group', { default: ExternalGroup })
  ...
  it('reports unknown tree-carrier identities instead of silently omitting their children', async () => {
    modules.set('@lyness/cordis-plugin-group', { default: Group })
    modules.set('@lyness/cordis-plugin-include', { default: Include })
