...
import { getLynRuntimeVersion } from '../src/index.ts'
import {
...
function profileFixture(): string {
  const dir = mkdtempSync(join(tmpdir(), 'lyn-compatibility-'))
...
it('requires explicit risk acknowledgement and the running runtime for a grant', async () => {
  ...
  await expect(setProfileVersionExemption(dir, 'plugin@1.0.0', '9.9.9', true, true)).rejects.toThrow(`runs LYN ${runtime}`)
