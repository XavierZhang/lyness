...
const names = ['@lyness/lyn-tools', 'react'] as const
...
class MatrixFixture {
  ...
  readonly root = realpathSync.native(mkdtempSync(join(tmpdir(), `lyn-linked-matrix-${++fixtureSequence}-`)))
  ...
  readonly installDir = join(this.root, 'installation', 'node_modules', '@lyness', 'lyn')
...
function writeNodeComparison(output: string, selected: readonly MatrixRow[]): void {
  ...
  const markdown = [
    ...
    '| ID | Layout | Package | Runtime | Request | Importer | Explicit paths start | JSON here | node_modules here | Parent JSON | Parent node_modules | Method | Native Node | Current LYN | Peer reference | Matches reference |',
    '|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|',
...
afterAll(() => {
  const parent = process.env.LYNESS_RESOLUTION_MATRIX_REPORT_DIR
