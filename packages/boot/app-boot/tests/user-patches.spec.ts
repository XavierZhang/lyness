/**
 * User patch-layer behavior of `lyn-app-boot`: the optional patch-list loader
...
import Hmr from '@lyness/cordis-plugin-hmr'
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'lyn-user-patches-'))
...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.LYNESS_SPEC_MODEL' } },
    })
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.LYNESS_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['LYNESS_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['LYNESS_APP_BOOT_USER_SPEC']
    }
  })
