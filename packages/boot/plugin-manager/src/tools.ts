...
import { approveEscalation } from '@lyness/lyn-sandbox'
import type { PluginEntryId } from './types.ts'
import { defineTool } from '@lyness/lyn-tools'
import { getLynRuntimeVersion } from '@lyness/lyn-app-boot'
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      runtimeVersion: { type: 'string', description: 'For set_version_exemption: exact LYN version from list_version_exemptions. Target must be the manifest package-name@version, not an alias or version range.' },
    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        case 'list_version_exemptions':
          return JSON.stringify({ runtimeVersion: getLynRuntimeVersion(), ...manager.listVersionExemptions() })
