/** Shared profile package operations used by lyn plugin and the running manager. */
...
export { setProfileVersionExemption, readProfileVersionExemptions } from '@lyness/lyn-app-boot'
...
export interface PackageOperationOptions {
  /** The pnpm executable name or path; resolved through `PATH` like the `lyn plugin` command. Defaults to `pnpm`. */
  command?: string
...
export function bundleManifest(name: string, dir: string, anchor: string): ProfileManifest | undefined {
  const packageDir = resolveBundleDir('lyn', name, anchor, dir)
  const manifest = readProfileManifest('lyn', packageDir)
  return manifest.lyn?.bundle?.patch === undefined ? undefined : manifest
}
...
async function reconcile(before: ProfileManifest, dir: string, anchor: string, options: PackageOperationOptions): Promise<void> {
  const after = readProfileManifest('lyn', dir)
  const dependencies = Object.keys(after.dependencies ?? {})
  const beforeDeps = new Set(Object.keys(before.dependencies ?? {}))
  const previous = after.lyn?.profile?.bundles ?? []
  ...
  for (const name of dependencies) {
    ...
    if (metadata?.lyn?.bundle === undefined) {
      options.onOutput?.(`lyn: warning: ${name} declares no lyn.bundle — installed as a plain dependency, not a profile layer\n`, 'stderr')
      continue
    }
    for (const file of bundlePatchPaths(resolveBundleDir('lyn', name, anchor, dir), metadata.lyn.bundle)) loadOverlayPatches('lyn', file)
    if (!bundles.includes(name)) {
      bundles.push(name)
    }
  }
  if (JSON.stringify(previous) === JSON.stringify(bundles)) return
  after.lyn = { ...after.lyn, profile: { ...after.lyn?.profile, bundles } }
...
async function activeRecordedRun(dir: string): Promise<string | undefined> {
  ...
  if (tree === undefined) {
    return `lyn: ${path} does not name a package run; delete it once no earlier package operation is still running in this profile\n`
  }
  await awaitTreeGone(tree)
  if (treeAlive(tree)) {
    return `lyn: process ${String(tree.pid)}, started by an earlier package operation whose own process ended, is still running in this profile; `
...
function bundleComponentManifests(manifest: ProfileManifest, dir: string, anchor: string): ProfileManifest[] {
  const bundle = manifest.lyn?.bundle
  if (bundle === undefined) return []
  const patches = bundlePatchPaths(dir, bundle).flatMap(file => loadOverlayPatches('lyn', file))
  ...
  return [...names].flatMap((name) => {
    ...
    try { packageDir = resolveBundleDir('lyn', name, anchor, dir) } catch (error) {
      // Resolution errors for uninstalled or dynamic rows remain subject to the startup loader's checks.
      void error
      return []
    }
    return [readProfileManifest('lyn', packageDir)]
  })
}
...
export async function runProfilePnpm(
  ...
  const before = readProfileManifest('lyn', dir)
  ...
  const rejected = async (warnings: readonly string[], restoration: string): Promise<PackageResult> => {
    const diagnostic = `\nlyn: installation rejected: ${warnings.join('\n')}\nlyn: ${restoration}.\n`
  ...
  try {
    ...
    if (drained) {
      ...
      const notice = 'lyn: pnpm output was cut short after its process exited\n'
    ...
    if (control.stalled) {
      const notice = `lyn: pnpm printed nothing for ${String(options.idleTimeoutMs)}ms and was terminated\n`
      await log.write(notice)
      options.onOutput?.(notice, 'stderr')
      append(Buffer.from(notice))
    }
    ...
    if (exitCode === 0 && !control.stalled) {
      ...
      for (const [name, spec] of Object.entries(directDependencies(after))) {
        ...
        if (!untouched) {
          ...
          const notice = `\nlyn: warning: ${found.join('\n')}\nlyn: it stays installed but profile startup denies it until you grant an exemption for those exact versions.\n`
          await log.write(notice)
          options.onOutput?.(notice, 'stderr')
        }
      }
      if (warnings.length > 0) {
        ...
        const restoration = repaired.exitCode === 0
          ...
          : "restored package.json and pnpm-lock.yaml, but node_modules could not be reinstalled; run 'lyn plugin install'"
        const diagnostic = `\nlyn: installation rejected: ${warnings.join('\n')}\nlyn: ${restoration}.\n`
        await log.write(diagnostic)
        options.onOutput?.(diagnostic, 'stderr')
        append(Buffer.from(diagnostic))
...
/** Initialize and run the lyn plugin command with the same write lock as the service.
...
export async function runPluginCommand(
  ...
  return withFileLock(join(dir, 'package.json'), async () => {
    if (!existsSync(join(dir, 'package.json'))) {
      ...
      options.onOutput?.(`lyn: initialized profile ${context.profile} at ${dir}\n`, 'stderr')
    }
...
export async function viewProfilePackage(dir: string, spec: string, options: PackageViewOptions): Promise<PackageViewResult> {
  const result = await execa(options.command ?? 'pnpm', [
    ...options.args ?? [], 'view', spec, 'name', 'version', 'description', 'lyn', '--json',
