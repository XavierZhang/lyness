...
describe('attributeFailure', () => {
  const name = parseInstallSpec('lyn-x')
  const git = parseInstallSpec('github:acme/lyn-x')
  const tarball = parseInstallSpec('https://cdn.example.com/lyn-x-1.0.0.tgz')
  const registryLog = 'ERR_PNPM_META_FETCH_FAIL  GET https://registry.npmjs.org/lyn-x: getaddrinfo ENOTFOUND registry.npmjs.org'
  ...
  it('lays a failure whose error line names the host a git or tarball spec is fetched from at that host', () => {
    expect(attributeFailure('network', 'fatal: unable to access \'https://github.com/acme/lyn-x/\': Could not resolve host: github.com', git)).toBe('spec-host')
    ...
    expect(attributeFailure('network', 'ERR_PNPM_FETCH_502  GET https://cdn.example.com/lyn-x-1.0.0.tgz: Bad Gateway', tarball)).toBe('spec-host')
  ...
  it('reads the host only off error lines, not off a warning that links to it', () => {
    const log = [
      'WARN  deprecated left-pad@1.0.0: see https://github.com/acme/lyn-x#readme',
