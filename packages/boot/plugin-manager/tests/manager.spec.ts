...
import {
  ...
  reconcileProfilePatches, OPTIONAL_BUNDLES, PluginPackages, readPluginMeta, getLynRuntimeVersion,
...
import { Group } from '@lyness/cordis-plugin-loader'
...
async function fixture(reload: 'live' | 'startup' = 'live', overlay = false, prepare?: (ctx: Context) => void, config: Config = {}, packageManager?: ProfileContext['packageManager'], prepareFiles?: (dir: string) => void) {
  ...
  const bundle = (name: string, rows: unknown[]) => {
    ...
    writeFileSync(join(path, 'package.json'), JSON.stringify({ name, version: '1.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }))
...
it.each(['network', 'timeout'] as const)('stops a GitHub %s before pnpm and attributes it to the repository', async (kind) => {
  ...
  expect(await manager.installBundle('https://github.com/acme/lyn-plugin.git')).toMatchObject({
...
it('cancels an active GitHub check before starting pnpm', async () => {
  ...
  const installing = manager.installBundle('github:acme/lyn-plugin', { requestId })
...
it('waits for the GitHub check before installing and activating the bundle', async () => {
  ...
  expect(readProfileManifest('test', dir).lyn?.profile?.bundles).toContain('addon')
})
...
it.each(['github:acme/connected', 'https://github.com/acme/connected.git', 'git+ssh://git@github.com/acme/connected.git'])(
  'installs %s through real Git and pnpm with private repository SSH fallback', async (spec) => {
    ...
    writeFileSync(join(repository, 'package.json'), JSON.stringify({ name, version: '1.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }))
...
it.each(['missing package', 'invalid manifest', 'not a bundle', 'missing patch', 'invalid patch'])(
  'boots with %s, lists its error, and permits deselection without enabling the broken bundle', async (failure) => {
    ...
    expect(readProfileManifest('test', dir).lyn?.profile?.bundles).toEqual(['core'])
...
it.each(['live', 'startup'] as const)('removes a bundle skipped at startup in a %s profile', async (mode) => {
  ...
  const remove = vi.spyOn(operations, 'runProfilePnpm').mockImplementation(async () => {
    const manifest = readProfileManifest('test', dir)
    expect(manifest.lyn?.profile?.bundles).toEqual(['core'])
...
it('describes a bundle by its manifest and patch: one-liner, rows without a live entry, and the built-in rows it changes', async () => {
  ...
  writeFileSync(join(dir, 'node_modules', 'described', 'package.json'), JSON.stringify({
    name: 'described', version: '2.0.0', description: 'Describes itself.', lyn: { bundle: { patch: './cordis.patch.yml' } },
  }))
...
it.each(['native', 'runtime'] as const)('reads a disabled bundle and its independent exported plugins without running them with %s resolution', async (mode) => {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'localized',
    exports: { './locale/*.json': './locale/*.json' }, lyn: {
...
it.each([
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({
    version: '1.0.0', exports, lyn: { bundle: { patch: './cordis.patch.yml' } },
  }))
...
it('retains installed dependencies when toggling a bundle and appends it when re-enabled', async () => {
  ...
  expect(readProfileManifest('test', dir).lyn?.profile?.bundles).toEqual(['core', 'third', 'extra'])
})
...
it('reports blocked scripts after a failed installation and retries only after explicit profile build approval', async () => {
  ...
  expect(readProfileManifest('test', dir).lyn?.profile?.bundles).not.toContain('addon')
})
...
it('runs a real pnpm dependency script only after approval and retry', async () => {
  ...
  writeFileSync(join(addon, 'package.json'), JSON.stringify({ name: 'approval-fixture-addon', version: '1.0.0',
    scripts: { install: 'node build.cjs' }, lyn: { bundle: { patch: './cordis.patch.yml' } } }))
...
it('reports a selected plain dependency as a problem, omits an unselected one, and reports missing versions', async () => {
  ...
  expect((await manager.listBundles()).some(row => row.name === 'extra')).toBe(false)
  expect(await manager.setBundleEnabled('extra', true)).toMatchObject({ changed: false, application: 'failed' })
  writeFileSync(join(dir, 'node_modules', 'core', 'package.json'), '{"name":"core","lyn":{"bundle":{"patch":"./cordis.patch.yml"}}}')
...
it('restores the manifest when the package pnpm added declares no bundle', async () => {
  ...
  expect(readProfileManifest('test', dir)).toMatchObject({ dependencies: { extra: '1.0.0' }, lyn: { profile: { bundles: ['core', 'extra'] } } })
...
it('reads what a spec names before installing it', async () => {
  ...
  answers(JSON.stringify({ name: 'lyn-x', version: '1.4.2', description: 'A sidebar.', lyn: { bundle: { patch: './cordis.patch.yml' } } }))
  expect(await manager.inspect('lyn-x')).toEqual({
    status: 'accepted', kind: 'registry', name: 'lyn-x', version: '1.4.2', description: 'A sidebar.', bundle: true, registry: null,
  })
  expect(view).toHaveBeenCalledWith(dir, 'lyn-x', { command: 'pnpm-test', timeoutMs: 1000, registry: null })
  const signal = AbortSignal.abort()
  answers(JSON.stringify([{ name: 'lyn-lib', version: '1.0.0', lyn: { bundle: {} } }, { name: 'lyn-lib', version: '1.1.0', lyn: null }]))
  expect(await manager.inspect('lyn-lib@^1', undefined, signal)).toEqual({ status: 'refused', problem: 'not-a-bundle', reason: 'lyn-lib declares no lyn.bundle', registries: [null] })
  expect(view).toHaveBeenLastCalledWith(dir, 'lyn-lib@^1', { command: 'pnpm-test', timeoutMs: 1000, signal, registry: null })
  // An answer that names no package keeps the name the spec gave; colour escapes around the JSON are dropped.
  answers('\x1b[36m' + JSON.stringify({ version: '0.0.1', description: '', lyn: { bundle: { patch: './p.yml' } } }) + '\x1b[39m\n')
  expect(await manager.inspect('lyn-bare')).toEqual({ status: 'accepted', kind: 'registry', name: 'lyn-bare', version: '0.0.1', bundle: true, registry: null })
  ...
  expect(await manager.inspect('github:acme/lyn-remote')).toEqual({ status: 'accepted', kind: 'git', bundle: null, registry: null, host: 'github.com' })
  ...
  expect(await manager.inspect('https://cdn.example.com/x/y/z/lyn-x-1.0.0.tgz')).toEqual({ status: 'accepted', kind: 'tarball', bundle: null, registry: null, host: 'cdn.example.com' })
  // A directory answers from its own manifest.
  const local = join(profile.home, 'dev', 'lyn-local')
  ...
  writeFileSync(join(local, 'package.json'), JSON.stringify({ name: 'lyn-local', version: '0.1.0', description: 'Local.' }))
  expect(await manager.inspect(local)).toEqual({ status: 'refused', problem: 'not-a-bundle', reason: 'lyn-local declares no lyn.bundle' })
  writeFileSync(join(local, 'package.json'), JSON.stringify({ name: 'lyn-local', version: '0.1.0', description: 'Local.', lyn: { bundle: { patch: './p.yml' } } }))
  expect(await manager.inspect(`file:${local}`)).toEqual({ status: 'accepted', kind: 'path', name: 'lyn-local', version: '0.1.0', description: 'Local.', bundle: true, registry: null })
  writeFileSync(join(local, 'package.json'), JSON.stringify({ name: 'core', lyn: { bundle: { patch: './p.yml' } } }))
...
it('offers the launcher\'s optional bundles switched off and never removable', async () => {
  ...
  writeFileSync(join(supplied, 'package.json'), JSON.stringify({
    name: offered, version: '3.0.0', description: 'Package one-liner.', lyn: { bundle: { patch: './cordis.patch.yml' } },
  }))
...
it('applies watched configuration while pnpm installation is still running', async () => {
  ...
  expect(readProfileManifest('test', dir).lyn?.profile?.bundles).toEqual(['core', 'extra', 'new-bundle'])
...
it('asks the registries in turn while one is unreachable or stale, and names the one that answered', async () => {
  ...
  const manifest = JSON.stringify({ name: 'lyn-x', version: '1.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } })
  // pnpm's own registry is unreachable; the mirror answers and the install is told to start there.
  failure('ERR_PNPM_META_FETCH_FAIL  GET https://registry.npmjs.org/lyn-x: ETIMEDOUT\n')
  answers(manifest)
  expect(await manager.inspect('lyn-x')).toEqual({ status: 'accepted', kind: 'registry', name: 'lyn-x', version: '1.0.0', bundle: true, registry: MIRROR })
  expect(asked(0)).toEqual([null, MIRROR])
  expect(read).toHaveBeenCalledWith(dir, { command: 'pnpm', timeoutMs: 1000 })
  expect(view).toHaveBeenLastCalledWith(expect.any(String), 'lyn-x', { command: 'pnpm', timeoutMs: 1000, registry: MIRROR })
  // pnpm prints a refusal as JSON on stdout with nothing on stderr: it is read the same way, and the reason is its message.
  view.mockResolvedValueOnce({ exitCode: 1, stdout: '{\n  "error": {\n    "code": "ERR_PNPM_META_FETCH_FAIL",\n    "message": "GET https://registry.npmjs.org/lyn-x: fetch failed"\n  }\n}\n', stderr: '', timedOut: false })
  answers(manifest)
  expect(await manager.inspect('lyn-x')).toMatchObject({ status: 'accepted', registry: MIRROR })
  ...
  view.mockResolvedValueOnce({ exitCode: 1, stdout: '{"error":{"code":"ERR_PNPM_FETCH_404","message":"GET https://registry.npmmirror.com/lyn-x: Not Found - 404"}}', stderr: '', timedOut: false })
  failure('ERR_PNPM_FETCH_404  GET https://registry.npmjs.org/lyn-x: Not Found - 404\n')
  expect(await manager.inspect('lyn-x', { registry: MIRROR })).toMatchObject({
    status: 'refused', problem: 'not-found', reason: 'ERR_PNPM_FETCH_404  GET https://registry.npmjs.org/lyn-x: Not Found - 404', registries: [MIRROR, null],
  })
  expect(asked(4)).toEqual([MIRROR, null])
  // A registry outside the configured set is asked alone.
  failure('ERR_PNPM_META_FETCH_FAIL  GET https://npm.corp.example/lyn-x: ECONNREFUSED\n')
  expect(await manager.inspect('lyn-x', { registry: 'https://npm.corp.example' })).toMatchObject({ status: 'refused', problem: 'network', registries: ['https://npm.corp.example/'] })
  expect(asked(6)).toEqual(['https://npm.corp.example/'])
  // A failure no registry changes ends the round at once.
  failure('', 4)
  expect(await manager.inspect('lyn-x')).toMatchObject({ status: 'refused', problem: 'unknown', registries: [null] })
  ...
  view.mockResolvedValueOnce({ exitCode: null, stdout: '', stderr: '', timedOut: true })
  expect(await manager.inspect('lyn-x')).toEqual({ status: 'refused', problem: 'network', reason: 'pnpm view timed out after 1000ms', registries: [null, MIRROR] })
  ...
  expect(await manager.inspect('lyn-x', {}, controller.signal)).toMatchObject({ status: 'refused', problem: 'network', registries: [null] })
  expect(asked(10)).toEqual([null])
  // The other forms ask no registry and carry the one the install starts with.
  expect(await manager.inspect('github:acme/lyn-remote', { registry: MIRROR })).toEqual({ status: 'accepted', kind: 'git', bundle: null, registry: MIRROR, host: 'github.com' })
  expect(await manager.inspect('github:acme/lyn-remote')).toEqual({ status: 'accepted', kind: 'git', bundle: null, registry: null, host: 'github.com' })
...
it('keeps pnpm\'s own registry alone when it names a private one, or cannot be read', async () => {
  ...
  const view = vi.spyOn(operations, 'viewProfilePackage')
  onTestFinished(() => { view.mockRestore() })
  view.mockResolvedValue({ exitCode: 1, stdout: '', stderr: 'ERR_PNPM_META_FETCH_FAIL  GET https://npm.corp.example/lyn-x: ETIMEDOUT\n', timedOut: false })
  expect(await manager.inspect('lyn-x')).toMatchObject({ status: 'refused', problem: 'network', registries: [null] })
  expect(await manager.inspect('lyn-x', { registry: MIRROR })).toMatchObject({ status: 'refused', problem: 'network', registries: [MIRROR] })
...
it('does not ask the next registry after the manager terminated a silent run', async () => {
  ...
  const install = vi.spyOn(operations, 'runProfilePnpm').mockResolvedValue({
    exitCode: 1, output: 'installing\nlyn: pnpm printed nothing for 600000ms and was terminated\n',
...
it('stops at a failure no registry changes, at a host the spec itself is fetched from, and after a registry outside the configured set', async () => {
  ...
  expect(install).toHaveBeenCalledTimes(1)
  install.mockResolvedValue(failing('fatal: unable to access \'https://github.com/acme/lyn-x/\': Could not resolve host: github.com'))
  expect(await manager.installBundle('github:acme/lyn-x')).toMatchObject({ application: 'failed', registries: [null], failedAt: 'spec-host', packageResult: { kind: 'network' } })
  expect(install).toHaveBeenCalledTimes(2)
  install.mockResolvedValue(failing('ERR_PNPM_META_FETCH_FAIL  GET https://npm.corp.example/lyn-x: ECONNREFUSED'))
  expect(await manager.installBundle('lyn-x', { registry: 'https://npm.corp.example' })).toMatchObject({
    application: 'failed', registries: ['https://npm.corp.example/'], failedAt: 'registry', packageResult: { kind: 'network' },
  })
  expect(install).toHaveBeenCalledTimes(3)
  expect(install).toHaveBeenLastCalledWith(expect.anything(), ['add', 'lyn-x', '--registry=https://npm.corp.example/'], expect.anything())
  // Every configured registry unreachable: the failure is the last attempt's, and all of them are named.
  install.mockResolvedValue(failing('ERR_PNPM_META_FETCH_FAIL  GET https://registry/lyn-x: ETIMEDOUT'))
  expect(await manager.installBundle('lyn-x')).toMatchObject({ application: 'failed', registries: [null, MIRROR], failedAt: 'registry', packageResult: { kind: 'network' } })
...
it('reports a stop that lands before a run starts, while the registries are read, as cancelled', async () => {
  ...
  const result = await manager.installBundle('lyn-x', { requestId })
...
it.each(['live', 'startup'] as const)('requires exact risk acknowledgement and preserves exemptions in a %s profile', async (mode) => {
  const { manager, dir, ctx } = await fixture(mode, false, undefined, {}, undefined, (dir) => {
    ...
    writeFileSync(file, JSON.stringify({ ...metadata, peerDependencies: { '@lyness/lyn': '999.0.0' } }))
  })
  ...
  expect((await manager.listBundles()).find(bundle => bundle.name === 'extra')?.error).toEqual({
    ...
    incompatible: [{ name: 'extra', version: '1.0.0', runtimeVersion: runtime, peers: { '@lyness/lyn': '999.0.0' } }],
  })
  expect(await manager.setBundleEnabled('extra', true)).toMatchObject({ changed: false, application: 'failed' })
  ...
  expect(readProfileManifest('test', dir).lyn?.profile?.bundles).toEqual(['core', 'extra'])
...
it('reports a package run refused for compatibility as a typed refusal', async () => {
  const { manager } = await fixture()
  const incompatible = [{ name: 'lyn-x', version: '2.0.0', runtimeVersion: getLynRuntimeVersion(), peers: { '@lyness/lyn': '999.0.0' } }]
  const install = vi.spyOn(operations, 'runProfilePnpm').mockResolvedValue({
    exitCode: 1, output: 'lyn: installation rejected', truncated: false, logPath: 'pnpm.log', kind: 'unknown', incompatible,
  })
  onTestFinished(() => { install.mockRestore() })
  const result = await manager.installBundle('lyn-x')
...
it.each([false, true])('rechecks installed bundle peers before accepting a disabled installation (exempted=%s)', async (exempted) => {
  const { manager, dir, bundle } = await fixture()
  if (exempted) await manager.setVersionExemption('incompatible@1.0.0', getLynRuntimeVersion(), true, true)
  const before = readFileSync(join(dir, 'package.json'), 'utf8')
  const install = vi.spyOn(operations, 'runProfilePnpm').mockImplementation(async () => {
    bundle('incompatible', [])
    const file = join(dir, 'node_modules', 'incompatible', 'package.json')
    const metadata = JSON.parse(readFileSync(file, 'utf8')) as Record<string, unknown>
    writeFileSync(file, JSON.stringify({ ...metadata, peerDependencies: { '@lyness/lyn': '<0.0.0' } }))
