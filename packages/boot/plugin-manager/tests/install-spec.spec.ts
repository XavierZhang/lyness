...
import { classifyInstallFailure, InvalidInstallSpecError, parseInstallSpec } from '@lyness/lyn-plugin-manager'

describe('parseInstallSpec', () => {
  it('reads registry names with an optional range, scoped or not', () => {
    expect(parseInstallSpec(' lyn-better-sidebar ')).toEqual({ kind: 'registry', spec: 'lyn-better-sidebar', name: 'lyn-better-sidebar' })
    expect(parseInstallSpec('@acme/lyn-tool@^1.2')).toEqual({ kind: 'registry', spec: '@acme/lyn-tool@^1.2', name: '@acme/lyn-tool', range: '^1.2' })
  ...
  it('reads absolute paths, with or without a file: or link: prefix, and tarballs on disk', () => {
    expect(parseInstallSpec('/plugins/lyn-x')).toEqual({ kind: 'path', spec: '/plugins/lyn-x', path: '/plugins/lyn-x' })
    expect(parseInstallSpec('file:/plugins/lyn-x')).toEqual({ kind: 'path', spec: 'file:/plugins/lyn-x', path: '/plugins/lyn-x' })
    expect(parseInstallSpec('link:/plugins/lyn-x')).toEqual({ kind: 'path', spec: 'link:/plugins/lyn-x', path: '/plugins/lyn-x' })
    expect(parseInstallSpec('/packs/lyn-x-1.0.0.tgz')).toEqual({ kind: 'tarball', spec: '/packs/lyn-x-1.0.0.tgz', path: '/packs/lyn-x-1.0.0.tgz' })
  })

  it('reads git hosts and tarball URLs, naming the host the spec itself is fetched from', () => {
    const hosts: Record<string, string> = {
      'github:someone/lyn-plugin': 'github.com', 'gitlab:a/b#main': 'gitlab.com', 'bitbucket:a/b': 'bitbucket.org', 'gist:0123abcd': 'gist.github.com',
    ...
    expect(parseInstallSpec('https://cdn.example.com/x/y/z/lyn-x-1.0.0.tgz')).toEqual({ kind: 'tarball', spec: 'https://cdn.example.com/x/y/z/lyn-x-1.0.0.tgz', host: 'cdn.example.com' })
  })
  ...
  it('refuses what neither the registry nor pnpm would take, naming why', () => {
    ...
    expect(refusal('./lyn-x')).toBe('a local path must be absolute')
    expect(refusal('../lyn-x')).toBe('a local path must be absolute')
    expect(refusal('file:./lyn-x')).toBe('a local path must be absolute')
    expect(refusal('https://example.com/not-a-package')).toBe('a URL must point at a git repository or a tarball')
    expect(refusal('Lyn-Upper')).toBe('not a package name the registry accepts')
