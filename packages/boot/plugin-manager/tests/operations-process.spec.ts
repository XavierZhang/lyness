...
import { initProfile } from '@lyness/lyn-app-boot'
...
setInterval(() => { if (existsSync(process.env.LYNESS_DESCENDANT_STOP_FILE)) process.exit(0) }, 50)
setTimeout(() => process.exit(0), 60000)
`

/** Writes a marker after the run is over, which only a stopped tree can prevent. */
const LATE_SCRIPT = `
setTimeout(() => { require('node:fs').writeFileSync(process.env.LYNESS_LATE_FILE, 'late') }, 1500)
setTimeout(() => process.exit(0), 60000)
`
...
const descendant = spawn(process.execPath, [process.env.LYNESS_DESCENDANT_SCRIPT], {
  stdio: ['ignore', 'inherit', 'inherit'],
})
writeFileSync(process.env.LYNESS_DESCENDANT_PID_FILE, String(descendant.pid))
...
function fixture(child: string, descendant: string) {
  ...
  const options: PackageOperationOptions = {
    ...
    env: {
      LYNESS_DESCENDANT_PID_FILE: pidFile, LYNESS_DESCENDANT_STOP_FILE: stopFile,
      LYNESS_DESCENDANT_SCRIPT: scriptFile, LYNESS_LATE_FILE: lateFile,
    },
  }
...
describe.skipIf(process.platform === 'win32')('a run with descendants', () => {
  it('settles a run whose pipes a descendant keeps open', async () => {
    ...
    expect(outcome.output).toContain('lyn: pnpm output was cut short after its process exited')
...
it('waits for a run recorded by an exited operation before starting its own', async () => {
  const { context, lateFile, options } = fixture(
    ...
    'console.log(require("node:fs").existsSync(process.env.LYNESS_LATE_FILE) ? "started after" : "started before")', '',
  )
