...
Existing configurations replace the module name `@lyness/cordis-plugin-hmr` with `@lyness/lyn-hmr`. The `hmr` service key, `baseDir`, `config`, `getLinked()`, `getOuterStack()`, `hmr/change` and `hmr/reload` remain available. The vendored package remains available; LYN profiles use this package.
