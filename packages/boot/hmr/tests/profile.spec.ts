...
import { boot, initProfile, readProfileManifest, readProfilePatches, type ProfileContext } from '@lyness/lyn-app-boot'
...
it('ignores dependency-only manifest changes and reloads a changed bundle list', async () => {
  ...
  delete manifest.lyn!.profile!.bundles
  ...
  writeFileSync(join(packageDir, 'package.json'), JSON.stringify({ name: 'added', lyn: { bundle: { patch: './cordis.patch.yml' } } }))
  writeFileSync(join(packageDir, 'cordis.patch.yml'), '- insert:\n    - id: bundled\n      name: cordis:probe\n      disabled: true\n')
  manifest.lyn!.profile!.bundles = ['added']
  writeFileSync(manifestPath, JSON.stringify(manifest))
  f.emit(2, manifestPath)
  await f.drain()
