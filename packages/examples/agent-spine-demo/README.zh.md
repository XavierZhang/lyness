...
| `includeHarnessIdentity` | `true` | 系统提示词是否点名 lyness 身份 |
...
| `lynHome` | `$LYNESS_HOME` 或 `~/.lyn` | bash 环境与本地 skill 目录使用的 harness 主目录 |
...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynessagent-spine-demo)是每个受支持字段及其源声明的穷尽式真源。
...
`apply(ctx, config)` 把每个子插件挂载到组合包 fiber 下，并把每个配置字段转发给拥有它的子插件。Cordis 会按 `inject` 声明挂起每个 fiber，直到其所需服务存在为止，因此加载顺序不影响正确性；下方的清单只是按依赖分层排列，便于阅读。`pickSpineConfig()` 只从应用配置中复制组合包拥有的字段，因此入口设置绝不会泄漏进主干；`lynHome` 值冲突会在组合时失败，因为本地 skill 与受管 bash 环境必须共享同一个 harness 主目录。重试策略让重试状态、提供方错误与失败的部分分片不进入模型历史，并重建保留先前前缀的重试请求，使提供方缓存保持可复用。
...
- [ACP 应用组合包](../../bundle/acp-app/README.zh.md)——不预创建 agent、组合本主干的 `lyn --profile acp` 应用。
- [SDK 应用组合包](../../bundle/sdk-app/README.zh.md)——为 JSON-RPC 客户端组合本主干的 `lyn --profile sdk` 应用。
- [核心子系统](../../../docs/subsystems/core.zh.md)——本主干挂载的服务与 agent loop 约定。
- [生成配置目录](../../../docs/config-catalog.zh.md#lynessagent-spine-demo)——每个受支持配置字段及其源声明。
...
模型体验由组合包挂载的面向模型子插件间接提供——`lyn-system-prompt`、`lyn-tools`、`lyn-tool-skill`、`lyn-tool-bash`、`lyn-tool-jobs` 与 `lyn-llm-retry`，启用 `goals` 时还包括 `lyn-tool-goal` 与目标轮次驱动器的提示词；组合包自身不添加任何面向模型的包装内容。

#### KV Cache 影响

组合包自身不添加请求前缀内容；提供方缓存复用取决于所挂载消费方的贡献，`lyn-llm-retry` 重建重试请求时会保留其先前前缀。
...
后备标题限制（5 个词、40 个后备字节、80 个可接受标题字节）是本组合包拥有、可覆盖的示例策略，而非 `lyn-session-title` 所有；需要不同上限的入口应传入自己的 `sessionTitle` 配置。`workspaceContext` 字段为必填（不设默认值），因为它改变模型可见输入；若该字段日后改型，请保留这一要求。
