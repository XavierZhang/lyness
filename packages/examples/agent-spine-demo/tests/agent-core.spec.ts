...
import { renderPrompt, TOOL_ORDER_REST } from '@lyness/system-prompt'
...
/**
 * Unit coverage for the @lyness/agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldLynHome = process.env.LYNESS_HOME
  const oldAgentsHome = process.env.LYNESS_AGENTS_HOME
  process.env.LYNESS_HOME = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-home-'))
  process.env.LYNESS_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldLynHome === undefined) {
      delete process.env.LYNESS_HOME
    } else {
      process.env.LYNESS_HOME = oldLynHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.LYNESS_AGENTS_HOME
    } else {
      process.env.LYNESS_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('lyn-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@lyness/agent$'] },
      { package_blocklist: ['^@lyness/session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by lyness.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level lynHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'lyn-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'lyn-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-lyn-home-test') as ToolExecution['token'],
      callId: ToolCallId('agent-core-lyn-home'),
      rootCallId: ToolCallId('agent-core-lyn-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ LYNESS_HOME: home, LYNESS_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested LYN home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        lynHome: '/global-lyn-home',
        workspaceContext: false,
        skills: { filesystem: { lynHome: '/nested-lyn-home' } },
      })
    }).toThrow('agent-spine-demo: lynHome and skills.filesystem.lynHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
