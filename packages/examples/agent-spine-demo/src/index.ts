 ...
 * @module @lyness/agent-spine-demo
 */
...
import SessionTitleService, { type Config as SessionTitleConfig } from '@lyness/session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@lyness/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@lyness/tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@lyness/skill'
import * as SkillFileSystem from '@lyness/skill-filesystem'
import AgentRegistry from '@lyness/agent'
import GoalService, { type Config as GoalDomainConfig } from '@lyness/goal'
import * as goalSession from '@lyness/goal-round-driver'
import * as toolGoal from '@lyness/tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@lyness/jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@lyness/invariants'
...
import * as bashEnv from '@lyness/shell-env'
import * as workspaceContext from '@lyness/agent-instructions'
import * as toolSkill from '@lyness/tool-skill'
import * as toolJobs from '@lyness/tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@lyness/agent-loop'
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'lynHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.lynHome !== undefined ? { lynHome: config.lynHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedLynHome = config.skills?.filesystem?.lynHome
  if (config.lynHome !== undefined && nestedLynHome !== undefined
    && resolveLynHome(config.lynHome) !== resolveLynHome(nestedLynHome)) {
    throw new Error('agent-spine-demo: lynHome and skills.filesystem.lynHome must resolve to the same directory')
  }
  const lynHome = resolveLynHome(config.lynHome ?? nestedLynHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { lynHome }))
  }
