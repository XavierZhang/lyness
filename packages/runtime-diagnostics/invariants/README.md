...
Use the registry for compositions that want live diagnostics. [`lyn-sdk-minimal`](../../bundle/sdk-minimal/README.md) mounts it with the four core stateful companions — `lyn-session`, `lyn-agent`, `lyn-scope`, and `lyn-agent-loop`; `lyn-base` deliberately omits runtime diagnostics. Custom compositions mount the registry and add companions for any other loaded package whose contracts they want checked. Loading the registry alone installs no checks: it ships no product checks of its own, so a composition that never mounts a companion observes no diagnostic behavior.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-invariants) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-invariants) — every accepted config field and its source declaration.
