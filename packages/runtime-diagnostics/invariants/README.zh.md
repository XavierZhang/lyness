...
`lyn-invariants` 在 lyness 组合中运行包自有的运行时检查——不变量：任何包都可以发布一个 `./invariant` 配套入口，在组合运行期间验证其自身的持久关系（权威事件流与可变快照）。检查自动运行，失败的检查会报告归因到拥有被违反关系的包的 `InvariantError`。需要带全局开关与包名过滤器的自检诊断时选择它；标准 agent 组合已挂载它及四个核心配套入口，而单独加载服务不会安装任何检查。
...
需要实时诊断的组合请使用注册表。[`agent-spine-demo`](../../../packages/examples/agent-spine-demo/README.zh.md) 中的标准 agent 组合已挂载它及四个核心有状态配套入口——`lyn-session`、`lyn-agent`、`lyn-scope` 与 `lyn-agent-loop`。自定义组合挂载注册表，并为任何其他已加载、且希望检查其约定的包添加配套入口。单独加载注册表不会安装任何检查：它自身不携带任何产品检查，因此从不挂载配套入口的组合不会观察到任何诊断行为。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynessinvariants)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
| 配套入口 | 检查 |
|---|---|
| `lyn-session`、`lyn-agent`、`lyn-scope`、`lyn-agent-loop` | 会话日志包含关系与调用/结果跟踪、agent 状态转换、作用域过滤分发的主体、loop 构建请求重建 |
| `lyn-llm`、`lyn-llm-retry`、`lyn-tools`、`lyn-system-prompt` | LLM 流语法、重试失败形状、工具流水线阶段配对与冻结结果、提示词组装章节名 |
| `lyn-compaction`、`lyn-hook-protocol`、`lyn-sandbox-policy` | 压缩流配对、钩子调用/结果配对、沙箱 mode 值 |
| `lyn-fs`、`lyn-subagent`、`lyn-workflow`、`lyn-tool-workflow` | 文件系统事件身份、subagent 提供方与开始/结束配对、workflow 生命周期身份、workflow 记录形状 |
| `lyn-goal`、`lyn-goal-round-driver` | 持久 goal 流折叠与重建的继续提示词 |
...
| `lyn-agent-presets`、`lyn-session-title`、`lyn-plan-mode`、`lyn-schedule`、`lyn-webserver` | preset 挂载位置、标题来源引用、plan-mode 载荷、schedule 流、路由 disposer 对称性 |
...
- [生成的配置目录](../../../docs/config-catalog.zh.md#lynessinvariants)——每个受支持配置字段及其源声明。
...
- **请求重建只覆盖 loop 构建的请求**——`lyn-agent-loop` 配套入口只重建 loop 显式构建的请求；直接一次性 LLM 调用即使由调用方冻结或附加会话 id，仍不在此约定内。
