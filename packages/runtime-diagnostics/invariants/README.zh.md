...
`lyn-invariants` 在 lyness 组合中运行包自有的运行时检查——不变量：任何包都可以发布一个 `./invariant` 配套入口，在组合运行期间验证其自身的持久关系（权威事件流与可变快照）。检查自动运行，失败的检查会报告归因到拥有被违反关系的包的 `InvariantError`。需要带全局开关与包名过滤器的自检诊断时选择它；标准 agent（智能体）组合已挂载它及四个核心配套入口，而单独加载服务不会安装任何检查。
...
需要实时诊断的组合请使用注册表。[`lyn-sdk-minimal`](../../bundle/sdk-minimal/README.zh.md) 挂载它及四个核心有状态配套入口——`lyn-session`、`lyn-agent`、`lyn-scope` 与 `lyn-agent-loop`；`lyn-base` 刻意省略运行时诊断。自定义组合挂载注册表，并为任何其他已加载、且希望检查其约定的包添加配套入口。单独加载注册表不会安装任何检查：它自身不携带任何产品检查，因此从不挂载配套入口的组合不会观察到任何诊断行为。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-invariants)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
| 配套入口 | 检查 |
|---|---|
| `lyn-session`、`lyn-agent`、`lyn-scope`、`lyn-agent-loop` | 会话日志包含关系与调用/结果跟踪、agent 状态转换、经过作用域过滤的分发主体、loop 所构建请求的重建 |
| `lyn-llm`、`lyn-llm-retry`、`lyn-tools`、`lyn-system-prompt` | LLM（大语言模型）流语法、重试失败形状、工具流水线阶段配对与冻结结果、提示词组装章节名 |
| `lyn-compaction`、`lyn-hook-protocol`、`lyn-sandbox-policy` | 压缩（compaction）流配对、钩子调用/结果配对、沙箱 mode 值 |
| `lyn-fs`、`lyn-subagent`、`lyn-workflow`、`lyn-tool-workflow` | 文件系统事件身份、subagent 提供方与开始/结束配对、工作流生命周期身份、工作流记录形状 |
...
- [生成的配置目录](../../../docs/config-catalog.zh.md#lynesslyn-invariants)——每个受支持配置字段及其源声明。
