...
import AgentRegistry, { Inbox, type Agent } from '@lyness/agent'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'lyn> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'lyn> \'',
        LYNESS_SHELL: '1', LYNESS_SESSION_ID: 'agent', LYNESS_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07lyn> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo lyn> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo lyn> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', LYNESS_SHELL: '1', LYNESS_SESSION_ID: 'agent', LYNESS_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'lyn> ' : "function prompt { 'lyn> ' }\n",
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'lyn> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
