...
async function fixture() {
  const directory = await mkdtemp(join(tmpdir(), 'lyn-primary-runtime-'))
  roots.push(directory)
  const source = join(directory, 'resources')
  const root = join(directory, 'home', 'lyn-runtimes', 'lyn-primary-runtime')
...
it('uses a payload in place without copying and tolerates a payload without Node.js or pnpm', async () => {
  ...
  await expect(readFile(join(directory, 'home', 'lyn-runtimes', 'lyn-primary-runtime', 'runtime.json'))).rejects.toMatchObject({ code: 'ENOENT' })
