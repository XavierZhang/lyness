...
async function tempDir(name: string): Promise<string> {
  const dir = await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `lyn-${name}-`)))
  tempDirs.push(dir)
...
describe('lyn-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.lyn/skills'), 'same', 'user lyn skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.lyn/skills'), 'same', 'project lyn skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.lyn/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project lyn skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-lyn')
    ...
    await writeSkill(join(noGit, '.lyn/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.lyn/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.lyn/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.lyn/skills')
  ...
  it.each([false, true])('publishes regular-file paths for linked skills while retaining their resource roots (filesystem service: %s)', async (withFileSystem) => {
    ...
    await mkdir(join(home, '.lyn/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.lyn/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.lyn/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.lyn/skills/broken-link'))
    await symlink('/dev/null', join(home, '.lyn/skills/device-link'))
    ...
    const fiber = ctx.plugin(SkillFileSystem, { lynHome: join(home, '.lyn'), agentsHome: join(home, '.agents'), watch: false })
    await fiber
    ...
    try {
      ...
      for (const name of ['linked-dir', 'linked-flat']) {
        ...
        expect(loaded?.resourceBase).toEqual({ kind: 'directory', path: name === 'linked-dir' ? join(home, '.lyn/skills', name) : join(home, '.lyn/skills') })
        expect((await lstat(path)).isFile()).toBe(true)
      }
      await writeFile(join(external, 'replacement.md'), '---\nname: linked-flat\ndescription: Replacement\n---\n\nReplacement body.\n')
      await rm(join(home, '.lyn/skills/linked-flat.md'))
      await symlink(join(external, 'replacement.md'), join(home, '.lyn/skills/linked-flat.md'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { lynHome: join(home, '.lyn'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.lyn/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.lyn/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousLynHome = process.env.LYNESS_HOME
    const previousAgentsHome = process.env.LYNESS_AGENTS_HOME
    const previousBundledSkillDir = process.env.LYNESS_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.LYNESS_HOME = join(envHome, '.lyn')
      process.env.LYNESS_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.LYNESS_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.lyn/skills'), 'env-skill', 'Env skill')
      ...
      process.env.LYNESS_HOME = join(envHome, 'empty-lyn')
      delete process.env.LYNESS_BUNDLED_SKILL_DIR
      process.env.LYNESS_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.LYNESS_AGENTS_HOME
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        signal: new AbortController().signal,
        invalidate() {},
      }, { lynHome: join(envHome, 'empty-lyn') }).name).toBe('filesystem')
    } finally {
      if (previousLynHome === undefined) {
        delete process.env.LYNESS_HOME
      } else {
        process.env.LYNESS_HOME = previousLynHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.LYNESS_AGENTS_HOME
      } else {
        process.env.LYNESS_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.LYNESS_BUNDLED_SKILL_DIR
      } else {
        process.env.LYNESS_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
