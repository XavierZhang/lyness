...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-agent-default-model) lists every accepted field. `reasoningEffort` is optional; saving a selection without it removes that field from the profile’s complete config override.
