...
import LlmRuntime, { createUserMessage, ToolCallId, LlmError, ReasoningEffortId, StreamChunk } from '@lyness/llm'
...
describe('agent loop', () => {
  ...
  it('renders harness identity, then the persona, then tool guidance — with {{variables}} resolved', async () => {
    ...
    expect(request!.system).toBe('You are an AI agent powered by lyness.\n\nYou are a test agent on mock.\n\nUse the noop tool wisely.')
  ...
  it('resolves {{cwd}} from the agent session workspace (factory create with meta.cwd)', async () => {
    ...
    expect(adapter.requests[0]!.system).toBe('You are an AI agent powered by lyness.\n\nWorking in /work/space.')
  })
