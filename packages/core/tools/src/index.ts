 ...
 * execution pipeline.
 * @module @lyness/lyn-tools
 */

import { Context, Service } from '@lyness/cordis'
import z from '@lyness/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@lyness/lyn-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@lyness/lyn-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@lyness/lyn-llm'
import { HarnessError } from '@lyness/lyn-llm'
import type { Agent } from '@lyness/lyn-agent'
import type { UserMessage } from '@lyness/lyn-session'
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@lyness/lyn-util-values'
import type { PromptSection, ToolProviderResult } from '@lyness/lyn-system-prompt'
import type { PtcRuntime } from '@lyness/lyn-ptc-runtime'
 ...
 * instead of deriving them: the seam's `lyn-ptc-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@lyness/lyn-tools`
...
declare module '@lyness/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@lyness/lyn-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@lyness/lyn-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@lyness/lyn-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@lyness/lyn-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@lyness/lyn-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `lyn-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): PromptSection {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`lyn-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requirePtcTransport(): ToolDefinition {
    this.ptcTransport ??= createRunCodeTool(this, {
      ...
      resolveSandboxPolicy: (exec) => {
        ...
        if (policy === undefined) throw new Error('lyn-tools: confined PTC runtime requires sandboxPolicy')
  ...
  private requirePtcRuntime(mode: ToolPresentationMode): PtcRuntime {
    ...
    if (!runtime) {
      throw new Error(`lyn-tools: mode "${mode}" requires a PTC runtime — load a ctx.ptcRuntime implementation (e.g. @lyness/lyn-ptc-runtime-node) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`lyn-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `lyn-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('lyn.tool.execution') as ToolExecutionToken
}
