...
const IDENTITY = 'You are an AI agent powered by lyness.'
...
describe('SystemPrompt', () => {
  ...
  it('keeps reusable instructions identical across local environments', async () => {
    ...
    try {
      ...
      let environment = { model: 'model-a', cwd: '/alice/project', platform: 'darwin', source: '/alice/lyn', url: 'http://127.0.0.1:3080' }
      ...
      environment = { model: 'model-a', cwd: 'C:/bob/project', platform: 'win32', source: 'C:/bob/lyn', url: 'http://127.0.0.1:4080' }
      const second = renderPrompt(await ctx.systemPrompt.assemble())
      const prefix = [IDENTITY, 'Model model-a.', ...reusable].join('\n\n') + '\n\n'
      expect(first).toBe(prefix + '/alice/lyn\n\nhttp://127.0.0.1:3080\n\nIn /alice/project on darwin.')
      expect(second).toBe(prefix + 'C:/bob/lyn\n\nhttp://127.0.0.1:4080\n\nIn C:/bob/project on win32.')
  ...
  describe('built-in sections', () => {
    ...
    it('registers the harness identity and the configured deployment persona', async () => {
      const ctx = new Context()
      await ctx.plugin(SystemPrompt, { personaPrefix: 'You are lyness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are lyness.', 'Be precise.', 'cwd: /tmp', ''])
    expect(assembly.contexts).toEqual([
