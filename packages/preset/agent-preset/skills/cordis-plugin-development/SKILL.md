...
2. Package documentation: `Config.listConfigs` with `name` set to the package finds its entries; querying one `entry` returns its `packageDir`, the resolved package directory. Read `<packageDir>/README.md`. Bundled packages resolve from the lyn installation and profile-installed bundles from the profile, so never guess the path from `$LYNESS_PROFILE_DIR`.
3. Source: installed packages ship built `lib/index.js` and `lib/types/**/*.d.ts` with JSDoc under that same `packageDir`, not `src/`; a source checkout of LYN has `packages/<group>/<name>/src`. Read them when inspection and the README leave a question open, and start source-level diagnosis from a concrete installation or runtime failure.

`LYNESS_PROFILE` (profile name) and `LYNESS_PROFILE_DIR` (its directory, whose `node_modules` holds only profile-installed bundles) are set in every shell call of a profile-launched Harness and absent when the Harness was booted without a profile. Bash reads them as `$LYNESS_PROFILE`; PowerShell, which the Windows preset uses, reads them as `$env:LYNESS_PROFILE`. With `lyn` on the PATH, `lyn --profile "$LYNESS_PROFILE" --dump-config` prints the composed profile.
