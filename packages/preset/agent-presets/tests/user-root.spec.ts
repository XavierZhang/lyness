 ...
 * authors into `<lynHome>/.agent-presets`, the way `lyn-skill-filesystem` owns
 * `<lynHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$LYNESS_HOME` is repointed per test because the derived root is resolved in the
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'lyn-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'lyn-preset-explicit-'))
