...
import { getLynRuntimeVersion, type ProfileContext } from '@lyness/lyn-app-boot'
...
it('mounts a profile-denied row disabled and the same row active once exempted', async () => {
  const ctx = await harness()
  onTestFinished(() => ctx.fiber.dispose())
  const dir = realpathSync(mkdtempSync(join(tmpdir(), 'lyn-preset-compat-')))
  ...
  writeFileSync(join(pluginDir, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@lyness/lyn': '<0.0.0' },
  }))
  ...
  await ctx.plugin({ inject: ['loader'], async apply(owner: Context) {
    ...
    writeFileSync(compatibilityPath, JSON.stringify({ 'incompatible-preset-plugin@1.0.0': [getLynRuntimeVersion()] }))
