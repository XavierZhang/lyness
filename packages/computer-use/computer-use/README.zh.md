...
服务没有配置项。提供方插件注入 `computerUse` 并调用 `ctx.computerUse.register(ComputerUseProviderName(name))`；该品牌类型从 `@lyness/lyn-computer-use/brand` 导出。返回的 effect 清理函数释放此次注册。
...
- **共享桌面** — 并发 Session 和独立 LYN 进程可以操作同一桌面；调用方协调完整的计算机操作流程。
