...
The service has no configuration. Provider plugins inject `computerUse` and call `ctx.computerUse.register(ComputerUseProviderName(name))`; the brand is exported from `@lyness/lyn-computer-use/brand`. The returned effect disposer releases that registration.
...
- **Shared desktop** — concurrent Sessions and separate LYN processes can operate the same desktop; callers coordinate whole computer-use workflows.
