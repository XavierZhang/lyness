...
`lyn-shell-env` provides the trusted `LYNESS_*` environment that every model shell call — bash or pwsh — runs with: built-in facts such as `LYNESS_HOME`, `LYNESS_SHELL=1`, the agent's `LYNESS_SESSION_ID`, and the launched profile's `LYNESS_PROFILE` and `LYNESS_PROFILE_DIR`. Plugin authors can register their own facts with declared keys, collected per execution and disposed with their plugin; duplicate ownership or undeclared runtime keys fail loudly instead of silently overwriting. The registry changes nothing else the model sees — the shell tools own their own schemas and prompts. Choose it in any composition that mounts a model shell tool; configuration only picks the Harness home directory.
...
Every call receives `LYNESS_HOME` (the absolute Harness home), `LYNESS_SHELL=1`, and, for agent calls, `LYNESS_SESSION_ID` (the calling session's id). When the launcher provided a profile context, every call also receives `LYNESS_PROFILE` (the profile name) and `LYNESS_PROFILE_DIR` (its absolute directory; its `node_modules` holds only profile-installed packages, while the harness's own bundles resolve from the lyn installation); compositions booted without a profile omit both.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-shell-env) is the exhaustive source for every accepted field and its JSDoc.

### What can go wrong

Two contributors declaring the same key, or a contributor claiming a reserved built-in (`LYNESS_HOME`, `LYNESS_SHELL`, `LYNESS_SESSION_ID`, `LYNESS_PROFILE`, `LYNESS_PROFILE_DIR`), fails plugin load loudly. A `LYNESS_*` key must be all-caps with underscores (for example `LYNESS_REGION`), and a missing description fails registration.
...
- **Built-ins stay here.** `LYNESS_HOME`, `LYNESS_SHELL`, `LYNESS_SESSION_ID`, `LYNESS_PROFILE`, and `LYNESS_PROFILE_DIR` are reserved for the registry; contributors cannot claim them.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-shell-env) — every accepted config field and its source declaration.
...
- **`list()` enumerates plugin-contributed variables only** — registry-owned built-ins (`LYNESS_HOME`, `LYNESS_SHELL`, `LYNESS_SESSION_ID`, `LYNESS_PROFILE`, `LYNESS_PROFILE_DIR`) are not included, so diagnostics, prompt, or UI code must not treat `list()` as an exhaustive environment catalog.
