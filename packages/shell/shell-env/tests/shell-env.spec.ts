...
describe('ShellEnvRegistry', () => {
    ...
    expect(ctx.shellEnv.list()).toEqual([
      {
        ...
        key: 'LYNESS_SESSION_JSONL',
      },
    ])
  })

  it('the persistence contributor resolves LYNESS_SESSION_JSONL only for a jsonl backend', async () => {
    const ctx = new Context()
    await ctx.plugin(BashEnvPlugin)
    ctx.provide('sessionPersistence', {
      locate: () => ({ kind: 'jsonl' as const, path: 'C:\\sessions\\s.jsonl' }),
    })
    expect(ctx.shellEnv.collect(execution('sess-p')).LYNESS_SESSION_JSONL).toBe('C:\\sessions\\s.jsonl')
  })
  ...
  it('the persistence contributor omits the variable for a non-jsonl backend', async () => {
    ...
    expect(ctx.shellEnv.collect(execution('sess-p'))).not.toHaveProperty('LYNESS_SESSION_JSONL')
  })
