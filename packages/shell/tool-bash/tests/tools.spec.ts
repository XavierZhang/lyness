...
import SystemPrompt, { FIRST_PARTY_SECTION_ORDER } from '@lyness/system-prompt'
...
const spillDir = mkdtempSync(join(tmpdir(), 'lyn-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-lyn' })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingLynHome = join(spillDir, 'lyn-home')
   ...
   * (the credential scrub in lyn-bash-local is the security control; see the
  ...
  class RecordingBashExecutor extends ShellExecutor {
    ...
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      return {
        ...
        ...request.lynEnv !== undefined ? { lynEnv: request.lynEnv } : {},
        sandboxPolicy: request.sandboxPolicy,
      }
    }
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { lynHome: recordingLynHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$LYNESS_*')
    expect(description).not.toContain('LYNESS_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.lynEnv).toEqual({
      LYNESS_HOME: recordingLynHome,
      LYNESS_SESSION_ID: 'request-fg',
      LYNESS_SESSION_JSONL: path,
      LYNESS_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { LYNESS_SESSION_ID: 'spoofed', LYNESS_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.LYNESS_SESSION_ID
    ...
    expect(bash.requests[0]?.lynEnv).toEqual({
      LYNESS_HOME: recordingLynHome,
      LYNESS_SESSION_ID: 'request-id-only',
      LYNESS_SHELL: '1',
    })
    expect(process.env.LYNESS_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.lynEnv)).toEqual([
      {
        LYNESS_HOME: recordingLynHome,
        LYNESS_SESSION_ID: 'request-parent',
        LYNESS_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        LYNESS_SHELL: '1',
      },
      {
        LYNESS_HOME: recordingLynHome,
        LYNESS_SESSION_ID: 'request-child',
        LYNESS_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        LYNESS_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.lynEnv?.LYNESS_SESSION_JSONL).not.toBe(bash.requests[1]?.lynEnv?.LYNESS_SESSION_JSONL)
  })
