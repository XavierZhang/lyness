...
`lyn-tool-bash` runs Bash commands and returns stdout, stderr, and exit markers. Each call uses a fresh shell; cwd, variables, and functions do not persist. With a job registry composed, every command is a job from its start: `run_in_background` returns the id at once, a foreground command that outlives its timeout returns the same id, and `job_output`/`job_kill` collect and stop it. Commands receive the managed `LYNESS_*` environment; sandbox denials can be retried once with wider `sandbox_permissions`, a `justification`, and user approval. Nonzero exits are results for the agent to interpret. Mount an executor such as `lyn-bash-local` or `lyn-bash-sandbox` with `lyn-shell-env`.
...
Load this plugin in any composition where the agent should run bash commands: it registers the `bash` tool once an executor provider and the `lyn-shell-env` registry are mounted, and stays pending until the `tools`, `shell`, `systemPrompt`, and `shellEnv` services exist.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesslyn-tool-bash) is the exhaustive source for every accepted field and its JSDoc; the generated [tool catalog](../../../docs/tool-catalog.md#lynesslyn-tool-bash) carries the full argument schema.
...
- **Request from named args only.** The tool never exposes `stdin`, `env`, or `stdoutMaxBytes`; it builds each request from command/workdir/timeout/signal fields plus the registry-collected `lynEnv`, so model-supplied keys cannot replace managed values.
...
- [shell-env](../shell-env/README.md) — the managed `LYNESS_*` environment every call receives.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#lynesslyn-tool-bash) — the exact `bash` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesslyn-tool-bash) — every accepted config field and its source declaration.
...
The model sees the generated [`bash` schema](../../../docs/tool-catalog.md#lynesslyn-tool-bash). `run_in_background` appears only when this producer enables it and a job registry is composed; `sandbox_permissions` and `justification` appear only when the mounted executor advertises sandboxing; the justification asks the model to use the language of the current user request. Agent-scoped tool restrictions can remove the definition for that agent.
