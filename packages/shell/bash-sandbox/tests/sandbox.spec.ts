...
import type { ShellRunResult, CollectedOutput } from '@lyness/shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@lyness/sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@lyness/sandbox'
...
import type { Config } from '@lyness/bash-sandbox'
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'lyn-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
