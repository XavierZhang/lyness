...
import type { ShellRunResult, CollectedOutput } from '@lyness/lyn-shell'
import SessionProjectionRegistry from '@lyness/lyn-session-projection'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@lyness/lyn-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@lyness/lyn-sandbox'
...
import type { Config } from '@lyness/lyn-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'lyn-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'LYNESS_WRAP=1', 'bash', '-c', 'printf "%s" "$LYNESS_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$LYNESS_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'lyn-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.LYNESS_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        lynEnv: { LYNESS_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'lyn-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const deniedRoot = mkdtempSync(join(tmpdir(), 'lyn-sandbox-denied-'))
    try {
