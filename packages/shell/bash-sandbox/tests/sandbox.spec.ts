...
import type { CollectedOutput, ShellExecSpec, ShellExecution, ShellRunResult } from '@lyness/lyn-shell'
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    ...
    const spawn = vi.spyOn(ctx.subprocess, 'spawn')
    const result = await run(bash, bash.resolve({ command: 'printf "%s" "$LYNESS_WRAP"' }))
