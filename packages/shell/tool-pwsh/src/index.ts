 ...
 * waits on its job until the timeout passes), the managed `LYNESS_*` environment through the
...
import type { GenericCallView, TerminalCallView, ToolDefinition, ToolExecution, ToolResult, ToolResultView } from '@lyness/lyn-tools'
import { HarnessError } from '@lyness/lyn-llm'
import type { Agent } from '@lyness/lyn-agent'
import type { JobId, JobRegistry, JobView } from '@lyness/lyn-jobs'
...
import { ESCALATION_TARGETS, approveEscalation, sandboxPermissionsDescription, validateEscalationArgs } from '@lyness/lyn-sandbox'
import type { SandboxPolicyService } from '@lyness/lyn-sandbox-policy'
import type { ShellExecRequest, ShellExecSpec, ShellExecution, ShellRunResult } from '@lyness/lyn-shell'
...
/* jscpd:ignore-start -- minimal mirror of lyn-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(windowsSandbox: boolean): string {
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Managed `$env:LYNESS_*` variables expose current harness environment facts. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror lyn-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of lyn-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of lyn-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  const pwshTool = (jobs: JobRegistry | undefined): ToolDefinition => {
    ...
    /* jscpd:ignore-start -- the job start and wait mirror lyn-tool-bash's by design (pwsh-tool-and-executor Agent Note). */
    ...
    return defineTool({
      ...
      /* jscpd:ignore-start -- deliberate mirror of lyn-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
      parameters: {
      ...
      /* jscpd:ignore-end */
      output: {
        // The foreground result wire shape mirrors lyn-tool-bash's by contract —
        // consumers of one must accept the other (see the pwsh-tool-and-executor
        // Agent Note).
        /* jscpd:ignore-start -- deliberate result-schema symmetry with lyn-tool-bash. */
        schema: {
          oneOf: [
            {
              type: 'object',
      ...
      /* jscpd:ignore-start -- the execute path mirrors lyn-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
      async execute(args: PwshToolArgs, exec) {
        ...
        const request: ShellExecRequest = {
          ...
          lynEnv: ctx.shellEnv.collect(exec),
