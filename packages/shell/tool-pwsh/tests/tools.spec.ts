 ...
 * managed `LYNESS_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellExecution, ShellProcess, ShellRunResult } from '@lyness/lyn-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, lynHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, lynHome === undefined ? {} : { lynHome })
...
async function setupWithJobs(toolConfig: Partial<ToolPwsh.Config> = {}, lynHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed LYNESS_* environment', async () => {
    const lynHome = mkdtempSync(join(tmpdir(), 'lyn-tool-pwsh-home-'))
    tempDirs.push(lynHome)
    const { ctx, bash } = await setup({}, lynHome)
    ...
    expect(request?.lynEnv).toEqual({
      LYNESS_HOME: lynHome,
      LYNESS_SHELL: '1',
      LYNESS_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const lynEnv = bash.requests[0]?.lynEnv
    expect(lynEnv).toBeDefined()
    expect(lynEnv?.['LYNESS_SHELL']).toBe('1')
    expect(lynEnv?.['LYNESS_HOME']).toEqual(expect.any(String))
    expect(lynEnv).not.toHaveProperty('LYNESS_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'lyn-tool-pwsh-policy-'))
