...
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputReader } from '@lyness/lyn-subprocess'
...
describe('LocalBashExecutor.run', () => {
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(run(bash, bash.resolve({ command: 'true', workdir: '/nonexistent-lyn' }))).rejects.toThrow(/ENOENT/)
  })
...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('an asynchronous creation failure settles as killed with a stage-neutral note', async () => {
    const { bash } = await setup()
    const proc = (await start(bash, bash.resolve({ command: 'true', workdir: '/nonexistent-lyn' })))
