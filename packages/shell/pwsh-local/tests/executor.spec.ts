...
import type { ShellExecSpec, ShellExecution, ShellProcess, ShellRunResult } from '@lyness/lyn-shell'
...
function commandBarrier() {
  const dir = mkdtempSync(join(tmpdir(), 'lyn-pwsh-barrier-'))
  tempDirs.push(dir)
  const path = join(dir, 'release')
  return {
    command: 'while (-not (Test-Path -LiteralPath $env:LYNESS_TEST_RELEASE)) { Start-Sleep -Milliseconds 20 }',
    env: { LYNESS_TEST_RELEASE: path },
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'lyn-pwsh-resolve-'))
    tempDirs.push(dir)
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'lyn-pwsh-resolve-link-'))
    tempDirs.push(dir)
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'lyn-pwsh-resolve-dir-'))
    tempDirs.push(dir)
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'lyn-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'lyn-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(run(bash, bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-lyn' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/lynEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:LYNESS_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    const { bash } = await setup()
    const proc = (await start(bash, bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:LYNESS_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
  ...
  it('an asynchronous creation failure settles as killed with a stage-neutral note', async () => {
    const { bash } = await setup()
    const proc = (await start(bash, bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-lyn' })))
