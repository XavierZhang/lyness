 ...
 * @module @lyness/lyn-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSessionId } from '@lyness/lyn-terminal'
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__LYNESS_PERSISTENT_BASH_START_${nonce}__`,
    end: `__LYNESS_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __lyn_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__lyn_persistent_bash_status"`
}
