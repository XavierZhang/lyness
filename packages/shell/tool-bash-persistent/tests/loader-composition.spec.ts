...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'lyn-persistent-bash-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@lyness/lyn-tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('LYNESS_PERSISTENT_BASH')
