 ...
 * harness home resolved by {@link resolveLynHome} (`$LYNESS_HOME` > `~/.lyn`),
 ...
 * machine: every process sharing one `$LYNESS_HOME` reports the same id, and
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `LYNESS_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveLynHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
