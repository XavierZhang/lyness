 ...
 * @module lyn-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for lyness-managed child environment facts. */
export const LYNESS_ENV_PREFIX = 'LYNESS_' as const

/** One environment key inside the managed {@link LYNESS_ENV_PREFIX} namespace. */
export type LynEnvironmentKey = `${typeof LYNESS_ENV_PREFIX}${string}`

/** Trusted lyness variables for one child-process execution. */
export type LynEnvironment = Readonly<Record<LynEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@lyness/lyn-terminal` without a cross-seam dependency;
