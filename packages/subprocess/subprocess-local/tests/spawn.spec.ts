...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${LYNESS_STALE:-absent}|$LYNESS_SHELL|$LYNESS_SESSION_ID]"':
      return node('console.log("[" + [process.env.LYNESS_STALE ?? "absent", process.env.LYNESS_SHELL, process.env.LYNESS_SESSION_ID].join("|") + "]")')
    case 'echo "[${LYNESS_TEST_API_KEY:-absent}|${LYNESS_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${LYNESS_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.LYNESS_TEST_API_KEY ?? "absent", process.env.LYNESS_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.LYNESS_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'lyn-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-lyn-test' })).done)
...
describe('OutputCollector', () => {
  ...
  it('keeps collecting when the spill directory has been removed (ENOENT on open)', () => {
    const removedDir = mkdtempSync(join(tmpdir(), 'lyn-subprocess-removed-'))
...
describe('spill failure reporting without an owner logger', () => {
  it('writes one stderr line for a bare spawn whose spill directory is gone', async () => {
    ...
    try {
      ...
      expect(result.stdout.text).toContain('line-0200')
      expect(result.stdout.spillPath).toBeUndefined()
      const lines = stderr.mock.calls.map(call => String(call[0])).filter(line => line.includes('lyn-subprocess-local:'))
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ LYNESS_X: '1' }).LYNESS_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-lyn-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-lyn-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient LYN env vars from child processes', async () => {
    process.env.LYNESS_TEST_API_KEY = 'super-secret'
    process.env.LYNESS_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.LYNESS_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${LYNESS_TEST_API_KEY:-absent}|${LYNESS_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${LYNESS_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.LYNESS_TEST_API_KEY
      delete process.env.LYNESS_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.LYNESS_TEST_PLAIN
    }
  })

  it('forwards explicit LYNESS_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient LYNESS_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.LYNESS_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${LYNESS_STALE:-absent}|$LYNESS_SHELL|$LYNESS_SESSION_ID]"', {
        env: { LYNESS_SHELL: '1', LYNESS_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.LYNESS_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/lyn-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    defaultSpillDir = dir
    expect(dir).toMatch(/lyn-subprocess-/)
