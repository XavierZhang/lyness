...
async function runWindows(
  ...
  start: unknown = { type: 'start', cwd: 'C:\\target', env: { TARGET: 'yes', lyn_subprocess_runner: 'restored' } },
...
describe('closed runner protocol', () => {
  ...
  it('removes the private directory when request creation fails partway through', () => {
    const isolatedTmp = mkdtempSync(join(tmpdir(), 'lyn-launch-failure-spec-'))
  ...
  it('contains cleanup failures and removes a substituted symlink only', () => {
    ...
    const target = join(tmpdir(), `lyn-runner-cleanup-target-${String(process.pid)}`)
    const link = join(tmpdir(), `lyn-runner-cleanup-link-${String(process.pid)}`)
...
describe('runner launch inputs', () => {
  ...
  it('resolves the source runner entry and checks concrete paths without executing it', () => {
    ...
    expect(runnerInvocationAvailable(['/definitely/missing-lyn-runner'])).toBe(false)
  ...
  it('loads the source runner from an isolated application cwd', () => {
    const directory = mkdtempSync(join(tmpdir(), 'lyn-runner-cwd-'))
  ...
  it('resolves Windows executables with target-cwd and PATH search semantics', () => {
    // A bare command consults the host environment for PATH and for the
    // cwd-search opt-out, so every such case pins it. Inheriting process.env
    // makes the expectations depend on the developer's shell: a host that
    // exports NoDefaultCurrentDirectoryInExePath drops the target-cwd probes.
    const noHostEnvironment: Readonly<Record<string, string | undefined>> = {}
    const probed: string[] = []
    ...
    }, exists, noHostEnvironment)).toBe('C:\\tools\\git\\bin\\bash.exe')
    ...
    expect(resolveWindowsExecutable('local.exe', 'C:\\target', {}, candidate =>
      candidate === 'C:\\target\\local.exe', noHostEnvironment)).toBe('C:\\target\\local.exe')
    ...
    expect(resolveWindowsExecutable('tool', 'C:\\target', {
      PATH: 'D:relative',
    }, candidate => candidate === 'D:relative\\tool.exe', noHostEnvironment)).toBe('D:relative\\tool.exe')
    expect(resolveWindowsExecutable('tool.', 'C:\\target', {}, candidate =>
      candidate === 'C:\\target\\tool.exe', noHostEnvironment)).toBe('C:\\target\\tool.exe')
    expect(resolveWindowsExecutable('.\\missing', 'C:\\target', {}, () => false))
      .toBeUndefined()

    expect(resolveWindowsExecutable('tool', 'C:\\target', {
      PATH: ';;C:\\bin',
    }, candidate => candidate === 'C:\\bin\\tool.exe', noHostEnvironment)).toBe('C:\\bin\\tool.exe')
    expect(resolveWindowsExecutable('tool', 'C:\\target', {
      PATH: '"";C:\\bin',
    }, candidate => candidate === 'C:\\bin\\tool.exe', noHostEnvironment)).toBe('C:\\bin\\tool.exe')
    expect(resolveWindowsExecutable('tool', 'C:\\target', {
      PATH: '"unterminated',
    }, candidate => candidate === 'C:\\target\\unterminated\\tool.exe', noHostEnvironment))
    ...
    const directory = mkdtempSync(join(tmpdir(), 'lyn-windows-resolver-'))
    scratch.push(directory)
...
describe('Linux one-shot exec bootstrap', () => {
  ...
  it.skipIf(process.platform === 'win32')('preserves symlink-sensitive parent traversal in PATH candidates', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-linux-path-symlink-'))
    scratch.push(root)
...
describe('Windows Job runner protocol owner', () => {
  ...
  it('sends target-exit only after suspended Job launch and closes runner stdio', async () => {
    ...
    expect(native.resolveWindowsExecutable).toHaveBeenCalledWith(
      'tool.exe',
      'C:\\target',
      { TARGET: 'yes', lyn_subprocess_runner: 'restored' },
      undefined,
      { SAFE: 'bootstrap' },
    )
    expect(native.spawnCurrentTokenJobProcess).toHaveBeenCalledWith(expect.anything(), {
      ...
      env: { TARGET: 'yes', lyn_subprocess_runner: 'restored' },
