...
import { SUBPROCESS_CONTROL_FD } from '@lyness/lyn-subprocess/control'

/** The one private environment variable consumed before target state is restored. */
export const SUBPROCESS_RUNNER_ENV = 'LYNESS_SUBPROCESS_RUNNER' as const
...
export function spawnRunnerInvocation(): RunnerInvocation {
  ...
  if (extname(fileURLToPath(import.meta.url)) !== '.ts') {
    return [process.execPath, fileURLToPath(import.meta.resolve('@lyness/lyn-subprocess-local/runner'))]
  }
  return [
    process.execPath,
    '--import',
