...
import type { SubprocessTerminalActivity, SubprocessTerminalSpawnSpec } from '@lyness/lyn-subprocess'
...
export function prepareShellActivity(
  ...
  const directory = mkdtempSync(join(tmpdir(), 'lyn-shell-'))
  ...
  try {
    if (shell === 'bash') {
      ...
      writeFileSync(rc, [
        ...
        '  __lyn_shell_pid=$BASHPID; __lyn_shell_sequence=0',
        '  __lyn_shell_idle() {',
        '    local result=$?',
        '    if [[ $BASHPID == "$__lyn_shell_pid" ]]; then',
        '      (( ++__lyn_shell_sequence ))',
        '      local activity=idle',
        `      builtin trap -p >| ${guards}`,
        `      [[ ! -s ${guards} ]] || activity=unknown`,
        `      builtin printf '%s:%s:%s\\n' "$BASHPID" "$__lyn_shell_sequence" "$activity" >| ${state}`,
        '    fi',
        '    return "$result"',
        '  }',
        '  if [[ $(declare -p PROMPT_COMMAND 2>/dev/null) == "declare -a "* ]]; then',
        '    PROMPT_COMMAND+=(__lyn_shell_idle)',
        '  else',
        '    PROMPT_COMMAND="${PROMPT_COMMAND}"$\'\\n\'"__lyn_shell_idle"',
        '  fi',
        `  PS0+=${quote(`$(builtin printf '%s:%s:busy' "$__lyn_shell_pid" "$__lyn_shell_sequence" >| ${state})`)}`,
        'fi',
        '',
      ].join('\n'), { mode: 0o600, flag: 'wx' })
    ...
    writeFileSync(join(directory, '.zshenv'), [
      ...
      'typeset -g __lyn_shell_pid=$$ __lyn_shell_sequence=0',
      '__lyn_shell_activity() {',
      '  (( ZSH_SUBSHELL == 0 && $$ == __lyn_shell_pid )) || return',
      '  (( ++__lyn_shell_sequence ))',
      `  builtin printf '%s:%s:%s\\n' "$$" "$__lyn_shell_sequence" "$1" >| ${state}`,
      '  return 0',
      '}',
      '__lyn_shell_idle() {',
      `  { builtin trap; zle -F; } >| ${guards}`,
      '  if [[ $CONTEXT != start || -n $BUFFER ]]; then __lyn_shell_activity busy',
      `  elif [[ -s ${guards}` + ' || -n ${(k)functions[(I)TRAP*]} ]]; then __lyn_shell_activity unknown',
      '  else __lyn_shell_activity idle; fi',
      '}',
      '__lyn_shell_busy() { __lyn_shell_activity busy }',
      '__lyn_shell_init() {',
      '  autoload -Uz add-zle-hook-widget add-zsh-hook',
      '  add-zle-hook-widget line-init __lyn_shell_idle',
      '  add-zle-hook-widget line-finish __lyn_shell_busy',
      '  add-zsh-hook preexec __lyn_shell_busy',
      '  precmd_functions=(${precmd_functions:#__lyn_shell_init})',
      '}',
      'typeset -ga precmd_functions',
      'precmd_functions+=(__lyn_shell_init)',
      '',
    ].join('\n'), { mode: 0o600, flag: 'wx' })
