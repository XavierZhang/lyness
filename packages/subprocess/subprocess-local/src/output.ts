...
import type { CollectedOutput } from '@lyness/lyn-subprocess'
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'lyn-subprocess-'))
...
function reportSpillFailureToStderr(error: unknown, label: string): void {
  process.stderr.write(`lyn-subprocess-local: ${label} spill failed; only the in-memory tail is retained: ${String(error)}\n`)
}
  ...
  private spillAll(spill: SpillOptions, chunk: Buffer): void {
    ...
    try {
      if (this.spillFd === undefined) {
        ...
        const file = join(
          spill.dir,
          `lyn-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
        )
      ...
      try {
        ...
        process.stderr.write(`lyn-subprocess-local: spill failure reporter threw: ${String(reporterFailure)}\n`)
      }
    }
  }
