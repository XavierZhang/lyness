...
Mount this provider when a deployment has local language servers — for example `typescript-language-server` — and wants the harness to navigate code through them. It needs filesystem and subprocess providers for the same execution world, plus the `lyn-lsp` seam and, for model access, `lyn-tool-lsp`.
...
| `env` | `{}` | Extra env merged over the credential-scrubbed ambient env; variables matching `KEY`/`PASSWORD`/`SECRET`/`TOKEN` and all `LYNESS_*` names are not forwarded |
...
- [lyn-lsp](../lsp/README.md) — the seam this provider registers against.
- [lyn-tool-lsp](../tool-lsp/README.md) — the model-facing tool over the seam.
...
Indirectly, through `lyn-tool-lsp`, which surfaces this provider's normalized results while this host contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `lyn-tool-lsp` owns request-prefix changes.
