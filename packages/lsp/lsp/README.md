...
- **Capability seam, Service Definition role.** The package owns `ctx.lsp` and the provider registry; providers register capabilities, not tools, and `lyn-tool-lsp` is the only owner of the model-facing surface.
...
- [lyn-lsp-stdio](../lsp-stdio/README.md) — the stdio provider that registers against this seam.
- [lyn-tool-lsp](../tool-lsp/README.md) — the model-facing tool over this seam.
...
Indirectly, through `lyn-tool-lsp`, which owns the model-facing `lsp` schema, prompt guidance, and rendered results while this registry contributes no prompt or schema itself.
