...
[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-experimental-browser-use-playwright-mcp)列出接受的字段。浏览器模式由 profile 或 preset 选择。子进程会清空继承的 `PLAYWRIGHT_MCP_*` 选项，避免其替换该配置。
...
- 工具 schema 跟随固定的实验依赖版本，不承诺 LYN 稳定性。
