...
import { boot, initProfile, readProfilePatches, type ProfileContext } from '@lyness/lyn-app-boot'
import ConfigEditor from '@lyness/lyn-config-editor'
import Settings from '@lyness/lyn-settings'
...
it('saves language and provider preferences without remounting and restores them at restart', async () => {
  const home = realpathSync(mkdtempSync(join(tmpdir(), 'lyn-speech-configuration-')))
  ...
  try {
    ...
    writeFileSync(join(bundle, 'package.json'), JSON.stringify({ name: 'test-bundle', version: '1.0.0', lyn: { bundle: { patch: 'cordis.patch.yml' } } }))
