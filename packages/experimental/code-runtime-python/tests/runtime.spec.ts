 ...
 * `lyn-code-runtime-python-*` directories inside the sampling window, which a
...
vi.mock('node:fs', async (importOriginal) => {
  ...
  return {
    ...
    mkdtempSync(prefix: string): string {
      ...
      if (basename(prefix).startsWith('lyn-code-runtime-python-')) stagedDirs.push(dir)
      return dir
    },
  }
})
...
describe('PythonCodeRuntime — seam descriptors and misuse', () => {
  ...
  it('runs the interpreter from materialized scripts outside the package, and removes them per run', async () => {
    ...
    expect(basename(dir)).toMatch(/^lyn-code-runtime-python-/)
