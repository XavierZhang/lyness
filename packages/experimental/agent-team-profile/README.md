...
`lyn-experimental-agent-team-profile` enables [Agent Teams](../agent-team/README.md) collaboration, tools, and Web UI with one bundle. Delegate work to teammates and view members, shared tasks, and teammate sessions in Web. Ordinary subagent delegation and overlapping global child controls are disabled; Workflow can still create fresh children. The bundle ships with lyn switched off. Enable it from the Plugins page or add it to an initialized profile.
...
In an existing profile’s `package.json`, keep `@lyness/lyn-experimental-agent-team-profile` in `lyn.profile.bundles` and remove the separate `@lyness/lyn-experimental-agent-team-web-profile` entry. User patches targeting `ui-agent-team` still apply.
...
- **Base profile required** — the patch depends on row ids and Subagent providers supplied by `lyn-base`; it is not a standalone profile.
