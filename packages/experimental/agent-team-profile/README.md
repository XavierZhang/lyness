---
description: "Published experimental Agent Teams profile layer over lyn-base with Team-scoped coordination tools and one-shot delegation."
kind: "package-bundle"
---
...
The package's runtime content is [`cordis.patch.yml`](cordis.patch.yml). Applied after `lyn-base`, the patch disables `tool-subagent-control` and `tool-subagent-list-agents`; sets the fresh and fork Subagent rows to `one-shot`; and inserts the Team service and tool rows with explicit providers and limits.
