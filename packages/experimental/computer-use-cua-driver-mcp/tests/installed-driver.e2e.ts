...
const executable = process.env.LYNESS_COMPUTER_USE_MCP_EXECUTABLE

function driverArguments(): string[] {
  const raw = process.env.LYNESS_COMPUTER_USE_MCP_ARGS
  ...
  if (!Array.isArray(args) || !args.every((arg: unknown): arg is string => typeof arg === 'string')) {
    throw new Error('LYNESS_COMPUTER_USE_MCP_ARGS must be a JSON array of strings')
  }
  return args
}
...
it.skipIf(executable === undefined || executable === '')(
  ...
  async ({ signal }) => {
    if (executable === undefined || !isAbsolute(executable)) {
      throw new Error('LYNESS_COMPUTER_USE_MCP_EXECUTABLE must be an absolute executable path')
    }
