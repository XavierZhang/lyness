...
import ComputerUse from '@lyness/lyn-computer-use'
import { ComputerUseProviderName } from '@lyness/lyn-computer-use/brand'
import SystemPrompt from '@lyness/lyn-system-prompt'
import ToolRuntime from '@lyness/lyn-tools'
import LlmRuntime, { LlmAdapter, ToolCallId, createUserMessage } from '@lyness/lyn-llm'
...
async function load(mode?: string): Promise<{ ctx: Context; root: string; model: ScreenshotModel }> {
  const root = await mkdtemp(join(tmpdir(), 'lyn-computer-use-mcp-'))
  roots.push(root)
  const model = new ScreenshotModel()
  const modules = new Map<string, unknown>([
    ['@lyness/lyn-computer-use', ComputerUse],
    ...
    ['@lyness/lyn-attachment-local', LocalAttachmentStore],
    ['@fixture/model', { inject: ['llm'], apply(ctx: Context) { ctx.effect(() => ctx.llm.registerAdapter(['fixture'], model)) } }],
    ['@lyness/lyn-experimental-computer-use-cua-driver-mcp', Provider],
  ])
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, JSON.stringify([...modules.keys()].map(name => ({
    id: name === '@lyness/lyn-experimental-computer-use-cua-driver-mcp' ? 'computer-use-driver' : undefined,
    name,
    config: name === '@lyness/lyn-experimental-computer-use-cua-driver-mcp'
      ? {
      ...
      : name === '@lyness/lyn-attachment-local'
        ? { lynHome: root }
        : name === '@lyness/lyn-agent-loop' ? { agents: [] } : {},
  }))))
