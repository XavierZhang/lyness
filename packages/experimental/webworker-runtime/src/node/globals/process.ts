 ...
 * @module @lyness/lyn-experimental-webworker-runtime/src/node/globals/process
 */
...
export interface ProcessShimOptions {
  ...
  readonly cwd: string
  /** Environment the tree reads; `LYNESS_HOME` belongs here. */
...
export function installProcessGlobal(options: ProcessShimOptions): ProcessShim {
  ...
  const shim: ProcessShim = {
    env: { ...options.env },
    argv: [...(options.argv ?? ['node', 'lyn-webworker'])],
    execArgv: [],
    execPath: '/lyn/bin/node',
    title: 'lyn-webworker',
    platform: 'linux',
    arch: 'x64',
    pid: 1,
