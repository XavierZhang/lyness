...
interface KoffiType {
  readonly __lynKoffiType: string
...
const token = (label: string, size: number, alignment = Math.min(size, 8) || 1): KoffiType =>
  ({ __lynKoffiType: label, size, alignment })
...
const typeOf = (target: unknown): KoffiType => {
  ...
  if (descriptor?.__lynKoffiType === undefined) {
...
const describe = (target: unknown): string =>
  typeof target === 'string' ? target : (target as KoffiType | undefined)?.__lynKoffiType ?? 'anonymous'
...
function array(target: unknown, length: number): KoffiType {
  ...
  return token(`array(${element.__lynKoffiType}, ${String(length)})`, element.size * length, element.alignment)
}
