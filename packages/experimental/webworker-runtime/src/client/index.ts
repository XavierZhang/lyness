...
interface ClientFileUploadGlobal {
  __LYNESS_FILE_UPLOAD__?: ClientFileUploadHooks
}
...
interface BootReadyGlobal {
  __LYNESS_BOOT_READY__?: PromiseWithResolvers<void>
}

function bootReadyGate(): PromiseWithResolvers<void> {
  return (globalThis as BootReadyGlobal).__LYNESS_BOOT_READY__ ??= Promise.withResolvers<void>()
}
 ...
 * bundles, `__LYNESS_BOOT__`, the theme bootstrap — in table order. The
 * boot-readiness deferred (`__LYNESS_BOOT_READY__`) is installed before the
...
export async function connectWorkerHost(worker: Worker, options?: WorkerHostConnectOptions): Promise<WorkerHostConnection> {
  ...
  try {
    ...
    ;(globalThis as ClientTransportGlobal).__LYNESS_TRANSPORT__ = {
    ...
    ;(globalThis as ClientFileUploadGlobal).__LYNESS_FILE_UPLOAD__ = {
      fetch: (input, init) => tunnel.fetch(input, init),
    }
