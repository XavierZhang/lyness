 ...
 * @module @lyness/lyn-experimental-webworker-runtime/src/worker-host
 */
import { setActiveModuleLoader, WorkerModuleLoader, type StaticModuleFactory } from './module-system/module-loader.ts'
import type { ProfileContext } from '@lyness/lyn-app-boot'
import type { TypertGateway } from '@lyness/lyn-api-gateway'
...
export interface WorkerHostOptions {
  ...
  /** Environment for the process shim; `LYNESS_HOME` defaults to `<root>/home`. */
...
export function createWorkerHost(options: WorkerHostOptions): WorkerHost {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      installProcessGlobal({ cwd: root, env: { LYNESS_HOME: home, HOME: home, ...options.env } })
      ...
      const appBoot = require('@lyness/lyn-app-boot') as {
        boot(
          binName: string,
          configPath: string,
          patches: unknown[],
      ...
      const cmdline = require('@lyness/lyn-cmdline') as {
      ...
      const ctx = await appBoot.boot('lyn-webworker', profileConfig, patches, (hostCtx) => {
...
export function installLogSink(ctx: HostContext, require: (specifier: string) => unknown): void {
  const { Logger } = require('@lyness/cordis') as { Logger: LogRenderer }
...
function bootPatches(
  ...
  const include = loader.load(loader.resolve('@lyness/cordis-plugin-include', root)) as { entryListSchema: unknown }
  ...
  vfs.seed(join(dir, 'package.json'), '{"private":true,"lyn":{"profile":{"bundles":[]}}}\n')
  ...
  const appBoot = loader.load(loader.resolve('@lyness/lyn-app-boot', root)) as {
    readProfilePatches(binName: string, profile: ProfileContext): unknown[]
  }
  return { patches: appBoot.readProfilePatches('lyn-webworker', profile), profile }
}
