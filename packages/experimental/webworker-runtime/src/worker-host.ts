 ...
 * @module @lyness/experimental-webworker-runtime/src/worker-host
 */
import { setActiveModuleLoader, WorkerModuleLoader, type StaticModuleFactory } from './module-system/module-loader.ts'
import type { TypertGateway } from '@lyness/api-gateway'
...
export function createWorkerHost(options: WorkerHostOptions): WorkerHost {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      const appBoot = require('@lyness/app-boot') as {
        boot(
          binName: string,
          configPath: string,
          patches: unknown[],
      ...
      const cmdline = require('@lyness/cmdline') as {
