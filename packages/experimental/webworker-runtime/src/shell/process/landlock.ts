...
import { LYNESS_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return LYNESS_TMP
  if (absolute.startsWith('/tmp/')) return `${LYNESS_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `lyn-bash-sandbox`. */
