...
describe('tar codec', () => {
  ...
  it('mounts as a VFS with directories synthesized along file paths', () => {
    ...
    expect(vfs.existsSync('/lyn/config/cordis.yml')).toBe(true)
    expect(vfs.readFileSync('/lyn/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
    expect(vfs.existsSync('/lyn/config')).toBe(true)
    expect(vfs.existsSync('/lyn/workspace')).toBe(true)
    expect(vfs.existsSync('/lyn/absent')).toBe(false)
  })
  ...
  it('applies ordered data overlays without exposing runtime paths', () => {
    ...
    expect(vfs.readFileSync('/lyn/workspace/status.txt', 'utf8')).toBe('fixture')
    expect(vfs.readFileSync('/lyn/home/sessions/example/session.jsonl', 'utf8')).toBe('{}\n')
    expect(() => loadVfsOverlay(packTar({
      'config/cordis.yml': encoder.encode('replaced'),
    }), '/lyn', vfs)).toThrow(/overlay entry must stay under home\/ or workspace/)
    expect(vfs.readFileSync('/lyn/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
  })
})
