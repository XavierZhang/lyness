...
it('keeps the native Landlock package API and CLI failure contract', async () => {
  ...
  const missingGrant = spawn(launcherPath(), ['--rw', '/lyn/missing', '--', 'touch', `${WORKSPACE}/never`], { cwd: WORKSPACE })
  expect(await collect(missingGrant)).toEqual({
    stdout: '',
    stderr: 'landlock-run: cannot open rule path: /lyn/missing: No such file or directory\n',
    code: 125,
  })
