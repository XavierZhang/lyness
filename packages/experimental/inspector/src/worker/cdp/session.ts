...
export class CdpSession implements NetworkSink {
  ...
  constructor(
    ...
    this.unsubscribeSources = sources.subscribeStatus(() => {
      if (this.diagnosticsEnabled) this.sendEvent('LYNInspector.sourcesChanged', { sources: this.sources.describe() })
    })
  }
  ...
  receive(value: unknown): void {
    ...
    try {
      ...
      } else if (request.method === 'LYNInspector.enable') {
        this.diagnosticsEnabled = true
        result = { sources: this.sources.describe() }
      } else if (request.method === 'LYNInspector.disable') {
        this.diagnosticsEnabled = false
        result = {}
      } else if (request.method === 'LYNInspector.getSources') {
        result = { sources: this.sources.describe() }
      } else if (request.method === 'LYNInspector.getCordisTree') {
