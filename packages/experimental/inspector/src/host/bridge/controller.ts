...
export async function startInspector(options: InspectorOptions = {}): Promise<InspectorHandle> {
  ...
  const clientProtocol = `lyn-inspector-v${String(INSPECTOR_PROTOCOL_VERSION)}-${randomBytes(32).toString('base64url')}`
  ...
  lifecycle.markRunning((error) => {
    try {
      ...
      console.error('lyn inspector: Host source cleanup after Worker failure failed', closeError)
    }
    void fetchObserver?.stop().catch((stopError: unknown) => {
      console.error('lyn inspector: fetch cleanup after Worker failure failed', stopError)
    })
    console.error('lyn inspector: Worker stopped unexpectedly', error)
  })
