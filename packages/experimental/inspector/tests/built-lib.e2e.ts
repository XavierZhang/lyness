...
const built = [
  'lib/index.js',
  'node_modules/@lyness/schemastery/lib/index.mjs',
...
describe.skipIf(!built)('experimental Inspector built artifact', () => {
  it('packs its sibling Worker and evaluates the Host from the tarball through plain Node', { retry: 0 }, async (test) => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-inspector-packed-'))
    ...
    linked = true
    const script = `
      const { startInspector } = await import('@lyness/lyn-experimental-inspector')
