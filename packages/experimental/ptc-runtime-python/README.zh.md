---
description: "CPython 子进程 PTC 运行时：为 Python 模型代码实现 lyn-ptc-runtime seam，及其使用的 fd-3 wire 协议。"
...
间接地，通过 `lyn-tools` 中的 PTC mode；当显式组合挂载本提供方时，它会把程序的完成值或失败渲染成保留的 `run_code` 结果，且已发布 profile 均不挂载这个实验包。
...
- **诊断与临时目录前缀省略包名中的 experimental 限定词**——标记 `[lyn-ptc-runtime-python] log capture truncated at <N> bytes` 与 `lyn-ptc-runtime-python-` 目录前缀独立于 npm 包名来标识本提供方。协议镜像检查 TypeScript 与 Python 的标记字节完全相同。
