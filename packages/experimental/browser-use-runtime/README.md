...
- **Shared host runtimes** — a profile installs this package beside the lyn installation, so `@lyness/lyn-scope` and `@lyness/lyn-mcp-client` stay peer dependencies. A dependency edge ships a second `lyn-scope` copy whose scope tags host registries cannot read: each Agent's MCP tools would register in the global tool layer and the second Agent's creation would fail.
