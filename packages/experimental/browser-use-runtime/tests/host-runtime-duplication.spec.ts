/**
 * Issue #4573: a profile install of this package sits beside the lyn
 * installation and installs its own copy of `@lyness/lyn-scope`, so two
 * scope module instances coexist in one host process. `lyn-scope` mints its
 ...
 * behavior to preserve. Delete it once `lyn-scope` stops depending on module
...
import BrowserUse from '@lyness/lyn-browser-use'
...
async function load(): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'lyn-browser-duplication-'))
  roots.push(root)
...
it('reproduces the second-Agent failure a profile install causes', async () => {
  ...
  vi.resetModules()
  const profileScope = await import('@lyness/lyn-scope')
  const profileMcpClient = await import('@lyness/lyn-mcp-client')
