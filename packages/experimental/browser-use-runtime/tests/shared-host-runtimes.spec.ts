/**
 * A profile installs this package from npm beside the lyn installation, so the
 * host runtimes it mounts must resolve to the installation's single module
 * instance. `lyn-scope` mints its scope-tag symbol and `lyn-mcp-client` keeps
...
const SHARED_HOST_RUNTIMES = ['@lyness/lyn-mcp-client', '@lyness/lyn-scope'] as const
