...
import LocalSubprocess from '@lyness/lyn-subprocess-local'
import { expect, it, vi } from 'vitest'
import { Config } from '../src/config.ts'
import { SenseVoiceWorker } from '../src/recognizer.ts'

const { LYNESS_SPEECH_E2E_ROOT: dataRoot, LYNESS_SPEECH_E2E_AUDIO: audioPath } = process.env
...
it.skipIf(!dataRoot || !audioPath)('prepares a real local recognizer, transcribes speech, and releases it', { timeout: 3_660_000, retry: 0 }, async () => {
  ...
  const worker = new SenseVoiceWorker(ctx, Config({ dataRoot: dataRoot!,
    modelDirectory: process.env.LYNESS_SPEECH_E2E_MODEL,
    vadModelPath: process.env.LYNESS_SPEECH_E2E_VAD,
    precision: process.env.LYNESS_SPEECH_E2E_PRECISION === 'fp32' ? 'fp32' : 'int8',
  }))
  try {
    worker.prepare()
