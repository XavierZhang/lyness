...
import { isSessionFormatJsonObject, parseSessionFormatLogFilename, sessionFormatLogFilename } from '@lyness/lyn-session-format'
import { createSessionFormatCatalogWithChildren, historicalSessionFormatCatalog, sessionFormatCatalog } from '@lyness/lyn-session-format-catalog'
import { historicalChildCatalogSource } from '@lyness/lyn-session-format-v3-to-v4'
...
export function packPreviewFixture(trees: readonly ImageTree[]): PackOverlayResult {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'lyn-preview-sessions-'))
  try {
