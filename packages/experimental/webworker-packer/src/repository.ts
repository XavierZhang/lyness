...
/** Composition entry point package: the `lyn` CLI, run from source. */
const CLI_PACKAGE = 'apps/cli'

/** Composition entry point: the `lyn` CLI, run from source. */
 ...
 * layers only, so the image is the shipped profile: the machine's `$LYNESS_HOME`
...
export function composeProfile(repoRoot: string, profile: string): string {
  const home = mkdtempSync(join(tmpdir(), 'lyn-pack-home-'))
  try {
    return execFileSync(
      process.execPath,
      ['--import', 'tsx/esm', join(repoRoot, CLI_ENTRY), '--profile', profile, '--dump-default-config'],
      { cwd: repoRoot, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, env: { ...process.env, [LYNESS_HOME_ENV]: home } },
    )
  } finally {
...
/** One `lyn.configTrees` declaration entry, validated field by field. */
 ...
 * (`lyn.configTrees` in its package.json): `path` is relative to the CLI
...
export function configTrees(repoRoot: string): ConfigTree[] {
  ...
  const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as {
    lyn?: { configTrees?: unknown }
  }
  const declared = manifest.lyn?.configTrees
  if (declared === undefined) return []
  if (!Array.isArray(declared)) {
    throw new Error(`vfs image: ${CLI_PACKAGE} lyn.configTrees must be an array`)
  }
  const mounts = new Set<string>()
  return declared.map((entry, index) => {
    const tree = entry as Partial<ConfigTreeDeclaration> | null
    const at = `${CLI_PACKAGE} lyn.configTrees[${String(index)}]`
