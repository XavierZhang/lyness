...
const packageNames = new Set(packages.map(name => `@lyness/lyn-experimental-${name}`))

it('loads both tarballs through plain Node and mounts their base image and overlay', { retry: 0 }, async (test) => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-webworker-packed-'))
  const links: string[] = []
  ...
  for (const name of packages) {
    ...
    const directory = join(root, 'node_modules/@lyness', `lyn-experimental-${name}`)
  ...
  const script = `
    ...
    import { fileURLToPath } from 'node:url'
    import * as packer from '@lyness/lyn-experimental-webworker-packer'
    import * as runtime from '@lyness/lyn-experimental-webworker-runtime'
    import * as client from '@lyness/lyn-experimental-webworker-runtime/client'
    import { sessionFormatCatalog } from '@lyness/lyn-session-format-catalog'
    for (const name of ['webworker-packer', 'webworker-runtime']) {
      assert.equal(import.meta.resolve('@lyness/lyn-experimental-' + name),
        new URL('./node_modules/@lyness/lyn-experimental-' + name + '/lib/index.js', import.meta.url).href)
    }
    assert.equal(typeof client.connectWorkerHost, 'function')
    const worker = fileURLToPath(import.meta.resolve('@lyness/lyn-experimental-webworker-runtime/worker'))
    ...
    const subject = '@lyness/lyn-image-export-fixture'
    ...
    assert.ok(vfs.existsSync('/lyn/' + packer.MANIFEST_PATH))
    mkdirSync('overlay')
    ...
    runtime.loadVfsOverlay(await runtime.inflateImage(overlay.image, 'packed overlay'), '/lyn', vfs)
    assert.equal(vfs.readFileSync('/lyn/workspace/hello.txt', 'utf8'), 'packed worker pair\\n')
    ...
    runtime.loadVfsOverlay(await runtime.inflateImage(prepared.image, 'prepared preview'), '/lyn', vfs)
    assert.equal(vfs.readFileSync('/lyn/home/sessions/project/preview/session.v3.jsonl', 'utf8'), historical)
    const current = JSON.parse(vfs.readFileSync('/lyn/home/sessions/project/preview/session.v' + sessionFormatCatalog.currentVersion + '.jsonl', 'utf8'))
