...
import { mountAgentLoopTestDependencies, mountAgentLoopTestHarness } from '@lyness/lyn-agent-loop-testkit'
import LocalAttachmentStore from '@lyness/lyn-attachment-local'
import BrowserUseRegistry from '@lyness/lyn-browser-use'
...
it.skipIf(process.env.LYNESS_STAGEHAND_E2E !== '1')('launches and attaches to installed Chromium while preserving the external browser', { timeout: 120_000, retry: 0 }, async ({ signal }) => {
  const executable = process.env.LYNESS_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('LYNESS_STAGEHAND_E2E requires LYNESS_BROWSER_EXECUTABLE')
  ...
  const root = await mkdtemp(join(tmpdir(), 'lyn-stagehand-live-'))
  ...
  try {
    ...
    await ctx.plugin(LocalAttachmentStore, { lynHome: root })
...
it.skipIf(process.env.LYNESS_STAGEHAND_E2E !== '1' || !process.env.LYNESS_STAGEHAND_MODEL || !process.env.LYNESS_STAGEHAND_MODEL_API_KEY)('extracts a controlled heading using an independently configured native Stagehand model', { timeout: 120_000, retry: 0 }, async ({ signal }) => {
  const server = createServer((_request, response) => {
    response.setHeader('content-type', 'text/html')
  ...
  try {
    ...
    await mountAgentLoopTestDependencies(ctx)
    await ctx.plugin(BrowserUseRegistry)
    await ctx.plugin(Provider, { model: stagehandModelSchema.parse({ modelName: process.env.LYNESS_STAGEHAND_MODEL, apiKey: process.env.LYNESS_STAGEHAND_MODEL_API_KEY }), mode: 'launch', ...process.env.LYNESS_BROWSER_EXECUTABLE === undefined ? {} : { executablePath: process.env.LYNESS_BROWSER_EXECUTABLE } })
    const harness = await mountAgentLoopTestHarness(ctx)
...
it.skipIf(process.env.LYNESS_STAGEHAND_E2E !== '1' || process.platform === 'win32')('scrubs real Chromium environment and closes it during canceled Stagehand initialization', { timeout: 120_000, retry: 0 }, async () => {
  const executable = process.env.LYNESS_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('LYNESS_STAGEHAND_E2E requires LYNESS_BROWSER_EXECUTABLE')
  const root = await mkdtemp(join(tmpdir(), 'lyn-stagehand-acquire-'))
  ...
  vi.stubEnv('LYNESS_BROWSER_FIXTURE_ID', 'do-not-forward')
  ...
  try {
    ...
    await writeFile(wrapper, [
      ...
      `writeFileSync(${JSON.stringify(marker)}, JSON.stringify({ pid: child.pid, token: process.env.BROWSER_FIXTURE_API_TOKEN ?? null, identity: process.env.LYNESS_BROWSER_FIXTURE_ID ?? null, publicValue: process.env.BROWSER_FIXTURE_PUBLIC, profile: process.argv.find(value => value.startsWith('--user-data-dir=')).slice('--user-data-dir='.length) }))`,
      'let output = \'\'',
...
it.skipIf(process.env.LYNESS_STAGEHAND_E2E !== '1')('drains canceled navigation before reconnecting to the same Chromium tabs', { timeout: 120_000, retry: 0 }, async () => {
  const executable = process.env.LYNESS_BROWSER_EXECUTABLE
  if (executable === undefined) throw new Error('LYNESS_STAGEHAND_E2E requires LYNESS_BROWSER_EXECUTABLE')
