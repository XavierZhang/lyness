...
显式启用的已安装浏览器测试使用受控本地页面和构建后的连接 Worker。请将 `LYNESS_BROWSER_EXECUTABLE` 设为已安装的 Chromium 可执行程序。只有同时提供 `LYNESS_STAGEHAND_MODEL` 和 `LYNESS_STAGEHAND_MODEL_API_KEY` 时，才会测试原生推理。
...
宿主在等待 CDP 就绪前即拥有启动的 Chromium 进程及其临时配置目录。Chromium 接收标准清理后的子进程环境，保留路径、区域设置和代理配置，排除凭据形式的变量及 LYN 身份信息。两种模式都在独立 Worker 中连接 SDK。Worker 除显式的源码 TypeScript 配置路径外不接收宿主环境，因此其 CDP 连接不继承宿主代理设置。SDK 关闭会等待活动操作。清理在配置的 SDK 宽限时间后终止连接 Worker；启动模式还会终止并等待自有 Chromium 进程退出，再移除配置目录。外部连接的浏览器保持运行。清理失败遵循下方的[所有权限制](#known-limitations-and-deferred-work)。

[原生运行时](src/native.ts)将显式模型配置传入 Stagehand 的公开初始化 API。Stagehand 在其浏览器扩展中负责模型请求、响应验证与 token 计量。LYN 通过现有 Session 日志记录浏览器工具输入和返回数据，包括 SDK 结果元数据。底层推理请求/响应捕获及其与 LYN Session 用量计量的集成均属暂缓工作。
...
[`stagehand_` 工具目录](../../../docs/tool-catalog.zh.md#lynesslyn-experimental-browser-use-stagehand-native)定义导航、标签页管理、截图、操作、观察和提取。结果包含当前页面事实或验证后的结构化数据。支持的截图以持久图像附件呈现。错误保持可见，让模型在重试前检查状态。

#### Token 影响

工具 schema 和结果增加主对话上下文。Stagehand 原生模型请求消耗额外 token，不计入 LYN Session 用量。
...
- **现有浏览器访问**——用户也可修改已连接的浏览器；占用机制只协调 LYN Session。
...
- **LYN 模型集成**——暂缓 Session 模型路由、LYN 凭据复用、底层推理请求/响应捕获，以及与 LYN Session 用量计量的集成。返回的工具数据和 SDK 元数据仍可回放。
