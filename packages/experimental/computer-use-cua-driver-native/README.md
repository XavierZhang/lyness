...
Use Cua Driver to inspect and operate desktop windows without installing its separate CLI or application. The native npm dependency runs inside the LYN host and exposes Cua Driver's own tools. Screenshots reach image-capable models through durable attachments. This published experimental package requires the launching host's desktop permissions and remains an explicit composition choice.
...
- name: '@lyness/lyn-experimental-computer-use-cua-driver-native'
```
...
```sh
env -u NODE_USE_ENV_PROXY LYNESS_COMPUTER_USE_NATIVE_E2E=1 node node_modules/vitest/vitest.mjs run --config vitest.e2e.config.ts packages/experimental/computer-use-cua-driver-native/tests/native.e2e.ts
```
...
- **Experimental release** — tool schemas follow the pinned upstream SDK and have no LYN stability promise.
