...
import ComputerUseRegistry from '@lyness/lyn-computer-use'
...
it('loads from cordis.yml and logs the native screenshot before the next model request', async () => {
  resetFixture()
  root = await mkdtemp(join(tmpdir(), 'lyn-native-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@lyness/lyn-computer-use', ComputerUseRegistry],
    ['@lyness/lyn-experimental-computer-use-cua-driver-native', NativeProvider],
  ])
