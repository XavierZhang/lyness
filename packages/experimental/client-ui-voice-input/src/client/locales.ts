...
export const zh = {
  ...
  'setup.local': '将在运行 LYN 的机器上下载本地模型，无需安装 Python 或编译工具。',
  'setup.disk': '硬盘空间',
  ...
  'downloadAdvice.network': '请检查运行 LYN 的机器能否访问下载来源及其模型下载服务；如需代理，请在该机器上配置后重试。',
  'downloadAdvice.dns': '请检查运行 LYN 的机器的 DNS 和代理设置，确认可以解析下载来源的域名后重试。',
  'downloadAdvice.timeout': '请检查运行 LYN 的机器的网络或代理连接，稍后重试。已下载并通过校验的文件会保留。',
  'downloadAdvice.certificate': '请检查运行 LYN 的机器的系统时间、受信任证书和代理设置后重试。',
  'downloadAdvice.http': '请确认下载地址可用、代理可正常连接下载服务，或稍后重试。',
  'downloadAdvice.integrity': '请重试下载。已完成并通过校验的其他文件会保留。',
  'downloadAdvice.storage': '请检查运行 LYN 的机器的剩余磁盘空间，以及模型目录的写入权限后重试。',
  'downloadAdvice.unknown': '请检查运行 LYN 的机器的网络、磁盘空间和模型目录权限后重试。',
  ...
  local: '音频在运行 LYN 的机器上识别。需要下载模型时，请确保该机器能访问所选下载源及其文件服务。如需代理，请在该机器上配置。',
  cloud: '音频将发送至所选云端服务。',
...
export const en: Record<VoiceKey, string> = {
  ...
  'setup.local': 'Local models will be downloaded to the machine running LYN. No Python or compiler is required.',
  ...
  'downloadAdvice.network': 'Check that the machine running LYN can reach the download source and its model download services. Configure a proxy on that machine if needed, then retry.',
  'downloadAdvice.dns': 'Check DNS and proxy settings on the machine running LYN. Confirm that it can resolve the download source domain, then retry.',
  'downloadAdvice.timeout': 'Check the network or proxy connection on the machine running LYN, then retry later. Verified downloaded files are retained.',
  'downloadAdvice.certificate': 'Check the system clock, trusted certificates and proxy settings on the machine running LYN, then retry.',
  ...
  'downloadAdvice.storage': 'Check available disk space and write permission for the model directory on the machine running LYN, then retry.',
  'downloadAdvice.unknown': 'Check the network, disk space and model directory permissions on the machine running LYN, then retry.',
  ...
  local: 'Audio is recognized on the machine running LYN. If models need downloading, that machine must be able to reach the selected source and its file services. Configure a proxy on that machine if needed.',
