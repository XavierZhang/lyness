...
import type {} from '@lyness/lyn-experimental-api-speech-to-text/remote'
...
import type {} from '@lyness/lyn-client-ui-plugin-manager/client'
...
function registerUi(ctx: Context): void {
  ...
  const actions: VoiceInputInjected = {
    openSettings: () => { ctx.pluginNavigation.openBundle('@lyness/lyn-experimental-voice-input-bundle') },
  ...
  ctx.slots.inject('plugins.bundle.config', () => ctx.slots.register({ name: 'plugins.bundle.config',
    key: '@lyness/lyn-experimental-voice-input-bundle', locale: NS, inject: () => actions,
  }, VoicePreparation))
