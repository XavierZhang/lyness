...
import type { SpeechProviderId } from '@lyness/lyn-experimental-speech-to-text/types'
import { RemoteError, type TypertRemoteContribution } from '@lyness/lyn-typert-protocol'
...
const REMOTE: TypertRemoteContribution = {
  package: '@lyness/lyn-experimental-api-speech-to-text',
  descriptors: [],
}
...
it('withdraws its Remote, localized slot and microphone captures on disposal', async () => {
  ...
  try {
    ...
    expect(b.openBundle).toHaveBeenCalledWith('@lyness/lyn-experimental-voice-input-bundle')
