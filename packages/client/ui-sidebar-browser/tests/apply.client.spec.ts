...
import type { ShortcutCommand, ShortcutPlatform } from '@lyness/lyn-client-shortcuts/client'
...
describe('ui-sidebar-browser apply', () => {
  it.each([0, 1])('binds and rebinds session controllers under desktop protocol %s', async (protocolVersion) => {
    ...
    vi.stubGlobal('lynDesktop', { protocolVersion, browser: bridge })
    const h = await boot()
    ...
    try {
      controller.mount({ tabId, signal: signal.signal, viewportId: host.id, applicationOrigin: 'https://lyn.example',
      ...
      controller.mount({ tabId, signal: reopened.signal, viewportId: host.id, applicationOrigin: 'https://lyn.example',
        initial: undefined, initialUrl: 'https://retained.example/', openTab: vi.fn() })
      h.openTabs.set([{ sessionId: 'session', tabId }])
      reopened.abort()
      expect(replacementStore.getSnapshot().byTab[tabId]).toBeDefined()
      const active = new AbortController()
      controller.mount({ tabId, signal: active.signal, viewportId: host.id, applicationOrigin: 'https://lyn.example',
