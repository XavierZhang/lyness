...
import type { SidebarRightTabActions } from '@lyness/lyn-client-ui-sidebar-right/client'
...
import type { WorkspaceDiffHunk } from '@lyness/lyn-workspace-changes/types'
import {
...
describe('review addresses', () => {
  it('round-trips coordinates and titles the tab by the turn', () => {
    expect(ADDRESS).toBe('lyn-resource://changes-review/session/viewed/5/2')
    expect(parseChangesReviewAddress(ADDRESS)).toEqual(COORDINATES)
    const definition = changesReviewDefinition(makeTranslate(zh))
    expect(definition).toMatchObject({ kind: 'changes-review', priority: 'builtin', patterns: ['lyn-resource://changes-review/**'] })
    expect(definition.canOpen?.(ADDRESS)).toBe(true)
    expect(definition.title(ADDRESS)).toBe('第 2 轮改动')
    for (const bad of [
      'lyn-resource://file/session/viewed/a.ts', 'lyn-resource://changes-review/session/viewed/5',
      'lyn-resource://changes-review/session//5/2', 'lyn-resource://changes-review/session/viewed/x/2',
      'lyn-resource://changes-review/session/viewed/5/0', 'lyn-resource://changes-review/session/%E0%A4%A/5/2',
    ]) {
...
describe('ReviewTab', () => {
  ...
  it('refuses an address it did not mint', () => {
    ...
    expect(() => mount({ address: 'lyn-resource://file/session/viewed/a.ts' })).toThrow('not a review address')
    error.mockRestore()
  })
})
