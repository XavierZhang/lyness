...
- **Inline local images require an HTTP(S) page**: Desktop’s `lyn-app:` pages do not support this file route. POSIX absolute paths and Windows drive-letter paths are supported; relative Chat image paths require the viewed Session’s workspace root. Separate image file links are optional.
