...
- **本地行内图片要求 HTTP(S) 页面**：Desktop 的 `lyn-app:` 页面不支持此文件路由。支持 POSIX 绝对路径与 Windows 盘符路径；Chat 中的相对图片路径需要当前查看的 Session 工作区根目录。图片的独立文件链接是可选的。
