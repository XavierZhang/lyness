...
import type { StartedToolCall, ToolResultNode } from '@lyness/lyn-client-ui-chat/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"lyn-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"lyn-manage-issues"}'): StartedToolCall {
  return {
...
describe('SkillRow', () => {
  ...
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: '加载技能lyn-manage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"lyn-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('lyn-manage-issues')
