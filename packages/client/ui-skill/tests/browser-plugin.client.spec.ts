...
describe('reference preview', () => {
  ...
  it('finishes the first click after a shared warm fetch and reloads after reconnect', async () => {
    ...
    await candidates
    expect(openResource).toHaveBeenCalledExactlyOnceWith('lyn-resource://file/session/preview//skills/review/SKILL.md')
  ...
  it('keeps pending clicks bound to their own Session when another Session opens', async () => {
    ...
    await secondDone
    expect(openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/second//second/SKILL.md')
    first.resolve({ ok: true, value: { skills: rows } })
    await firstDone
    expect(openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/first//skills/review/SKILL.md')
