...
describe('StateDot.module.css', () => {
  ...
  it('keeps ongoing on the rotating spinner rather than a solid-dot rule', () => {
    ...
    expect(css).toContain('@keyframes lyn-state-dot-spin')
    expect(css).toContain('@keyframes lyn-state-dot-dash')
    expect(css).toMatch(/\.spinnerMotion[^{]*\{[^}]*animation: lyn-state-dot-spin 1\.5s linear infinite/su)
