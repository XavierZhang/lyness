...
const TYPE_COLORS: Readonly<Record<TraditionalFileType, string>> = {
  ...
  image: 'var(--lyn-file-type-violet)',
  ...
  video: 'var(--lyn-file-type-violet)',
...
describe('FileTypeIcon.module.css', () => {
  it.each(Object.entries(TYPE_COLORS) as [TraditionalFileType, string][])(
    ...
    (type, color) => {
      ...
      expect(rule).toContain(`--lyn-file-type-default-color: ${color}`)
    },
  )

  it('keeps one caller override and the supplied violet in named variables', () => {
    expect(css).toContain('color: var(--lyn-file-type-icon-color, var(--lyn-file-type-default-color))')
    expect(css).toContain('--lyn-file-type-violet: rgb(139, 118, 246)')
  })
})
