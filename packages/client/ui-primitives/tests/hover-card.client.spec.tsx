...
import { HoverCard, Tooltip } from '@lyness/lyn-client-ui-primitives'
...
describe('HoverCard', () => {
  ...
  it('keeps previews below the desktop frame clearance', () => {
    const prior = document.documentElement.style.getPropertyValue('--lyn-frame-top-clearance')
    const priority = document.documentElement.style.getPropertyPriority('--lyn-frame-top-clearance')
    document.documentElement.style.setProperty('--lyn-frame-top-clearance', '120px')
    try {
      const { wrapper } = mount({ variant: 'preview' })
      wrapper.getBoundingClientRect = () => DOMRect.fromRect({ x: 100, y: 600, width: 600, height: 32 })
      fireEvent.pointerEnter(wrapper)
      ...
      if (prior === '') document.documentElement.style.removeProperty('--lyn-frame-top-clearance')
      else document.documentElement.style.setProperty('--lyn-frame-top-clearance', prior, priority)
    }
  })
