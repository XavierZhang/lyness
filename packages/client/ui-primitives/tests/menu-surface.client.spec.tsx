...
it('keeps each backing after its menu anchor and removes it when unmounted', () => {
  ...
  expect(backing.style.getPropertyValue('--lyn-menu-anchor')).toBe(menu.style.getPropertyValue('--lyn-menu-anchor'))
...
it('gives simultaneously open menus independent anchors and follows visibility changes', () => {
  ...
  const anchors = screen.getAllByRole('menu').map(menu => menu.style.getPropertyValue('--lyn-menu-anchor'))
