 ...
 * published `--lyn-frame-top-clearance` (the macOS window strip).
 */
...
afterEach(() => {
  cleanup()
  vi.restoreAllMocks()
  document.documentElement.style.removeProperty('--lyn-frame-top-clearance')
})
...
describe('useAnchoredMaxHeight', () => {
  ...
  it('widens the margin to the frame top clearance when published', () => {
    document.documentElement.style.setProperty('--lyn-frame-top-clearance', '48px')
    stubBottom(300)
    const { getByTestId } = render(<Probe cap={400} />)
