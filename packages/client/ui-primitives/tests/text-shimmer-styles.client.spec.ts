...
describe('TextShimmer styles', () => {
  it('animates active text and disables motion and transparent fill for reduced motion', () => {
    expect(css).toMatch(/\.root\[data-text-shimmer\]\s*\{[^}]*animation: lyn-text-shimmer/s)
    expect(css).toMatch(/@keyframes lyn-text-shimmer\s*\{[^}]*background-position: 0% center/s)
