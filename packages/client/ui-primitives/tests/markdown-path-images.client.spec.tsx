...
describe('MarkdownText local-path images', () => {
  ...
  it('accepts only the Desktop file route from a local image resolver', () => {
    const src = 'lyn-app://app/api/file?path=%2Ftmp%2Fgraph.png'
    ...
    for (const result of ['lyn-app://shell/api/file?path=x', 'lyn-app://app/assets/image.png',
      'lyn-app://app/api/file-other?path=x', 'lyn-app://app.example/api/file?path=x']) {
