...
import { Button, ConnectionIndicator, Input, Menu, MenuItemButton, Modal, Pill } from '@lyness/lyn-client-ui-primitives'
...
describe('Menu', () => {
  ...
  it('Tab settles the focused row; Shift+Tab closes back to the anchor', () => {
    ...
    expect(alpha.getAttribute('data-lyn-automatic-focus')).toBe('')
    expect(fireEvent.keyDown(alpha, { key: 'Tab' })).toBe(false)
    expect(alpha.getAttribute('data-lyn-automatic-focus')).toBeNull()
