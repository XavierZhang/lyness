...
import type { PeerScope } from '@lyness/lyn-typert-protocol'
import {
  RpcId,
  type ClientRequest,
...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  private register(
    ...
    const fetchHandler = rpcFetchHandler(channel, handler, this.operator, this.scoped)
  ...
  private registerInterceptor(
    ...
    const interceptor: ConnectionRpcInterceptor = {
      matches,
      fetchHandler: rpcFetchHandler(channel, handler, this.operator, this.scoped),
    }
    return owner.effect(() => {
...
function rpcFetchHandler(
  ...
  return {
    ...
    async fetch(request: Request): Promise<Response> {
      ...
      try {
        const result = await scoped(request, () => handler(endpoint, message.payload, request.signal, peer))
