...
import type { ClientEntryState } from '@lyness/lyn-client-modules/client'
import type { PluginEntryId } from '@lyness/lyn-api-remotes/client'
...
describe('PluginInventorySettingsTab', () => {
  it.each(['global', 'preset'])('shortens package and module name fallbacks in the %s inventory', async (scope) => {
    const names = [
      ['@lyness/lyn-tool-subagent', 'tool-subagent'],
      ['@lyness/lyn-host-web', 'web'],
      ['@lyness/lyn-client-tabs', 'tabs'],
      ['@lyness/cordis-plugin-hmr', 'hmr'],
      ['cordis:timer', 'timer'],
      ['@acme/lyn-sidebar/navigation', 'sidebar/navigation'],
    ...
    fireEvent.change(screen.getByRole('searchbox', { name: en.search }), {
      target: { value: '@lyness/lyn-tool-subagent' },
    })
  ...
  it('keeps authored locale titles verbatim when they resemble module names', async () => {
    const { locale, pageProps } = localizedProps(async () => ({
      entries: [{
        entryId: 'include:navigation' as PluginEntryId, moduleName: '@acme/lyn-navigation', enabled: true, fiberPhase: null,
        meta: { title: { en: 'lyn-Navigation', zh: 'lyn-导航' } },
      }],
    }))
    const view = render(<PluginInventorySettingsTab {...pageProps} />)
    await screen.findByRole('searchbox', { name: en.search })
    fireEvent.click(globalToggle())
    expect(screen.getByRole('button', { name: 'lyn-Navigation, include:navigation, Enabled' })).toBeTruthy()
    locale.setLocale('zh')
    view.rerender(<PluginInventorySettingsTab {...pageProps} />)
    expect(screen.getByRole('button', { name: 'lyn-导航, include:navigation, 已启用' })).toBeTruthy()
  })
  ...
  it.each(['global', 'preset'])('resolves each %s row field independently and hides empty English descriptions', async (scope) => {
    const rows: Snapshot['entries'] = [
      {
        entryId: 'include:navigation' as PluginEntryId, moduleName: '@acme/lyn-sidebar/navigation', enabled: true, fiberPhase: null,
        meta: { description: { en: 'Navigation description.' } },
      },
      {
        entryId: 'include:commands' as PluginEntryId, moduleName: '@acme/lyn-sidebar/commands', enabled: true, fiberPhase: null,
        meta: { title: { en: 'English title' }, description: { en: 'Package description.', zh: '中文命令说明。' } },
      },
      {
        entryId: 'include:theme' as PluginEntryId, moduleName: '@acme/lyn-theme/client', enabled: true, fiberPhase: null,
    ...
    const theme = screen.getByRole('button', { name: '@acme/lyn-theme, include:theme, Enabled' })
    ...
    expect(screen.getByText(zh.moduleLabel).nextElementSibling?.textContent).toBe('@acme/lyn-theme/client')
    ...
    fireEvent.change(screen.getByRole('searchbox', { name: en.search }), { target: { value: '@acme/lyn-theme/client' } })
    expect(screen.getAllByRole('listitem')).toHaveLength(1)
    expect(screen.getByRole('button', { name: '@acme/lyn-theme, include:theme, Enabled' }).getAttribute('aria-describedby')).toBeNull()
    expect(list).toHaveBeenCalledOnce()
  })
  ...
  it('keeps metadata-error-only rows inspectable by their full module specifier', async () => {
    ...
    await renderReady({
      entries: [{
        entryId: 'include:legacy' as PluginEntryId, moduleName: '@acme/lyn-legacy', enabled: false, fiberPhase: null,
        meta: { error },
      }],
    })
    fireEvent.click(globalToggle())
    ...
    fireEvent.click(card)
    expect(screen.getByText(en.moduleLabel).nextElementSibling?.textContent).toBe('@acme/lyn-legacy')
  })
  ...
  it('renders a rosterless deployment as one global list, folded until opened', async () => {
    const view = await renderReady({
      entries: [
        { entryId: 'hmr', moduleName: '@lyness/cordis-plugin-hmr', enabled: true, fiberPhase: 'active' },
