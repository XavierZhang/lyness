...
import type { ShortcutCatalogEntry, ShortcutCommandId, ShortcutFixedCatalogEntry } from '@lyness/lyn-client-shortcuts/client'
...
import { initialShortcutConfig, bindingIssue, normalizeBinding, presentBinding } from '@lyness/lyn-client-shortcuts/protocol'
import type { ShortcutConfigSnapshot } from '@lyness/lyn-client-shortcuts/protocol'
...
it('saves individual edits and disables changes when configuration cannot be read', async () => {
  ...
  expect(screen.getByRole('alert').textContent).toContain('this site’s localStorage entry lyn.keybindings.v1')
...
('identifies the $runtime document and preserves $error data in the selected locale', ({ runtime, error, dictionary }) => {
  ...
  expect(text).toContain(runtime === 'web' ? 'lyn.keybindings.v1' : 'userData/keybindings.json')
