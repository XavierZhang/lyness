...
import { Modal } from '@lyness/lyn-client-ui-primitives'
import type {} from '@lyness/lyn-client-shortcuts/client'
import type { DesktopKeyboardApi, DesktopShortcutInput, ShortcutCommandId } from '@lyness/lyn-client-shortcuts/protocol'
...
describe('assembled shortcut command owners', () => {
  it('keeps desktop defaults out of Web and removes commands on owner unload', async ({ start }) => {
    ...
    await client.unload('@lyness/lyn-client-ui-shortcuts')
    expect(fixedIds()).toEqual(expect.arrayContaining(conversationIds))
    expect(fixedIds()).not.toContain('fixed.move')
    await client.unload('@lyness/lyn-client-ui-conversation')
    for (const id of conversationIds) expect(fixedIds()).not.toContain(id)
    for (const owner of ['ui-settings-general', 'ui-layout']) await client.unload(`@lyness/lyn-client-${owner}`)
  ...
  it('toggles foreground dialogs, permits shortcuts above settings, and blocks settings behind shortcuts', async ({ start }) => {
    ...
    const previousBridge = Object.getOwnPropertyDescriptor(window, 'lynDesktop')
    const listeners = new Set<(input: DesktopShortcutInput) => void>()
    const keyboard: DesktopKeyboardApi = {
      subscribe: (listener) => { listeners.add(listener); return () => { listeners.delete(listener) } },
      closeWindow: async () => {},
    }
    Object.defineProperty(window, 'lynDesktop', { configurable: true, value: { keyboard } })
    ...
    onTestFinished(() => {
      cleanup()
      modal.remove()
      if (previousBridge === undefined) Reflect.deleteProperty(window, 'lynDesktop')
      else Object.defineProperty(window, 'lynDesktop', previousBridge)
