...
import { SlotTestRuntime, stubConfigForm } from '@lyness/lyn-client-test-runtime'
import { LocaleRuntime } from '@lyness/lyn-client-locale/client'
import { apply as applyConversation, inject as injectConversation } from '@lyness/lyn-client-ui-conversation/client'
import { apply as applyTool, inject as injectTool } from '@lyness/lyn-client-ui-tool/client'
...
describe('keyed toolview hole through the real machinery', () => {
  ...
  it('file-path clicks travel owner openFile → chat inject → the right Sidebar', async () => {
    ...
    await vi.waitFor(() => {
      expect(b.sidebarRight.openResource).toHaveBeenCalledWith('lyn-resource://file/session/s1/src/a.ts')
    })
