/**
 * Node half of the client module system (`lyn.client` dual-face package): scans
 * the host Loader's entries for packages declaring `lyn.client`, composes the
 * `window.__LYNESS_BOOT__` entry graph (wire single source: {@link WebBootEntry}
...
/** package.json `lyn.client` declaration fields, validated one by one after reading the file. */
interface LynClientDeclaration {
  inject?: string[]
  platform: string
...
/** Narrow an unknown parsed JSON value to the `lyn.client` declaration, throwing on malformed fields. */
function parseLynClient(pkgName: string, value: unknown): LynClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object lyn.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} lyn.client.platform must be a string`)
  }
  const inject = optionalStringArray(pkgName, 'lyn.client.inject', decl.inject)
  const external = optionalStringArray(pkgName, 'lyn.client.external', decl.external)
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} lyn.client.immediately must be a boolean`)
  }
  return {
...
function comboSectionMap(record: WebPluginRecord): Record<string, unknown> {
  ...
  const base = new URL(`/plugins/${record.entry.id}/client.js.map`, 'http://lyn.invalid')
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in lyn.client.external',
        )
      }
...
window.__ModuleLoader__={
  ...
  rows.push({ kind: 'global', name: '__LYNESS_BOOT__', value: graph })
  return rows
}

/**
 * The web plugin table service: incremental `lyn.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
   ...
   * @returns the graph served as `window.__LYNESS_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(loaderName: string, baseUrl: string): ResolvedPkgMeta | null {
    ...
    const lyn = pkg.lyn
    const decl = parseLynClient(
      packageName,
      lyn !== null && typeof lyn === 'object' ? (lyn as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${packageName} declares lyn.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
