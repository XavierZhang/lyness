...
import type { SessionAddress } from '@lyness/lyn-api-session-controller/types'
...
export function planAddress(target: PlanAddress): string {
  ...
  return `lyn-resource://plan/${parts.map(encodeURIComponent).join('/')}`
}
...
export function parsePlanAddress(address: string): PlanAddress | undefined {
  const match = /^lyn-resource:\/\/plan\/([^?#]+)$/.exec(address)
