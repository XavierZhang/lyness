 ...
 */
import { ConversationEventRegistry } from '@lyness/lyn-client-ui-conversation/client'
...
import { createSidebarRightController } from '@lyness/lyn-client-ui-sidebar-right/src/client/service.ts'
import { SidebarRightTabRegistry } from '@lyness/lyn-client-ui-sidebar-right/src/client/tab-registry.ts'
import { createSidebarRightStore } from '@lyness/lyn-client-ui-sidebar-right/src/client/stores.ts'
...
describe('ui-plan browser apply', () => {
  ...
  it('opens an embedded child plan in the visible parent sidebar without adopting a child store', async () => {
    ...
    try {
      ...
      expect(resources()).toContain('lyn-resource://plan/subagent/visible-parent/embedded-child/continuable/child-plan')
      ...
      expect(resources().some(address => address.startsWith('lyn-resource://plan-review/embedded-child/'))).toBe(true)
      expect(store.getSnapshot().bySession[child]).toBeUndefined()
    } finally {
      await fiber.dispose()
      unbind()
  ...
  it('binds both entry points to the same Session resource and removes the sidebar seats', async () => {
    ...
    try {
      ...
      removeFileEntry()
      const address = 'lyn-resource://plan/s-plan/call'
      ...
      injected.openPlan(plan.callId)
      expect(b.openResource).toHaveBeenLastCalledWith('lyn-resource://plan/subagent/parent/s-plan/continuable/call')
      reviewInjected.openReview(pending, 'question:1')
      expect(b.openResource).toHaveBeenLastCalledWith('lyn-resource://plan/subagent/parent/s-plan/continuable/call')
      ...
      expect(first).toEqual([expect.stringMatching(/^lyn-resource:\/\/plan-review\/s-plan\//), {
