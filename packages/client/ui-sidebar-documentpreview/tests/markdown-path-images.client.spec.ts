...
const BASE = 'https://example.test/tools/lyn/'
...
describe('Markdown preview image URLs', () => {
  ...
  it.each(['about:blank', 'lyn-app://shell/', 'file:///app', 'ws://localhost/'])(
  ...
  it('resolves relative and absolute images through the Desktop file route', () => {
    for (const destination of ['图%20片.png', '/work/guide/图%20片.png']) {
      const url = new URL(markdownImageUrl('lyn-app://app/', '/work/guide/notes.md', destination)!)
