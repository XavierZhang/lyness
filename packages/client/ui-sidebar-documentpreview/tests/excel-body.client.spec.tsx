...
const props = { content: { kind: 'bytes', data: new Uint8Array([1]) }, limits: Config({}).excel, t: makeTranslate(en), resourceAddress: 'lyn-resource://file/session/s1/book.xlsx' } as ExcelBodyProps
...
it.each(['xlsx', 'xls', 'csv', 'tsv'])('loads the spreadsheet chunk for %s', async (format) => {
  mocked.parse.mockResolvedValue(value)
  render(<LazyExcelBody {...props} resourceAddress={`lyn-resource://file/session/s1/book.${format}`} />)
  await waitFor(() => { expect(mocked.workbook).toHaveBeenCalledOnce() })
