...
import { DeveloperToolsPreference } from '@lyness/lyn-client-ui-settings/src/client/developer-tools.ts'
import type { DeveloperToolsSettings } from '@lyness/lyn-client-ui-settings/src/developer-tools-settings.ts'
import type { Resources, ResourceSnapshot } from '@lyness/lyn-client-resources/client'
import type { WorkspaceFileStat } from '@lyness/lyn-api-workspace-files/types'
...
function props(text = '<p>hello</p>'): HtmlBodyProps {
  ...
  return {
    ...
    resourceAddress: 'lyn-resource://file/session/html/index.html',
...
describe('HtmlBody', () => {
  ...
  it('renders static HTML without reading related files, running scripts or retaining an advanced frame', async () => {
    ...
    expect(frame.getAttribute('name')).toBe(`lyn-sidebar-html-${TAB_ID}`)
    ...
    expect(advanced.getAttribute('name')).toBe(`lyn-sidebar-html-${TAB_ID}`)
  ...
  it('destroys the old frame and revokes its Blob when bytes or source file change', async () => {
    ...
    view.rerender(<HtmlBody {...changed} resourceAddress="lyn-resource://file/session/html/other.html" />)
