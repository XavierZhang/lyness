...
import type { OfficeToPdfGeneration } from '@lyness/lyn-office-to-pdf/types'
import { Context } from '@lyness/cordis'
import { expect, it, vi } from 'vitest'
import type { SessionId } from '@lyness/lyn-session/types'
import type { ClientRemote } from '@lyness/lyn-api-remotes/client'
import type {} from '@lyness/lyn-office-to-pdf/remote'
...
it.each(['remote', 'render', 'files'] as const)('keeps Word and PowerPoint registration and guidance when %s is absent', async (missing) => {
  ...
  expect(h.register).toHaveBeenCalledWith(expect.objectContaining({
    name: 'sidebar.right.tab.document', key: '@lyness/lyn-client-ui-sidebar-documentpreview/office', locale: 'sidebarOffice',
  }), OfficeBody)
