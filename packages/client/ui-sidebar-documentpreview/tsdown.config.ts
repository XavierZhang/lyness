...
const bundle = clientBundle('@lyness/lyn-client-ui-sidebar-documentpreview', ['lib/types/index.js'], {
...
const workerModule = '\0lyn-pdf-worker.mjs'
...
const excelWorker: NonNullable<UserConfig['plugins']> = [{
  name: 'lyn-excel-worker-source',
  resolveId(source) { return source === './worker.ts?raw' ? '\0lyn-excel-worker-source' : null },
  async load(id) {
    if (id !== '\0lyn-excel-worker-source') return null
    const parent = this
    const worker = await Rolldown.rolldown({
      ...
      plugins: [{
        name: 'lyn-excel-worker-dependencies',
...
export default (options: Parameters<typeof bundle>[0]): UserConfig[] => bundle(options).map(config =>
  config.name?.endsWith('/client') === true ? {
    ...
    define: { ...config.define, __LYNESS_PDFJS_ASSETS__: pdfAssets() },
  } : config,
)
