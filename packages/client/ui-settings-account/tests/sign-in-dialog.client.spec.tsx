...
import type { AccountView, SignInAttemptId } from '@lyness/lyn-deepseek-account/types'
...
it.each([en, zh])('shows waiting actions and cancels before dismissing', async (copy) => {
  const props = mount({ id, phase: 'waiting-browser', authorizeUrl: 'https://platform.deepseek.com/lyn/authorize?state=example' }, copy)
...
it.each([en, zh])('restores the copy label two seconds after the latest successful copy', async (copy) => {
  ...
  try {
    ...
    const authorizeUrl = 'https://platform.deepseek.com/lyn/authorize?state=example'
    ...
    expect(writeText).toHaveBeenLastCalledWith('https://platform.deepseek.com/lyn/authorize?state=example&theme=dark')
...
it('copies the authorization link with the palette of the current render', async () => {
  ...
  try {
    Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText } })
    const attempt = { id, phase: 'waiting-browser' as const, authorizeUrl: 'https://platform.deepseek.com/lyn/authorize?state=example' }
    ...
    expect(writeText).toHaveBeenLastCalledWith('https://platform.deepseek.com/lyn/authorize?state=example&theme=light')
  } finally {
    if (clipboard) Object.defineProperty(navigator, 'clipboard', clipboard)
    else Reflect.deleteProperty(navigator, 'clipboard')
  }
