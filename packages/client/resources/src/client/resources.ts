...
interface ResourceRecord {
  ...
  /** The address's protocol key (`lyn-resource://` host); absent when the address is not a resource address. */
...
/**
 * The one URL scheme resource addresses use: `lyn-resource://<type>/…`, where
 * the host names the protocol. Other schemes (`sidebar://…`) are navigation
 * addresses and name no resource.
 */
export const RESOURCE_SCHEME = 'lyn-resource'

/**
 * The protocol key of one address: the host of a `lyn-resource://` URL, as the
