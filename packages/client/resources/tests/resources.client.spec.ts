...
describe('protocolOf', () => {
  it('reads the lyn-resource host, lower-cased, and reports none for any other address', () => {
    expect(protocolOf('lyn-resource://file/session/s1/home/ys/b.txt')).toBe('file')
    expect(protocolOf('LYN-RESOURCE://File/session/s1/a')).toBe('file')
    expect(protocolOf('lyn-resource://chat/node/1')).toBe('chat')
    ...
    expect(protocolOf('lyn-resource:///no-host')).toBeUndefined()
...
describe('ResourceRegistry stream generations', () => {
  ...
  it('streams another protocol as plain numbers', async () => {
    ...
    const source = b.registry.source('lyn-resource://counter/one')
    const unsubscribe = source.subscribe(() => {})
