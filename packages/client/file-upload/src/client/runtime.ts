...
export class FileUploadRuntime extends Service implements FileUploadService {
  ...
  constructor(ctx: Context) {
    ...
    const hook = (globalThis as ClientFileUploadGlobal).__LYNESS_FILE_UPLOAD__
...
function workerTransport(): FileUploadTransport {
  return {
    post(request) {
      ...
      const worker = new Worker(workerUrl, { name: 'lyn-file-upload' })
...
function resolveUrl(path: string): URL {
  ...
  const origin = typeof pageLocation === 'object' && pageLocation !== null
    ...
    ? pageLocation.origin
    : undefined
  return new URL(path, origin === undefined || origin === 'null' ? 'http://lyn.internal' : origin)
}
