...
const MOUNTED_BASE = 'http://127.0.0.1:3080/tools/lyn/'
...
describe('localPathMediaUrl', () => {
  it('maps an absolute POSIX path to the file route of the document, root or mount', () => {
    ...
    for (const [base, root] of [
      ...
      ['lyn-app://app/', 'lyn-app://app/'],
      ['lyn-app://app/index.html', 'lyn-app://app/'],
      ['http://127.0.0.1:3080/tools/lyn/index.html', MOUNTED_BASE],
    ]) {
  ...
  it('keeps unsupported application transports inert', () => {
    ...
    expect(localPathMediaUrl('lyn-app://shell/', '/tmp/graph.png')).toBeUndefined()
...
describe('AssistantMarkdown local-path images', () => {
  it.each([BASE, 'lyn-app://app/'])('renders a local image in closing prose through %s', (base) => {
