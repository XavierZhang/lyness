...
import type { ISession, SessionReference } from '@lyness/lyn-api-session-controller/client'
...
describe('Chat inject API', () => {
  ...
  it('addresses file paths under the Session\'s scope and opens them in the right Sidebar', async () => {
    ...
    expect(b.sidebarRight.openResource).toHaveBeenCalledWith('lyn-resource://file/session/root-1/src/a.ts')
    ...
    expect(b.sidebarRight.openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/root-1/src/a.ts')

    // A name a URL would otherwise mangle survives the round trip.
    await injected.openFile('src/a b#c.ts')
    expect(b.sidebarRight.openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/root-1/src/a%20b%23c.ts')

    // A line travels as the `file` type's navigation parameter, not in the address.
    await injected.openFile('src/a.ts', { line: 7 })
    expect(b.sidebarRight.openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/root-1/src/a.ts', { params: { line: 7 } })
    await b.runtime.dispose()
  })
  ...
  it('keeps a relative path under the Session without a cwd, and addresses a path outside the workspace absolutely', async () => {
    ...
    await injected.openFile('src/a.ts')
    expect(b.sidebarRight.openResource).toHaveBeenCalledWith('lyn-resource://file/session/root-2/src/a.ts')
    // An absolute path outside every known root still names its Session.
    await injected.openFile('/abs/a.ts')
    expect(b.sidebarRight.openResource).toHaveBeenLastCalledWith('lyn-resource://file/session/root-2//abs/a.ts')
    await b.runtime.dispose()
  })
