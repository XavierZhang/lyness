...
使用本包可为 lyn Web 客户端提供 Settings 面板、连接恢复控件、由功能包贡献的导航，以及依次进行的首次运行引导。用户可以从侧边栏打开面板、立即重试失败的连接，并在宿主为回环浏览器提供本地配置文件时访问该文件。各功能包提供自己的设置行、分区和引导步骤；本包提供共享的界面展示和代码工作工具开关，但不添加引导文案。
...
Web 与桌面端的通用设置底部显示当前发布版本，使用构建注入的 `LYNESS_CLIENT_VERSION` 元数据和当前语言。缺少版本元数据的局部构建不显示该行。
