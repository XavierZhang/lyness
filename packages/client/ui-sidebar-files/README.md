...
- **The type** — `ctx.sidebarRightTabs.register(...)` with kind `files`, id `@lyness/lyn-client-ui-sidebar-files`, band `builtin`, no patterns, and one guide entry (order 10, its title and description from the `sidebarFiles` namespace, its glyph a yellow folder) that opens the type.
