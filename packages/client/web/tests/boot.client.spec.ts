...
const transportGlobal = globalThis as {
  __LYNESS_TRANSPORT__?: { loadBundle(url: string): Promise<void> }
}
...
afterEach(() => {
  ...
  delete transportGlobal.__LYNESS_TRANSPORT__
...
describe('bootstrap failure rendering', () => {
  it('leaves the loading page visible when a carrier owns fatal presentation', async () => {
    ...
    expect(container.querySelector('[data-lyn-boot-spinner]')).not.toBeNull()
    await entry.dispose()
  })
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__LYNESS_BOOT__
    }, 'window.__LYNESS_BOOT__ is missing or not an object')
  })
...
describe('plugin activation', () => {
  it('prefetches a parser-loaded immediate row through the injected bundle transport', async () => {
    ...
    transportGlobal.__LYNESS_TRANSPORT__ = {
...
it('draws the shared boot page before Host injections and resumes without replacing the document', async () => {
  ...
  vi.stubGlobal('__LYNESS_BOOT_READY__', gate)
  const container = document.createElement('div')
  document.body.append(container)
  ...
  try {
    const page = container.querySelector('[data-lyn-boot]')
    const spinner = container.querySelector('[data-lyn-boot-spinner]')
    ...
    expect(container.querySelector('[data-lyn-boot]')).toBe(page)
