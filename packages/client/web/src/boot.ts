...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      await (globalThis as { __LYNESS_BOOT_READY__?: { promise: Promise<void> } }).__LYNESS_BOOT_READY__?.promise
      ...
      const transport = (globalThis as {
        __LYNESS_TRANSPORT__?: { loadBundle?: ClientModuleCreateOptions['loadBundle'] }
      }).__LYNESS_TRANSPORT__
      this.modules = moduleLoader.create({
        boot: win.__LYNESS_BOOT__,
