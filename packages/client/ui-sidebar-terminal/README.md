...
# @lyness/lyn-client-ui-sidebar-terminal
...
The terminal background, default text, cursor, and selection follow the LYN theme, including system preference and theme-token overrides. Theme changes preserve the running shell, output, and application OSC color overrides. Reset commands restore colors to the current LYN defaults. xterm adjusts text toward 4.5:1 contrast; the cursor keeps at least 3:1 contrast against its cell background, including Vim colorschemes.
