/** LYN defaults and application OSC colors for one xterm screen. */
import type { IDisposable, ITheme, Terminal } from '@xterm/xterm'

/* oxlint-disable typescript/no-non-null-assertion -- OSC widths/indices are checked; LYN defaults precede cursor reads. */
...
/** Keep program color overrides separate from the LYN defaults used by OSC resets. */
export class TerminalTheme implements IDisposable {
  ...
  /**
   * Apply changed LYN colors without replacing application-defined palette entries.
   * @param background - resolved opaque LYN background.
   * @param foreground - resolved LYN text color.
   */
  ...
  /** Preferred cursor color after the initial LYN update, including OSC 12 overrides. */
