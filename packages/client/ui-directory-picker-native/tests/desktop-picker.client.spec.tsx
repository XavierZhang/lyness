...
import { ClientRoster, createClientTest, webApp } from '@lyness/lyn-client-test-runtime/src/assembly/index.ts'
...
const test = createClientTest({ roster: ClientRoster.of([...webApp.rows, {
  name: manifest.name, inject: manifest.lyn.client.inject, immediately: false,
}]) })
...
test('the composed native flow cancels through Desktop without invoking the Host chooser', async ({ start, remote }) => {
  ...
  vi.stubGlobal('__LYNESS_DIRECTORY_PICKER__', { pick })
