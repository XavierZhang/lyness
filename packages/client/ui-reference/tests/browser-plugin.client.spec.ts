...
import { RemoteError, TestSessions } from '@lyness/lyn-client-test-runtime'
...
async function bench(
  ...
  sessions: RemoteLookup<SessionReferenceMentionCandidate> = vi.fn(() => Promise.resolve({
    ...
    value: [{
      ...
      mention: '@[Research](lyn-session:InNvdXJjZSI)',
    }],
  })),
...
describe('candidates', () => {
  ...
  it('labels a session without a cwd and still dates it', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: false,
        createdAt: CREATED_AT,
        mention: '@[same](lyn-session:InNhbWUi)',
      }],
    }))
    const { source } = await bench(files, sessions)
  ...
  it('falls back to the candidate createdAt for a session the Host list does not carry', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Unlisted run](lyn-session:InVubGlzdGVkIg)',
      }],
    }))
  ...
  it('uses current Session titles and groups direct subagents above ordinary Sessions', async () => {
      ...
      value: [
        {
          ...
          sameWorkspace: true,
          createdAt: CREATED_AT,
          mention: '@[Ordinary title](lyn-session:Im9yZGluYXJ5Ig)',
        },
        {
          ...
          sameWorkspace: true,
          createdAt: CREATED_AT,
          mention: '@[researcher](lyn-session:IndvcmtlciI)',
        },
      ],
    }))
    const { source } = await bench(files, sessions, {
    ...
    expect(source.onPick({
      ...
      insert: {
        source: 'reference',
        ref: '@[researcher](lyn-session:IndvcmtlciI)',
        label: 'researcher',
        appearance: 'session',
        clipboardText: '@[researcher](lyn-session:IndvcmtlciI)',
      },
    })
  })
  ...
  it('reads a session opened moments ago as the present, not a zero distance', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: NOW - 1_000,
        mention: '@[Just now](lyn-session:Imp1c3Qtbm93Ig)',
      }],
    }))
  ...
  it('dates a session in the current workspace without repeating that workspace', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Sibling run](lyn-session:InNpYmxpbmdyIg)',
      }],
    }))
    const { source } = await bench(files, sessions)
...
describe('pick and codec', () => {
  ...
  it('inserts sessions as atomic chips whose clipboard and model forms are canonical mentions', async () => {
    ...
    const mention = '@[Research](lyn-session:InNvdXJjZSI)'
...
describe('reference preview', () => {
  it('opens plain and quoted file references without treating folders or sessions as files', async () => {
    ...
    expect(openResource).toHaveBeenNthCalledWith(1, 'lyn-resource://file/session/target/notes/readme.md')
    expect(openResource).toHaveBeenNthCalledWith(2, 'lyn-resource://file/session/target/docs/a%20b.md')
    expect(source.openReference?.(session, { ref: '@docs/', appearance: 'folder' })).toBe(false)
    expect(source.openReference?.(session, { ref: '@[Research](lyn-session:abc)', appearance: 'session' })).toBe(false)
