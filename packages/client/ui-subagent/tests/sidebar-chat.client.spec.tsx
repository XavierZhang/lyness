...
import { sessionSnapshot } from '@lyness/lyn-client-test-runtime'
import type { ConversationViewsProps } from '@lyness/lyn-client-ui-conversation/client'
...
describe('Sidebar chat address', () => {
  it('round-trips the child identity and direct-parent routing metadata', () => {
    ...
    expect(resource).toBe('lyn-resource://subagentchat/session/child%231?parent=parent%2Fa&mode=continuable')
  ...
  it.each([
    ...
    'lyn-resource://other/session/child?parent=parent&mode=continuable',
    'lyn-resource://subagentchat/other/child?parent=parent&mode=continuable',
    'lyn-resource://subagentchat/session/child?mode=continuable',
    'lyn-resource://subagentchat/session/child?parent=&mode=continuable',
    'lyn-resource://subagentchat/session/child?parent=parent&mode=invalid',
    'lyn-resource://subagentchat/session/%?parent=parent&mode=continuable',
  ])('rejects %s', (address) => {
...
describe('Sidebar chat registration', () => {
  it('registers the resource, tab, and child slot and retains the child for the resource lifetime', async () => {
    ...
    expect(definition?.canOpen?.('lyn-resource://subagentchat/invalid')).toBe(false)
