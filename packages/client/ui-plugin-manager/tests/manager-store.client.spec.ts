...
import type { BundleInfo, ChangeResult, ManagementError, PluginEntryId, PluginInfo, PluginInstallRequestId } from '@lyness/lyn-api-remotes/client'
...
const INCOMPATIBLE = { name: 'lyn-late', version: '2.0.0', runtimeVersion: '0.1.0', peers: { '@lyness/lyn': '^0.2.0' } }
...
const BUNDLE: BundleInfo = {
  ...
  optional: false,
  removable: true,
  rows: [{ rowId: 'sidebar', moduleName: 'lyn-better-sidebar', entryId: ROW_ENTRY }, { rowId: 'theme', moduleName: 'lyn-better-sidebar/theme' }],
  overrides: [],
}

const PLUGINS: PluginInfo[] = [
  { entryId: ROW_ENTRY, moduleName: 'lyn-better-sidebar', enabled: true, fiberPhase: 'active', patchId: 'sidebar' },
  { entryId: 'include:core' as PluginEntryId, moduleName: '@lyness/lyn-base', enabled: true, fiberPhase: 'active', readOnlyReason: 'management-required' },
]
...
const INSPECTED = { status: 'accepted' as const, kind: 'registry' as const, name: 'lyn-better-sidebar', version: '1.0.0', bundle: true, registry: null }

const APPLIED: ChangeResult = { changed: true, application: 'applied', stage: 'enable', target: 'lyn-better-sidebar' }
...
function failed(error?: ManagementError, packageResult?: ChangeResult['packageResult']): ChangeResult {
  return {
    changed: false, application: 'failed', stage: 'enable', target: 'lyn-better-sidebar',
...
function bench(overrides: Partial<Record<string, ReturnType<typeof vi.fn>>> = {}) {
  ...
  const plugins = {
    ...
    installBundle: vi.fn(() => Promise.resolve(ok({ ...APPLIED, bundle: 'lyn-new' }))),
...
describe('packageView', () => {
  it('joins a bundle with the entries its rows run as', () => {
    expect(packageView(BUNDLE, PLUGINS)).toEqual({
      name: 'lyn-better-sidebar', version: '0.16.0', description: 'A sidebar.',
      installed: true, optional: false, enabled: false,
      rows: [
        { rowId: 'sidebar', moduleName: 'lyn-better-sidebar', entryId: ROW_ENTRY, enabled: true, phase: 'active' },
        { rowId: 'theme', moduleName: 'lyn-better-sidebar/theme', enabled: false, phase: null },
      ],
    })
    // A row the inventory no longer lists, a protected row, and a bundle the Host cannot read.
    const protectedBundle: BundleInfo = {
      name: '@lyness/lyn-base', enabled: true, installed: false, optional: false, removable: false, readOnlyReason: 'management-required',
      error: { code: 'operation-error', diagnostic: 'broken' },
      rows: [{ rowId: 'core', moduleName: '@lyness/lyn-base', entryId: 'include:core' as PluginEntryId }, { rowId: 'gone', moduleName: 'x', entryId: 'include:gone' as PluginEntryId }],
      overrides: [],
    }
    expect(packageView(protectedBundle, PLUGINS)).toEqual({
      name: '@lyness/lyn-base', installed: false, optional: false, enabled: true, readOnlyReason: 'management-required',
      error: { code: 'operation-error', diagnostic: 'broken' },
      rows: [
        { rowId: 'core', moduleName: '@lyness/lyn-base', entryId: 'include:core', enabled: true, phase: 'active', readOnlyReason: 'management-required' },
...
describe('sortPackages', () => {
  it('orders packages by the short name a person reads, not by the Host order or enablement', async () => {
    ...
    const zeta: BundleInfo = { ...plain, name: 'lyn-zeta' }
    const alpha: BundleInfo = { ...plain, name: '@acme/lyn-alpha', enabled: false }
    const views = [zeta, BUNDLE, alpha].map(bundle => packageView(bundle, PLUGINS))
    expect(sortPackages(views).map(pkg => pkg.name)).toEqual(['@acme/lyn-alpha', 'lyn-better-sidebar', 'lyn-zeta'])
    ...
    expect(state().packages.map(pkg => pkg.name)).toEqual(['@acme/lyn-alpha', 'lyn-better-sidebar', 'lyn-zeta'])
  })
})
...
describe('PluginManagerController', () => {
  ...
  it('checks the spec, hands the run to the Host, and folds the chunks that carry its request id', async () => {
    ...
    expect(state().install.spec).toBe('  lyn-new ')
    await vi.waitFor(() => { expect(plugins.inspect).toHaveBeenCalledTimes(1) })
    expect(plugins.inspect).toHaveBeenCalledWith('lyn-new', { registry: null }, expect.any(AbortSignal))
    const requestId = await started()
    expect(state().install.subject).toEqual({ spec: 'lyn-new', ...INSPECTED })
    expect(plugins.installBundle).toHaveBeenCalledTimes(1)
    expect(plugins.installBundle).toHaveBeenCalledWith('lyn-new', { enabled: false, requestId, registry: null })
    ...
    expect(state().install.runs).toEqual([
      { jobId: 'j1', command: 'pnpm add lyn-new', cwd: '/p', output: 'Progress\nDone\n' },
      { jobId: 'j2', command: 'pnpm remove lib', cwd: '/p', output: '- lib\n', exitCode: 0 },
    ])
    face.toggleInstallDetails()
    expect(state().install.detailsOpen).toBe(true)
    gate.resolve(ok({ ...APPLIED, bundle: 'lyn-new' }))
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
    expect(state().install).toMatchObject({ installed: 'lyn-new', restartRequired: false, detailsOpen: true })
  ...
  it('refuses a spec the list already shows without asking the Host, and words what the Host refused', async () => {
    const { plugins, face, state, controller } = bench({
      inspect: vi.fn()
        ...
        .mockResolvedValueOnce(ok({ status: 'refused', problem: 'not-a-bundle', reason: 'plain declares no lyn.bundle' }))
        .mockResolvedValueOnce(refused('gateway/internal', 'offline')),
    })
    await controller.load()
    face.openInstall()
    ...
    face.runInstall()
    await vi.waitFor(() => { expect(state().install.inputError).toEqual({ problem: 'not-a-bundle', reason: 'plain declares no lyn.bundle' }) })
  ...
  it('leaves the check or the failed screen for the spec at once', async () => {
    ...
    expect(state().install).toMatchObject({ open: true, phase: 'idle', spec: 'lyn-x', inputError: null })
    ...
    face.cancelInstall()
    expect(state().install).toMatchObject({ open: true, phase: 'idle', spec: 'lyn-x', failure: null, subject: null })
  })
  ...
  it('enables what a finished install added from its screen, closes, and marks it in the list', async () => {
    const { plugins, face, state, controller } = bench({
      installBundle: vi.fn()
        .mockResolvedValueOnce(ok({ ...APPLIED, bundle: 'lyn-a' }))
        .mockResolvedValueOnce(ok({ ...APPLIED, bundle: 'lyn-a' }))
        .mockResolvedValueOnce(ok({ ...APPLIED, application: 'overridden' })),
      setBundleEnabled: vi.fn()
        .mockResolvedValueOnce(ok({ ...APPLIED, application: 'restart-required' }))
    ...
    expect(plugins.setBundleEnabled).toHaveBeenCalledExactlyOnceWith('lyn-a', true)
    // A restart it waits for is said in passing; the list marks it.
    expect(state().notice).toEqual({ kind: 'restart', packageName: 'lyn-a', seq: 1 })
    expect(state().highlight).toBe('lyn-a')
    face.clearHighlight()
    face.clearHighlight()
    ...
    expect(state().notice).toEqual({ kind: 'failed', action: 'enable', code: 'operation-error', reason: 'the tree rejected it', packageName: 'lyn-a', seq: 2 })
    expect(state().highlight).toBe('lyn-a')
  ...
  it('offers the scripts a blocked run left pending, and retries the same spec with them allowed', async () => {
    ...
    expect(state().install).toMatchObject({ subject: { spec: 'x', name: 'lyn-better-sidebar' }, failure: null })
    gates[1]!.resolve(ok({ ...APPLIED, bundle: 'lyn-better-sidebar', approvedBuilds: ['native'] }))
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
    expect(state().install).toMatchObject({ installed: 'lyn-better-sidebar', approvedBuilds: ['native'] })
  ...
  it('drops an enable from the installed screen that settles after disposal', async () => {
    ...
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
    face.enableInstalled()
    expect(plugins.setBundleEnabled).toHaveBeenCalledWith('lyn-new', true)
    const before = state()
    controller.dispose()
  ...
  it('offers the registries the Host configured, starts from its first, and asks the check and the run to start where the person chose', async () => {
    ...
    face.editInstallSpec('lyn-new')
    face.runInstall()
    expect(plugins.inspect).toHaveBeenCalledWith('lyn-new', { registry: MIRROR }, expect.any(AbortSignal))
    ...
    expect(plugins.installBundle).toHaveBeenCalledWith('lyn-new', { enabled: false, requestId, registry: MIRROR })
    ...
    gate.resolve(ok({ ...APPLIED, bundle: 'lyn-new', registries: [MIRROR, null] }))
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
  ...
  it('lists pnpm\'s own configuration once when it names the registry the Host also offers', async () => {
    ...
    const storage = new Map([['lyn.plugin-manager.install-registry', JSON.stringify({ kind: 'offered', registry: MIRROR })]])
  ...
  it('keeps a remembered pnpm configuration that names its own registry', async () => {
    const storage = new Map([['lyn.plugin-manager.install-registry', JSON.stringify({ kind: 'offered', registry: null })]])
    vi.stubGlobal('localStorage', {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => { storage.set(key, value) },
      removeItem: (key: string) => { storage.delete(key) },
  ...
  it('starts from the registry the Host configured first while nothing is remembered', async () => {
    ...
    face.editInstallSpec('lyn-new')
    face.runInstall()
    expect(plugins.inspect).toHaveBeenCalledWith('lyn-new', { registry: CORP }, expect.any(AbortSignal))
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
    expect(plugins.installBundle).toHaveBeenCalledWith('lyn-new', expect.objectContaining({ registry: CORP }))
  })
  ...
  it('refuses a custom registry that is not an http(s) URL before asking the Host, and remembers the registry last used', async () => {
    ...
    try {
      ...
      face.runInstall()
      expect(plugins.inspect).toHaveBeenCalledWith('lyn-new', { registry: 'https://npm.corp.example' }, expect.any(AbortSignal))
      await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
      expect(plugins.installBundle).toHaveBeenCalledWith('lyn-new', expect.objectContaining({ registry: 'https://npm.corp.example/' }))
      ...
      storage.set('lyn.plugin-manager.install-registry', JSON.stringify({ kind: 'offered', registry: 'https://old.example/' }))
      const stale = bench()
  ...
  it('goes back to the spec with the registries open from a failed run, keeping the registries the Host asked', async () => {
    ...
    face.changeRegistry()
    expect(state().install).toMatchObject({ phase: 'idle', spec: 'lyn-new', registryOpen: true, runs: [], failure: null })
  })

  it.each(['network', 'timeout'] as const)('recovers a GitHub %s failure without retrying its URL through a mirror', async (kind) => {
    const spec = 'https://github.com/example/lyn-plugin.git'
    ...
    const { face, state, plugins } = bench({
      ...
      installBundle: vi.fn().mockResolvedValueOnce(ok({
        ...
        failedAt: 'registry',
      })).mockResolvedValue(ok({ ...APPLIED, bundle: 'lyn-new' })),
    })
    face.openInstall()
    await vi.waitFor(() => { expect(state().install.registries).toEqual(REGISTRIES) })
    ...
    await vi.waitFor(() => { expect(state().install.phase).toBe('failed') })
    face.changeRegistry()
    expect(state().install).toMatchObject({ phase: 'idle', spec: 'lyn-new', mirrorRecovery: true, registryOpen: true })
    face.runInstall()
    await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
    expect(plugins.inspect).toHaveBeenLastCalledWith('lyn-new', { registry: MIRROR }, expect.any(AbortSignal))
    expect(plugins.installBundle).toHaveBeenLastCalledWith('lyn-new', expect.objectContaining({ registry: MIRROR }))
    face.closeInstall()
    face.openInstall()
    await vi.waitFor(() => { expect(state().install.registries).toEqual(REGISTRIES) })
    expect(state().install.registry).toEqual({ kind: 'offered', registry: MIRROR })
  ...
  it.each([
    ...
    face.editInstallSpec('https://github.com/example/lyn-plugin.git')
    face.runInstall()
    await vi.waitFor(() => { expect(state().install.phase).toBe('failed') })
    face.useGithubMirror()
...
describe('Host registry response recommendation', () => {
  it('inspects the remembered registry when the offered choice becomes custom during the initial read', async () => {
    const key = 'lyn.plugin-manager.install-registry'
    ...
    try {
      ...
      expect(plugins.inspect).toHaveBeenCalledWith('lyn-new', { registry }, expect.any(AbortSignal))
      ...
      await vi.waitFor(() => { expect(state().install.phase).toBe('done') })
      expect(plugins.installBundle).toHaveBeenCalledWith('lyn-new', expect.objectContaining({ registry }))
    } finally {
      registries.resolve(ok(REGISTRIES))
      vi.unstubAllGlobals()
    }
