...
import type { PluginEntryId, PluginInstallRequestId } from '@lyness/lyn-api-remotes/client'
import { bindSnapshotSelector, stubConfigForm } from '@lyness/lyn-client-test-runtime'
import type { ConfigForm, ConfigFormSnapshot, SettingsMirrorSnapshot } from '@lyness/lyn-client-ui-settings/client'
...
function row(overrides: Partial<PackageRow> = {}): PackageRow {
  return { entryId: 'include:sidebar' as PluginEntryId, rowId: 'sidebar', moduleName: 'lyn-better-sidebar', enabled: true, phase: 'active', ...overrides }
}

const INCOMPATIBLE = { name: 'lyn-late', version: '2.0.0', runtimeVersion: '0.1.0', peers: { '@lyness/lyn': '^0.2.0', '@lyness/lyn-core': '^0.2.0' } }
...
const incompatibleText = (name = INCOMPATIBLE.name): string => en.reasonIncompatibleVersion
  .replace('{plugin}', `${name}@2.0.0`).replace('{runtime}', '0.1.0').replace('{peers}', '@lyness/lyn ^0.2.0, @lyness/lyn-core ^0.2.0')
...
describe('PluginManagerPage', () => {
  it('opens the requested bundle after its inventory arrives and falls back when it is absent', () => {
    ...
    act(() => { b.navigation.actions.setView({ kind: 'package', name: 'lyn-better-sidebar' }) })
    b.set({ status: 'ready', packages: [pkg()] })
    expect(document.querySelector('[data-plugin-detail="lyn-better-sidebar"]')).not.toBeNull()
    act(() => { b.navigation.actions.setView({ kind: 'package', name: 'missing' }) })
    expect(document.querySelector('[data-plugin-detail]')).toBeNull()
    expect(document.querySelector('[data-plugin-package="lyn-better-sidebar"]')).not.toBeNull()
  })
  ...
  it('keeps the read failure and its retry visible while a detail page is open', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
  ...
  it('lists the installed bundles as cards, the installation\'s offered ones as official, and tags a problem the Host reports', () => {
    const { actions } = renderTab({
      packages: [
        ...
        pkg({ name: 'lyn-broken', enabled: false, error: { code: 'not-bundle' } }),
        pkg({ name: '@lyness/lyn-web-app', installed: false }),
        pkg({ name: 'lyn-protected', readOnlyReason: 'management-required' }),
        pkg({ name: '@acme/lyn-tool', enabled: false }),
        // Selected by the profile but not a bundle: a problem the person can switch off, in the profile's own group.
        pkg({ name: 'lyn-selected', installed: false, error: { code: 'not-bundle' } }),
        pkg({ name: '@lyness/lyn-experimental-agent-team-profile', installed: false, optional: true, enabled: false }),
      ],
      busy: ['lyn-protected'],
    })
    ...
    expect(cards.map(card => card.getAttribute('data-plugin-package'))).toEqual([
      '@lyness/lyn-experimental-agent-team-profile', 'lyn-better-sidebar', 'lyn-broken', 'lyn-protected', '@acme/lyn-tool', 'lyn-selected',
    ])
    ...
    expect(screen.getByRole('switch', { name: en.enableToggle.replace('{name}', '@acme/lyn-tool') })).toHaveProperty('disabled', false)
    ...
    fireEvent.click(screen.getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-better-sidebar') }))
    expect(actions.setEnabled).toHaveBeenCalledWith('lyn-better-sidebar', false)
    expect(screen.getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-broken') })).toHaveProperty('disabled', true)
    const locked = screen.getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-protected') })
  ...
  it('omits built-in profile dependencies from cards and counts while retaining optional and third-party bundles', () => {
    renderTab({
      packages: [
        ...
        pkg({ name: '@acme/lyn-base', readOnlyReason: 'management-required' }),
        pkg({ name: 'lyn-better-sidebar' }),
        pkg({ name: '@lyness/lyn-experimental-agent-team-profile', installed: false, optional: true }),
      ],
    })
    expect(screen.getAllByRole('listitem').map(card => card.getAttribute('data-plugin-package'))).toEqual([
      '@lyness/lyn-experimental-agent-team-profile', '@acme/lyn-base', 'lyn-better-sidebar',
    ])
  ...
  it.each([false, true])('shows an empty list for built-in bundles with errors and installed=%s', (installed) => {
    renderTab({
      packages: ['@lyness/lyn-base', '@lyness/lyn-web-app'].map(name => pkg({
  ...
  it('opens an official bundle\'s page with its beta tag and no uninstall, and switches it on', () => {
    ...
    const { actions } = renderTab({
      packages: [pkg({ name: '@lyness/lyn-experimental-agent-team-profile', meta: { title }, installed: false, optional: true, enabled: false })],
    })
    ...
    expect(actions.setEnabled).toHaveBeenCalledExactlyOnceWith('@lyness/lyn-experimental-agent-team-profile', true)
  })

  it.each([
    '@lyness/lyn-experimental-agent-team-profile',
    '@lyness/lyn-experimental-auto-review',
    '@lyness/lyn-experimental-fixture-input',
    '@acme/lyn-local-tools',
    ...
    const assertCard = (dict: typeof en) => {
      ...
      expect(screen.queryByText(dict.statusBeta) !== null).toBe(name.startsWith('@lyness/lyn-experimental-'))
    }
    assertCard(en)
    setLanguage(zh)
    assertCard(zh)
  ...
  it('renders manifest icons for arbitrary bundles and rows, with decode fallback and source recovery', () => {
    ...
    const { set } = renderTab({ packages: [bundle] }, { rows: new Set(['lyn-better-sidebar#sidebar']) })
    ...
    set({ packages: [bundle] })
    expect(image().getAttribute('src')).toBe(icon)
    fireEvent.click(screen.getByRole('button', { name: 'View lyn-better-sidebar' }))
    expect(image().getAttribute('src')).toBe(icon)
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Configure lyn-better-sidebar' }))
  ...
  it('shows metadata diagnostics without blocking management or displaying legacy descriptions', () => {
    ...
    const enable = screen.getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-better-sidebar') })
    expect(enable).toHaveProperty('disabled', false)
    fireEvent.click(enable)
    expect(actions.setEnabled).toHaveBeenCalledExactlyOnceWith('lyn-better-sidebar', true)
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
    expect(screen.getByText(en.metadataError.replace('{error}', error))).toBeTruthy()
    const uninstall = screen.getByRole('button', { name: en.uninstallLabel.replace('{name}', 'lyn-better-sidebar') })
    expect(uninstall).toHaveProperty('disabled', false)
    fireEvent.click(uninstall)
    expect(actions.uninstall).toHaveBeenCalledExactlyOnceWith('lyn-better-sidebar')
  ...
  it.each<{
    ...
    {
      field: 'title',
      meta: { title: { en: '@acme/lyn-sidebar', zh: '侧栏套件' }, description: 'Package description.' },
      englishTitle: '@acme/lyn-sidebar', chineseTitle: '侧栏套件',
    ...
    const { setLanguage } = renderTab({ packages: [pkg({ name: '@acme/lyn-sidebar', meta })] })
    const languages = [
  ...
  it('resolves row fields independently from bundle metadata and preserves subpath specifiers', () => {
    const rows = [
      row({ moduleName: '@acme/lyn-sidebar/navigation', meta: { description: { en: 'Navigation description.' } } }),
      row({
        rowId: 'theme', entryId: 'include:theme' as PluginEntryId, moduleName: '@acme/lyn-theme/client',
        meta: { title: { en: '@acme/lyn-theme', zh: '主题插件' }, description: { en: '', zh: '中文主题说明。' } },
      }),
    ]
    const { actions, setLanguage } = renderTab(
      { packages: [pkg({ meta: { title: 'Bundle title', description: 'Bundle description.' }, rows })] },
      { rows: new Set(['lyn-better-sidebar#theme']) },
      { 'plugins.row.config:lyn-better-sidebar#theme': view => view === 'page' ? <form aria-label="theme settings" /> : null },
    )
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'Bundle title') }))
    expect(screen.getByRole('switch', { name: en.partToggle.replace('{name}', '@acme/lyn-sidebar/navigation') })).toBeTruthy()
    expect(screen.getByRole('switch', { name: en.partToggle.replace('{name}', '@acme/lyn-theme') })).toBeTruthy()
    ...
    fireEvent.click(within(navigation).getByRole('switch', { name: zh.partToggle.replace('{name}', '@acme/lyn-sidebar/navigation') }))
    ...
    expect(document.querySelector('[data-plugin-row-detail]')?.getAttribute('data-plugin-row-detail')).toBe('lyn-better-sidebar#theme')
    expect(screen.getByRole('heading', { level: 3 }).textContent).toBe('主题插件')
    expect(screen.getByText('中文主题说明。')).toBeTruthy()
    setLanguage(en)
    expect(screen.getByRole('heading', { level: 3 }).textContent).toBe('@acme/lyn-theme')
    expect(screen.getByText('@acme/lyn-theme/client')).toBeTruthy()
  ...
  it('shows a row id only once when it is the localized title in lists and configuration pages', () => {
    const moduleName = '@acme/lyn-sidebar/navigation'
    const { setLanguage } = renderTab(
      { packages: [pkg({ rows: [row({ moduleName, meta: { title: { en: 'Sidebar component', zh: 'sidebar' } } })] })] },
      { rows: new Set(['lyn-better-sidebar#sidebar']) },
      { 'plugins.row.config:lyn-better-sidebar#sidebar': view => view === 'page' ? <form aria-label="sidebar settings" /> : null },
    )
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
    ...
    const detail = document.querySelector('[data-plugin-row-detail="lyn-better-sidebar#sidebar"]') as HTMLElement
  ...
  it('translates row text, searches current copy and technical identities, and keeps configuration keys unchanged', () => {
    ...
    const localized = row({
      moduleName: '@acme/lyn-sidebar-widget',
      meta: {
    ...
    const { actions, setLanguage } = renderTab(
      ...
      { rows: new Set(['lyn-better-sidebar#sidebar']) },
      { 'plugins.row.config:lyn-better-sidebar#sidebar': view => view === 'summary' ? 'Config summary' : <form aria-label="row settings" /> },
    )
    ...
    for (const query of ['导航组件', '侧边导航', 'sidebar', '@acme/lyn-sidebar-widget']) {
    ...
    expect(screen.getByText('@acme/lyn-sidebar-widget')).toBeTruthy()
    ...
    expect(document.querySelector('[data-plugin-row-detail]')?.getAttribute('data-plugin-row-detail')).toBe('lyn-better-sidebar#sidebar')
  ...
  describe('configuration pages', () => {
    const bodies: SlotBodies = {
      ...
      'plugins.bundle.config:lyn-better-sidebar': view => view === 'page' ? <form aria-label="sidebar form" /> : null,
      'plugins.row.config:lyn-better-sidebar#sidebar': view => view === 'summary' ? 'The sidebar row.' : <form aria-label="row form" />,
    }

    it('lists an official plugin after the official bundles with its summary, and opens its page', () => {
      renderTab(
        { packages: [pkg({ name: '@lyness/lyn-experimental-agent-team-profile', installed: false, optional: true, enabled: false })] },
      ...
      expect(within(official).getAllByRole('listitem').map(card => card.getAttribute('data-plugin-item') ?? card.getAttribute('data-plugin-package')))
        .toEqual(['@lyness/lyn-experimental-agent-team-profile', 'bash'])
    ...
    it('renders a bundle\'s own configuration on its page, and no configure control on a row without one', () => {
      renderTab({ packages: [pkg({ rows: [row()] })] }, { bundles: new Set(['lyn-better-sidebar']) }, bodies)
      fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
      const detail = document.querySelector('[data-plugin-detail]') as HTMLElement
      expect(within(detail).getByRole('form', { name: 'sidebar form' })).toBeTruthy()
      expect(within(detail).queryByRole('button', { name: en.configureRow.replace('{name}', 'lyn-better-sidebar') })).toBeNull()
    })

    it('opens a row\'s configuration page from its configure control and leads back to the bundle', () => {
      const theme = row({ rowId: 'theme', moduleName: 'lyn-better-sidebar/theme', entryId: 'include:theme' as PluginEntryId })
      renderTab({ packages: [pkg({ rows: [row(), theme] })] }, { rows: new Set(['lyn-better-sidebar#sidebar']) }, bodies)
      fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
      expect(screen.queryByRole('button', { name: en.configureRow.replace('{name}', 'lyn-better-sidebar/theme') })).toBeNull()
      fireEvent.click(screen.getByRole('button', { name: en.configureRow.replace('{name}', 'lyn-better-sidebar') }))
      const page = document.querySelector('[data-plugin-row-detail="lyn-better-sidebar#sidebar"]') as HTMLElement
      expect(within(page).getByRole('heading', { level: 3 }).textContent).toBe('lyn-better-sidebar')
      expect(within(page).getByText('lyn-better-sidebar', { selector: 'code' })).toBeTruthy()
      ...
      fireEvent.click(within(page).getByRole('button', { name: en.backToPackage.replace('{name}', 'lyn-better-sidebar') }))
      expect(document.querySelector('[data-plugin-row-detail]')).toBeNull()
      expect(document.querySelector('[data-plugin-detail="lyn-better-sidebar"]')).toBeTruthy()
    })
  })
  ...
  describe('detail contributions', () => {
    ...
    it('renders the contributed actions, badges, and sections on each page, told what the page is about', () => {
      renderTab(
        ...
        { items: [{ id: 'bash', label: 'Shell' }], rows: new Set(['lyn-better-sidebar#sidebar']) },
        bodies,
      )
      ...
      fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
      const detail = document.querySelector('[data-plugin-detail]') as HTMLElement
      const act = within(detail).getByRole('button', { name: 'act bundle lyn-better-sidebar' })
      expect(within(detail).getByText('badge bundle lyn-better-sidebar')).toBeTruthy()
      const section = within(detail).getByRole('region', { name: 'section bundle lyn-better-sidebar' })
      // The contributed actions come before the page's own switch; the sections after the rows.
      const toggle = within(detail).getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-better-sidebar') })
      ...
      expect(subjects.at(-1)).toEqual({
        kind: 'bundle',
        pkg: { name: 'lyn-better-sidebar', version: '0.16.0', installed: true, enabled: true, rows: [{ rowId: 'sidebar', moduleName: 'lyn-better-sidebar', enabled: true }] },
      })

      fireEvent.click(screen.getByRole('button', { name: en.configureRow.replace('{name}', 'lyn-better-sidebar') }))
      const page = document.querySelector('[data-plugin-row-detail]') as HTMLElement
      expect(within(page).getByRole('button', { name: 'act row lyn-better-sidebar#sidebar' })).toBeTruthy()
      expect(within(page).getByText('badge row lyn-better-sidebar#sidebar')).toBeTruthy()
      expect(within(page).getByRole('region', { name: 'section row lyn-better-sidebar#sidebar' })).toBeTruthy()
      expect(subjects.at(-1)).toMatchObject({ kind: 'row', row: { rowId: 'sidebar', moduleName: 'lyn-better-sidebar', enabled: true } })
      fireEvent.click(within(page).getByRole('button', { name: en.backToPackage.replace('{name}', 'lyn-better-sidebar') }))
    ...
    it('leaves the version out of a bundle the Host reports none for', () => {
      const unversioned: PackageView = { name: 'lyn-better-sidebar', installed: true, optional: false, enabled: true, rows: [] }
      renderTab({ packages: [unversioned] }, {}, bodies)
      fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
      expect(subjects.at(-1)).toEqual({ kind: 'bundle', pkg: { name: 'lyn-better-sidebar', installed: true, enabled: true, rows: [] } })
    })
  })

  it('preserves the full package name and description for a third-party scope', () => {
    const name = '@acme/lyn-experimental-agent-team-profile'
  ...
  it('opens a bundle\'s page with its facts and rows, and uninstalls from it', () => {
    const { actions, set } = renderTab({
      packages: [pkg({
        ...
        rows: [row(), row({ rowId: 'theme', moduleName: 'lyn-better-sidebar/theme', entryId: 'include:theme' as PluginEntryId, enabled: false, phase: null })],
      })],
    })
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', 'lyn-better-sidebar') }))
    const detail = document.querySelector('[data-plugin-detail="lyn-better-sidebar"]') as HTMLElement
    expect(within(detail).getByRole('heading', { level: 3 }).textContent).toBe('lyn-better-sidebar')
    // The version sits beside the name as a tag; the crumb only leads back.
    expect(within(detail).getByText('v0.16.0')).toBeTruthy()
    expect(document.querySelector('[data-plugin-name]')?.textContent).toBe('lyn-better-sidebar')
    ...
    expect(within(detail).getByRole('switch', { name: en.partToggle.replace('{name}', 'lyn-better-sidebar/theme') })).toBeTruthy()
    expect(within(detail).getByText(en.rowPhaseActive)).toBeTruthy()
    expect(within(detail).getByText(en.partOff)).toBeTruthy()
    fireEvent.click(within(detail).getByRole('button', { name: en.uninstallLabel.replace('{name}', 'lyn-better-sidebar') }))
    expect(actions.uninstall).toHaveBeenCalledWith('lyn-better-sidebar')
    fireEvent.click(within(detail).getByRole('switch', { name: en.enableToggle.replace('{name}', 'lyn-better-sidebar') }))
    expect(actions.setEnabled).toHaveBeenCalledWith('lyn-better-sidebar', false)
    ...
    expect(within(detail).getByRole('button', { name: en.uninstallLabel.replace('{name}', 'lyn-better-sidebar') })).toHaveProperty('disabled', true)
  ...
  it('switches the rows of a bundle that is on, filters a long list, and locks what the Host will not address', () => {
    ...
    const toggle = (id: string): HTMLElement => within(detail.querySelector<HTMLElement>(`[data-plugin-row="${id}"]`)!)
      .getByRole('switch', { name: en.partToggle.replace('{name}', 'lyn-better-sidebar') })
    ...
    expect(within(detail).queryByRole('switch', { name: en.partToggle.replace('{name}', 'lyn-better-sidebar') })).toBeNull()
  ...
  it('takes a spec, checks it, and words what the check refused', () => {
    ...
    fireEvent.change(field, { target: { value: 'lyn-x' } })
    expect(actions.editInstallSpec).toHaveBeenCalledWith('lyn-x')
    expect(document.querySelector('[data-terminal]')).toBeNull()
    expect(screen.queryByRole('checkbox')).toBeNull()

    set({ install: { ...IDLE_INSTALL, open: true, spec: ' lyn-x ' } })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'checking' } })
    ...
    for (const [problem, sentence] of problems) {
      set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', inputError: { problem: problem as never, reason: 'r' } } })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', inputError: { problem: 'network', reason: 'r', registries: [null, MIRROR] } } })
  ...
  it('shows the subject while installing, folds the pnpm output behind the details, and stops through the Host', () => {
    const subject = { spec: 'lyn-x', status: 'accepted', kind: 'registry', name: 'lyn-x', version: '1.4.2', description: 'A sidebar.', bundle: true, registry: null } as const
    const run = { jobId: 'j1', command: 'pnpm add lyn-x', cwd: '/home/u/.lyn/profiles/web', output: 'Progress: resolved \x1b[96m1\x1b[39m\n' }
    const { actions, set } = renderTab({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, runs: [run] } })
    ...
    expect(screen.getByText('lyn-x')).toBeTruthy()
    expect(screen.getByText('A sidebar.')).toBeTruthy()
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, runs: [run], detailsOpen: true } })
    expect(screen.getByRole('button', { name: en.installDetailsHide }).getAttribute('aria-expanded')).toBe('true')
    expect(screen.getByText(en.installLocation.replace('{dir}', '/home/u/.lyn/profiles/web'))).toBeTruthy()
    // The run streams as a terminal: its command line, its coloured output so far, the running label.
    expect(screen.getByText('pnpm add lyn-x')).toBeTruthy()
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, detailsOpen: true, runs: [{ ...run, output: `${lines}\n` }] } })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, detailsOpen: true } })
  ...
  it('offers to enable what a finished install added, and says when it waits for a restart', () => {
    const subject = { spec: '/plugins/lyn-x', status: 'accepted', kind: 'path', name: 'lyn-x', bundle: true, registry: null } as const
    const { actions, set } = renderTab({
      install: {
        ...IDLE_INSTALL,
        open: true,
        ...
        phase: 'done',
        subject,
        runs: [{ jobId: 'j1', command: 'pnpm add /plugins/lyn-x', cwd: '/p', output: 'Done in 1s\n', exitCode: 0 }],
        installed: 'lyn-x',
      },
    })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: '/plugins/lyn-x', phase: 'done', subject, installed: 'lyn-x', restartRequired: true, enabling: true } })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'done', subject: { spec: 'lyn-x', status: 'accepted', kind: 'registry', name: 'lyn-x', bundle: true, registry: null } } })
  ...
  it('asks to allow the scripts a blocked install left pending, retries with them, and says what was allowed', () => {
    const subject = { spec: 'lyn-x', status: 'accepted', kind: 'registry', name: 'lyn-x', bundle: true, registry: null } as const
    const { actions, set } = renderTab({
      install: {
        ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject,
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject, failure: { reason: 'ERR', kind: 'build-blocked' } } })
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'done', subject, installed: 'lyn-x', approvedBuilds: ['native'] } })
  ...
  it('offers the registries under the spec, folded by default, and picks or types one', () => {
    ...
    expect(actions.runInstall).not.toHaveBeenCalled()
    set({ install: { ...open, spec: 'lyn-x', registryOpen: true, registry: { kind: 'custom', url: 'https://npm.corp/' } } })
  ...
  it('names the registry each attempt asks while installing, badges each run, and says when every registry failed', () => {
    const subject = { spec: 'lyn-x', status: 'accepted', kind: 'registry', name: 'lyn-x', bundle: true, registry: null } as const
    const runs = [
      { jobId: 'j1', command: 'pnpm add lyn-x', cwd: '/p', output: 'ERR\n', exitCode: 1 },
      { jobId: 'j2', command: `pnpm add lyn-x --registry=${MIRROR}`, cwd: '/p', output: 'Progress\n' },
    ]
    const { actions, set } = renderTab({
      install: {
        ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, runs, detailsOpen: true, attempts: { registries: [null, MIRROR], total: 2 },
      },
    })
    expect(screen.getByRole('status').textContent).toBe(en.installingTitle)
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'running', subject, runs: [runs[0] as never], detailsOpen: true, attempts: { registries: [null], total: 2 } } })
    ...
    set({
      install: {
        ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject, runs,
    ...
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject, attempts: { registries: [null], total: 1 }, failure: { reason: 'ERR', kind: 'network', failedAt: 'registry' } } })
    expect(screen.getByText(en.installFailureNetwork)).toBeTruthy()
    expect(screen.getByRole('button', { name: en.installChangeRegistry })).toBeTruthy()
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject, attempts: { registries: [null], total: 1 }, failure: { reason: 'ERR', kind: 'not-found', failedAt: 'registry' } } })
    expect(screen.getByRole('button', { name: en.installChangeRegistry })).toBeTruthy()
    // A failure the Host laid at neither offers no other registry.
    set({ install: { ...IDLE_INSTALL, open: true, spec: 'lyn-x', phase: 'failed', subject, attempts: { registries: [null], total: 1 }, failure: { reason: 'ERR', kind: 'disk-full' } } })
    expect(screen.queryByRole('button', { name: en.installChangeRegistry })).toBeNull()
  })
  ...
  it('scrolls to and marks the package an install enabled, then lets the mark go', () => {
    ...
    try {
      ...
      set({ highlight: 'lyn-better-sidebar' })
      expect(document.querySelector('[data-plugin-package="lyn-better-sidebar"]')?.hasAttribute('data-plugin-highlight')).toBe(true)
      expect(scrollIntoView).toHaveBeenCalledWith({ block: 'center', behavior: 'smooth' })
      act(() => { vi.advanceTimersByTime(2_400) })
      expect(actions.clearHighlight).toHaveBeenCalledTimes(2)
      set({ highlight: null })
      expect(document.querySelector('[data-plugin-package="lyn-better-sidebar"]')?.hasAttribute('data-plugin-highlight')).toBe(false)
    } finally {
  ...
  it('confirms an uninstall by the package\'s title and runs the action through it', () => {
    const { actions, set } = renderTab({
      packages: [pkg(), pkg({ name: 'lyn-other' })],
      confirm: { action: 'uninstall', packageName: 'lyn-better-sidebar' },
    })
    expect(screen.getByRole('dialog', { name: en.confirmUninstallTitle.replace('{name}', 'lyn-better-sidebar') })).toBeTruthy()
    ...
    set({ confirm: { action: 'uninstall', packageName: 'lyn-other' } })
    expect(screen.getByRole('dialog', { name: en.confirmUninstallTitle.replace('{name}', 'lyn-other') })).toBeTruthy()
    set({ packages: [] })
    expect(screen.getByRole('dialog', { name: en.confirmUninstallTitle.replace('{name}', 'lyn-other') })).toBeTruthy()
