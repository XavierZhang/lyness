...
import type { GlobalStandardProps, PropsRuntime } from '@lyness/lyn-client-ui-slots'
...
import type { WorkspaceSnapshot, WorkspaceView } from '@lyness/lyn-api-workspace-controller/client'
...
import type { SessionStatusSnapshot } from '@lyness/lyn-client-ui-session/client'
...
describe('ConversationRoot resident composer', () => {
  ...
  it('publishes the column width as a px variable for the shared width axis', () => {
    ...
    expect(root.style.getPropertyValue('--lyn-conversation-column-width')).toBe('1200px')
    // No dragged preference: the user-width override stays absent so the
    // adaptive clamp term applies.
    expect(root.style.getPropertyValue('--lyn-chat-user-width')).toBe('')
  })
  ...
  it('drag → persist → window clamp round-trip on a width handle', () => {
    ...
    try {
      ...
      expect(handle.style.getPropertyValue('--lyn-width-handle-pointer-y')).toBe('300px')
      fireEvent.pointerUp(handle, { pointerId: 1, clientX: 825, clientY: 300 })
      expect(handle.hasAttribute('data-dragging')).toBe(false)
      expect(root.style.getPropertyValue('--lyn-chat-user-width')).toBe('970px')
      expect(localStorage.getItem('lyn.conversation.contentWidth')).toBe('970')
      ...
      act(() => { fireResize(content) })
      expect(root.style.getPropertyValue('--lyn-chat-user-width')).toBe('724px')
      expect(localStorage.getItem('lyn.conversation.contentWidth')).toBe('970')
  ...
  it('starts width dragging only from the primary pointer button', () => {
    ...
    expect(handle.style.getPropertyValue('--lyn-width-handle-pointer-y')).toBe('')
  })
