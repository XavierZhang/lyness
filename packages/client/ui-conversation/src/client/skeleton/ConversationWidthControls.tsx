...
function WidthHandle(props: {
  ...
  const onPointerMove = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    event.currentTarget.style.setProperty('--lyn-width-handle-pointer-y', `${event.clientY - box.top}px`)
    latest.current = event.clientX
...
export function ConversationWidthControls({ container, phase }: ConversationWidthControlsProps) {
  const publishWidths = useCallback((container: HTMLDivElement): void => {
    ...
    target.style.setProperty('--lyn-conversation-column-width', `${column}px`)
    const preference = readWidthPreference()
    if (preference === null) target.style.removeProperty('--lyn-chat-user-width')
    else target.style.setProperty('--lyn-chat-user-width', `${resolveContentWidth(column, preference)}px`)
  }, [])
  ...
  const onDrag = useCallback((width: number): void => {
    if (container === null) return
    const target = container.parentElement ?? container
    target.style.setProperty('--lyn-chat-user-width', `${resolveContentWidth(container.offsetWidth, width)}px`)
  }, [container])
