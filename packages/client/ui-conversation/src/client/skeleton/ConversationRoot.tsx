...
const WIDTH_PREF_KEY = 'lyn.conversation.contentWidth'
...
function WidthHandle(props: {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    e.currentTarget.style.setProperty('--lyn-width-handle-pointer-y', `${e.clientY - box.top}px`)
...
export function ConversationRoot({
  ...
  // scroll body: the seat's height as --lyn-composer-height, so controls clear
  // the composer as it grows, and the scrollport's own height as
  // --lyn-conversation-viewport-height, so a control can sit in the band the
  ...
  const seatResizeRef = useCallback((seat: HTMLDivElement | null): void => {
    ...
    seatObserver.current = new ResizeObserver(() => {
      scroller.style.setProperty('--lyn-composer-height', `${seat.offsetHeight}px`)
      scroller.style.setProperty(
        '--lyn-conversation-viewport-height',
  ...
  // Publishes the column's live width as --lyn-conversation-column-width so
  ...
  const publishWidths = useCallback((root: HTMLDivElement): void => {
    ...
    root.style.setProperty('--lyn-conversation-column-width', `${column}px`)
    const preference = readWidthPreference()
    if (preference === null) {
      root.style.removeProperty('--lyn-chat-user-width')
    } else {
      root.style.setProperty('--lyn-chat-user-width', `${resolveContentWidth(column, preference)}px`)
    }
  }, [])
  ...
  const onHandleDrag = useCallback((width: number): void => {
    ...
    root.style.setProperty('--lyn-chat-user-width', `${clamped}px`)
  }, [])
