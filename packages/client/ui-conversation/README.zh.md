...
控件组的尺寸、内容、可见性或字体加载状态变化后，composer 测量展开状态下的控件组。若无法排在同一行，控制栏为模型位设置 `--lyn-composer-model-text-display: none` 和 `--lyn-composer-model-icon-display: block`；两者默认值分别为 `block` 和 `none`。若连图标也放不下，仍允许换行。
...
当会话被其他写句柄占用时，发送失败的 toast 提示用户退出其他正在运行的 LYN 后重试。
