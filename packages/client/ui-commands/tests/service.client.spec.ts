...
import { CommandDefinitionId } from '@lyness/lyn-commands/brand'
import { scopeOf } from '@lyness/lyn-api-session-controller/client'
import type { SessionId } from '@lyness/lyn-session/types'
import { RemoteError, TestRemote, TestSessions } from '@lyness/lyn-client-test-runtime'
import type { SessionFixture } from '@lyness/lyn-client-test-runtime'
import { IconGoalOutlineRegular } from '@lyness/lyn-client-ui-primitives'
import type { ClientSessionContext, ConsumeTokenRequest, InputTriggerPick, InputTriggerSource, SubmitAttachment } from '@lyness/lyn-client-ui-input-trigger/client'
...
describe('candidates', () => {
  ...
  it('localizes canonical built-in and contribution descriptions on every candidate request', async () => {
    let locale = 'zh'
    const commands: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@lyness/lyn-command-compact'), name: 'compact', description: 'Compact older conversation history' },
  ...
  describe('menu presentation (design doc for #3567)', () => {
    ...
    const SHIPPED: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@lyness/lyn-command-compact'), name: 'compact', description: 'Compact older conversation history' },
      { definitionId: CommandDefinitionId('@lyness/lyn-session-log-export'), name: 'export', description: 'Download this Session log as a ZIP archive' },
      { definitionId: CommandDefinitionId('@lyness/lyn-command-feedback'), name: 'feedback', description: 'Record feedback about this session', input: { hint: '<text>' } },
      { definitionId: CommandDefinitionId('@lyness/lyn-command-goal'), name: 'goal', description: 'Set or view the goal for a long-running task', input: { hint: '<objective>', attachments: true } },
      { definitionId: CommandDefinitionId('@lyness/lyn-permission-presets'), name: 'permission', description: 'Switch the permission preset (sandbox mode + approval policy)', input: { hint: '<preset>' } },
      { definitionId: CommandDefinitionId('@lyness/lyn-plan-mode'), name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
