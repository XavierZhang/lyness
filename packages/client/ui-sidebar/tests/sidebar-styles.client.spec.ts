...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('--lyn-sidebar-inline-padding')).toBe('12px')
    expect(root?.get('padding')).toBe('6px var(--lyn-sidebar-inline-padding)')
    ...
    expect(declarations('.regionArea')?.get('margin-right')).toBe(
      'calc(-1 * var(--lyn-sidebar-inline-padding))',
    )
