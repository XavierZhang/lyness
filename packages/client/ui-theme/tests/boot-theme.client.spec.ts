...
describe('theme bootstrap row', () => {
  it('colors the body with head CSS before applying body state', () => {
    ...
    expect(head.text).toBe(':root{color-scheme:dark}body{background-color:#151517;--lyn-boot-bg:#151517}')
  ...
  it('lets durable light override a dark OS and clears stale dark state', () => {
    ...
    if (head?.kind !== 'style') throw new Error('theme head bootstrap row is not a style')
    expect(head.text).toBe(':root{color-scheme:light}body{background-color:#fff;--lyn-boot-bg:#fff}')
  ...
  it('uses a media query for the system canvas palette', () => {
    ...
    expect(head.text).toBe(
      ':root{color-scheme:light}body{background-color:#fff;--lyn-boot-bg:#fff}'
      + '@media(prefers-color-scheme:dark){:root{color-scheme:dark}body{background-color:#151517;--lyn-boot-bg:#151517}}',
    )
  })
  ...
  it('writes the durable content font size and defaults it to 14px', () => {
    ...
    expect(document.body.style.getPropertyValue('--lyn-content-font-size')).toBe('17px')
    executeBootstrap('light')
    expect(document.body.style.getPropertyValue('--lyn-content-font-size')).toBe('14px')
  })
})
