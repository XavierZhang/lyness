...
function bootThemeStyle(preference: ThemePreference): string {
  const light = `:root{color-scheme:light}body{background-color:${LIGHT_BACKGROUND};--lyn-boot-bg:${LIGHT_BACKGROUND}}`
  const dark = `:root{color-scheme:dark}body{background-color:${DARK_BACKGROUND};--lyn-boot-bg:${DARK_BACKGROUND}}`
...
function bootThemeBodyScript(preference: ThemePreference, fontSize: number): string {
  ...
  document.body.style.setProperty('--lyn-content-font-size', ${JSON.stringify(`${fontSize}px`)})
})()`
}
