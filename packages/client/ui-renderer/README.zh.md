---
description: "浏览器 UI 渲染器：React slot 绑定、ctx.uiRenderer 与 lyn Web 客户端组装后应用的应用根。"
...
插件在 `slots`、`sessions` 与 `layout` 就绪后激活；它安装 `createSlotRenderer()` 并 reflect `uiRenderer` 服务。`mountApp` 会查找启动内核的 `[data-lyn-boot]` 元素：存在时经 `BootHandoff`（一个保留加载 DOM 的单帧透传）hydrate，否则创建全新 root 并同步 flush 渲染。
