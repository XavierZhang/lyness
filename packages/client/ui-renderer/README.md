---
description: "Browser UI renderer: React bindings for ordinary Slots and reusable Component Factories, ctx.uiRenderer, and the assembled lyn web application root."
...
This package is infrastructure: the web shell and the boot kernel are its only direct consumers. A composition needs it whenever it wants a React-rendered GUI — `lyn-client-web` loads the roster, waits for every entry to activate, then calls `ctx.uiRenderer.mount(container)`.
