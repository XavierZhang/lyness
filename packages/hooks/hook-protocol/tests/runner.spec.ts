...
import type { ShellExecRequest, ShellExecSpec, ShellExecution, ShellExecutor, ShellRunResult } from '@lyness/lyn-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@lyness/lyn-hook-protocol'
import type { RunHookOptions } from '@lyness/lyn-hook-protocol'
 ...
 * hook — the REAL executor (lyn-bash-local) is exercised end-to-end by the
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash: Pick<ShellExecutor, 'resolve' | 'execute'> = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what lyn-bash-local's resolve does for the fields runHook sets.
      return {
