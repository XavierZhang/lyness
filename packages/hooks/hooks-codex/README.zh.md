...
`lyn-hooks-codex` 在 agent（智能体）运行期间执行现有 Codex `hooks.json` 中的 command 钩子，让提示词与工具把关逻辑无需重写即可生效。它支持 5 个 Codex hook 点：会话开始、提示词提交、工具执行前后以及停止。钩子可以用模型可见的原因阻塞提示词或工具调用、添加对话上下文，或强制 agent 再执行一步。需要在 harness 中复用 Codex command 钩子时选择本包；超出这一受支持子集的行为应使用原生插件。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-hooks-codex)是每个受支持字段的穷尽式真源。
...
- [生成的配置目录](../../../docs/config-catalog.zh.md#lynesslyn-hooks-codex)——每个受支持配置字段及其源声明。
