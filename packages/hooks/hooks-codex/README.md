...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesshooks-codex) is the exhaustive source for every accepted field.
...
`SessionStart` is the one emit point and runs detached — no extension point awaits it. Each run chain is tracked, and disposing the bridge aborts a still-running hook process, then drains the continuation before the dispose resolves (`createDetachedRuns` in `lyn-hook-protocol`).
...
| [`src/invariant.ts`](src/invariant.ts) | Invariant companion (no runtime invariant; the `hook/*` pairing checks live in `lyn-hook-protocol`) |
...
- [Generated configuration catalog](../../../docs/config-catalog.md#lynesshooks-codex) — every accepted config field and its source declaration.
