...
describe('ripgrep resolution', () => {
  ...
  it('uses the unpacked executable path for an Electron ASAR dependency', async () => {
    ...
    dependency.rgPath = '/Applications/lyness.app/Contents/Resources/app.asar/lyn/node_modules/@vscode/ripgrep/bin/rg'
    const { resolveRgPath } = await import('@lyness/lyn-tool-fs-search')

    await expect(resolveRgPath()).resolves.toBe(
      '/Applications/lyness.app/Contents/Resources/app.asar.unpacked/lyn/node_modules/@vscode/ripgrep/bin/rg',
    )
  })
})
