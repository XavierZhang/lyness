...
import { SESSION_FORMAT_VERSION, Session, SessionId, SessionLogOffset, SessionSeq } from '@lyness/lyn-session'
...
async function seed(version: 0 | 1, compression: JsonlCompression, refuse = false) {
  const root = await mkdtemp(join(tmpdir(), 'lyn-multi-edge-publication-'))
  roots.push(root)
