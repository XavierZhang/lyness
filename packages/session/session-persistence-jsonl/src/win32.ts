 ...
 *
 * @module lyn-session-persistence-jsonl/win32
 */
...
export async function acquireLockHandleWin32(path: string): Promise<number> {
  const api = await win32()
  const name = `Local\\lyn-session-lock-${createHash('sha256').update(resolve(path).toLowerCase()).digest('hex')}`
...
async function createLeafDirectoryWin32(parent: string, target: string): Promise<void> {
  ...
  const staging = await mkdtemp(toNamespacedPath(join(parent, '.lyn-mkdir-')))
  try {
