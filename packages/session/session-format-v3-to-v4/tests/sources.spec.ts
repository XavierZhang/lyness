...
describe('rewritePluginSource', () => {
  it('rewrites renamed and role-sensitive producers, dropping the plugin field', () => {
    ...
    expect(rewritePluginSource({ kind: 'plugin', plugin: '@lyness/lyn-system-prompt' }, 1, 'system')).toEqual({ kind: 'system-prompt' })
    expect(rewritePluginSource({ kind: 'plugin', plugin: '@lyness/lyn-system-prompt' }, 1, 'user')).toEqual({ kind: 'runtime-context' })
  ...
  it('namespaces external plugin names while retaining released first-party producers', () => {
    ...
    expect(rewritePluginSource({ kind: 'plugin', plugin: '@lyness/lyn-system-prompt' }, 3, 'system')).toEqual({ kind: 'system-prompt' })
  })
})
