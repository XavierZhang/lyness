...
import type { SessionFormatArtifact, SessionFormatEvent } from '@lyness/lyn-session-format'
...
function source() {
  const user = { id: 'context', role: 'user', source: { kind: 'plugin', plugin: '@lyness/lyn-system-prompt' }, content: [{ type: 'text', text: 'context' }] }
