...
# @lyness/lyn-session-format-v3-to-v4
...
| `@lyness/lyn-system-prompt`, on a system-role message | `system-prompt` |
| `@lyness/lyn-system-prompt`, on another role | `runtime-context` |
...
| `session/title`, `session/title-llm-request` | Cite distinct earlier human `user/message` events. User-assigned titles have no citations; other titles have citations. An LLM title request has nonempty citations and one user-role text message sourced from `lyn-session-title-llm`. |
