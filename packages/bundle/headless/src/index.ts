...
function currentPreset(header: AdoptableHeader, events: Iterable<SessionEvent>, sessionId: SessionId): string | undefined {
  ...
  for (const event of events) {
    // Owned by lyn-agent-preset-registry, which this bundle does not compose, so the
...
async function resolveAgent(
  ...
  if (query === undefined) {
    throw new Error('headless --session-id requires the sessionQuery service; lyn-base provides it')
  }
...
async function run(ctx: Context, config: Config, io: HeadlessIo): Promise<void> {
  ...
  const task = config.task === undefined || config.task === '-'
    ...
    : config.task
  if (task.trim() === '') {
    throw new Error('a task is required, for example: lyn --profile headless "run the tests"')
  }
  ...
  // (@lyness/lyn-agent-preset-registry README, "Composing a child agent").
  ...
  try {
    ...
    if (outcome.reason?.kind === 'error') {
      io.stderr.write(`lyn: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
    }
