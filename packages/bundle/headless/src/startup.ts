...
function headlessCommand(): Command {
  return new Command()
    ...
    .addHelpText('after', `
Examples:
  lyn --profile headless "run the tests"          answer one task and exit
  echo "run the tests" | lyn --profile headless   read the task from stdin
  lyn --profile headless --json "run the tests"   emit machine-readable run events
  lyn --profile headless --session-id session-… "continue"   resume an existing Session
`)
}
...
export function apply(ctx: Context): void {
  ...
  if (jsonRequested(ctx.get('cmdlineArgs')?.get() ?? [])) {
    program.error = (message: string, errorOptions?: Parameters<Command['error']>[1]): never => {
      ...
      // The JSON contract keeps stderr to `lyn:` diagnostics, so commander's
  ...
  program.action(() => {
    ...
    if (program.args.length > 0 && joined.trim() === '') {
      program.error('error: a task is required, for example: lyn --profile headless "run the tests"')
    }
