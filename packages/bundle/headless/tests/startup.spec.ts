...
import { internals as cmdlineInternals, provideCmdline } from '@lyness/lyn-cmdline'
...
async function bootStartup(
  ...
  const dir = mkdtempSync(join(tmpdir(), 'lyn-headless-startup-'))
  tempDirs.push(dir)
...
describe('headless command-line provider', () => {
  ...
  it('writes the JSON error event for a --json usage error', async () => {
    ...
    expect(first).toEqual({
      type: 'error',
      message: 'a task is required, for example: lyn --profile headless "run the tests"',
    })
    expect(task).toBeUndefined()
  ...
  it('prints its own help and leaves the runner pending', async () => {
    ...
    expect(observed.out).toContain('lyn --profile headless')
