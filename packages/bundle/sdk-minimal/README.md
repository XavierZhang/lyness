...
Use `lyn --profile sdk-minimal` when an SDK client needs a small, explicit coding-agent runtime. By default, the profile advertises only a platform-selected persistent shell, persists sessions as uncompressed JSONL, and selects the model from the SDK initialization request. It supplies a complete Cordis tree and deliberately excludes `lyn-base`, Web, settings, managed credentials, telemetry, compaction, filesystem tools, workspace instructions, skills, jobs, and subagents. Its danger-full-access policy lets the shell modify any path available to the process, so use it only with an isolated workspace.
...
Launch the profile directly or select it from the Python SDK. Supply an explicit `LYNESS_HOME`, use a disposable workspace, and provide the model credential through `DEEPSEEK_API_KEY`.

```sh
export LYNESS_HOME=/absolute/path/to/example-lyn-home
lyn --profile sdk-minimal
```

`LYNESS_CONTEXT_WINDOW` sets the fallback capacity for a model absent from the adapter's advisory catalog. `LYNESS_SYSTEM_PROMPT` replaces the default persona. The SDK initialization request is the sole model selection and overrides environment defaults.

Use `lyn plugin --profile sdk-minimal` to manage persistent external dependencies. Profile, home, and ordered `--patch` files can replace rows or insert bundles above the complete default tree. The shipped template applies patches only at startup.
...
The bundle's single insert is the complete application tree: SDK stdio startup and JSON-RPC serving, one environment-configured DeepSeek adapter, the explicit agent core, conditional MCP resource tools, local subprocess execution, a platform-selected persistent shell PTY, and uncompressed JSONL persistence under `$LYNESS_HOME/sessions`. It does not inherit another bundle, so every extra row is an explicit profile change.
...
- **The composition intentionally omits shared product services** — select `lyn --profile sdk` when settings, managed credentials, policy presets, telemetry, Web tools, or the full default tool roster are required.
