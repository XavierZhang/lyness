...
import { OfficeSourceKey, type OfficeToPdfRequest } from '@lyness/lyn-office-to-pdf'
import * as LibreOfficeProvider from '@lyness/lyn-office-to-pdf'
import { expect, it, onTestFinished } from 'vitest'

it('loads one shared conversion row and retains caller-owned PDFs after disposal', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'lyn-conversion-loader-'))
  ...
  const rows = loadOverlayPatches('conversion-test', fileURLToPath(new URL('../cordis.patch.yml', import.meta.url)))
    ...
    .filter(row => row.name === '@lyness/lyn-office-to-pdf')
  ...
  ctx.loader.internal = {
    ...
    async import(specifier: string) {
      if (specifier !== '@lyness/lyn-office-to-pdf') throw new Error(`Unexpected plugin: ${specifier}`)
