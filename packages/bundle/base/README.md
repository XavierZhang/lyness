---
description: "The shared lyn core: model access, tools, durable sessions, and safety defaults for every lyn --profile surface, for users composing or customizing a profile."
kind: "package-bundle"
---
...
Every base-backed `lyn --profile` surface runs on `lyn-base`, so those surfaces share a model connection, the full tool set, durable session history, and workspace safety defaults. The shipped `sdk-minimal` profile deliberately uses a complete standalone tree instead. You rarely touch this bundle directly — shipped base-backed profiles already include it, and a custom base-backed profile names it first. When you need different defaults, change your profile patch or add a later bundle; this package is not a library you import.
...
You get the lyn core automatically: the shipped `web`, `headless`, `sdk`, and `acp` profiles already include it, and a custom profile names it as its first bundle. After that, everything works with no further configuration.
...
Run `lyn --profile my-profile "your task"` and you get a working agent with model access, tools, persistence, and the default permission policy. The shipped `web`, `headless`, `sdk`, and `acp` profiles are created for you on first use. To add more bundles, run `lyn plugin --profile <name> add <package>`; in-box bundles resolve from the lyn installation. The profile contract is documented in the [app-boot profile section](../../boot/app-boot/README.md).
