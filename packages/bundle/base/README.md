...
You get the lyn core automatically: the shipped `web` and `headless` profiles already include it, and a custom profile names it as its first bundle. After that, everything works with no further configuration.
...
Run `lyn --profile my-profile "your task"` and you get a working agent with model access, tools, persistence, and the default permission policy. The shipped `web` and `headless` profiles are created for you on first use. To add more bundles, run `lyn plugin --profile <name> add <package>`; in-box bundles resolve from the lyn installation. The profile contract is documented in the [app-boot profile section](../../boot/app-boot/README.md).
