...
interface FixtureGlobal {
  __lynLazyRequireLoads?: number
}

const fixtureGlobal = globalThis as FixtureGlobal

afterEach(() => {
  delete fixtureGlobal.__lynLazyRequireLoads
})
...
it('resolves from the caller and caches the successful module value', () => {
  ...
  expect(fixtureGlobal.__lynLazyRequireLoads).toBeUndefined()
  const first = load()
  expect(first).toEqual({ value: 42 })
  expect(load()).toBe(first)
  expect(fixtureGlobal.__lynLazyRequireLoads).toBe(1)
})
