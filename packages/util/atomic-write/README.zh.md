...
`lyn-atomic-write` 一步原子地替换文件内容：目标的读取方总是看到完整的旧内容或完整的新内容，绝不看到部分写入。它还通过写锁跨进程串行化读-渲染-提交循环，因此同一文件的并发写入方无法复活彼此替换掉的状态。调用方为每次替换声明权限位，全新 inode 会带着这些权限位走完交换，因此替换权限过宽的旧文件时会直接收窄，不存在 chmod 竞态。它是一个零依赖库，由用户设置文档与凭据存储这类文件型存储共享；`cordis.yml` 无法加载它，而且由于没有 `fsync`，崩溃持久性由调用方负责。
...
await writeFileAtomic('/home/u/.lyn/settings.yaml', text, { mode: 0o600 })
```
...
await withFileLock('/home/u/.lyn/settings.yaml', async () => {
  const previous = await readCurrent()
  await writeFileAtomic('/home/u/.lyn/settings.yaml', render(previous), { mode: 0o600 })
})
```
