...
# @lyness/lyn-atomic-write

[English](README.md) | 中文

## 概述

使用 `lyn-atomic-write` 替换文件时，不会暴露部分内容，也不会跟随临时路径上的符号链接。它的写锁会跨进程串行化读-修改-写入循环，因此并发写入方不会用陈旧状态相互覆盖。每次替换都会在全新 inode 上使用调用方选择的权限位，从而安全地收窄现有文件的权限。这个零依赖库只接受字符串；它不提供 `cordis.yml` 插件，也不保证崩溃持久性，因为它不调用 `fsync`。
...
await writeFileAtomic('/home/u/.lyn/cordis.patch.yml', text, { mode: 0o600 })
```
...
await withFileLock('/home/u/.lyn/cordis.patch.yml', async () => {
  const previous = await readCurrent()
  await writeFileAtomic('/home/u/.lyn/cordis.patch.yml', render(previous), { mode: 0o600 })
})
```
