...
```ts
import { brandNumber, type BrandedNumber } from '@lyness/lyn-brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `lyn-llm`, the shared agent/session `SessionId` in `lyn-session`, `JobId` in `lyn-jobs`, and `SessionSeq` versus `SessionLogOffset` in `lyn-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `lyn-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
