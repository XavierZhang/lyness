 ...
 *
 * Policy: a package brands the ids it owns — `ToolCallId` in lyn-llm (tool-call
 * correlation), the shared agent/session `SessionId` in lyn-session, and
 * `JobId` in lyn-jobs. Branding is for ids that cross package boundaries and
