---
description: "lyness 主目录与用户数据路径的共享解析，供需要统一根目录、波浪号展开与稳定监听路径的包使用。"
...
`lyn-home-paths` 解析所有用户数据所在的统一 lyness 主目录，并把子路径拼接上去，让每个产品包都就文件存放位置达成一致。优先级是显式的：显式配置的路径优先，然后是 `$LYNESS_HOME`，最后是 `~/.lyn`；空或仅含空白的 `$LYNESS_HOME` 视为未设置。该包还针对操作系统主目录展开 `~`、`~/...` 与 `~\...` 前缀，并规范化监听目标，让原生文件系统 watcher 即使在最终路径段尚不存在时也能获得一种稳定的路径写法。它是一个零依赖库，由产品包直接导入；`cordis.yml` 无法加载它。
...
显式配置的路径优先级最高，然后是 `$LYNESS_HOME`，最后是默认的 `~/.lyn`。空或仅含空白的 `$LYNESS_HOME` 视为未设置，因此空白的覆盖值绝不会把主目录解析到当前工作目录。

### 展示主目录

面向用户的路径请以符号形式渲染根目录，而不是机器路径：默认主目录显示为 `~/.lyn`，任何已配置的主目录显示为 `$LYNESS_HOME`。展示形式绝不会泄露机器的绝对路径。
...
`resolveLynHome` 先读显式覆盖值，然后读 `$LYNESS_HOME`，最后回退到操作系统主目录拼接 `.lyn`。选中的值经过波浪号展开并规范化为绝对路径；`lynHomePath` 用 Node 的平台路径规则拼接子路径段。`lynHomeDisplay` 把解析出的路径与默认根目录比较并返回符号标签，因此已配置的主目录绝不泄露其绝对路径。
...
- [shell 环境](../../shell/shell-env/README.zh.md)——`LYNESS_HOME` 如何到达模型 shell 调用。
