---
description: "Shared resolution of the lyness home and user-data paths for packages that need one consistent root, tilde expansion, and stable watch paths."
...
`lyn-home-paths` resolves the single lyness home that all user data lives under, and joins child paths onto it, so every product package agrees on where its files go. Precedence is explicit: a configured path wins, then `$LYNESS_HOME`, then `~/.lyn`, and an empty or whitespace-only `$LYNESS_HOME` counts as unset. The package also expands `~`, `~/...`, and `~\...` prefixes against the operating-system home, and canonicalizes a watch target so a native filesystem watcher gets one stable path spelling even when the final components do not exist yet. It is a zero-dependency library that product packages import directly; a `cordis.yml` cannot load it.
...
For user-facing paths, render the root symbolically rather than as a machine path: the default home displays as `~/.lyn` and any configured home displays as `$LYNESS_HOME`. The display form never leaks an absolute machine path.
...
`resolveLynHome` reads the explicit override, then `$LYNESS_HOME`, then falls back to the operating-system home joined with `.lyn`. The chosen value is tilde-expanded and normalized to an absolute path; `lynHomePath` joins child segments with Node's platform path rules. `lynHomeDisplay` compares the resolved path against the default root and returns the symbolic label, so a configured home never leaks its absolute path.
...
- [Shell environment](../../shell/shell-env/README.md) — how `LYNESS_HOME` reaches model shell calls.
