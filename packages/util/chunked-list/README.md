...
# @lyness/lyn-chunked-list

English | [中文](README.zh.md)

## Summary

`lyn-chunked-list` lets callers append values while retaining earlier list versions without copying the whole collection. Callers can iterate every value in insertion order and validate JSON checkpoints with their own value schema. The subagent catalog uses it for immutable projection state.
...
```ts
import { appendChunkedList, iterateChunkedList } from '@lyness/lyn-chunked-list'
