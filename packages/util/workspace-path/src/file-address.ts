/**
 * The `lyn-resource://file/…` address grammar: how a file is named across the
 ...
 *
 * Every resource address is `lyn-resource://<type>/…`, the URI host naming the
 * resource protocol; for `file` the path opens with the scope:
 *
 * - `lyn-resource://file/session/<sessionId>/<path>` names a file by its path,
 *   relative to that Session's workspace root or absolute; the
 *   Host resolves it against the root it holds for the Session.
 * - `lyn-resource://file/absolute/<path>` names a file by its absolute path with
 *   the leading `/` dropped (`lyn-resource://file/absolute/home/ys/notes.txt`;
 *   Windows `lyn-resource://file/absolute/C:/x/y.txt`; a UNC path keeps an empty
 *   first segment, `lyn-resource://file/absolute//server/share/x.txt`). It carries
 *   no Session.
 *
...
const FILE_ADDRESS_PREFIX = 'lyn-resource://file/'
 ...
 * @returns the `lyn-resource://file/session/<sessionId>/<path>` address.
 */
 ...
 * @returns the `lyn-resource://file/absolute/<path>` address.
 */
 ...
 * @returns the parts, or `undefined` when the string is not a `lyn-resource://file/` URI in a known scope with a path, or a segment is not validly encoded.
 */
