...
public static class LynFileAssociations {
...
${application === null ? 'ConvertTo-Json -InputObject @([LynFileAssociations]::List($path)) -Depth 4 -Compress' : '[LynFileAssociations]::Open($path, $application)'}
`
  ...
  const directory = await mkdtemp(join(tmpdir(), 'lyn-native-command-'))
