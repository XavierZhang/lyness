...
it.skipIf(process.platform !== 'win32')('queries and invokes a registered Windows handler through the system Shell', async ({ task, signal: testSignal, onTestFailed }) => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-windows-association-'))
  const suffix = randomUUID().replaceAll('-', '')
  const extension = `.lyn${suffix}`
  const progId = `LYN.Test.${suffix}`
  const appName = `lyn-handler-${suffix}.exe`
