...
it.skipIf(!gioAvailable)('queries and launches an installed Linux desktop entry through GIO', async ({ task }) => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-gio-associations-'))
  ...
  expect(apps).toContainEqual({ id: desktop, name: 'LYN Test Handler', default: true, icon: null })
