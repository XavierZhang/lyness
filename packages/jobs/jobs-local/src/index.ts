 ...
 * @module @lyness/jobs-local
 */

import { Context } from '@lyness/cordis'
import z from '@lyness/schemastery'
import type { Agent } from '@lyness/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@lyness/scope'
import type { ScopeLayer } from '@lyness/scope'
 ...
 * `@lyness/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @lyness/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @lyness/agent)')
    }
