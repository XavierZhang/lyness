 ...
 * @module @lyness/lyn-jobs-local
 */

import { Context } from '@lyness/cordis'
import z from '@lyness/schemastery'
import type { Agent } from '@lyness/lyn-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@lyness/lyn-scope'
import type { ScopeLayer } from '@lyness/lyn-scope'
 ...
 * `@lyness/lyn-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @lyness/lyn-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @lyness/lyn-agent)')
    }
