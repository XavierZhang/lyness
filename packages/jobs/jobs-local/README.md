...
`lyn-jobs-local` runs background jobs inside the harness process: work keeps running while the agent moves on, and the owning agent can read, wait on, list, and cancel it, with completion delivered as an in-session notice when `lyn-tool-jobs` is also mounted. It implements the `lyn-jobs` contract with in-memory records handed out as fresh snapshots, never live state. A per-owner concurrency limit (default 10) bounds how many jobs one agent can have running or stopping at once; jobs die with the harness process and are not durable across restarts.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynessjobs-local) is the exhaustive source for the accepted field.
...
| [`src/invariant.ts`](src/invariant.ts) | Invariant companion (no runtime invariant; snapshot checks live in `lyn-jobs/invariant`) |
