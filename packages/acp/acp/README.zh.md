---
description: "面向程序化客户端与维护者的仅自动化 ACP（Agent Client Protocol）服务器，用于通过 JSON-RPC stdio 驱动 lyness agent（智能体）。"
...
`lyn-acp` 让受信程序通过标准 [ACP](https://agentclientprotocol.com) 自动操作持久 lyness agent：创建或恢复会话、选择模型与推理强度、挂载 MCP 服务器、提交或取消工作、接收语义更新，并独立关闭会话。进程外 subagent、测试运行器与脚本化控制器适合选择它；它刻意不提供 LYN 专用呈现数据与交互式 UI 功能。持久化支持跨进程重启列出、恢复与关闭会话，但不支持删除、fork、transcript（文本记录）回放与附加目录。运行 `pnpm lyn --profile acp` 可启动服务器；仓库客户端使用 `lyn-subagent-acp`。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#lynesslyn-acp)是每个受支持字段及其 JSDoc 的穷尽式真源。

### 启动服务器

`pnpm lyn --profile acp` 会启动随附的 stdio 服务器。`acp` profile 会挂载会话持久化，因此客户端可以列出、恢复和关闭持久会话。[`@lyness/lyn-subagent-acp`](../../subagent/subagent-acp/README.zh.md) 会启动同一 profile 来执行进程外委派。
