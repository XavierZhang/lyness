...
export class WebRuntime extends Service {
   ...
   * `$LYNESS_WEB_SEARCH_PROVIDER` / `$LYNESS_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.LYNESS_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.LYNESS_WEB_FETCH_PROVIDER
  }
