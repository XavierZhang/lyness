...
import type { SessionId, SessionStore } from '@lyness/session'
import type { SessionPersistence, SessionRawArtifact } from '@lyness/session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `lyn-session-${safeSessionIdSegment(sessionId)}.zip`
}
