...
Choose it when you want full-text recall over prior sessions with ranking and paging. Choose it together with `lyn-session-query` and the session service; a persistence backend is optional but recommended so persisted history is searchable after restarts. Avoid pointing `path` at the session-persistence database — this package owns a separate derived index.
...
The generated [configuration catalog](../../../docs/config-catalog.md#lynesssession-query-sqlite) is the exhaustive source for every accepted field and its JSDoc.
...
- [lyn-session-query](../session-query/README.md) — the service definition: exact reads, filters, and traces this backend inherits.
- [lyn-tool-session-query](../tool-session-query/README.md) — the model-facing consumer that calls these search methods.
