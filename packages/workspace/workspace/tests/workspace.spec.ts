...
import type { StorageBackend } from '@lyness/lyn-storage'
...
import { SessionPersistenceRevision } from '@lyness/lyn-session-persistence'
import type { SessionPersistenceSnapshot } from '@lyness/lyn-session-persistence'
...
async function makeDir(name: string): Promise<string> {
  base ??= await realpath(await mkdtemp(join(tmpdir(), 'lyn-workspace-')))
