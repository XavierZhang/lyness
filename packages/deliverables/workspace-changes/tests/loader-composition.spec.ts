...
import * as WorkspaceChangesPlugin from '@lyness/lyn-workspace-changes'
...
describe('real Loader composition', () => {
  it('loads the shipped rows and records a turn’s changes', async () => {
    root = await mkdtemp(join(tmpdir(), 'lyn-workspace-changes-loader-'))
    const cwd = join(root, 'ws')
    await writeFile(join(root, 'cordis.yml'), [
      "- name: '@lyness/lyn-session'",
      "- name: '@lyness/lyn-subprocess-local'",
      "- name: '@lyness/lyn-workspace-changes'",
      '',
    ].join('\n'))
    context = new Context()
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@lyness/lyn-workspace-changes', WorkspaceChangesPlugin],
    ])
