...
describe.skipIf(!built)('built workflow PTC runtime', () => {
  it('runs a structured child and confines direct writes through installed exports', async () => {
    ...
    try {
      root = await mkdtemp(join(homedir(), '.lyn-built-workflow-'))
...
import PtcWorkflowEngine from '@lyness/lyn-workflow-ptc'
const ctx = new Context()
try {
  for (const name of ['session', 'session-projection', 'fs-local', 'subprocess-local', 'sandbox-local']) {
    await ctx.plugin((await import('@lyness/lyn-' + name)).default, {})
  }
  await ctx.plugin((await import('@lyness/lyn-sandbox-policy')).default, { mode: 'read-only', workspaceRoot: process.argv[2] })
  await ctx.plugin((await import('@lyness/lyn-ptc-runtime-node')).default, {})
  await ctx.plugin((await import('@lyness/lyn-subagent')).default, {})
