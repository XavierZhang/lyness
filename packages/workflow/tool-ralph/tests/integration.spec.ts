...
import { STRUCTURED_OUTPUT_TOOL } from '@lyness/lyn-subagent-in-process-driver'
...
async function mountExecution(ctx: Context): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'lyn-ralph-'))
...
describe('lyn-tool-ralph over the real spawn and sandboxed PTC stack', () => {
