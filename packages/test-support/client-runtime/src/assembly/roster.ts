 ...
 * @module @lyness/lyn-client-test-runtime/src/assembly/roster
 */
import type { Context } from '@lyness/cordis'
import type { WebBootEntry, WebBootGraph } from '@lyness/lyn-client-modules/client'
import { PLATFORM_MODULES } from '@lyness/lyn-client-web/src/platform.ts'

/** One browser plugin row as `lyn.client` declares it, keyed by package name. */
export interface ClientRosterRow {
  /** Package name (== manifest entry id == Loader entry name). */
  readonly name: string
  /** Package-name dependency edges from `lyn.client.inject` ([] when absent). */
  readonly inject: readonly string[]
  /** Stage-one prefetch mark from `lyn.client.immediately` (false when absent). */
