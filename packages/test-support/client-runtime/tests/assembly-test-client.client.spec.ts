...
import { RemoteMock, ok, openStream } from '@lyness/lyn-remote-mock'
...
const MODULES = '@lyness/lyn-client-modules'
const SIDEBAR = '@lyness/lyn-client-ui-sidebar'
const PARALLEL_PROBE = '@lyness/lyn-client-test-parallel-probe'
...
const BRAND = '@lyness/lyn-client-ui-brand-official'
...
describe('TestClient (jsdom)', () => {
  it('boots the whole web-app roster, connects, mounts, and disposes with nothing unmatched', async () => {
    ...
    expect('__LYNESS_TRANSPORT__' in globalThis).toBe(false)
  ...
  it('boots separate client instances against their own mocks and keeps shared shims until the last dispose', async () => {
    ...
    await a.reload('@lyness/lyn-client-connection')
  ...
  it('creates no mount element when the roster cannot be loaded', async () => {
    const roster = ClientRoster.of([{ name: '@lyness/lyn-client-test-runtime-missing', inject: [], immediately: true }])
    const before = document.body.childElementCount
  ...
  it('reports the log when the connection never becomes ready', async () => {
    ...
    const roster = webApp.closure(['@lyness/lyn-api-workspace-controller'])
