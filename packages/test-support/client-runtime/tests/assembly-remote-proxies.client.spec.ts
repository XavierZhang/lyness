...
import { RemoteMock, frames, ok, openStream } from '@lyness/lyn-remote-mock'
...
const API_ROSTER = webApp.closure(['@lyness/lyn-api-remotes'])
...
describe('remote proxies over a booted client', () => {
  it('boots the Office Remote subset from source and routes calls through its mock', async () => {
    const name = '@lyness/lyn-api-remotes'
  ...
  it('drops the api-remotes row and answers ctx.remote.<ns>.<method> with the mock value, logging positional args', async () => {
    ...
    expect([...client.ctx.loader.entries()].map(entry => entry.options.name)).not.toContain('@lyness/lyn-api-remotes')
