...
import { LlmAdapter, LlmError, ReasoningEffortId, expandAssistantStream, offloadedImageText, requestImageHandleText, resolveRetryPolicy } from '@lyness/lyn-llm'
...
function normalizeProjectedHeader(header: Readonly<Record<string, unknown>>): Record<string, unknown> {
  ...
  if (typeof header['cwd'] === 'string' && /^\{\{cwd\}\}(?:\/|$)/.test(header['cwd'])) {
    normalized['cwd'] = header['cwd'].replace('{{cwd}}', '/lyn-snapshot-cwd')
  }
  return normalized
}
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.LYNESS_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $LYNESS_SNAPSHOT_FILE)')
  }
  validateConfiguredModels(config.providers)
  const overrideFile = config.overrideFile ?? process.env.LYNESS_SNAPSHOT_OVERRIDE
  const childEnv = process.env.LYNESS_SNAPSHOT_CHILD_FILES
