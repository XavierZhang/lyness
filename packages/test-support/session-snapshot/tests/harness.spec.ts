...
describe('runScenario', () => {
  ...
  it('builds lyn profile argv and rebases relative modules in live and replay patches', async () => {
    ...
    expect(await realpath(join(dir, '.lyn', 'profiles', 'acp', 'node_modules', 'example-package')))
      .toBe(await realpath(packageDir))
    expect(await realpath(join(dir, '.lyn', 'profiles', 'acp', 'node_modules', '@fixture', 'example-package')))
    ...
    const conflictLink = join(dir, '.lyn', 'profiles', 'acp', 'node_modules', 'conflict-package')
    await Promise.all([
