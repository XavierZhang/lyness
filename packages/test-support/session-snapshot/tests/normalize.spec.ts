  ...
  it('scrubs fixed snapshot spill paths', () => {
    const ev = JSON.stringify({
      ...
      data: {
        content: [{
          type: 'text',
          text: 'Full formatted result stored at: /tmp/lyn-acp-snapshot-spill/session-c22bc3f1d2af/8a7b6c5d4e3f-bash.txt. Use read with offset/limit, or grep this path to search within it.',
        }],
      },
    })
    const out = normalizeSessionLog(`${header({ cwd: ctx.cwd })}\n${ev}\n`, ctx)
    expect(out).toContain('{{spillLocator:bash.txt}}')
    expect(out).not.toContain('/tmp/lyn-acp-snapshot-spill')
  })
  ...
  it('scrubs scenario-owned snapshot spill paths', () => {
    const ev = JSON.stringify({
      ...
      data: {
        content: [{
          type: 'text',
          text: 'Full formatted result stored at: /tmp/lyn-acp-snap-012345678/session-c22bc3f1d2af/8a7b6c5d4e3f-bash.txt. Use read with offset/limit, or grep this path to search within it.',
        }],
      },
    })
    const out = normalizeSessionLog(`${header({ cwd: ctx.cwd })}\n${ev}\n`, ctx)
    expect(out).toContain('{{spillLocator:bash.txt}}')
    expect(out).not.toContain('/tmp/lyn-acp-snap-012345678')
  })
  ...
  it('scrubs scenario-owned snapshot spill paths with Windows drive and separators', () => {
    const ev = JSON.stringify({
      ...
      data: {
        content: [{
          type: 'text',
          text: String.raw`Full formatted result stored at: C:\t\lyn-acp-snap-012345678\session-c22bc3f1d2af\8a7b6c5d4e3f-bash.txt. Use read with offset/limit, or grep this path to search within it.`,
        }],
      },
    })
    const out = normalizeSessionLog(`${header({ cwd: ctx.cwd })}\n${ev}\n`, ctx)
    expect(out).toContain('{{spillLocator:bash.txt}}')
    expect(out).not.toContain('C:\\t\\lyn-acp-snap-012345678')
  })
...
describe('extractSnapshotSpillPaths', () => {
  it.each([
    ...
    const locator = `${root}/lyn-acp-snap-123456789/session-123456abcdef/abcdef123456-session-reference-1.txt`.replaceAll('/', separator)
  ...
  it('maps each spill filename to its full matched path, last match wins per name', () => {
    const log = [
      'Full formatted result stored at: /tmp/lyn-acp-snapshot-spill/session-c22bc3f1d2af/8a7b6c5d4e3f-bash.txt. Use read with offset/limit, or grep this path to search within it.',
      'stale copy at /tmp/lyn-acp-snap-012345678/session-aaaaaaaaaaaa/bbbbbbbbbbbb-grep.txt then',
      'fresh copy at /tmp/lyn-acp-snap-012345678/session-cccccccccccc/dddddddddddd-grep.txt then',
    ].join('\n')
    expect(extractSnapshotSpillPaths(log)).toEqual(new Map([
      ['bash.txt', '/tmp/lyn-acp-snapshot-spill/session-c22bc3f1d2af/8a7b6c5d4e3f-bash.txt'],
      ['grep.txt', '/tmp/lyn-acp-snap-012345678/session-cccccccccccc/dddddddddddd-grep.txt'],
    ]))
  })
...
describe('scrubSessionSnapshot', () => {
  ...
  it('preserves the header while projecting and scrubbing each body record', () => {
    ...
    expect(scrubSessionSnapshot(`${header}\n${system}\n${request}\n${event}\n`)).toBe([
      header,
      '{"type":"system/message","data":{"turn":1,"step":1,"message":{"id":"11111111-1111-4111-8111-111111111111","role":"system","content":[{"type":"text","text":"{{system}}"}],"source":{"kind":"plugin","plugin":"@lyness/lyn-system-prompt"}}}}',
