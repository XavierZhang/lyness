...
export interface AgentUnderTest {
  ...
  configPath: string
  /** Named lyn profile; omitted only for test-only fake bins with their own config grammar. */
  profile?: string
...
export function launchAcpTestAgent(options: AcpTestLaunchOptions): LaunchedAcpTestAgent {
  ...
  const launch = resolveExampleLaunch({
    ...
    configArgs: agent.profile === undefined
      ...
      : profileArgs(agent.profile, agent.configPath, selectedConfig, options.env?.LYNESS_SNAPSHOT, cwd),
  ...
  const clientApp = createAcpClientApp({ name: 'lyness-acp-test-client' })
...
/** Build one lyn profile invocation from the base and optional scenario patches. */
function profileArgs(
  ...
  const materializedRoot = join(cwd, '.lyn-profile-patches')
 ...
 * profile fallback. This mirrors `lyn plugin` while retaining the bare entry
...
function linkProfilePackage(source: string, cwd: string, packageName: string): void {
  ...
  // The package may instead belong to the lyn installation; profile boot heals those links.
  if (packageDir === undefined) return
  const link = join(cwd, '.lyn', 'profiles', 'node_modules', packageName)
