...
export interface RunOptions {
   ...
   * them to `lyn-llm-replay` via `$LYNESS_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const initialWorkspace = await captureWorkspaceSnapshot(cwd, {
      ignoredRootEntries: ['.agents', '.lyn', '.lyn-profile-patches', '.lyn-snapshot-stream-ready'],
    })
    const env: NodeJS.ProcessEnv = {
      ...
      ...clearedProxyEnv(),
      LYNESS_SNAPSHOT: opts.mode,
      LYNESS_SNAPSHOT_FILE: opts.fixtureFile,
      LYNESS_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      LYNESS_SNAPSHOT_SPILL_ROOT: spillRoot,
      LYNESS_SNAPSHOT_SPILL_LOCATOR_ROOT: snapshotSpillRoot(opts.fixtureFile),
      LYNESS_HOME: join(cwd, '.lyn'),
      LYNESS_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { LYNESS_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { LYNESS_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in lyn-llm-replay's loadSessionScripts
