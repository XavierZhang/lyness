...
- [lyn-llm service](../llm/README.md) — the model-call service whose capacity metadata `resolveModelInfo()` serves.
...
Indirectly, through consumers such as `lyn-compaction-basic`; the service itself adds no prompt, message, schema, tool, or model call.
