...
import { ACCOUNT_QUOTA_EXCEEDED_CODE, LlmError, QUOTA_EXCEEDED_CODE } from '@lyness/lyn-llm'
import type {} from '@lyness/lyn-deepseek-account'
import { launchEnvironmentOf } from '@lyness/lyn-launch-environment'
import { plainOptions, resolveAdapterOptions, registerDeepSeekProvider, catalogModelInfo } from '@lyness/lyn-llm-deepseek'
import type { DeepSeekRequestAuth, ResolvedDeepSeekOptions } from '@lyness/lyn-llm-deepseek'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const resolveAuth = async (connection: ResolvedDeepSeekOptions): Promise<DeepSeekRequestAuth> => {
    ...
    return {
      headers: { 'x-lyn-auth-token': token },
