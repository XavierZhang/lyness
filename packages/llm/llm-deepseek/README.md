...
The generated [configuration catalog](../../../docs/config-catalog.md#lynessllm-deepseek) is the exhaustive source for every accepted field and its JSDoc.
...
When `ctx.deepseekLlmApiExtensions` is present, the adapter prepares its registered top-level fields from the exact serialized base request before `fetch`. Preparation or field collisions fail before HTTP; after a 2xx response, the adapter accepts every captured contribution before consuming SSE. Transport and non-2xx failures do not accept them. Shipped compositions use this for the optional incremental `lyn_session_log` field and the default-on active `lyn_plugin_packages` inventory; both stay outside model input.
...
- [Session-log upload](../../session/session-log-deepseek/README.md) — the opt-in incremental `lyn_session_log` contribution.
