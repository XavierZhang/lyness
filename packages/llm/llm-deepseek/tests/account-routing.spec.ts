...
import LlmRuntime, { createUserMessage, LlmAdapter, type GenerateOptions, type StreamChunk, ToolCallId } from '@lyness/lyn-llm'
...
it('propagates account sign-out to the HTTP request signal', async () => {
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url, init) => {
    ...
    expect(new Headers(init!.headers).get('x-lyn-auth-token')).toBe('fixture-account-token')
