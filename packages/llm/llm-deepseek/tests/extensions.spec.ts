...
import { Context, LoggerLevel } from '@lyness/cordis'
import LlmRuntime from '@lyness/lyn-llm'
import DeepSeekLlmApiExtensionRegistry from '@lyness/lyn-deepseek-llm-api-extensions'
import type { DeepSeekLlmApiExtensionRequest } from '@lyness/lyn-deepseek-llm-api-extensions'
import { SessionId } from '@lyness/lyn-session'
import * as DeepSeek from '@lyness/lyn-llm-deepseek-api-key'
import { adapter, assemble, options, sse, textEvents } from './helpers.ts'

declare module '@lyness/lyn-deepseek-llm-api-extensions' {
  interface DeepSeekLlmApiExtensionMap {
    lyn_messages_test: { value: string }
  }
}
...
async function boot() {
  const home = await mkdtemp(join(tmpdir(), 'lyn-messages-extensions-'))
...
describe('Messages request extensions', () => {
  it('prepares the native body and accepts its contribution before yielding content', async () => {
    ...
    ctx.deepseekLlmApiExtensions.register('lyn_messages_test', {
      prepare: (value) => {
        request = value
    ...
    expect(request?.body).not.toHaveProperty('lyn_messages_test')
    expect(fetch.mock.calls[0]?.[0]).toBe('https://messages.example.test/root/v1/messages')
    const body = fetch.mock.calls[0]?.[1]?.body
    if (typeof body !== 'string') throw new Error('Expected a serialized Messages request')
    expect(JSON.parse(body)).toMatchObject({ lyn_messages_test: { value: 'inventory' } })
  })

  it('rejects preparation before dispatch', async () => {
    const ctx = await boot()
    ctx.deepseekLlmApiExtensions.register('lyn_messages_test', { prepare() { throw new Error('inventory unavailable') } })
  ...
  it.each(['http', 'transport', 'stream'] as const)('records acceptance only for HTTP success despite a later %s failure', async (failure) => {
    const ctx = await boot()
    const accept = vi.fn()
    ctx.deepseekLlmApiExtensions.register('lyn_messages_test', { prepare: () => ({ value: { value: 'log' }, accept }) })
  ...
  it('sends the base request without extension fields when they fail to serialize', async () => {
    ...
    vi.stubGlobal('fetch', fetch)
    const { result, overflow } = await withSerializationOverflow('lyn_messages_test', () => assemble(ctx.llm.stream(options())))
    expect(JSON.stringify({ lyn_messages_test: { value: 'log' } })).toBe('{"lyn_messages_test":{"value":"log"}}')
    ...
    expect(sentBody(fetch)).not.toHaveProperty('lyn_messages_test')
    expect(warnings).toEqual([[
      'llm-deepseek: sending route "deepseek-official/deepseek-v4-flash" without request extension fields lyn_messages_test'
  ...
  it('omits unserializable extension fields when the adapter has no reporter', async () => {
    ...
    const llm = adapter({ baseURL: 'https://messages.example.test' }, {
      prepareExtensions: () => Promise.resolve({ fields: { lyn_messages_test: { value: 'log' } }, accept }),
    })
    const fetch = vi.fn<typeof globalThis.fetch>().mockResolvedValue(new Response(sse(textEvents)))
    vi.stubGlobal('fetch', fetch)
    const { result } = await withSerializationOverflow('lyn_messages_test', () => assemble(llm.stream(options())))
    expect(result.assembler.finish.kind).toBe('stop')
    expect(accept).not.toHaveBeenCalled()
    expect(sentBody(fetch)).not.toHaveProperty('lyn_messages_test')
  })
