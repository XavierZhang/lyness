...
import LlmRuntime, { createToolResultMessage, createUserMessage, ToolCallId, ReasoningEffortId, createMessage, createSystemMessage } from '@lyness/lyn-llm'
...
import type { Options as Config } from '@lyness/lyn-llm-deepseek'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'lyn-e2e-user-id-'))
  vi.stubEnv('LYNESS_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  ...
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.DEEPSEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without DEEPSEEK_API_KEY')
    const dir = await mkdtemp(join(tmpdir(), 'lyn-e2e-credentials-'))
    try {
      ...
      const observedFetch: typeof fetch = async (input, init) => {
        ...
        if (body.model === model) {
          ...
          if (wireRequests.length === 1) {
            expect(body.system).toBe(initial)
            expect(body.messages).toEqual([{ role: 'user', content: [{ type: 'text', text: 'ping' }] }])
          } else if (wireRequests.length === 2) {
            expect(body).toEqual(wireRequests[0])
          } else if (wireRequests.length === 3) {
            expect(body.system).toBe(initial)
            expect(body.messages[0]).toEqual(wireRequests[0]!.messages[0])
            expect(body.messages.at(-1)).toEqual({ role: 'system', content: [{ type: 'text', text: latest }] })
          } else if (wireRequests.length === 4) {
            expect(body.system).toBe(latest)
            expect(body.messages).toEqual(wireRequests[2]!.messages.filter(message => message.role !== 'system'))
          }
        }
