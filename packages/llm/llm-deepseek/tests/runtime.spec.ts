import * as Protocol from '@lyness/lyn-llm-deepseek'
...
import { DeepSeekAdapter, resolveAdapterOptions, parseModelIds, parseMaxTokens }
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'lyn-llm-deepseek-'))
  vi.stubEnv('LYNESS_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    expect(server.requests[0]).toMatchObject({ lyn_test: { version: 1 } })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const adapter = new DeepSeekAdapter({
      ...
      resolveAuth: () => Promise.resolve({ headers: { 'x-api-key': 'k' } }),
      resolveUserId: () => TEST_USER_ID,
      prepareExtensions: () => Promise.resolve({ fields: { lyn_test: 1 }, accept: async () => { accept() } }) as never,
    })
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-lyness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-lyness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-lyness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/v1/files',
      filename: `lyn-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    expect(server.headers[1]?.['x-lyness-compact']).toBe('1')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-lyness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-lyness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-lyness-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.lyn/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
  ...
  it('takes the catalog and the completion cap the environment names for that endpoint', () => {
    const gateway = createLaunchEnvironmentSnapshot([
      {
        source: 'project-env',
        path: '/work/.env',
        values: { DEEPSEEK_MODELS: 'gateway-flash,gateway-pro', DEEPSEEK_MAX_TOKENS: '8192' },
      },
    ])
    const resolved = resolveAdapterOptions({}, gateway)
    expect(resolved.models.map(model => model.id)).toEqual(['gateway-flash', 'gateway-pro'])
    expect(resolved.maxTokens).toBe(8192)
    // The cap stands on its own, because a composition may already name models
    // the endpoint serves while still capping completions lower than official.
    const capOnly = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { DEEPSEEK_MAX_TOKENS: '4096' } },
    ])
    const capped = resolveAdapterOptions({ models: [{ id: 'deepseek-v4-flash' }] }, capOnly)
    expect(capped.models.map(model => model.id)).toEqual(['deepseek-v4-flash'])
    expect(capped.maxTokens).toBe(4096)
  })

  it('refuses a catalog or cap the endpoint could never serve', () => {
