/** Durable DeepSeek attachment-to-file-id index. @module lyn-llm-deepseek/upload-index */
...
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@lyness/lyn-attachment'
...
/** Atomic local index shared by every DeepSeek session in this LYN home. */
export class DeepSeekUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `LYNESS_HOME/llm-deepseek/files-v3.json`.
   */
  constructor(path = join(resolveLynHome(), 'llm-deepseek', 'files-v3.json')) {
    this.path = path
  }
