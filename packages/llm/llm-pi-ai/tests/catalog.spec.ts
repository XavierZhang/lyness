...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@lyness/lyn-llm'
...
/** A throwaway $LYNESS_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'lyn-pi-catalog-'))
  homes.push(dir)
