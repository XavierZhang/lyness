...
} from '@lyness/lyn-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  ...
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@lyness/lyn-tools', 'ToolInput')).toBe('@lyness/lyn-tools#ToolInput')
    expect(typertPackageKey('@lyness/lyn-tools', 'host')).toBe('@lyness/lyn-tools#host')
    expect(ctx.typert.get('@lyness/lyn-tools#ToolInput')).toMatchObject({
      package: '@lyness/lyn-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@lyness/lyn-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.create())
    expect(ctx.typert.getPackage('@lyness/lyn-tools', 'host')).toMatchObject({
      key: '@lyness/lyn-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@lyness/lyn-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@lyness/lyn-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@lyness/lyn-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@lyness/lyn-tools#ToolInput')?.schema).toBe(original.schemas[0]?.create())
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@lyness/lyn-tools#ToolInput').name).toBe('ToolInput')
    expect(create).toHaveBeenCalledOnce()
    expect(ctx.typert.resolve('@lyness/lyn-tools#ToolInput').schema).toBe(schema)
    expect(create).toHaveBeenCalledOnce()
    expect(() => ctx.typert.resolve('@lyness/lyn-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@lyness/lyn-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@lyness/lyn-tools#ToolInput')).not.toBe(projected)
  })
