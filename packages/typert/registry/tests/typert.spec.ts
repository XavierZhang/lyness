...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@lyness/tools', 'ToolInput')).toBe('@lyness/tools#ToolInput')
    expect(typertPackageKey('@lyness/tools', 'host')).toBe('@lyness/tools#host')
    expect(ctx.typert.get('@lyness/tools#ToolInput')).toMatchObject({
      package: '@lyness/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@lyness/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@lyness/tools', 'host')).toMatchObject({
      key: '@lyness/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@lyness/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@lyness/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@lyness/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@lyness/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@lyness/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@lyness/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@lyness/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@lyness/tools#ToolInput')).not.toBe(projected)
  })
