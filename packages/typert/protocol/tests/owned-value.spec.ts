...
describe('TypertOwnedValue', () => {
  ...
  it('recognizes a separately produced marker without accepting false markers or borrowed values', () => {
    expect(TYPERT_OWNED_VALUE).toBe(Symbol.for('lyn.typert.owned-value'))
    const siblingBundle = { [Symbol.for('lyn.typert.owned-value')]: true, value: {}, [Symbol.dispose]() {} }
    expect(isTypertOwnedValue(siblingBundle)).toBe(true)
    for (const value of [undefined, null, 1, {}, { [Symbol.for('lyn.typert.owned-value')]: false }]) {
