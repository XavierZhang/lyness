...
/**
 * File name of the single user-global instruction file under `$LYNESS_HOME`.
 * Discovery (`$LYNESS_HOME/<name>`) and reconciliation (the user-global scope key's
...
export function scopeForDisplayPath(displayPath: string): string {
  if (displayPath === '~/.lyn/AGENTS.md' || displayPath === '$LYNESS_HOME/AGENTS.md') return USER_GLOBAL_DIRECTORY
