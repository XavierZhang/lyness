...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.lynHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.lynHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.lynHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(lynHome: string): string {
  return `${lynHomeDisplay(lynHome)}/AGENTS.md`
}
