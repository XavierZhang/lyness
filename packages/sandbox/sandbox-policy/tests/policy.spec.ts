...
describe('sandbox:policy request context', () => {
  ...
  it('reflects the latest durable switch on the next assembly and stays byte-stable otherwise', async () => {
    ...
    expect(await policyContext(ctx, active)).toBe(`Current LYN file policy: workspace-write. Any available operation enforced by the LYN file sandbox may modify files under the session workspace: ${JSON.stringify('/projects/current')}. Some platform temporary areas may also be writable.`)
  })
