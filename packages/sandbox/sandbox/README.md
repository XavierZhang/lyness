...
Through [`lyn-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`lyn-tool-bash`](../../shell/tool-bash/README.md), a requested confined mode with no usable backend produces code `SANDBOX_UNAVAILABLE` and the exact error below; an execution-time runner failure appends ` Runner failure: <detail>`.
