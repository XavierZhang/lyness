...
const platformPackageName = `@lyness/node-addon-landlock-run-linux-${process.arch}`

const NATIVE_PACKAGE_PREFIX = '@lyness/node-addon-landlock-run'
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'lyn-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'lyn-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'lyn-packed-work-'))
