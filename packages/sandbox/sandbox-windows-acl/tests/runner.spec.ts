 ...
 * the argv shape lyn-sandbox-local's confine() builds), with piped stdio
...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  beforeAll(() => {
    scratchRoot = mkdtempSync(join(tmpdir(), 'lyn-acl-runner-'))
    writableDir = join(scratchRoot, 'writable')
    mkdirSync(writableDir)
    isolatedTemp = mkdtempSync(join(tmpdir(), 'lyn-acl-runner-temp-'))
    ...
    try {
      publicProbeDir = mkdtempSync(join(process.env.PUBLIC ?? 'C:\\Users\\Public', 'lyn-acl-public-'))
    } catch {
  ...
  it('a FullControl open inside a granted root still works for files (the deny inherits to containers only)', () => {
    ...
    // Administrators, SYSTEM, or the LYN host. Directories inside a granted
