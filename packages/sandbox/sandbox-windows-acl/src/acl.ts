 ...
 * @module @lyness/lyn-sandbox-windows-acl/acl
 */
...
/**
 * One lock file per protected path: `<GetTempPathW()>\lyn-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or LYNESS_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'lyn-acl-locks', `${digest}.lock`)
}
