 ...
 * @module @lyness/lyn-sandbox-windows-acl/runner
 */

import { SUBPROCESS_CONTROL_ENV, SUBPROCESS_CONTROL_FD } from '@lyness/lyn-subprocess/control'
  ...
  try {
    ...
    if (parsed.mode === 'workspace-write') {
      ...
      if (seamManaged) {
        ...
        ownedTempDir = mkdtempSync(join(parsed.temp, 'lyn-'))
