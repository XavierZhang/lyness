...
`lyn-sdk-client` lets TypeScript programs start and drive a complete lyness runtime over stdio JSON-RPC. Use `Lyness` to open sessions, send text or image prompts, collect event and notification streams, and obtain the last committed assistant response when the runtime becomes idle; use `HarnessClient` for direct protocol requests and subscriptions. Callers may provide `lynBin`; otherwise the client resolves the same-version `@lyness/lyn` executable. The client owns the subprocess across runs, exposes typed transport and protocol failures, and reaps it on `close()` or `await using`. It is suitable when the caller can choose the runtime profile and launch settings.
