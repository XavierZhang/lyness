...
/**
 * JSON-RPC client for the lyness SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
  /** Original public lyn launch and timeout options for this client. */
  ...
  /** @param options - lyn profile, patch, home, process, environment, and timeout options. */
  constructor(options?: HarnessClientOptions)
  constructor(options: HarnessClientOptions = {}, runtime?: RuntimeProcessOptions) {
    this.options = options
    this.runtime = runtime ?? resolveLynLaunch(options)
  }
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('lyness runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('lyness runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('lyness runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('lyness runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('lyness runtime is not running')
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('lyness runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('lyness runtime closed'))
  }
