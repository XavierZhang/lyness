/**
 * High-level run API over {@link HarnessClient}: `Lyness` owns one
...
/**
 * Reusable SDK for running lyness agent turns in a runtime
 ...
 * child is reaped.
 */
export class Lyness implements AsyncDisposable {
  private clientInstance: HarnessClient
  private readonly createClient: () => HarnessClient
  private readonly cwd: string
  private readonly provider: string
  private readonly model: string
  private readonly reasoningEffort: LynessOptions['reasoningEffort']
  private readonly maxTokens: number | undefined
  private initialized: Promise<void> | undefined
  private closed = false

  /** @param options - lyn launch configuration plus the session route, effort, and output cap. */
  constructor(options?: LynessOptions)
  constructor(options: LynessOptions = {}, clientFactory?: () => HarnessClient) {
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'lyness initialization and cleanup failed',
          )
        }
...
export function createProcessLyness(
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: Lyness, readonly id: string) {}
