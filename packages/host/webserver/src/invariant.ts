/**
 * Package-owned invariant companion for `@lyness/host-webserver`.
 * @module @lyness/host-webserver/invariant
 */
...
const PACKAGE_NAME = '@lyness/host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__lyn_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__lyn_invariant_upgrade_probe__', handler: () => {} }
