...
export async function loadWin32DialogBindings(): Promise<Win32DialogBindings> {
  ...
  const protoShow = koffi.proto('int32 __stdcall LynDialogShow(void *self, void *owner)')
  const protoSetOptions = koffi.proto('int32 __stdcall LynDialogSetOptions(void *self, uint32 options)')
  const protoSetTitle = koffi.proto('int32 __stdcall LynDialogSetTitle(void *self, str16 title)')
  const protoGetResult = koffi.proto('int32 __stdcall LynDialogGetResult(void *self, _Out_ void **item)')
  const protoGetDisplayName = koffi.proto('int32 __stdcall LynItemGetDisplayName(void *self, int32 form, _Out_ void **name)')
  const protoRelease = koffi.proto('uint32 __stdcall LynComRelease(void *self)')
...
export async function closeThreadWindows(threadId: number): Promise<void> {
  ...
  const protoEnumProc = koffi.proto('int __stdcall LynEnumThreadWndProc(void *hwnd, intptr lparam)')
