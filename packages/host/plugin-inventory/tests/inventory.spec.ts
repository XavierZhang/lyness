...
import { PluginPackages, readPluginMeta, type RuntimeResolution } from '@lyness/lyn-app-boot'
import Loader from '@lyness/cordis-plugin-loader'
import { remoteMethods } from '@lyness/lyn-typert-protocol'
import type { AgentPresetRegistry } from '@lyness/lyn-agent-preset-registry'
...
function metadataFixture(mode: 'native' | 'runtime') {
  const root = mkdtempSync(join(tmpdir(), 'lyn-inventory-meta-'))
  roots.push(root)
  ...
  const expectUnlinked = () => {
    ...
    for (const path of [
      ...
      join(root, 'node_modules'), join(profileDir, '.lyn-module-fallback'),
