...
- **Presets appear only with a roster** — a deployment without `lyn-agent-preset-registry` serves Loader entries alone; the `agentPresets` field is absent rather than empty.
