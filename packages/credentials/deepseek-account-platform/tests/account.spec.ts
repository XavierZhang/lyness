...
import type { AccountClientMetadata } from '@lyness/lyn-deepseek-account'
...
async function fixture(
  ...
  const home = await mkdtemp(join(tmpdir(), 'lyn-account-'))
  ...
  const handle = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    receivedHeaders.push({ clientPlatform: req.headers['x-client-platform'] as string | undefined,
      ...
      path: req.url, cookie: req.headers.cookie, authorization: req.headers['x-lyn-auth-token'] as string | undefined })
    if (redirect) { res.writeHead(302, { location: `${origin}/redirect-target` }).end(); return }
    if (req.url === '/auth-api/v0/users/logout') {
      logoutCount++
      logoutHeaders.push(req.headers['x-lyn-auth-token'] as string | undefined)
    ...
    if (req.method === 'GET') {
      detailRequests.push({ path: req.url!, authorization: req.headers['x-lyn-auth-token'] as string | undefined })
    ...
    if (req.url?.endsWith('auth_cancel')) {
      ...
      init = body
      onInit()
      value = { authorize_url: `${rewriteBrowserOrigin ? 'https://platform.deepseek.com' : origin}/lyn/authorize?authorize_id=test`, expires_in: 600, authorize_id: 'test', ...initOverride }
    } else {
      count++
      exchanged.resolve(undefined)
      if (hold) await release.promise
      value = { user: null, token: 'lyn_mock_test', authorized_url: `${origin}/lyn/authorized?result=test&locale=zh_CN`, ...exchangeOverride }
    }
    res.setHeader('content-type', 'application/json')
...
it('stores a grant before redirecting, restores account presence, and signs out without deleting device identity', async () => {
  ...
  expect(response.headers.get('location')).toBe(`${f.origin}/lyn/authorized?result=test&locale=zh_CN&login_source=desktop`)
  expect((await f.account.getState()).status).toBe('credential-stored')
  expect(await readFile(join(f.home, 'credentials.yaml'), 'utf8')).toContain('lyn_mock_test')
  ...
  expect(await f.account.getPlatformSession()).toEqual({ origin: f.origin, userId: 'test-user', token: 'lyn_mock_test',
    requestHeaders: {} })
...
it('restricts platform destinations to the configured origin and route', () => {
  ...
  expect(() => browserUrl('https://evil.test/lyn/authorized', 'https://platform.deepseek.com', '/lyn/authorized')).toThrow()
  expect(() => browserUrl('https://platform.deepseek.com/other', 'https://platform.deepseek.com', '/lyn/authorized')).toThrow()
})

it('maps both returned browser pages to the development origin across the login flow', async () => {
  const f = await fixture(undefined, {}, true)
  f.exchangeResponse({ authorized_url: 'https://platform.deepseek.com/lyn/authorized?result=a%2Fb&locale=zh_CN' })
  await f.account.startSignIn(clientMetadata(), f.callbackOrigin, 'desktop')
  expect((await f.wait('waiting-browser')).attempt?.authorizeUrl).toBe(`${f.origin}/lyn/authorize?authorize_id=test`)
  const response = await fetch(f.callback(), { redirect: 'manual' })
  expect(response.status).toBe(302)
  expect(response.headers.get('location')).toBe(`${f.origin}/lyn/authorized?result=a%2Fb&locale=zh_CN&login_source=desktop`)
  expect((await f.account.getState()).status).toBe('credential-stored')
})
...
it.each([
  ...
  const f = await fixture()
  f.exchangeResponse({ authorized_url: `${f.origin}/lyn/authorized?result=a%2Fb&locale=zh_CN${query}` })
  await f.account.startSignIn(clientMetadata(), f.callbackOrigin, client)
  await f.wait('waiting-browser')
  const response = await fetch(f.callback(), { redirect: 'manual' })
  expect(response.status).toBe(302)
  expect(response.headers.get('location')).toBe(`${f.origin}/lyn/authorized?result=a%2Fb&locale=zh_CN&login_source=${client}`)
})

it('preserves Platform business client_type in the completion URL', async () => {
  const f = await fixture()
  f.exchangeResponse({ authorized_url: `${f.origin}/lyn/authorized?client_type=LYN` })
  await f.account.startSignIn(clientMetadata(), f.callbackOrigin, 'web')
  await f.wait('waiting-browser')
  const response = await fetch(f.callback(), { redirect: 'manual' })
  expect(response.headers.get('location')).toBe(`${f.origin}/lyn/authorized?client_type=LYN&login_source=web`)
})

it.each([undefined, 'https://other.example/lyn/authorized', '/lyn/authorized'])('rejects an invalid exchange completion URL %s before storing a token', async (authorizedUrl) => {
  const f = await fixture()
...
it('adds private deployment cookies to every Platform request without exposing them in account state', async () => {
  ...
  expect(paths.slice(0, 2)).toEqual(['/auth-api/v0/lyn/auth_init', '/auth-api/v0/lyn/auth_exchange'])
  ...
  expect(headers.filter(item => grantedPaths.has(item.path ?? '')).every(item => item.authorization === 'lyn_mock_test')).toBe(true)
...
it('rejects reserved, duplicate and malformed deployment headers without disclosing values', () => {
  for (const headers of [
    { Authorization: 'secret-value' }, { 'X-LYN-Auth-Token': 'secret-value' }, { HOST: 'secret-value' }, { 'Content-Length': '5' },
...
it('does not forward deployment cookies through a Platform redirect', async () => {
  ...
  await f.account.startSignIn(clientMetadata(), f.callbackOrigin, 'desktop')
  await f.wait('failed')
  expect(f.receivedHeaders.map(item => item.path)).toEqual(['/auth-api/v0/lyn/auth_init'])
})
...
it('closes the failed Web authorization tab without redirecting and keeps the shared HTTP server alive', async () => {
  ...
  expect(success.headers.get('location')).toBe(`${f.origin}/lyn/authorized?result=test&locale=zh_CN&login_source=web`)
...
it('completes login through a local TCP forward using the browser port rather than the Host port', async () => {
  ...
  const response = await fetch(f.callback(), { redirect: 'manual' })
  expect(response.headers.get('location')).toBe(`${f.origin}/lyn/authorized?result=test&locale=zh_CN&login_source=web`)
  expect((await f.account.getState()).status).toBe('credential-stored')
})
...
it('uses the initialization payload ID for cancellation without extracting it from the browser URL', async () => {
  const f = await fixture()
  f.initResponse({ authorize_id: 'payload-id', authorize_url: `${f.origin}/lyn/authorize?opaque=value` })
  await f.account.startSignIn(clientMetadata(), f.callbackOrigin, 'desktop')
  const state = await f.wait('waiting-browser')
  await f.account.cancelSignIn(state.attempt!.id)
  await f.cancellationReceived.promise
...
it('carries the configured embedded frontend selector in the private Platform session', async () => {
  ...
  await fetch(f.callback(), { redirect: 'manual' })
  await f.account.getProfile(clientMetadata())
  expect(await f.account.getPlatformSession()).toEqual({ origin: f.origin, userId: 'test-user', token: 'lyn_mock_test', embeddedPageDist: 'feat/test',
    requestHeaders: {} })
})
...
it.each([
  ...
  expect(paths.slice(0, 4)).toEqual([
    '/auth-api/v0/lyn/auth_init', '/auth-api/v0/lyn/auth_cancel',
    '/auth-api/v0/lyn/auth_init', '/auth-api/v0/lyn/auth_exchange',
  ])
...
it('keeps one login attempt on the identity captured by its own initiating call', async () => {
  ...
  expect(f.receivedHeaders.map(header => [
    ...
    ['/auth-api/v0/lyn/auth_init', '', 'web', '1.0.0', 'zh_CN', '28800'],
    ['/auth-api/v0/lyn/auth_cancel', '', 'web', '1.0.0', 'zh_CN', '28800'],
    ['/auth-api/v0/lyn/auth_init', '', 'web', '9.9.9', 'en_US', '-18000'],
    ['/auth-api/v0/lyn/auth_exchange', '', 'web', '9.9.9', 'en_US', '-18000'],
  ])
})
...
it('returns no credentials when signed out or disposed', async () => {
  ...
  expect(await f.account.resolveToken('https://api.deepseek.com')).toBeUndefined()
  expect(await f.account.getPlatformSession()).toBeNull()
  await f.account.cancelSignIn('missing' as import('@lyness/lyn-deepseek-account').SignInAttemptId)
  await f.dispose()
  expect(await f.account.getProfile(clientMetadata())).toBeNull()
  expect(await f.account.getPlatformSession()).toBeNull()
