...
The `credentials/` group lets configuration name secrets instead of embedding their values. Use `credentials/` to store, look up, and remove credentials, `credentials-local/` for private on-machine storage with per-run environment overrides, and `authorization/` when obtaining a credential requires asking a human. Rotated stored values apply to the next model request, while `DEEPSEEK_API_KEY=… lyn` takes precedence for that run. Configuration files contain only credential names; local secret values remain readable only by the same OS user.
...
- [Generated configuration catalog](../../docs/config-catalog.md#lynesslyn-credentials-local) — every accepted field of the local store.
