...
`credentials/` 组让配置引用机密的名字，而不嵌入机密值。使用 `credentials/` 存储、查询和移除凭据；使用 `credentials-local/` 将凭据私密地存储在本机，并支持按次运行的环境覆盖；当需要向人询问以获取凭据时，使用 `authorization/`。轮换后的存储值会作用于下一次模型请求，而 `DEEPSEEK_API_KEY=… lyn` 在该次运行中优先。配置文件只包含凭据名称；本地机密值只有同一 OS 用户可读。
...
- [生成配置目录](../../docs/config-catalog.zh.md#lynesslyn-credentials-local)——本地存储的每个受支持字段。
