...
vi.mock('@lyness/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@lyness/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@lyness/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
