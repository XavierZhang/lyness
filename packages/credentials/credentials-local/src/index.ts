/**
 * File-backed credentials provider over `$LYNESS_HOME/.credentials.yaml`, layered
 ...
 * > $LYNESS_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… lyn`, a CI
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveLynHome(config.lynHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `lyn-atomic-write`, this is a
...
/** File-backed credentials provider (`$LYNESS_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start lyn from instead',
      )
    }
  }
