...
| `lynHome` | `$LYNESS_HOME` or `~/.lyn` | Harness home used when `path` is omitted |
...
- **A key the launching environment supplies is read-only** — `DEEPSEEK_API_KEY=… lyn` wins for this run; saving or removing it is refused. Clear the variable in the launching shell first.
...
| — | No runtime invariant companion is published; the Service Definition companion (`lyn-credentials/invariant`) owns the `credentials/reference-updated` lifecycle contract; this provider's file/environment layering is asynchronous I/O pinned by its unit suite. |
...
- **Atomic, not crash-durable** — inherited from `lyn-atomic-write`; the store re-reads on boot.
