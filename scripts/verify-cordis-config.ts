 ...
 *
 * Both have happened. `shell-env` in a preset realm left `LYNESS_WEB_URL` reaching
...
function validateAppResolution(): string[] {
  ...
  // lyn app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.lyn?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).lyn?.bundle?.patch === 'string')
