/** Application-entrypoint classification and lyn-launch enforcement. */
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-application-entrypoints-'))
  cleanups.push(root)
  return root
}
...
describe('application entrypoints', () => {
  ...
  it('rejects a package-level application bin', () => {
    ...
    expect(applicationEntrypointViolations(root)).toEqual([
      'packages/example/app/package.json: package bin bypasses the lyn launcher; applications use apps/cli profiles',
    ])
  })
  ...
  it('rejects a private Python application carrier outside lyn', () => {
    const root = fixture()
  ...
  it('rejects a start:web script that bypasses the lyn launcher', () => {
    const root = fixture()
  ...
  it('rejects a dev:web wrapper that serves without the lyn launcher', () => {
    const root = fixture()
  ...
  it('rejects a new root demo until its launch role is classified', () => {
    const root = fixture()
    write(root, 'package.json', JSON.stringify({ scripts: { 'demo:new-app': 'lyn --profile new-app' } }))

    expect(applicationEntrypointViolations(root)).toEqual([
      'package.json scripts.demo:new-app: demo launcher has no explicit lyn or in-process classification',
    ])
  })
})
