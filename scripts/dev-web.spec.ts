...
describe('StageSupervisor', () => {
  ...
  it('forwards one signal, escalates survivors after the grace period, and reports none of them', async () => {
    ...
    const stubborn = fakeStage('lyn web', ['SIGKILL'])
  ...
  it('waits for stages the terminal already interrupted before signaling survivors', async () => {
    ...
    const survivor = fakeStage('lyn web', ['SIGTERM'])
...
describe('parseDevWebArguments', () => {
  ...
  it('separates its own flags from the arguments forwarded to lyn web', () => {
  ...
  it('rejects lyn web arguments when no server is started', () => {
    expect(() => parseDevWebArguments(['--no-serve', '--no-open'])).toThrow('--no-serve leaves no lyn web process for --no-open')
  })
})

it('samples one local environment at startup without validating watcher outputs', async () => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-dev-web-environment-'))
  try {
    ...
    writeClientBuildRecord(root, {
      ...
      LYNESS_CLIENT_COMMIT_HASH: 'fffffff',
      LYNESS_CLIENT_TITLE: 'lyness',
      LYNESS_CLIENT_VERSION: '1.2.2',
    })
    ...
    expect(devWebBuildEnvironment(root, {
      ...
      LYNESS_CLIENT_COMMIT_HASH: 'abc1234',
      LYNESS_CLIENT_EXTRA: 'launch-value',
    })).toEqual({
      PATH: '/bin',
      LYNESS_CLIENT_COMMIT_HASH: 'abc1234',
      LYNESS_CLIENT_EXTRA: 'launch-value',
...
it('discovers lyn.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-dev-web-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-dev-web-library-'))
  try {
    ...
    await write('packages/client/delivered', { lyn: { client: { platform: 'web' } } }, clientPreset)
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'lyn-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@lyn-test/dev-web-watch', private: true, type: 'module' }))
