/**
 * Enforce lyn profiles as the only supported Node application launcher.
...
interface LauncherPolicy {
  readonly kind: 'lyn-direct' | 'lyn-wrapper'
  readonly wrapper?: string
}

/** Public product launcher plus the build-only WebWorker packer. */
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ['apps/cli/package.json', { lyn: 'lib/bin.js' }],
  ['packages/experimental/webworker-packer/package.json', { 'lyn-pack-vfs-image': './bin.js' }],
])
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ['apps/cli/src/bin.ts', 'supported lyn application launcher'],
  ...
  ['packages/subagent/subagent-lyn-sdk/tests/fixtures/loader/driver.ts', 'test-only subprocess driver'],
...
/**
 * Root scripts that start an application must visibly select lyn: every `demo:*`
...
const ROOT_LAUNCHER_POLICIES = new Map<string, LauncherPolicy>([
  ['demo:ptc', { kind: 'lyn-wrapper', wrapper: 'scripts/demo-ptc.mjs' }],
  ['demo:inspector', { kind: 'lyn-direct' }],
  ['start:web', { kind: 'lyn-direct' }],
  ['dev:web', { kind: 'lyn-wrapper', wrapper: 'scripts/dev-web.ts' }],
])
...
function manifestBinViolations(root: string): string[] {
  ...
  for (const rawPath of manifests) {
    ...
    if (expected === undefined) {
      failures.push(`${path}: package bin bypasses the lyn launcher; applications use apps/cli profiles`)
      continue
    }
...
function referencesLynCli(source: string): boolean {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy === undefined) {
      if (name.startsWith('demo:')) {
        failures.push(`package.json scripts.${name}: demo launcher has no explicit lyn or in-process classification`)
      }
      continue
    }
    const command = typeof commandValue === 'string' ? commandValue : ''
    if (policy.kind === 'lyn-direct') {
      if (!referencesLynCli(command)) failures.push(`package.json scripts.${name}: application launcher script must launch apps/cli/src/bin.ts`)
    ...
    if (!referencesLynCli(source)) failures.push(`${wrapper}: application launcher wrapper must launch apps/cli/src/bin.ts`)
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (failures.length > 0) {
    ...
    console.log('verify-application-entrypoints: lyn is the only supported Node application launcher.')
  }
}
