...
/** Root demos are application wrappers and therefore must visibly select lyn. */
...
function rootDemoViolations(root: string): string[] {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy.kind === 'lyn-direct') {
      if (!referencesLynCli(command)) failures.push(`package.json scripts.${name}: application demo must launch apps/cli/src/bin.ts`)
    ...
    if (!referencesLynCli(source)) failures.push(`${wrapper}: application demo wrapper must launch apps/cli/src/bin.ts`)
