...
const TAGS = ['lyn-v0.1.0-alpha.1', 'lyn-v0.1.0-rc.1', 'lyn-v0.1.0-rc.2'] as const
...
function fixture(): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'lyn-persistence-releases-'))
  temporary.push(root)
...
describe('pinned persistence releases', () => {
  ...
  it('rejects malformed manifest identities and version metadata', () => {
    ...
    for (const changes of [
      { tag: 'lyn-v0.1.0-beta.1' }, { sourceDate: '2026-09-01' },
