 ...
 * @param input - Parsed npm lockfile v3 from a consumer with only lyn as its dependency.
...
async function refreshResolution(): Promise<void> {
  const temporary = await mkdtemp(join(tmpdir(), 'lyn-dependency-catalog-'))
  try {
    writeFileSync(join(temporary, 'package.json'), `${JSON.stringify({
      name: 'lyn-dependency-catalog', version: '0.0.0', private: true, dependencies: { [PACKAGE]: 'latest' },
    }, null, 2)}\n`)
