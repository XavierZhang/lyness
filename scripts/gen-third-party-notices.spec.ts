...
describe('tierExternalDeps', () => {
  it('limits the LibreOffice exception to its reviewed package identity and MPL terms', () => {
    ...
    for (const dependency of [
      ...
      { name: '@lyness/lyn-libreoffice-kit', license: 'MPL-2.0' },
...
describe('virtualManifest', () => {
  it('resolves a manifest from an ordinary prefix-matching store directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-notices-prefix-'))
    try {
  ...
  it('falls back to a content scan when pnpm 11 truncates the store directory name', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-notices-truncated-'))
    try {
      const name = '@scope/pkg'
      const version = '2.0.0'
      const store = join(root, 'store')
  ...
  it('selects the requested version when the store retains historical copies', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-notices-version-'))
    try {
      const name = '@scope/pkg'
      const store = join(root, 'store')
  ...
  it('returns undefined when neither the prefix nor the content scan finds the package', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-notices-miss-'))
    try {
      const store = join(root, 'store')
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@lyness/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
...
describe('collectPythonDependencies', () => {
  ...
  it('excludes normalized local project names without exempting a third-party prefix', () => {
    const pyprojects = [
      '[project]\nname = "lyness-runtime-bin"\ndependencies = ["pydantic"]\n',
      '[project]\nname = "lyness-sdk"\ndependencies = ["Lyness_Runtime-Bin", "lyness-unrelated"]\n',
    ]
    expect(() => collectPythonDependencies(pyprojects)).toThrow(
      'python dependency lyness-unrelated is missing from PYTHON_METADATA',
    )
  })
})
