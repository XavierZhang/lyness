...
const CLIENT_BUILD_ENV_PREFIX = 'LYNESS_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'LYNESS_BUILD_CLIENT_PROFILE'

/** Public client environment required by official LYN artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'LYNESS_CLIENT_COMMIT_HASH'

/** Public variable carrying the repository package version embedded in client artifacts. */
const CLIENT_VERSION_VARIABLE = 'LYNESS_CLIENT_VERSION'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.lyn-build/client-build-environment.json'
...
export function repositoryClientBuildEnvironment(
  ...
  delete inherited.LYNESS_CLIENT_COMMIT_HASH
  delete inherited.LYNESS_CLIENT_GIT_DIRTY
  delete inherited.LYNESS_CLIENT_VERSION
  const dirty = repositoryGitDirty(root)
  return {
    ...inherited,
    LYNESS_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    ...(dirty === true ? { LYNESS_CLIENT_GIT_DIRTY: 'true' } : {}),
    LYNESS_CLIENT_VERSION: repositoryVersion(root),
  }
}
...
export function officialClientBuildEnvironment(
  root: string,
  environment: NodeJS.ProcessEnv = process.env,
): Readonly<Record<`LYNESS_CLIENT_${string}`, string>> {
  return {
    LYNESS_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    LYNESS_CLIENT_VERSION: repositoryVersion(root),
 ...
 * @returns defined `LYNESS_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return {
      LYNESS_CLIENT_COMMIT_HASH: commitHash,
      LYNESS_CLIENT_VERSION: version,
      ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT,
    }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`LYNESS_CLIENT_${string}`, string>>,
): void {
