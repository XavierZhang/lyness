...
export function packageReadmeSummaryErrors(file: string, source: string): string[] {
  ...
  const template = kind === undefined
    ? '.agents/skills/lyn-doc/templates/'
    : `.agents/skills/lyn-doc/templates/${kind}.md`
  return [
    `${file}: Summary has ${String(words)} words; the limit is ${String(MAX_PACKAGE_README_SUMMARY_WORDS)}. Read .agents/skills/lyn-doc/SKILL.md and ${template} before rewriting it.`,
  ]
}
