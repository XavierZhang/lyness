...
const MERGE_HEAD = /declare module ['"](?:@lyness\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@lyness/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@lyness/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@lyness/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@lyness/cordis'` Context merge
...
/**
 * Every event name a `declare module '@lyness/cordis'` Events merge declares in one
