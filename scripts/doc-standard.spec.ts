...
const KIND_TEMPLATES: Readonly<Record<string, string>> = {
  ...
  'persistence-change': '.agents/skills/lyn-doc/templates/persistence-change.md',
  'persistence-release': '.agents/skills/lyn-doc/templates/persistence-release.md',
  'persistence-format': '.agents/skills/lyn-doc/templates/persistence-format.md',
}
...
function validateSessionFormatRelease(source: string, currentWriterVersion: number): SessionFormatRelease {
  ...
  if (typeof evidenceTag !== 'string'
    || !/^lyn-v\d+\.\d+\.\d+(?:-[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?(?:\+[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?$/u.test(evidenceTag)) {
    throw new Error('evidenceTag must be a non-empty lyn-v version tag without URL delimiters')
  }
...
describe('Session format release authority', () => {
  ...
  it('rejects empty, malformed, and URL-injecting evidence tags', () => {
    const { record, evidence } = sessionFormatReleaseFixture()
    for (const tag of [
      null, true, 1, '', ' ', 'lyn-v', record.evidenceTag.replace('lyn-v', 'v'),
      ...
      expect(() => validateSessionFormatRelease(source, record.latestReleasedVersion), String(tag)).toThrow('lyn-v version tag')
    }
  })
...
describe('lyn-doc skill consolidation', () => {
  ...
  it('maps retrospective release comparisons to their dedicated document kind', () => {
    const files = globSync('docs/persistence-changes/releases/lyn-*.md', { cwd: root })
    expect(files.length).toBeGreaterThan(0)
    for (const file of files) {
      const metadata = readFrontmatter(file)
