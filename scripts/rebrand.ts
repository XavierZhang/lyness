...
const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    from: "'@deepseek-ai', 'dsh-",
    to: "'@lyness', '",
    note: 'A node_modules directory built from path fragments, which must spell the same name the manifest beside it declares. The scope rule drops the product segment from the name but cannot see it split across two string arguments, leaving the directory and the manifest naming different packages.',
  },
  {
 ...
 * dependency patch as readily as in TypeScript. A NUL byte in the first block
 * is the same signal `git diff` uses to call a file binary.
...
/** Placeholder character pair; a source file never contains a NUL byte. */
const MASK = '\u0000'
 ...
 * @param from - the source text for the running direction.
 * @param to - the target text for the running direction.
 * @returns The rewritten line, with archived references restored verbatim.
 */
function applyPreservingArchivedReferences(line: string, rule: Rule, from: string, to: string): string {
  const frozen: string[] = []
  const masked = line.replace(ARCHIVED_REFERENCE, (match) => {
    frozen.push(match)
    return `${MASK}${String(frozen.length - 1)}${MASK}`
  })
  const rewritten = applyRule(masked, rule, from, to)
  if (frozen.length === 0) return rewritten
  return rewritten.replace(/\u0000(\d+)\u0000/g, (_match, index: string) => frozen[Number(index)] ?? '')
}
