...
function gitSupportsObjectFormat(format: 'sha256'): boolean {
  const root = mkdtempSync(join(tmpdir(), 'lyn-git-object-format-'))
  try {
...
describe('translation pairing snapshots', () => {
  it('stores exact uncommitted bytes for later recovery by object ID', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-translation-pairing-'))
    try {
      ...
      expect(execFileSync('git', [
        '-C', root, 'rev-parse', `refs/lyn/translation-pairing/snapshots/${objectId}`,
