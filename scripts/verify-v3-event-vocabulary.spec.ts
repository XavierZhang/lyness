...
import { RELEASED_V3_EVENT_TYPES } from '@lyness/lyn-session-format-v3-to-v4'
...
function fixture(options: { writer?: string; events?: string } = {}): { root: string; sourceRef: string } {
  const root = mkdtempSync(join(tmpdir(), 'lyn-v3-vocabulary-'))
  roots.push(root)
...
describe('explicit V3 source vocabulary verification', () => {
  ...
  it.each(['HEAD', 'origin/master', 'lyn-v0.1.5-alpha.1', 'a'.repeat(10), '', '--help'])('rejects a mutable or incomplete source pin %j', (ref) => {
