...
Defaults to ~/.lyn/sessions. Already-V4 Sessions are opened read-only.
...
async function migrate(root: string, jobs: number): Promise<number> {
  const reportDirectory = mkdtempSync(join(tmpdir(), 'lyn-migrate-v4-'))
...
if (process.argv[1] !== undefined && realpathSync(process.argv[1]) === realpathSync(import.meta.filename)) {
  try {
    ...
    if (values.help) console.log(usage)
      ...
      process.exitCode = await migrate(resolve(values['sessions-dir'] ?? join(homedir(), '.lyn', 'sessions')), jobs)
    }
  } catch (error: unknown) {
