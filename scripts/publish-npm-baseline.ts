...
const RELEASE_ENTRY_PACKAGE = '@lyness/lyn'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"lyn web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed lyn web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed lyn web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'lyn-npm-baseline-'))
...
/** Installs one complete bundle outside the workspace and probes the shipped lyn entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'lyn-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'lyn-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@lyness/lyn/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed lyn bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed lyn --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed lyn entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed lyn Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-lyn-web-probe'], result)
    }
  }
}
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.LYNESS_HOME = resolve(consumerRoot, '.lyn')
  environment.LYNESS_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.LYNESS_TELEMETRY_DISABLED = '1'
