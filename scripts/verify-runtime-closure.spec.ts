...
function fixture(files: Record<string, string | Record<string, unknown>>): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-runtime-closure-'))
  roots.push(root)
  for (const [relative, value] of Object.entries(files)) {
    ...
    writeFileSync(path, preset === null ? (typeof value === 'string' ? value : `${JSON.stringify(value, null, 2)}\n`) : JSON.stringify([{ insert: [{ name: '@lyness/lyn-agent-preset', config: { id: preset[1], plugins: typeof value === 'string' ? loadCordisYaml(value) : value } }] }]))
  }
  return root
}
...
- id: conditional
  ...
  disabled: !!js process.env.LYNESS_DISABLE_CONDITIONAL === '1'
`,
    })
