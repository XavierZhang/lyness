...
import type { LynBundleManifest } from '../packages/util/package-manifest/src/types.ts'
...
const publicNativePackages = new Set([
  '@lyness/node-addon-system',
  '@lyness/node-addon-system-darwin-arm64',
  '@lyness/node-addon-system-darwin-x64',
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@lyness/lyn': ['lib/*.js', 'lib/types/*.d.ts'],
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@lyness/lyn-app-boot': ['lib/worker/profile-resolution-bootstrap.js'],
  ...
  '@lyness/lyn-client-ui-sidebar-documentpreview': ['lib/client.*.js'],
  '@lyness/lyn-client-ui-sidebar-terminal': ['lib/client.*.js'],
  '@lyness/lyn-client-web': ['lib/**/*.css', 'lib/apply-injections.js'],
  '@lyness/lyn-client-ui-theme': ['lib/styles'],
  // The physical-key protocol is a public entry usable without the browser service.
  '@lyness/lyn-client-shortcuts': ['lib/protocol.js'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@lyness/lyn-experimental-ptc-runtime-python': ['py/**/*.py'],
  '@lyness/lyn-experimental-speech-to-text-sensevoice': ['runtime/assets.json'],
  // The isolated Node bootstrap is a separately launched bundle.
  '@lyness/lyn-ptc-runtime-node': ['lib/process.js'],
  // The Host entry starts its sibling Worker by URL rather than a package export.
  '@lyness/lyn-experimental-inspector': ['lib/worker.js'],
  // Creator's composition guidance travels with the declaration package.
  '@lyness/lyn-agent-preset': ['skills'],
  ...
  '@lyness/lyn-skill-office': ['assets'],
  '@lyness/lyn-subprocess': ['lib/control.js'],
  ...
  '@lyness/lyn-headless': ['lib/json-stream-*.js'],
}
...
/**
 * Require a lyn-family manifest to carry the workspace version.
 *
 * The lyn release sequence publishes packages/ and apps/ members and every
 * private lyn package on one shared version, written by `release:lyn` and
 ...
 * @param expected - the version every lyn-family manifest must carry (the root's).
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const label = manifest.name ?? dir
  const familyVersionError = checkLynFamilyVersion(manifest, repositoryVersion)
  ...
  if (isPublicNativePackage) {
    ...
    // them, and the lyn family published publicly with its own sequence on
  ...
  if (dir.startsWith('apps/') && dir !== desktopApplicationDirectory && manifest.name?.startsWith('@lyness/')) {
...
/**
 * Prevent an official runtime from requiring an experimental package. The lyn installation's `dependencies`
...
export function checkExperimentalDependencyIsolation(
  ...
  for (const { dir, manifest } of manifests) {
    ...
    const offered = manifest.name === '@lyness/lyn' ? new Set(optionalBundles) : new Set<string>()
...
/**
 * Require exact LYN ranges, tilde vendor/native ranges, and the workspace protocol elsewhere.
 *
  ...
  for (const { dir, manifest } of manifests) {
    for (const section of dependencySections) {
      for (const [name, range] of Object.entries(manifest[section] ?? {})) {
        ...
        const expected = name === '@lyness/lyn' || name.startsWith('@lyness/lyn-')
          ? 'workspace:*'
