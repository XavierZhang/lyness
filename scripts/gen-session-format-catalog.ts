...
export interface SessionFormatMigrationManifest extends Readonly<Omit<LynSessionFormatMigrationManifest, 'export'>> {
...
export function collectSessionFormatMigrations(
  ...
  for (const discovered of globSync('packages/session/session-format-v*-to-v*/package.json', { cwd: scanRoot }).sort()) {
    ...
    const metadata = manifest.lyn?.sessionFormatMigration
    if (metadata === undefined) {
      throw new Error(`gen-session-format-catalog: ${rel} lacks lyn.sessionFormatMigration`)
    }
    const allowed: ReadonlySet<string> = new Set<keyof LynSessionFormatMigrationManifest>([
