...
async function acquireInstallLock(commonDirectory) {
  ...
  let observeBarrier = process.env.LYNESS_TEST_LEFTHOOK_LOCK_OBSERVE_BARRIER
  while (true) {
    try {
      ...
      try {
        ...
        const publicationBarrier = process.env.LYNESS_TEST_LEFTHOOK_LOCK_PUBLISH_BARRIER
