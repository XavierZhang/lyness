/**
 * The Web development loop: build once, serve through `lyn web`, and keep every
 * browser-side artifact rebuilt on source edits, in one terminal.
 *
 * Stages, in order: `pnpm run build` (skipped by `--skip-build`), three
 * long-lived watchers, then `lyn web` (skipped by `--no-serve`). The watchers
 ...
 * `lyn web` serves. A missing watcher does not fail — it silently shows the
 ...
 *
 * Usage: `pnpm run dev:web [--skip-build] [--no-serve] [--poll[=ms]] [lyn web arguments]`.
 ...
 * `lyn web` started elsewhere. `--poll` switches the source watchers to polling
...
/** Resolved `dev-web` command line: the script's own flags plus the arguments forwarded to `lyn web`. */
export interface DevWebArguments {
  /** Skip the complete `pnpm run build` that otherwise precedes the watchers. */
  readonly skipBuild: boolean
  /** Start `lyn web`; false keeps only the rebuild watchers beside an already running server. */
  ...
  /** Arguments forwarded verbatim to `lyn web`, in order. */
 ...
 * when `--no-serve` leaves forwarded arguments without a `lyn web` process.
 */
export function parseDevWebArguments(argv: readonly string[]): DevWebArguments {
  ...
  if (!serve && appArgs.length > 0) {
    throw new Error(`dev-web: --no-serve leaves no lyn web process for ${appArgs[0] ?? ''}`)
  }
 ...
 * whose package.json carries `lyn.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      lyn?: { client?: { platform?: unknown } }
    }
    if (manifest.lyn?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `lyn.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.lyn?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
/**
 * Grace period per escalation step. `lyn web` gives its plugin tree five seconds
...
if (isMain) {
  ...
  try {
    ...
    console.error('dev-web: usage: pnpm run dev:web [--skip-build] [--no-serve] [--poll[=ms]] [lyn web arguments]')
    process.exit(1)
  }
  ...
  for (const name of Object.keys(process.env)) {
    if (name === CLIENT_BUILD_PROFILE_SELECTOR || name.startsWith('LYNESS_CLIENT_')) {
      Reflect.deleteProperty(process.env, name)
    }
  }
  for (const [name, value] of Object.entries(buildEnvironment)) {
    if (name.startsWith('LYNESS_CLIENT_') && value !== undefined) process.env[name] = value
  }

  const pluginDirs = discoverPluginDirs()
  const libraryDirs = discoverLibraryDirs()
  if (pluginDirs.length === 0) {
    console.error('dev-web: no lyn.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  if (!shutdownRequested()) {
    ...
    // The same launch vector as the root `lyn` script, so the served Host runs
    // from source exactly as `pnpm lyn web` would.
    if (options.serve) {
      spawnStage(supervisor, 'lyn web', process.execPath, [
        '--import', 'tsx/esm', 'apps/cli/src/bin.ts', 'web', ...options.appArgs,
      ], false)
    }
    console.log(
      `dev-web: watching ${String(pluginDirs.length)} lyn.client plugin packages`
      ...
      + (options.serve ? ', serving through lyn web' : '')
      + ':\n  '
