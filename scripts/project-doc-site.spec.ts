...
function fixture(): { root: string; pages: DocsPage[] } {
  const root = mkdtempSync(join(tmpdir(), 'lyn-doc-site-'))
  roots.push(root)
...
describe('documentation site build', () => {
  it.each([
    ...
    const root = mkdtempSync(join(tmpdir(), 'lyn-doc-build-'))
    roots.push(root)
  ...
  it('refuses to remove the site root or an outside directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-doc-build-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'lyn-doc-build-outside-'))
  ...
  it('unlinks a link-shaped output without removing its target', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-doc-build-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'lyn-doc-build-link-target-'))
    roots.push(root, outside)
    const outDir = join(root, '.dist')
  ...
  it('refuses output whose nearest existing parent resolves outside the site root', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-doc-build-parent-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'lyn-doc-build-parent-link-target-'))
    roots.push(root, outside)
...
describe('publishableImage', () => {
  ...
  it('refuses a target whose real path escapes the repository', () => {
    ...
    const { root } = fixture()
    const outside = mkdtempSync(join(tmpdir(), 'lyn-doc-site-outside-'))
    roots.push(outside)
...
describe('rewriteMarkdown', () => {
  it('maps published pages and pins unpublished source links', () => {
    ...
    expect(rewriteMarkdown(source, {
      ...
      + '[source](https://github.com/XavierZhang/lyness/blob/abc123/packages/tool.ts#L2) '
  ...
  it('uses raw GitHub content for unpublished images when nothing places them', () => {
    ...
    expect(rewriteMarkdown('![logo](../packages/logo.svg)\n', {
      ...
      repoRoot: root,
      repositoryRef: 'abc123',
    })).toBe('![logo](https://raw.githubusercontent.com/deepseek-ai/lyness/abc123/packages/logo.svg)\n')
  })
  ...
  it('replaces the destination token without changing repeated titles or escapes', () => {
    ...
    expect(rewriteMarkdown(source, {
      ...
      + '[escaped](https://github.com/XavierZhang/lyness/blob/abc123/docs/x(y).md)\n',
    )
  })
...
describe('docsPages locale routes', () => {
  it('redirects both locale roots to their locale-relative quick-start page', () => {
    ...
    for (const page of homes) {
      ...
      expect(projected).not.toContain('# lyness')
    }
  })
...
describe('projectedPageContent', () => {
  ...
  it('drops the repository badge every page links from its footer', () => {
    const badge = '[![](https://img.shields.io/badge/powered_by-lyn-4D6BFE?style=flat-square)](https://github.com/XavierZhang/lyness)'
    expect(projectedPageContent(`# Guide\n\nBody.\n\n${badge}\n`, page('zh-guide')))
      .toBe('# Guide\n\nBody.\n')
  })
...
describe('emitRawMarkdownPages', () => {
  function mirrorDir(): string {
    const out = mkdtempSync(join(tmpdir(), 'lyn-doc-mirror-'))
    roots.push(out)
    return out
  }
...
describe('raw Markdown projection of the published manifest', () => {
  ...
  beforeAll(() => {
    mirror = mkdtempSync(join(tmpdir(), 'lyn-doc-mirror-real-'))
  ...
  it('emits home pages with their bodies instead of the frontmatter stub', () => {
    for (const route of ['index.md', 'en/index.md']) {
      ...
      expect(home, route).toContain('# lyness')
    }
  })
...
describe('llmsTxt', () => {
  const site = { base: '/x/', title: 'lyness', description: '插件化 SDK' }
