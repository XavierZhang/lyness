...
export async function preparePrimaryRuntime(options: PreparePrimaryRuntimeOptions): Promise<void> {
  ...
  const staging = mkdtempSync(join(tmpdir(), 'lyn-primary-'))
  try {
  ...
  const require = createRequire(import.meta.url)
  await prepareOfficeSkillAssets(join(dirname(require.resolve('@lyness/lyn-skill-office/package.json')), 'assets'),
...
if (import.meta.main) {
  ...
  await preparePrimaryRuntime({
    ...
    cache: values.cache ?? join(tmpdir(), 'lyn-primary-runtime-downloads'), version,
