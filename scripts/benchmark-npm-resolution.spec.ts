...
describe('npm resolution benchmark', () => {
  ...
  it('combines installed metadata with current publishable workspace fields', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-npm-registry-index-'))
    roots.push(root)
  ...
  it('runs npm against the local registry without requesting an archive', async () => {
    const index: RegistryIndex = new Map([[
      '@lyness/lyn',
      new Map([['0.1.0', { name: '@lyness/lyn', version: '0.1.0' }]]),
    ]])
  ...
  it('returns npm placement for two aliased package versions without requesting archives', async () => {
    const index: RegistryIndex = new Map([[
      '@lyness/lyn',
      new Map([
        ['0.1.0', { name: '@lyness/lyn', version: '0.1.0' }],
        ['0.2.0', { name: '@lyness/lyn', version: '0.2.0' }],
      ]),
    ]])

    const result = await resolveNpmPackageLock(index, {
      '@lyness/lyn': '0.2.0',
      'lyn-previous': 'npm:@lyness/lyn@0.1.0',
    }, 10_000)

    expect(result.archiveRequests).toBe(0)
    expect(result.packageLock.packages['node_modules/@lyness/lyn']?.version).toBe('0.2.0')
    expect(result.packageLock.packages['node_modules/lyn-previous']).toMatchObject({
      name: '@lyness/lyn',
      version: '0.1.0',
    })
  })
  ...
  it('isolates peer resolution from inherited npm configuration', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-hostile-npm-config-'))
    roots.push(root)
    const userConfig = join(root, 'user.npmrc')
    writeFileSync(userConfig, '@lyness:registry=http://127.0.0.1:1/\nlegacy-peer-deps=true\nomit=peer\n')
    const previous = {
