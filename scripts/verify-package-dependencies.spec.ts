...
function sourceFacts(
  ...
  const root = mkdtempSync(join(tmpdir(), 'lyn-dependency-source-'))
  roots.push(root)
...
function generatedHostFixture(mode: 'schema' | 'object'): { root: string; manifestPath: string; source: string } {
  const root = mkdtempSync(join(tmpdir(), 'lyn-generated-host-dependencies-'))
  roots.push(root)
...
describe('package dependency scope', () => {
  ...
  it('discovers the Client directory, lyn.client declarations, and configured Host packages', () => {
    const packages = [
      ...
      pkg('@f/dynamic-client', 'packages/client/dynamic/package.json', { lyn: { client: {} } }),
      pkg('@f/dual', 'packages/api/dual/package.json', { lyn: { client: {} } }),
      ...
      pkg('@f/excluded', 'packages/api/excluded/package.json', { lyn: { client: {} } }),
  ...
  it('rejects stale, redundant, overlapping, and unknown configuration', () => {
    ...
    expect(found.violations).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining('clientFaceExclude names @f/host, which declares no lyn.client entry'),
...
describe('face-aware source classification', () => {
  ...
  it('fails when a managed Host package has no Host entry', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-package-missing-host-'))
    roots.push(root)
  ...
  it('counts Host values as dependencies and Client values as development inputs', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-package-faces-'))
    roots.push(root)
    const subject = pkg('@f/dual', 'packages/g/dual/package.json', {
      lyn: { client: { inject: ['@f/injected'] } },
    })
    const files = {
  ...
  it('identifies exact runtime exports without treating type imports as values', () => {
    const source = [
      ...
      "import { createLazyRequire as lazy } from '@lyness/lyn-lazy-require'",
      "import * as lazyModule from '@lyness/lyn-lazy-require'",
...
describe('dependency sections', () => {
  ...
  it('repairs owned relationships without changing unrelated dependencies', () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-package-dependencies-'))
    roots.push(root)
