...
function workspaceRange(name: string): 'workspace:*' | 'workspace:~' {
  return name === '@lyness/lyn' || name.startsWith('@lyness/lyn-') ? 'workspace:*' : 'workspace:~'
}
...
export function discoverPackageDependencyScope(
  ...
  for (const name of include) {
    ...
    if (pkg !== undefined
      && (pkg.manifestPath.startsWith('packages/client/') || hasClientDeclaration(pkg.manifest.lyn))) {
  ...
  for (const name of exclude) {
    ...
    } else if (pkg !== undefined && !hasClientDeclaration(pkg.manifest.lyn)) {
      violations.push(`clientFaceExclude names ${name}, which declares no lyn.client entry`)
    }
  }
  ...
  for (const pkg of packages) {
    ...
    const clientHost = (hasClientDeclaration(pkg.manifest.lyn) || include.has(pkg.name)) && !exclude.has(pkg.name)
...
export function collectRuntimeSourceExportUses(path: string, source: string): RuntimeSourceExportUse[] {
  ...
  for (const statement of sourceFile.statements) {
    if (!ts.isImportDeclaration(statement)
      ...
      || statement.moduleSpecifier.text !== '@lyness/lyn-lazy-require') continue
...
export function readPackageDependencyFacts(
  ...
  const inject = pkg.manifest.lyn?.client?.inject ?? []
...
export function expectedPackageDependencies(
  ...
  for (const name of facts.clientInject) {
    if (facts.workspaceNames.has(name)) add(name, 'devDependencies', 'lyn.client.inject')
  }
