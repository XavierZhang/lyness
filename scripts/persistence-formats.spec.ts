...
function saveReference(root: string, version: number, currentVersion: number, snapshot = inventory(version)): void {
  ...
  const record = {
    ...
    source: version === 1 ? { pullRequest: 3349 } : { tag: 'lyn-v0.1.0-rc.5' },
...
function fixture(currentVersion = 3): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-persistence-formats-'))
  temporary.push(root)
...
describe('complete persistence format references', () => {
  ...
  it('loads a contiguous archive and the current catalog without Git or historical package sources', () => {
    ...
    expect(formats.entries[0]?.source).toEqual({ tag: 'lyn-v0.1.0-rc.5' })
  ...
  it('rejects a source tag outside the LYN release namespace', () => {
    const root = fixture()
    editPair(root, 0, 'tag: lyn-v0.1.0-rc.5', 'tag: latest')
