...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.LYNESS_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('LYNESS_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$LYNESS_GATE_CONCURRENCY'
  const failFast = flagEnabled('LYNESS_GATE_FAIL_FAST')
 ...
 * @param failFast - whether `LYNESS_GATE_FAIL_FAST` is enabled.
...
export function ciWorkerEnvironment(
  ...
  setDefault('LYNESS_OXLINT_THREADS', shared)
  setDefault('LYNESS_PUBLINT_CONCURRENCY', shared)
  setDefault('LYNESS_SNAPSHOT_MAX_WORKERS', 1)
  setDefault('LYNESS_SNAPSHOT_MAX_CONCURRENCY', shared)
  setDefault('LYNESS_WEB_SNAPSHOT_WORKERS', env.LYNESS_GATE_CONCURRENCY === '1' ? 1 : available)
  setDefault('LYNESS_COVERAGE_MAX_WORKERS', available)
  const coverageBudget = env.LYNESS_COVERAGE_MAX_WORKERS || String(available)
  const total = Number(coverageBudget)
  if (!Number.isSafeInteger(total) || total < 1) {
    throw new Error(`run-gates: LYNESS_COVERAGE_MAX_WORKERS must be a positive integer, got ${JSON.stringify(coverageBudget)}.`)
  }
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { LYNESS_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('lyn-package-licenses', 'verify-lyn-package-licenses', { label: 'LYN package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('LYNESS_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('lyn-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'lyn source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { LYNESS_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { LYNESS_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[], after?: string[]): Gate {
  ...
  const workerRaw = process.env.LYNESS_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 1 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: LYNESS_WEB_SNAPSHOT_WORKERS must be a positive integer, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `LYNESS_SNAPSHOT=replay LYNESS_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.LYNESS_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `LYNESS_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// LYNESS_COVERAGE_MAX_WORKERS is shared between the two parallel gates. CI
// defaults its partition count to the instrumented share; an explicit
// LYNESS_COVERAGE_PARTITIONS overrides that share independently. The exempt
...
// total of one (the serial reference jobs) also set LYNESS_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// LYNESS_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test, expect.poll, and hook
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('LYNESS_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.LYNESS_GATE_VERBOSE === '1'
