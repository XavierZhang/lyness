...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'lyn-python-office-'))
...
it('rejects a missing required dependency before producing a sidecar', async () => {
  const { staging, destination, packageAt } = await fixture()
  await packageAt('@deepseek-ai/libreoffice-kit', { dependencies: { 'lyn-missing-office-fixture': '1' } })

  await expect(copyOfficeSidecar(staging, destination, { platform: 'linux', arch: 'x64' }))
    .rejects.toThrow('lyn-missing-office-fixture required by @deepseek-ai/libreoffice-kit is missing')
