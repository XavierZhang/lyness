...
const SESSION_PACKAGE = '@lyness/lyn-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@lyness/lyn-session/types'
 ...
 * top-level declaration (in `@lyness/lyn-session`) and any declaration
 * merge inside a `declare module '@lyness/lyn-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @lyness/lyn-session/known-event-types',
    ' */',
    '',
    '/**',
