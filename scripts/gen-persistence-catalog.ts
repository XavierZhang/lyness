...
const SESSION_PACKAGE = '@lyness/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@lyness/session/types'
 ...
 * top-level declaration (in `@lyness/session`) and any declaration
 * merge inside a `declare module '@lyness/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @lyness/session/known-event-types',
    ' */',
    '',
    '/**',
