 ...
 * `@lyness/*` listed 49 candidate globs and `@lyness/*\/invariant`
...
interface PackageAlias {
  /** Bare specifier, e.g. `@lyness/session`. */
 ...
 * @returns Every directory whose manifest names a `@lyness/` package and that carries `src`.
 */
 ...
 * `@lyness/<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@lyness/` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@lyness\/[^"/]+)":/gm)) {
    const key = match[1]
