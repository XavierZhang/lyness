 ...
 * `@lyness/lyn-*` listed 49 candidate globs and `@lyness/lyn-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@lyness/lyn-` package and that carries `src`.
 */
 ...
 * `@lyness/lyn-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@lyness/lyn-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@lyness\/lyn-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@lyness\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
