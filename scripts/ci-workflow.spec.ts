...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests]] as const) {
      ...
      expect(evaluateRunsOn(job['runs-on'], { vars: { LYNESS_CI_FAILOVER_WINDOWS: 'blacksmith' } }))
    ...
    expect(windowsCoverage.env).toMatchObject({ LYNESS_COVERAGE_PARTITIONS: "${{ vars.LYNESS_CI_FAILOVER_WINDOWS == 'selfhosted' && github.event.pull_request.user.login != 'dependabot[bot]' && '4' || '' }}" })
    ...
    expect(observational).toMatchObject({
      ...
      'timeout-minutes': 15,
      env: {
        LYNESS_GATE_FAIL_FAST: '',
        LYNESS_PUBLINT_CONCURRENCY: "${{ vars.LYNESS_CI_FAILOVER_WINDOWS == 'selfhosted' && github.event.pull_request.user.login != 'dependabot[bot]' && '8' || '' }}",
      },
    })
    ...
    expect(serialGate!.env).toMatchObject({ LYNESS_COVERAGE_TEST_TIMEOUT_MS: '90000' })
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      ...
      expect(evaluateRunsOn(job['runs-on'], { vars: { LYNESS_CI_FAILOVER_LINUX: 'blacksmith' } }))
    ...
    for (const [name, selector, variable, pool, hosted] of [
      ['linux gates', selectors.linux, 'LYNESS_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'lyn-ubuntu-24-04-16core'],
      ['linux aggregate', selectors.linuxAggregate, 'LYNESS_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'ubuntu-latest'],
      ['windows lanes', selectors.windows, 'LYNESS_CI_FAILOVER_WINDOWS', ['self-hosted', 'lyn-win-ci', 'windows'], 'lyn-windows-2025-16core'],
    ] as const) {
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers], ['node-compat', nodeCompat]] as const) {
      expect(job.env, `${jobName} must enable fail-fast`).toMatchObject({ LYNESS_GATE_FAIL_FAST: '1' })
    }
    ...
    expect(windowsBuild.env, 'windows-build must enable fail-fast').toMatchObject({ LYNESS_GATE_FAIL_FAST: '1' })
    expect(windowsCoverage.env, 'windows-coverage must enable fail-fast').toMatchObject({ LYNESS_GATE_FAIL_FAST: '1' })
    ...
    expect(observational?.env).toMatchObject({ LYNESS_GATE_FAIL_FAST: '' })
  })
  ...
  it('gates standalone keyless blacksmith jobs and benchmark tiers on the failover variables', () => {
    ...
    expect(expectedFilenames['runs-on']).toContain('LYNESS_CI_FAILOVER_LINUX')
    ...
    expect(sandbox['runs-on']).toContain('LYNESS_CI_FAILOVER_LINUX')
    ...
    for (const name of ['larger-runner-benchmark', 'consolidated-runner-benchmark'] as const) {
      ...
      expect(benchmark['runs-on']).toContain('LYNESS_CI_FAILOVER_LINUX')
      expect(benchmark['runs-on']).toContain('LYNESS_CI_FAILOVER_WINDOWS')
...
describe('Runtime and LLM e2e Blacksmith routing', () => {
  it('routes DeepSeek e2e only through the Linux Blacksmith switch', () => {
    ...
    for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
      expect(evaluateRunsOn(job['runs-on'], { vars: { LYNESS_CI_FAILOVER_LINUX: mode, LYNESS_CI_FAILOVER_WINDOWS: 'blacksmith' } }))
  ...
  it('keeps native release and dispatch builders hosted while routing x64 CI by platform', () => {
    ...
    for (const [target, runner, variable, blacksmith] of [
      ['node24-linux-x64', 'ubuntu-latest', 'LYNESS_CI_FAILOVER_LINUX', 'blacksmith-2vcpu-ubuntu-2404'],
      ['node24-win-x64', 'windows-2025', 'LYNESS_CI_FAILOVER_WINDOWS', 'blacksmith-2vcpu-windows-2025'],
      ['node24-linux-arm64', 'ubuntu-24.04-arm', 'LYNESS_CI_FAILOVER_LINUX', 'ubuntu-24.04-arm'],
      ['node24-macos-arm64', 'macos-latest', 'LYNESS_CI_FAILOVER_LINUX', 'macos-latest'],
      ['node24-macos-x64', 'macos-15-intel', 'LYNESS_CI_FAILOVER_LINUX', 'macos-15-intel'],
    ] as const) {
      for (const ci of [false, true]) {
        for (const release of [false, true]) {
          for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
            const vars = { LYNESS_CI_FAILOVER_LINUX: 'blacksmith', LYNESS_CI_FAILOVER_WINDOWS: 'blacksmith', [variable]: mode }
  ...
  it.each(['plan', 'sdk-wheel'])('routes runtime %s only for non-release CI', (name) => {
    ...
    for (const ci of [false, true]) {
      for (const release of [false, true]) {
        for (const mode of ['', 'selfhosted', 'unexpected', 'blacksmith']) {
          expect(evaluateRunsOn(job['runs-on'], { inputs: { ci, release }, vars: { LYNESS_CI_FAILOVER_LINUX: mode } }))
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/lyness_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/lyness_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
...
describe('Issue lifecycle workflow', () => {
  ...
  it('mints Project credentials only after preflight and always revalidates current metadata', () => {
    ...
    expect(tokenStep).toMatchObject({
      ...
      with: {
        'client-id': '${{ vars.LYNESS_ISSUE_APP_CLIENT_ID }}',
        'private-key': '${{ secrets.LYNESS_ISSUE_APP_PRIVATE_KEY }}',
        owner: 'lyness',
        repositories: 'lyness',
...
describe('npm release workflows', () => {
  ...
  it('runs dependency policy and npm layout checks in the LYN release workflow', () => {
    ...
    if (!isRecord(workflow.on) || !Array.isArray(dependencies.steps)) {
      throw new TypeError('LYN release workflow must define triggers and dependency steps')
    }
