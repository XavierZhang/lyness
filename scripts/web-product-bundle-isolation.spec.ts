...
function fixture() {
  ...
  const root = realpathSync.native(mkdtempSync(join(tmpdir(), 'lyn-web-bundle-isolation-')))
  ...
  write('apps/web/package.json', '{"name":"@lyness/lyn-web-frontend","type":"module"}')
  ...
  write('packages/experimental/prototype/package.json', '{"name":"@lyness/lyn-experimental-prototype","type":"module"}')
...
describe('default Web bundle input isolation', () => {
  ...
  it('checks npm alias package identity independently of its directory name', async () => {
    ...
    test.write('apps/web/node_modules/ordinary-name/package.json',
      '{"name":"@lyness/lyn-experimental-aliased","type":"module","main":"index.js"}')
...
describe('bundler input ownership', () => {
  ...
  it('distinguishes missing actual inputs from omitted upstream source-map files', () => {
    ...
    test.write('apps/web/node_modules/upstream/package.json', '{"name":"@lyness/lyn-experimental-upstream"}')
    ownership.reset()
