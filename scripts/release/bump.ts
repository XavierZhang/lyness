 ...
 *
 * The lyn family shares one version across its publishable members, private
...
/** Release types the lyn family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the lyn family's version. */
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:lyn <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Plan the lyn family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the lyn family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateLynVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <lyn|vendor> [version]')
  ...
  if (family.id === 'lyn') {
    ...
    if (request === undefined) throw new Error('usage: release:lyn <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:lyn takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
