 ...
 * owns: `lyn` and `vendor`. Each family carries its own version baseline, tag
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@lyness/')) throw new Error(`${normalized} must name an @lyness package`)
...
class LynFamily extends ReleaseFamily {
  readonly id = 'lyn'
  readonly patterns = [
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`lyn release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@lyness/lyn', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@lyness/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new LynFamily(), new VendorFamily()]
}
