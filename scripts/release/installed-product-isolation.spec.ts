...
const roots: string[] = []
const experimental = '@lyness/lyn-experimental-example'

function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-installed-isolation-'))
  roots.push(root)
  return root
}
...
describe('installed default-product isolation', () => {
  it('ignores development dependencies and unrelated installed experimental packages', () => {
    const root = fixture()
    const entry = writePackage(root, '@lyness/lyn', {
      dependencies: { core: '1.0.0' },
      devDependencies: { [experimental]: '1.0.0' },
    })
    writePackage(root, 'core', { peerDependencies: { '@lyness/lyn': '1.0.0' } })
  ...
  it('skips the entry package\'s optional bundles, which must be installed, and still rejects other experimental edges', () => {
    const root = fixture()
    const entry = writePackage(root, '@lyness/lyn', { dependencies: { core: '1.0.0', [experimental]: '1.0.0' } })
    writePackage(root, 'core', {})
    writePackage(root, experimental, { dependencies: { '@lyness/lyn-experimental-inner': '1.0.0' } })
    ...
    const missing = writePackage(root, '@lyness/lyn-missing', { dependencies: { absent: '1.0.0' } })
    expect(() => verifyInstalledProductIsolation(missing, ['absent'])).toThrow('optional bundle is missing: @lyness/lyn-missing -> absent')
  })

  it('rejects experimental identities hidden behind an installed alias', () => {
    const root = fixture()
    const entry = writePackage(root, '@lyness/lyn', { dependencies: { safeName: 'file:../prototype' } })
  ...
  it('follows nested installations rather than an unrelated hoisted package', () => {
    const root = fixture()
    const entry = writePackage(root, '@lyness/lyn', { dependencies: { core: '1.0.0' } })
    writePackage(root, 'core', {})
  ...
  it('permits missing optional packages and optional peers but rejects missing dependencies', () => {
    ...
    writePackage(root, '@lyness/lyn', { dependencies: { missing: '1.0.0' } })
