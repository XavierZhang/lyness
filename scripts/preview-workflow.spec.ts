...
describe('PR preview workflow', () => {
  ...
  it('keeps the immutable full build and restore-only dependency cache', () => {
    ...
    expect(workflow.env.LYNESS_TELEMETRY_DISABLED).toBe('1')
    const commands = preview.steps.map(step => step.run)
    expect(commands).toContain('pnpm install --frozen-lockfile')
    expect(commands).toContain('pnpm run build')
    expect(commands).toContain('pnpm --filter @lyness/lyn-web-frontend run build:preview')
    expect(commands.indexOf('pnpm run build')).toBeLessThan(commands.indexOf('pnpm --filter @lyness/lyn-web-frontend run build:preview'))
  ...
  it('retains per-PR deployment, protected image verification, and idempotent URL comments', () => {
    ...
    expect(workflow.env.CF_PROJECT).toBe('lyn-build-preview')
    ...
    expect(comment.run).toContain('<!-- lyn-preview-url -->')
