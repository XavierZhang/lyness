...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-persistence-changes-'))
  roots.push(root)
...
describe('persistence changes current-tree commands', () => {
  ...
  it('executes the real CLI against a source-only temporary checkout without Git history', { timeout: 90_000 }, () => {
    ...
    writeFileSync(join(session, '../package.json'), JSON.stringify({ name: '@lyness/lyn-session' }))
    writeFileSync(join(root, 'tsconfig.host.json'), JSON.stringify({ compilerOptions: {
      ...
      paths: { '@lyness/lyn-session/types': ['./packages/core/session/src/types.ts'] },
    } }))
    const source = [
