...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@lyness/subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@lyness/subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@lyness/subprocess-local')
    expect(bootstrap).toContain("import('@lyness/lyn/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@lyness/subprocess-local/runner')")
