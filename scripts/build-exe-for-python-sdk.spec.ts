...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@lyness/lyn-subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@lyness/lyn-subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@lyness/lyn-subprocess-local')
    expect(bootstrap).toContain("import('@lyness/lyn/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@lyness/lyn-subprocess-local/runner')")
  ...
  it('runs pnpm through its JavaScript entrypoint without a command shell', () => {
    ...
    expect(result.stdout).toContain(`${process.execPath} C:\\tools\\pnpm.cjs --filter lyn-python-runtime-closure deploy`)
    const deploy = result.stdout.split('\n').find(line => line.includes(' --filter lyn-python-runtime-closure deploy'))
  ...
  it('resolves the pnpm package behind a Windows command shim', () => {
    const setup = mkdtempSync(join(tmpdir(), 'lyn-pnpm-home-'))
