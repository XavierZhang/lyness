...
The public package family belongs to the `@lyness` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@lyness/node-addon-landlock-run
@lyness/node-addon-landlock-run-<platform>
```
