...
Prebuilt `bin/landlock-run` Landlock launcher for linux-x64 — a static musl binary compiled natively (no cross toolchain) from the C source shipped in [`@lyness/node-addon-landlock-run`](https://www.npmjs.com/package/@lyness/node-addon-landlock-run). npm's `os`/`cpu` fields select this package at install time; the entry package resolves it to a file path — it ships no JavaScript and is never imported.
...
Sibling: `@lyness/node-addon-landlock-run-linux-arm64`.
