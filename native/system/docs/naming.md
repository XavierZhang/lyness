...
The public package family belongs to the `@lyness` scope and uses the `node-addon-system` package prefix; platform packages append platform information only:

```text
@lyness/node-addon-system
@lyness/node-addon-system-<platform>
```
