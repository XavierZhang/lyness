...
| `x-client-version` | 共享的客户端构建版本，在打包时内联为 `LYNESS_CLIENT_VERSION`；完整 SemVer，保留预发布标识，一期等于内置 lyn 版本 |
...
| `x-client-bundled-lyn-version` | 发布元数据中的完整内置 lyn 版本 |
...
按平台、架构、Desktop 版本、内置 lyn 版本与通道匹配，由服务端维护版本范围和优先级。使用完整 SemVer，不按字符串排序或截断预发布部分。一期两个版本相等、通道固定 Nightly，独立修订与通道切换延后。不得要求降级，也不得仅因客户端曾查询或展示过就停止返回 `40005`。
...
**依赖远程业务拦截。** 本地 lyn 请求不一定到达远程网关，需要独立的未登录查询。
...
| 相等的 Desktop/lyn 预发布版本、固定 Nightly | 完整 SemVer 匹配，不要求降级或切换通道 |
