...
x-client-bundled-lyn-version: 0.1.3-rc.2
```
...
| `x-client-version` | Shared client build version, inlined as `LYNESS_CLIENT_VERSION` at packaging; full SemVer retaining prerelease identifiers, initially equal to the bundled lyn version |
...
| `x-client-bundled-lyn-version` | Full bundled lyn version from release metadata |
...
**Relying on remote business interception.** Local lyn requests need not reach the remote gateway; an independent guest query is necessary.
...
| Equal prerelease Desktop/lyn versions, fixed Nightly | Full SemVer matching, no downgrade or channel-switch requirement |
