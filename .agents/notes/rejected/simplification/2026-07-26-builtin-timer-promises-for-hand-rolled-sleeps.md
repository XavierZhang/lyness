...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`lyn-llm-mock-server` `pause()`, `lyn-lsp-stdio`, `lyn-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
