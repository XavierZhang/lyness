...
Each disabled row carries its restore recipe in a local comment. An overlay row re-enables the capability for a base-backed profile from `$LYNESS_HOME/cordis.patch.yml` or a `--patch` file. Shipped presets are `@lyness/lyn-agent-preset` rows in the Web bundle patch (`packages/bundle/web-app/cordis.patch.yml`), so a Web session that wants `ralph` overrides that row's `config.plugins` from the profile patch, or saves a preset under a **new id** through the Web editor, which persists the declaration in `$LYNESS_HOME/profiles/<profile>/cordis.patch.yml`; the registry refuses a duplicate preset id (`packages/preset/agent-preset-registry/src/index.ts`). In `ptc` the override drops `disabled` from the tool row and the engine row together, because `tool-ralph` injects `ctx.workflowEngine`.
