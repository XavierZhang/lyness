...
**Format version.** This event simplification predates the released v0 baseline. `lyn-session` owns the current writer constant, while the static catalog and adjacent packages now own supported historical decoding and migration. The identity v0-to-v1 edge proves that lifecycle without changing this event representation.
