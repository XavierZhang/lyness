# Agent Note: Omit runtime invariants from shipped lyn config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@lyness/*/invariant` entry.
...
- Ordinary `lyn` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
