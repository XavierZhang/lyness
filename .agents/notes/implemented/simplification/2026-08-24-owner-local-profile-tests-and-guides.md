...
The `demo:acp` and `demo:cordis` scripts are absent. ACP starts through `lyn --profile acp`; the Cordis guide starts `lyn web` with its explicit overlay. `demo:ptc` remains as a thin wrapper over `lyn --profile headless` with `LYNESS_TOOLS_MODE=ptc`.
