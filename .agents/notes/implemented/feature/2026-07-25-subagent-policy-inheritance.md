...
- Each delegation adds at most three log-only events. `lyn-subagent` owns the optional peer types for permission presets, sandbox policy, and approval — its shared helpers hold the `ctx.get` consumption; compositions without those services behave unchanged. Out-of-process children retain their own deployment policy, and a running child does not follow later parent switches.
