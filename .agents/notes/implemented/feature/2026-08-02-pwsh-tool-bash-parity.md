...
- `@lyness/shell-env` is a new shipped package; `lyn-tool-bash`'s `lynHome` config moved there, so compositions mounting the shell tools must also mount `shell-env` (the spine bundles do).
