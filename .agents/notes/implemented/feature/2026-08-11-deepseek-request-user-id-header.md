...
`lyn-llm-deepseek` sends `x-lyness-user-id` on every provider request sent after successful credential resolution. The value comes from `@lyness/anonymous-user-id` and therefore matches the OpenTelemetry Resource `user.id` and `/feedback` acknowledgement for the same `$LYNESS_HOME`. The adapter continues to send `x-lyness-session-id` only when `GenerateOptions.sessionId` is present; the agent loop supplies the current durable `Session.id` for ordinary agent, title-generation, and compaction requests.
