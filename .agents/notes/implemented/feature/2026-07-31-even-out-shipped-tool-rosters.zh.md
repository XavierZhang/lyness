...
`apps/cli` 增加了五个 workspace 依赖:四个是交付树当时挂载的,外加 `lyn-mcp-client`——它并不被挂载,存在的意义是让已安装的 `lyn` 能挂。四个保留了下来——[session-search-not-shipped-default 决策](2026-08-02-session-search-not-shipped-default.zh.md)把 `@lyness/tool-session-query` 连同它的行一起移除了。
