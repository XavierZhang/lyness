...
The rail sits against the scrollport's right edge and centers on the band the sticky composer leaves visible. That band is the scrollport's own height minus the seat's, so ConversationRoot publishes `--lyn-conversation-viewport-height` beside the `--lyn-composer-height` it already measures on the same element, and the rail centers on their difference instead of a viewport height that ignores the Session header.
