...
[Schedule 指南](../../../../docs/user/guide/schedule.zh.md)使用显式加载 `@lyness/time-context` 与 `@lyness/schedule` 的 overlay；默认 Web 配置树保持不变。Schedule 只观察插件加载后发布的根 Agent，并在该 Agent scope 中安装三个工具和一个可丢弃 owner。cold history 读取、已发布的根、child Agent 与其他 host 都不会激活它。
