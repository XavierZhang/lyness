...
该功能的第一方归属是一对包：`@lyness/host-open-in-app` 位于 `packages/host/open-in-app/`（探测、目录与启动路由），`@lyness/client-ui-open-in-app` 位于 `packages/client/ui-open-in-app/`（分体按钮），由 `lyn-web-app` bundle 的 `open-in-app` 与 `ui-open-in-app` 两行挂载进 Web profile。转正是重写，不是 vendoring：
