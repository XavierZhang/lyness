...
The event vocabulary lives in contract packages (`lyn-agent` declares the `agent/*` events); `@lyness/lyn-agent-loop` is the only concrete loop plugin and is itself swappable — nothing outside it may depend on it.
