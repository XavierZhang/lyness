...
这些辅助函数位于基础消息词汇旁的 `lyn-llm` 中，因为它们的完整约定只依赖该词汇。`createToolResultMessage()` 与其他创建辅助函数同属此处：它将工具调用 id 与确切的 user-role 工具结果块及来源耦合起来，不依赖会话状态或事件。`lyn-session` 只消费完整消息，不负责构造它们。
