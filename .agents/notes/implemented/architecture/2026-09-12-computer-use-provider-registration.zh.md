...
桌面提供方暴露不同的操作、观测格式和平台设施。LYN 需要防止在一个组合中意外启用两个提供方，同时让各提供方的集成正常工作，而不承诺通用操作 API。

## Decision

LYN 能力称为 **computer use（计算机操作）**。[`lyn-computer-use`](../../../../packages/computer-use/computer-use/README.zh.md) 拥有 `ctx.computerUse`，注册一个提供方自定的名称并返回其 effect 清理函数。第二次注册无论名称为何都会失败。服务不包含提供方对象、共享操作类型、分派方法、Session 锁或运行时选择器。
...
各集成暴露上游工具目录。MCP 结果转换保留在 `lyn-mcp-client` 中，其基于回调的工具适配函数也转换原生 Cua Driver 结果。计算机操作服务不依赖该适配函数或任一提供方。
...
**仅嵌入原生运行时。** 原生集成让 LYN 拥有运行时生命周期，并与后端进程共享原生故障。MCP 提供方保留独立安装驱动的选项。
