...
`lyn-python-runtime-closure` 是私有部署 manifest，`lyness-sdk-runtime-<platform>-<arch>` 是可执行文件族。协议字段 `serverInfo.name` 是 `lyness-sdk-runtime`；Python 分发包名是 `lyness-sdk` / `lyness-runtime-bin`，导入模块名是 `deepseek_harness` / `deepseek_harness_runtime`。
