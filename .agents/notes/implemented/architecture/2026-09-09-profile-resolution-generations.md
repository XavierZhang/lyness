...
The package traversal belongs to `@lyness/lyn-app-boot` beside profile loading. Ordinary Node, source launches, packaged executables, and the Electron Host consume the same runtime resolution and interception.
...
- Pkg and Electron carriers select runtime resolution; Electron executes its Host in Node mode from the ASAR-backed lyn tree while native executable entries remain unpacked.
