...
There is no dedicated model-facing Consumer package. The Consumer is the existing `ctx.tools` execution pipeline: `lyn-spill-policy` consumes final tool results through the `tools/post-execute` waterfall, and the model follows the backend-supplied retrieval hint for the returned locator.
...
`lyn-spill-policy` is a `tools/post-execute` result transformer with one configuration knob:
