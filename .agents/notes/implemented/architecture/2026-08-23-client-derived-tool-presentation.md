...
The Client connection stops re-exporting `ToolCallView` and `ToolResultView` from `lyn-tools/presentation` for Session consumers.
...
- Session history does not reference `lyn-tools/presentation`, `ctx.tools`, `presenterScopeFor`, or `backscanArgs`.
...
Changes to this decision use `lyn-pre-push-checks` to select commands for the final diff. Required evidence includes:
...
- `LYNESS_SNAPSHOT=replay pnpm run test:web`, without refreshing existing presentation goldens;
