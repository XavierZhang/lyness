...
**Teach the shipped skill to run the CLI dump.** It requires the profile name in the agent's shell and a `lyn` executable on its PATH, neither of which the session guarantees, and its output is unbounded.
