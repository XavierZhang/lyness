...
- Persistence stays a plugin concern; the in-memory store ships in lyn-session.
...
- Derivation cost grows with log length — compaction (lyn-compaction) is the intended mitigation, not log mutation.
