...
`@lyness/lyn-brand-studio` 是一个以 `lyn --profile brand-studio` 启动的 bundle，其 patch 只插入一行，且不叠加在 `lyn-base` 之上。该行用 commander 解析 launcher 传入的参数，运行一次，并经 `ctx.appExit` 请求退出。它以参数接收产品名、图标 PNG、字体文件、可选的主题色、资产目录与目标 profile；不带 `--accept-trademark` 时拒绝运行。
...
字体由运营方提供。字标排版的决策本就要求调用方提供字体，而随附字体会与仓库的运行时许可证门禁冲突——该门禁不接受任何 SIL OFL 字体。
...
**把 Inter 作为运行时依赖随附，提供预设字体清单。** 运营方就不必自备字体文件。`scripts/gen-third-party-notices.ts` 会拒绝非宽松许可证的运行时依赖；把 OFL 加进它的清单或加一条具名例外，改变的是整个仓库的分发策略，而不只是本包。
...
AI 生成路径、交互式询问与预设字体清单仍待完成；其中第一项需要图像生成能力。
