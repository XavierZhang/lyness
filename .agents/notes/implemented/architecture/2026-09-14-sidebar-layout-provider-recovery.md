...
OpenCode's `packages/app/src/context/layout.tsx` saves Session tabs, while `context/terminal.tsx` separately saves terminal identities. LYN uses its existing Session-scoped store and provider services for the same separation, retaining Session ownership for terminals.
