...
The provider-neutral identity is owned by `lyn-llm` (`packages/llm/llm/src/attribution.ts`), not by individual adapters. `AppIdentity` contains only public product facts needed to build `User-Agent`, and the default `APP_IDENTITY` values:

- product token for `User-Agent`: `lyness` (continuity with the pre-Agent Note wire value and the repo/org identity)
- version: read from the owning package's manifest via `createRequire`, never a hand-copied constant
- app URL: `https://github.com/XavierZhang/lyness` - the repository home
...
| Direct DeepSeek endpoint | `User-Agent` for app attribution; `x-lyness-user-id` and conditional `x-lyness-session-id` are separate request identity under the DeepSeek-specific decision. Do not send OpenRouter-only headers unless DeepSeek documents an equivalent contract. |
...
- `lyn-llm` documents the mandatory `User-Agent` attribution contract for `LlmAdapter` authors (`LlmAdapter` JSDoc, package README, and the adapter-contract section of `docs/subsystems/llm-streaming.md`).
...
- `lyn-llm-deepseek` sends the shared `User-Agent` on every request and its mock-server suite asserts the exact value.
- `lyn-llm-pi-ai` sends the same `User-Agent` through pi-ai's `StreamOptions.headers` hook and its mock-server suite asserts the exact value.
...
**SDK-named token (`lyness-sdk`).** Considered for the `User-Agent` token because the supported runtime client stack uses the SDK name. `lyness` won because it names the lyness product, matches the org/repo identity and package scope, and keeps wire attribution stable without calling the complete product an SDK.
