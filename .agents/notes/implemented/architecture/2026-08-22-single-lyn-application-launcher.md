...
Every supported Node application starts through the `lyn` CLI and one named profile. The shipped profiles are `web`, `headless`, `sdk`, `sdk-minimal`, and `acp`, selected with `lyn --profile <name>` or `lyn <name>`. `plugin` names the management command; a profile with that name requires `--profile plugin`.
