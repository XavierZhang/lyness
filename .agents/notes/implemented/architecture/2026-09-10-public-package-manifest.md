...
Plugin authors need npm identity, runtime requirements, and LYN declarations from one public import. Internal image-packaging, Session catalog, and generated proxy metadata do not define extension points for community plugins. Exposing those fields together makes internal mechanisms appear available to external authors.

## Decision

[`LynPackageManifest`](../../../../packages/util/package-manifest/src/types.ts) describes the package.json fields LYN uses, with required `name` and `version`. Its optional `lyn` member uses `LynManifest` for public composition and author metadata. The type is a selected npm field set, not a complete package.json schema. App-boot adapts it with `Partial` for local profiles, which need no published identity.

Runtime requirements live at top-level `engines`: `lyn`, `node`, and `npm` are optional version strings, and other engine names are allowed. `lyn.manifestVersion` identifies declaration format `1`. Format and LYN compatibility declarations are not enforced by current installers or loaders.
...
**Put LYN compatibility under `lyn.engines`.** [VS Code](https://code.visualstudio.com/api/references/extension-manifest) places its host requirement in top-level `engines.vscode`. Top-level `engines.lyn` gives authors one location for runtime requirements; LYN still owns enforcement of its custom key.

**Use peer dependencies as the sole host requirement.** Peer dependencies constrain installed npm packages, including the CLI package `@lyness/lyn`. They do not identify the currently running LYN process when plugins live in a separate profile project.
