...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `lyn-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `lyn-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `lyn-timeout` (`idleWatchdog`) |
| Clear the timer | `lyn-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `lyn-timeout` (`timeoutOf`) |
