...
**Mount the registration-only `computer-use` and `browser-use` services in `lyn-base`.** The shared composition would carry rows that only an optional provider bundle needs; a provider bundle can insert the service row from its own patch and dependencies.
