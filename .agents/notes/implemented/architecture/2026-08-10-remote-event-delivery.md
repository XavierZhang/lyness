...
### Consumer contract (`lyn-typert-protocol`)
...
JSON safety remains a runtime concern. Before queueing, the API Remotes Host source checks every argument with `lyn-session`'s `isJsonValue` and fails loudly when one is invalid, because this is an allowlist composition error rather than untrusted input.
...
| Root `tsconfig.base.json` | Adds source-plane `paths` entries for `lyn-settings/types`, `lyn-credentials/types`, and `lyn-api-remotes/types` |
| `lyn-commands` / `lyn-settings` / `lyn-credentials` | Moves each `interface Events` member to the owner's Client-safe `./types`; settings and credentials add that export, move brands and pure types with it, retain constructors in index, and include `lib/types/**/*.js` in published files |
| `lyn-session` | Exposes `isJsonValue` for validation of every event argument by the API Remotes Host source |
