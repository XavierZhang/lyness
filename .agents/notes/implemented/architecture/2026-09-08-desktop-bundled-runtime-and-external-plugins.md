...
[Runtime preparation](../../../../apps/desktop/scripts/prepare-lyn.ts) materializes the production graph once at build time and ships it through `extraResources/lyn`. The Electron shell stays in ASAR. An Electron RunAsNode process runs the private Desktop Host from resources and loads enabled plugins from `$LYNESS_HOME/profiles/desktop`.
