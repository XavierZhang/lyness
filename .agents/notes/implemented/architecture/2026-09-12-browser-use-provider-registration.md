...
Stagehand uses an explicitly configured native model from its pinned SDK catalog. The configured API key and optional headers are forwarded into the browser extension, where Stagehand performs inference. Browser tool inputs and returned data, including SDK result metadata, use the existing Session log. LYN model routing, credential reuse, underlying inference request/response capture, and integration into Session usage accounting remain deferred; this integration adds no persistence events or Session schema changes.
...
**Delegated browser agents.** The experiments compare browser-control backends. Delegating the whole task to another planner would alter LYN's control of the task loop.

**LYN model bridge.** Native Stagehand configuration keeps its browser integration independent of LYN model-request adaptation and persistence. Session model selection, credential reuse, underlying inference capture, and Session usage accounting are deferred as one coordinated integration.
...
Attachment reservations apply within one provider instance; they do not coordinate separate LYN processes or external browser clients. Browser state is absent from Session replay, and cancellation does not undo delivered browser actions. Provider READMEs own engine support, model requirements, and upstream restrictions.
