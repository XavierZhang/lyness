...
- The probe runs on the contender's host. Writers on several hosts sharing one `LYNESS_HOME` over a network filesystem are unsupported; the `flock` that guards Session files does not reliably exclude them either.
...
**Signal handlers in `lyn plugin` only.** This covers one of the paths that leave a lock and leaves crashes, forced exits, and the other lock files behind.
