...
|---|---|---|---|---|
| lyn | Publish set: non-experimental `packages/*/*` + `apps/*`; private experimental packages join only the shared version bump | one version for the publish set, private lyn packages, and workspace root, `0.0.x` | `lyn-v<version>` | `release.yml` (pack) / `release-publish.yml` (publish) |
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@lyness/lyn-invariants` for the same reason.
