...
Every experimental directory outside the denylist is public by default. Workspace constraints require public packages to omit `private` and set `publishConfig.access: public`; all experimental packages retain the `@lyness/lyn-experimental-*` npm prefix. Adding a private prototype requires a denylist entry as well as its private manifest.
