...
|---|---|---|---|---|
| lyn | 发布集：非 experimental 的 `packages/*/*` + `apps/*`；私有实验性包仅加入共享版本 bump | 发布集、私有 lyn 包与 workspace 根共用一个 `0.0.x` | `lyn-v<版本>` | `release.yml`（pack）/ `release-publish.yml`（发布） |
...
`scripts/check-workspace-constraints.ts` 要求这个协议，所以新包无法再引入硬写的范围；同理，invariant companion 规则要求 `@lyness/lyn-invariants` 用 `workspace:^`。
