...
**Declare the three packages as runtime `dependencies` so electron-builder ships them in the asar.** This is how `@lyness/cordis` and `@lyness/lyn-api-gateway` reach the asar today, and it removes the ordering problem. `lyn-app-boot` would bring a large dependency tree into `app.asar/node_modules`, duplicating the copy already inside the packed lyn runtime and enlarging the payload the runtime file policy must screen.
