...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `lyn-tool-bash` (`bash`), `lyn-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `lyn-tool-subagent` (`subagent`). Example-only tools are excluded.
