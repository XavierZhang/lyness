...
- **Installation guidance.** A plugin package mounts through `cordis.yml` rows. Only a package declaring `lyn.bundle.patch` installs as a profile layer via `lyn plugin --profile <name> add <package>` — if this package lacks that declaration, say how it mounts in a composition, never `lyn plugin add`.
