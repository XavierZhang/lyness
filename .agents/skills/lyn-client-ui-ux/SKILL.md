...
- Any new header element or full page must account for macOS and Windows window chrome. On macOS the titlebar clearance is `--lyn-frame-top-clearance`, consumed by Menu, Modal, and overlay primitives as their top safe margin — **never change the global variable to fix one page**. Add a scoped `[data-platform='darwin']` override on the page's own inset instead, and assert the override in the page's stylesheet spec.
