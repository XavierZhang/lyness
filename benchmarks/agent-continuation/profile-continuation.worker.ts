/** End-to-end SDK continuation through built lyn sdk-minimal with an explicitly mounted file editor. */
...
async function run(root: string): Promise<ProfileReport> {
  ...
  await writeFile(patch, [
    ...
    '- insert:',
    '    - id: fs-local', "      name: '@lyness/lyn-fs-local'",
    '    - id: str-replace-editor', "      name: '@lyness/lyn-tool-str-replace-editor'",
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LYNESS_AGENTS_HOME: join(home, 'agents'),
  }
  const harness = new Lyness({
    lynBin: join(import.meta.dirname, '..', '..', '..', 'apps', 'cli', 'lib', 'bin.js'),
    profile: 'sdk-minimal', lynHome: home, processCwd: cwd, cwd,
...
assertBuiltBenchmarkRuntime(import.meta.url, { '@lyness/lyn-sdk-client': import.meta.resolve('@lyness/lyn-sdk-client') })
