...
async function verifyProviderCwdResume(
  ...
  const providerCwd = scenario.manifest.environment?.LYNESS_SNAPSHOT_PROVIDER_CWD
  ...
  const otherHostCwd = await mkdtemp(join(tmpdir(), 'lyn-provider-resume-'))
  const env = {
    LYNESS_HOME: join(cwd, '.lyn'),
    LYNESS_SNAPSHOT: 'replay',
    LYNESS_SNAPSHOT_FILE: fixture,
    LYNESS_SNAPSHOT_PROVIDER: model.provider,
    LYNESS_SNAPSHOT_MODEL: model.model,
    LYNESS_PERMISSION_MODE: 'read-only',
  ...
  try {
    const refused = await runLoaderSmoke({
      ...
      env: { ...env, LYNESS_SNAPSHOT_PROVIDER_CWD: `${providerCwd}/other` },
    })
    ...
    const resumed = await runLoaderSmoke({
      ...
      env: { ...env, LYNESS_SNAPSHOT_PROVIDER_CWD: providerCwd },
    })
...
describe('headless recorded-session snapshots', () => {
  ...
  it('replays original inbox mentions before normalized user messages', () => {
    ...
    const original = 'Use @[Research](lyn-session:InJlZmVyZW5jZS1zb3VyY2Ui)'
    const log = [
  ...
  it.each(['assistant/message', 'assistant/attempt'] as const)(
    ...
    (eventType) => {
      ...
      expect(stderrFromSession(log)).toBe('lyn: reasoning:\nfirst thought\n')
    },
  )
  ...
  it.each([10, 20])('assigns sibling roles by catalog order when the first child timestamp is %i', async (firstCreatedAt) => {
    const cwd = await mkdtemp(join(tmpdir(), 'lyn-headless-catalog-order-'))
    try {
      const logs = [
        [
      ...
      for (const content of logs) {
        const directory = join(cwd, '.lyn', 'sessions', String(headerOf(content).id))
  ...
  it('writes header sidecars without replacing a retained Session generation', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'lyn-headless-sidecars-'))
    try {
  ...
  for (const { scenario, retainedToolInput } of runs) {
    ...
    scenarioTest(`${mode}s ${scenario.name}${inputLabel} through lyn --profile headless`, async () => {
      ...
      try {
        result = await runLoaderSmoke({
          ...
          env: {
            ...
            LYNESS_SNAPSHOT_SPILL_LOCATOR_ROOT: locatorRoot,
            LYNESS_SNAPSHOT_FILE: join(scenario.dir, fixtureFiles[0] as string),
            ...
            ...(scenario.name === 'mcp-resources' || scenario.name === 'mcp-resources-ptc' ? {
              LYNESS_MCP_RESOURCES_FIXTURE: join(repoRoot, 'packages/mcp/mcp-client/tests/fixtures/resources-server.ts'),
            } : {}),
            NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
            LYNESS_TELEMETRY_DISABLED: '1',
            ...(mcpDemo === undefined ? {} : { LYNESS_MCP_DEMO_URL: mcpDemo.url }),
          },
          prepare: async (cwd) => {
            ...
            if (mcpDemo !== undefined) {
              const profileDir = join(cwd, '.lyn/profiles/headless')
          ...
          inspect: async (cwd) => {
            ...
            if (mcpDemo !== undefined) {
              ...
              const saved = await readFile(join(cwd, '.lyn/profiles/headless/cordis.patch.yml'), 'utf8')
