...
| 约 20 个上游 e2e 文件（`apps/cli/tests/**`、`apps/web/tests/shipped-composition.e2e.ts` 及各包 `tests/*.e2e.ts`）、`apps/cli/tests/profiles/{acp/cordis.yml,headless/real-model.patch.yml}`、`docs/testing{,.zh}.md` | 写死的 `'deepseek-v4-flash'` 改为 `E2E_TARGET.model`；两个 YAML fixture 改为 `!!js` 读 `DEEPSEEK_MODELS`；请求缓存与 Claude Code 子代理两个测试在非官方目标上跳过（后者原先直接抛错） | e2e 调用哪个平台要由配置决定而不是写死；每处都是一行替换，合并冲突好解 | 2026-09-22 |
