...
export interface Config {
  /** Harness home containing the fixed user-global `AGENTS.md`; defaults to `$LYNESS_HOME` or `~/.lyn`. */
  lynHome?: string
 ...
 * `lynHome` to bash environment and local skill discovery, `sessionTitle` to
 ...
 */
export interface Config {
  /** The agent-loop `agents` list (see lyn-agent-loop's `Config`). */
  ...
  /** The deployment persona (see lyn-system-prompt's `Config`). */
  persona?: SystemPromptConfig['persona']
  /** The explicit model-facing tool order (see lyn-system-prompt's `Config`). */
  toolOrder?: SystemPromptConfig['toolOrder']
  /** The tool registry's config — its presentation `mode` (see lyn-tools' `Config`). */
  tools?: ToolsConfig
  /** lyness home directory shared by shell context and local skill discovery. */
  lynHome?: string
...
依赖：[`AgentLoopConfig`](#lynessagent-loop) · [`GoalDomainConfig`](#lynessgoal) · [`InvariantConfig`](#lynessinvariants) · [`JobsConfig`](#lynessjobs-local) · [`SessionTitleConfig`](#lynesssession-title) · [`SkillFileSystem`](../packages/skill/skill-filesystem/src/index.ts) · [`SkillRegistryConfig`](#lynessskill) · [`SystemPromptConfig`](#lynesssystem-prompt) · [`toolBash`](../packages/shell/tool-bash/src/index.ts) · [`toolGoal`](../packages/goal/tool-goal/src/index.ts) · [`toolJobs`](../packages/jobs/tool-jobs/src/index.ts) · [`ToolsConfig`](#lynesstools) · [`toolSkill`](../packages/skill/tool-skill/src/index.ts) · [`workspaceContext`](../packages/context/agent-instructions/src/index.ts)
...
export interface Config {
  /** Explicit harness home; omitted follows `LYNESS_HOME`, then `~/.lyn`. */
  lynHome?: string
...
export interface Config {
  /** Override the fixture path; defaults to `$LYNESS_SNAPSHOT_FILE`. */
  file?: string
  /** Override the sidecar path; defaults to `$LYNESS_SNAPSHOT_OVERRIDE`. */
  overrideFile?: string
  /**
   * Override the child-log paths; defaults to `$LYNESS_SNAPSHOT_CHILD_FILES` (a
...
<a id="lynesssession-persistence-sqlite"></a>
 ...
 * and one LYN-owned shutdown bound. Uploading modes validate their endpoint
...
export interface Config {
  ...
  /** lyness config root. Defaults to `$LYNESS_HOME` or `~/.lyn`. */
  lynHome?: string
  /** Shared agent config root. Defaults to `$LYNESS_AGENTS_HOME` or `~/.agents`. */
  agentsHome?: string
  ...
  /** Bundled skill root; defaults to `$LYNESS_BUNDLED_SKILL_DIR` when default roots are included, otherwise mounts none. */
...
export interface Config {
  /** Provider name on `ctx.subagents` (default `lyn-sdk`). */
  providerName: string
  /** Explicit lyn CLI module, resolved and checked at plugin load; omission uses the SDK dependency. */
  lynBin?: string
...
export interface Config {
  /** Include the fixed lyness identity before the deployment persona (default true). */
...
- `@lyness/client-modules` — 需要 `webServer` · `loader`（[`packages/client/modules/src/index.ts`](../packages/client/modules/src/index.ts)）
...
- `@lyness/client-ui-deliverables` — 需要 `systemPrompt`（[`packages/client/ui-deliverables/src/index.ts`](../packages/client/ui-deliverables/src/index.ts)）
...
- `@lyness/code-runtime-python`（[`packages/code-runtime/code-runtime-python/src/index.ts`](../packages/code-runtime/code-runtime-python/src/index.ts)）
