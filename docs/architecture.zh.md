...
所有受支持的 Node 应用都从 `lyn` CLI 与具名 profile 启动。随附应用是 `lyn web`（刻意为 `--profile web` 保留的别名）、`lyn --profile headless`、`lyn --profile sdk`、`lyn --profile sdk-minimal`、`lyn --profile acp` 与 `lyn --profile brand-studio`。TypeScript SDK 会解析其同版本 `lyn` 依赖并选择 `sdk`；自定义插件组合继续由 profile 与有序 patch 文件表达，而不是另一个可执行文件或内联应用树。`sdk-minimal` 是位于同一 launcher 后的仓库自有独立组合包，而不是由调用方提供的 Cordis 配置树。
...
[Electron 桌面应用](../apps/desktop/README.zh.md)持有保留的 `$LYNESS_HOME/profiles/desktop` npm 项目。每个签名 Electron 发行版绑定一个确切 lyn 版本并携带第一方离线 seed；启动时通过内置 pnpm 把该版本安装进可写 profile，同时保留旧 profile 中桌面插件的确切版本。CLI profile 与 Desktop 共享 `$LYNESS_HOME` 下受支持的产品数据，但绝不共享可执行包、插件激活、lockfile 或 `node_modules`。

Electron 通过内置的上游 Node.js 进程启动私有 Desktop Host 包；该包从保留 profile 加载已安装的 lyn 后端与匹配的客户端图。一元 RPC、Remote stream 与版本匹配的客户端资源经带版本的分帧字节管道传输，Node IPC 只保留生命周期控制，再通过安全的 `lyn-app://` 协议到达渲染进程；因此桌面组合不会开放 Web server 或 loopback 端口。只有壳自有 UI 能通过内置 pnpm 及其私有 `$LYNESS_HOME/desktop/pnpm/store` 执行插件事务。
