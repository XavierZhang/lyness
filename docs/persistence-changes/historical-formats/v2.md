...
Inspect the V2 persistence types captured at `lyn-v0.1.3-alpha.2`. Assistant streams are embedded in message or attempt events, and the physical header requires `isSeeded` instead of storing `seedLength`. This snapshot retains V2 declarations before the V3 system-message and envelope changes.
...
The source is the published tag `lyn-v0.1.3-alpha.2`, whose writer constant is 2. The [tag record](../releases/lyn-v0.1.3-alpha.2.md) describes the adjacent release changes. This complete inventory includes unchanged roots as well as that tag’s changed roots.
