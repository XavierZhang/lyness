...
Inspect the V0 persistence types captured at `lyn-v0.1.2-rc.1`. Assistant chunks are separate events, and the physical header stores an optional inherited-event count as `seedLength`. The complete schemas retain the declared types at this checkpoint, including a broad numeric header version.
...
The source is the published tag `lyn-v0.1.2-rc.1`, whose writer constant is 0. This is the last V0 tag in the [captured prerelease archive](../releases/README.md); the [tag record](../releases/lyn-v0.1.2-rc.1.md) preserves its release context. Earlier V0 tags have additional type differences and are not replaced by this snapshot.
