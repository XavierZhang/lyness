...
查阅 `lyn-v0.1.2-rc.1` 对应的 V0 持久化类型。Assistant chunk 是独立事件，物理 header 以可选的 `seedLength` 存储继承事件数。完整 schema 保留该检查点声明的类型，包括宽泛的数值 header 版本。
...
来源为已发布 tag `lyn-v0.1.2-rc.1`，其写入器常量为 0。这是[已收录预发布归档](../releases/README.zh.md)中最后一个 V0 tag；[tag 记录](../releases/lyn-v0.1.2-rc.1.zh.md)保留其发布背景。更早的 V0 tag 还存在其他类型差异，本快照不替代那些记录。
