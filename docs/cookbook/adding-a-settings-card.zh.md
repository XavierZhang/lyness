...
```ts
import type { Context, Volatile } from '@lyness/cordis'
...
浏览器半侧由[客户端模块系统](../../packages/client/modules)送到页面：它扫描已启用的 Loader 条目，找出声明了 `lyn.client` 的包，送出每个包构建好的 `./client` 导出——但它只把一个包的半侧挂在说明符恰为裸包名的那一行上。从子路径导出挂载的行永远不带半侧，因此把一个包拆成多行的组合包，其半侧留在根行上，它注册的每个页面都随根行关闭而消失。需要在其他行关闭时仍保留页面的子插件，应作为独立的包发布。
...
```jsonc
{
  ...
  "lyn": { "client": { "platform": "web", "inject": ["@lyness/lyn-client-ui-settings"] } }
}
```
