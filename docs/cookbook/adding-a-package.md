...
The file-by-file checklist for a new `@lyness/lyn-<name>` package. This checklist is validated against the bash and adapter packages as templates; if it drifts from them, fix it here.
...
packages/<group>/<pkg>/
  ...
  tsconfig.json    # extends ../../../tsconfig.base.json, rootDir src,
                   ...
                   # you use Config, + ../../<group>/<dep> for each lyn dep)
...
| File | Change |
|---|---|
| `tsconfig.base.json` | no edit for an existing group; for a new group, add a `./packages/<group>/*/src` candidate to the `@lyness/lyn-*` wildcard |
...
A `packages/client/*` package additionally extends `tsconfig.base.client.json` instead of `tsconfig.base.json`, and a client plugin package declares `lyn.client` in package.json, exports `./client`, and calls the shared tsdown preset (`packages/client/tsdown.client.ts`) — see [packages/client/AGENTS.md](../../packages/client/AGENTS.md) for the client-side contract.
...
Only Settings shortens literal package-name and module-name fallbacks by removing npm scope and Cordis/LYN prefixes; Plugin Manager keeps complete names. Locale titles and descriptions remain unchanged. A row configuration page can use its registered summary when the plugin has no display description.
