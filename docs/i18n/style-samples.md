...
> Dependency rule: extension plugins depend on interfaces, never on `lyn-agent-loop` (the loop is swappable); composition bundles such as `lyn-base` and `lyn-sdk-minimal` may assemble the concrete loop.

依赖约束规范：各类扩展插件仅依赖抽象接口，严禁直接依赖 `lyn-agent-loop`（该主循环支持替换实现）；`lyn-base` 与 `lyn-sdk-minimal` 等组合包可以组装具体循环。
