...
自动生成的[插件配置目录](../../config-catalog.zh.md)列出每个插件的所有受支持字段与默认值；[`lyn-llm-pi-ai`](../../config-catalog.zh.md#lynessllm-pi-ai) 就是本页所配置的那个提供方段落。[`lyn-llm-pi-ai`](../../../packages/llm/llm-pi-ai/README.zh.md) 和 [`lyn-llm-deepseek`](../../../packages/llm/llm-deepseek/README.zh.md) 参考文档负责直接 `settings.yaml` 配置、目录解析、推理控制、凭据与适配器错误。
...
全部开关、各自接受的取值，以及接受它们的协议，都列在[生成的 `lyn-llm-pi-ai` 配置参考](../../config-catalog.zh.md#lynessllm-pi-ai)的 `PiAiCompatProfile` 之下——该参考派生自源码，因此不会落后于适配器实际接受的内容。
