...
- **Service Definition** (`lyn-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`lyn-bash-local`) — executes commands on the local machine
- **Consumer** (`lyn-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  lyn-shell   │────▶│  lyn-bash-local  │     │ lyn-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@lyness/lyn-my-cap'
...
```yaml
- name: '@lyness/lyn-my-cap-local'
- name: '@lyness/lyn-tool-my-cap'
```
