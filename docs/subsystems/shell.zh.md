...
bash 执行 seam 分为 Service Definition（[lyn-shell](../../packages/shell/shell)，`ctx.shell`）、Service Provider（[lyn-bash-local](../../packages/shell/bash-local) 与 [lyn-bash-sandbox](../../packages/shell/bash-sandbox)）和 Consumer（[lyn-tool-bash](../../packages/shell/tool-bash)，即 `bash` schema）。通用后台任务的 job id、所有权与控制位于 [jobs.md](jobs.zh.md)；本 seam 返回一个不含任务概念的进程句柄。managed-range 机制封装在[子进程 seam](subprocess.zh.md)之后。
...
`stdin` 和 `env` 是受信任的进程内插件输入，不由 `lyn-tool-bash` 暴露。本地执行器会先清除环境中的凭据，再合并调用方显式提供的 env。
...
使用沙箱的执行器通过 `ShellExecutor.sandboxMode` 暴露其已配置的模式回退值。工具层请求 [`@lyness/lyn-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.zh.md)，把每个调用会话的持久 `sandbox/mode` 覆盖值与不可变 cwd 解析为 `ShellExecRequest.sandboxPolicy`；经用户批准、严格更宽松的调用只替换模式。模式/root/enforcement 词汇归 [`@lyness/lyn-sandbox` 沙箱 seam](sandbox.zh.md) 所有；模式仅管辖文件效果。
...
`start()` 返回不含 id 或所有者的句柄。`lyn-tool-bash` 将它适配为 `ctx.jobs.start()` 钩子；随后由通用运行时拥有任务标识与生命周期。`done` 会在底层进程结算时完成且绝不 reject；subprocess 提供方的 rejection 会生成状态为 `killed` 的进程，并把不声明阶段的错误写入 stderr。进程结算后仍可读取，并且沙箱事实会在 `done` 完成前写入。
