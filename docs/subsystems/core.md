...
The `Branded<B>` primitive lives in its own type-only package, [lyn-brand](../../packages/util/brand) (no runtime code, no harness-package dependency), so any package can brand the ids it owns without depending on an unrelated capability package.
