...
|---|---|---|---|
| `ContentBlockMap` | lyn-llm | `ContentBlock` | [llm-streaming.md](llm-streaming.md#content-blocks-and-messages) |
| `MessageSourceMap` | lyn-llm | `MessageSource` | [llm-streaming.md](llm-streaming.md#content-blocks-and-messages) |
| `FinishReasonMap` | lyn-llm | `FinishReason` | [llm-streaming.md](llm-streaming.md#the-model-request-and-result) |
| `TurnEndReasonMap` | lyn-session | `TurnEndReason` | [session.md](session.md) |
| `SessionEventMap` | lyn-session | `SessionEvent` | [session.md](session.md) |
...
The `Branded<B>` primitive and stateless constructor live in [lyn-brand](../../packages/util/brand), which has no harness capability dependency. `brandString<T>()` applies a compile-time-only string brand.
...
The two core IDs are `ToolCallId` (correlates a tool call with its result; lyn-llm) and `SessionId` (the shared live agent and durable session identity; lyn-session). Capability packages brand their own ids too, such as `JobId` in [jobs.md](jobs.md).
