...
The bash execution seam is split across a Service Definition ([lyn-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([lyn-bash-local](../../packages/shell/bash-local) and [lyn-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([lyn-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `lyn-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env. See [the bash-stdin-env Agent Note](../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md).
...
A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@lyness/sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@lyness/sandbox` seam](sandbox.md); modes govern file effects only.
...
`start()` returns a handle with no id or owner. `lyn-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the process closes and never rejects, reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
