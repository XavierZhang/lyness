...
The bash execution seam is split across a Service Definition ([lyn-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([lyn-bash-local](../../packages/shell/bash-local) and [lyn-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([lyn-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Managed-range mechanics live behind the [subprocess seam](subprocess.md).

Source: [`packages/shell/shell/src/types.ts`](../../packages/shell/shell/src/types.ts)

## Managed shell environment namespace

`LYNESS_*` variables are Harness-owned child-process facts. The model-facing bash tool collects them through `ctx.shellEnv` and passes them through `ShellExecRequest.lynEnv`; the subprocess service removes inherited `LYNESS_*` names before merging the current snapshot. The `LynEnvironmentKey`/`LynEnvironment` vocabulary is owned by the [subprocess seam](subprocess.md) and re-exported by `lyn-shell`.
...
interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link lynEnv}, which merges after this
  ...
  /**
   * Harness-owned `LYNESS_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `LYNESS_*` entries before merging this
   ...
   */
  lynEnv?: LynEnvironment | undefined
...
interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link lynEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `LYNESS_*` snapshot (typed to managed keys); merges after {@link env}. */
  lynEnv?: LynEnvironment | undefined
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `lyn-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env.
...
Each stream is a `CollectedOutput` — the (possibly truncated) text plus recovery info; when truncated, `text` is the **tail** and the complete stream spills to a private file. The fields are owned by the [subprocess seam](subprocess.md) and re-exported by `lyn-shell`.

## File sandbox: `ShellSandboxInfo`

A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@lyness/lyn-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@lyness/lyn-sandbox` seam](sandbox.md); modes govern file effects only.
...
`start()` returns a handle with no id or owner. `lyn-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the underlying process settles and never rejects; a subprocess provider rejection becomes a `killed` process with a stage-neutral error on stderr. Reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
...
`ShellExecutor` owns `resolve`, foreground `run`, background-process `start`, and the `sandboxMode` capability fact. `lyn-bash-local` owns command defaulting, timeout/abort classification, the terminal environment, and the background read merge; managed-range termination, bounded collectors, spill files, credential scrubbing, and disposal quiescence are the [subprocess service](subprocess.md)'s. `lyn-tool-bash` owns model-facing rendering and adapts background handles into the [generic job runtime](jobs.md). `lyn-shell` owns the shell tools' shared exit-status contract: the exported `parseExitStatus`/`ParsedExitStatus` inverts the `[exit code: N]` / `[killed by signal: X]` markers `lyn-tool-bash`'s `renderResult` and `lyn-tool-pwsh`'s `renderPwshResult` append, and both tools' `presentResult` use it to split the rendered text into the terminal card's output body and its exit-status pill.
...
Registry (`ctx.shellEnv`) for trusted, per-execution `LYNESS_*` variables. The namespace is rebuilt for every model shell call: ambient `LYNESS_*` values are discarded by the executor, then the registry's current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.
...
Types: [LynEnvironment](subprocess.md) · [ToolExecution](tools.md)
