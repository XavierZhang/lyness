...
Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@lyness/lyn-scope`): agent-scoped listeners receive only that agent.
 ...
 * @mode emit
 */
'goal/changed'(this: import('@lyness/lyn-scope').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void
```
