...
Every supported Node application starts at the `lyn` CLI with a named profile. The shipped applications are `lyn web` (the deliberate alias for `--profile web`), `lyn --profile headless`, `lyn --profile sdk`, `lyn --profile sdk-minimal`, `lyn --profile acp`, and `lyn --profile brand-studio`. The TypeScript SDK resolves its same-version `lyn` dependency and selects `sdk`; custom plugin composition remains a profile plus ordered patch files, not another executable or inline application tree. `sdk-minimal` is a repository-owned standalone bundle behind the same launcher, not a caller-supplied Cordis tree.
...
The [Electron desktop application](../apps/desktop/README.md) owns the reserved `$LYNESS_HOME/profiles/desktop` npm project. Each signed Electron release binds one exact lyn version and carries a first-party offline seed; startup installs that version into the writable profile with the bundled pnpm, while retaining exact desktop-plugin versions from the previous profile. CLI profiles share supported product data under `$LYNESS_HOME`, but never executable packages, plugin activation, lockfiles, or `node_modules` with Desktop.

Electron starts the private Desktop Host package under its bundled upstream Node.js process; that package loads the installed lyn backend and matching client graph from the reserved profile. Unary RPC, Remote streams, and version-matched client assets cross versioned framed byte pipes with Node IPC reserved for lifecycle control, then reach the renderer through the secure `lyn-app://` protocol; the desktop composition opens no Web server or loopback port. Only shell-owned UI can run plugin transactions through the bundled pnpm and its private `$LYNESS_HOME/desktop/pnpm/store`.
