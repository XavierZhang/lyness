...
describe('raw Markdown development middleware', () => {
  it.each([undefined, 'document', 'empty'])('serves an explicit raw fetch with destination %s', (destination) => {
    const { res, next } = request('/lyness/en/reference/index.md?lyn-raw=1#section', destination, 'GET', '/lyness/')
  ...
  it.each([
    ...
    ['/en/reference/index.md?lyn-raw=1', 'script'],
    ['/en/reference/index.md', 'empty'],
    ['/en/reference/index.md?lyn-raw=0', 'empty'],
    ['/en/reference/index.md?lyn-raw=1', 'style'],
    ['/en/reference/index.md?lyn-raw=1', 'image'],
  ...
  it.each(['/missing.md', '/en/reference.md'])('returns 404 for explicit unpublished route %s', (url) => {
    const { res, next } = request(`${url}?lyn-raw=1`, 'empty')
  ...
  it('preserves llms.txt and omits bodies for HEAD', () => {
    ...
    for (const url of ['/llms.txt', '/en/reference/index.md?lyn-raw=1']) {
  ...
  it('delegates missing paths, unrelated routes, other bases and non-read methods', () => {
    for (const { next } of [
      ...
      request('/en/reference/index.md?lyn-raw=1', 'empty', 'GET', '/lyness/'),
      request('/en/reference/index.md?lyn-raw=1', 'empty', 'POST'),
    ]) expect(next).toHaveBeenCalledExactlyOnceWith()
  })
  ...
  it('does not turn a projection error into a successful raw response', () => {
    ...
    expect(() => request('/en/reference/index.md?lyn-raw=1', 'empty')).toThrow('Source missing')
  })
})
