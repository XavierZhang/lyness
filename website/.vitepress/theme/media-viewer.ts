...
export function viewerButton(doc: Document, label: string, icon: keyof typeof icons): HTMLButtonElement {
  ...
  svg.classList.add('lyn-media-icon')
...
export class MediaViewer {
  ...
  open(content: MediaContent, trigger: HTMLElement, onClose: () => void): () => void {
    ...
    dialog.className = 'lyn-media-viewer'
    dialog.setAttribute('aria-labelledby', 'lyn-media-title')
    const toolbar = doc.createElement('div')
    toolbar.className = 'lyn-media-toolbar'
    const title = doc.createElement('span')
    title.className = 'lyn-media-title'
    title.id = 'lyn-media-title'
    ...
    scaleLabel.className = 'lyn-media-scale'
    const fit = viewerButton(doc, copy.fit, 'fit')
    fit.className = 'lyn-media-fit'
    const close = viewerButton(doc, copy.close, 'close')
    close.className = 'lyn-media-close'
    close.autofocus = true
    toolbar.append(zoomOut, scaleLabel, zoomIn, fit)
    const helpToggle = viewerButton(doc, copy.helpLabel, 'help')
    helpToggle.className = 'lyn-media-help-toggle'
    helpToggle.setAttribute('aria-expanded', 'false')
    helpToggle.setAttribute('aria-controls', 'lyn-media-help-text')
    const help = doc.createElement('p')
    help.className = 'lyn-media-help'
    help.id = 'lyn-media-help-text'
    help.hidden = true
    help.textContent = copy.help
    const viewport = doc.createElement('div')
    viewport.className = 'lyn-media-viewport'
    const paper = doc.createElement('div')
    paper.className = 'lyn-media-paper'
