...
export function installMermaidViewer(
  ...
  const scan = (): void => {
    ...
    for (const [container, entry] of entries) {
      if (!containers.has(container) || container.querySelector('svg:not(.lyn-media-icon)') !== entry.svg || !entry.button.isConnected) {
    ...
    for (const container of containers) {
      ...
      const svg = container.querySelector<SVGSVGElement>('svg:not(.lyn-media-icon)')
      if (!svg || !dimensions(svg)) continue
      const trigger = viewerButton(doc, copy.open, 'open')
      trigger.className = 'lyn-diagram-open'
