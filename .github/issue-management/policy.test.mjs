...
test('removes reserved labels from Issues before validation', async (t) => {
  ...
  assert.deepEqual(requests, [
    {
      url: 'https://api.github.com/repos/lyness/lyness/issues/42/labels/kind%2Fbug-fix',
      method: 'DELETE',
    },
    {
      url: 'https://api.github.com/repos/lyness/lyness/issues/42/labels/bug-fix',
      method: 'DELETE',
    },
  ])
})
...
test('deletes a stale audit comment after repairing its only violation', async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      { path: '/repos/lyness/lyness/issues/42', method: 'GET' },
      { path: '/graphql', method: 'POST' },
      {
        path: '/repos/lyness/lyness/issues/42/labels/kind%2Fbug-fix',
        method: 'DELETE',
      },
      {
        path: '/repos/lyness/lyness/issues/42/comments?per_page=100',
        method: 'GET',
      },
      {
        path: '/repos/lyness/lyness/issues/comments/99',
        method: 'DELETE',
      },
    ],
  )
...
test('separates resolving and informational references', () => {
  assert.deepEqual(
    parseReferences({
      body: 'Fixes #12\nRelated to #4\nRefs deepseekharness/lyn-test#7',
      repository: 'deepseekharness/lyn-test',
    }),
...
test('reads Priority and Status from Project custom fields', async (t) => {
  ...
  assert.deepEqual(urls, [
    'https://api.github.com/repos/lyness/lyness/issues/42',
...
const mockPolicyApi = (t, { pull = {}, requested = true, reviews = [], issues = {}, priority = 'P1', projectError = false } = {}) => {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'lyn-policy-'))
  t.after(() => {
...
test('runs trusted rollout selection with absent and present capability markers', { skip: process.platform === 'win32' ? 'The policy workflow executes under hosted Ubuntu bash' : false }, (t) => {
  const directory = mkdtempSync(join(tmpdir(), 'lyn-policy-rollout-'))
...
test('reads policy snapshots in reference order and only resolving Project priorities', async (t) => {
  ...
  const repo = '/repos/lyness/lyness'
  assert.deepEqual(fixture.requests, [
    repo + '/pulls/10',
