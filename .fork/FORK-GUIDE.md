...
```text
upstream（官方，只读）  https://github.com/XavierZhang/lyness.git  分支 master
        │ git fetch / git merge
        ▼
...
pnpm install      # 安装依赖
pnpm lyn --profile headless "your prompt"        # 冒烟验证
```
...
pnpm install
pnpm run test
pnpm lyn --profile headless "your prompt"
...
- [ ] `pnpm install` / `pnpm run build` / `pnpm run test` / `pnpm lyn --profile headless "your prompt"` 全部通过
- [ ] 不占用官方包名、镜像名、域名、Logo
