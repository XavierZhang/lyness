...
| 1.3 | `TenantConfig`：本组织可用的模型（语言、图片、视频、音乐）与供应商白名单、功能授权、Agent 身份、文案覆盖（**不含资产与产品名**，那些在部署层）；API Key 按租户隔离存储 | done（2026-09-20：`packages/tenant/{tenant-config,tenant-config-static}`，只读后端 + 后端自报可写能力；密钥只存凭据引用，授权可带自建端点；身份拆为约束（叠加）与个性（替换），本次只定形状，接进提示词随 1.5；功能先用开关清单；不回落部署级模型。[决策](../.agents/notes/implemented/architecture/2026-09-20-tenant-configuration-seam.md)） |
| 1.4 | 租户配置 Typert RPC（替代文档的 REST 方案，冲突 #2） | done（2026-09-21：`packages/api/tenant-controller`，`ctx.remote.tenant` 的 `describe`／`save`；租户只来自请求，不做参数——为此给连接层加了每请求钩子 `rpc.scope` 并新增 `packages/tenant/tenant-request`，同时补完 1.5 的另一半（会话按创建它的请求打租户戳）。授权只到「调用方配置自己的组织」，成员级 RBAC 待用户体系。未进任何已发布 profile。[决策](../.agents/notes/implemented/architecture/2026-09-21-tenant-configuration-over-rpc.md)、[钩子](../.agents/notes/implemented/architecture/2026-09-21-the-tenant-of-a-request.md)） |
| 1.5 | ⚠️ `tenant_id` 进 session log：新增 `SessionEventMap` 成员 | done（2026-09-20：`packages/tenant/tenant-session` 写入仅日志事件 `tenant/identity`，含租户 id、标识与身份文本原文；`tenant` 投影读回并兼作幂等守卫；新增事件类型**不需要**升 `SESSION_FORMAT_VERSION`。2026-09-21 补完按请求绑定：租户取自创建该会话的那次调用（`ctx.requestTenant`），调用之外（CLI、恢复、子 Agent）仍用配置的租户；最终只改了 1 个官方文件的 2 个模块（连接层的 `rpc.scope`），不需要改创建请求类型、Remote 方法与浏览器端。[决策](../.agents/notes/implemented/architecture/2026-09-20-tenant-in-the-session-log.md)、[请求钩子](../.agents/notes/implemented/architecture/2026-09-21-the-tenant-of-a-request.md)） |
| 1.6 | 数据隔离：租户层配置入库；backend 走 `storage` 缝（既有 sqlite 实现，或新增 MySQL provider） | done（2026-09-21：`packages/tenant/tenant-config-store`，`tenant_config` 存储域 / 版本 1 / `per-record`，介质由 `storage-domain` 路由决定（json 或 sqlite），本包不绑定介质；自报 `writable`，保存前校验并一次列出全部问题；与只读后端二选一。MySQL provider 未做，属 `storage` 缝自己的工作。[决策](../.agents/notes/implemented/architecture/2026-09-21-durable-tenant-configuration.md)） |
...
| 1.10 | `lyn --profile brand-studio`（仅私有化）：图标 PNG 二选一——上传（见 4.8）或填写品牌简介由模型生成；之后统一由 1.9 生成图标与 favicon 的 SVG，字标一律由 1.8 字体排版生成；写入 `assetDirectory` 与 `brand-deployment` 的 patch 层。上传路径依赖 1.8～1.9，生成路径另依赖 1.7；均不依赖 1.1～1.6 | 部分完成（2026-09-15 上传路径已交付：`packages/bundle/brand-studio`，参数式、`--accept-trademark` 与 `--accept-font-license` 必填、品牌方自带字体、写目标 profile 补丁层，[决策](../.agents/notes/implemented/architecture/2026-09-15-brand-studio-profile.md)。2026-09-16 起 `--font` 可选，不传即用内置多语言字体（`packages/host/brand-fonts`，[决策](../.agents/notes/implemented/architecture/2026-09-16-built-in-wordmark-fonts.md)）。待做：AI 生成路径（需 1.7）、交互式引导、首次模型设置） |
