...
| 1.5 | ⚠️ `tenant_id` 进 session log：新增 `SessionEventMap` 成员 | 部分完成（2026-09-20：`packages/tenant/tenant-session` 写入仅日志事件 `tenant/identity`，含租户 id、标识与身份文本原文；`tenant` 投影读回并兼作幂等守卫；新增事件类型**不需要**升 `SESSION_FORMAT_VERSION`。租户来源目前取自插件配置（一个部署一个租户）——按请求绑定需要改 3 个官方文件（创建请求类型、Remote 方法、浏览器端），已决定随 1.4 管理后台一并改。[决策](../.agents/notes/implemented/architecture/2026-09-20-tenant-in-the-session-log.md)） |
