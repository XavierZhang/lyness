...
/** One request forwarded from Electron's `lyn-app://` handler. */
...
export interface DesktopHostController {
  /** Installed lyn version carried by this host. */
  readonly lynVersion: string
...
const DESKTOP_STREAM_PATH = '/.lyn/remote-stream'

const DESKTOP_TRANSPORT_SCRIPT = `globalThis.__LYNESS_TRANSPORT__={
  ownsHost:true,
...
function readManifest(path: string): PackageManifest {
  ...
  if (!isRecord(value)) throw new Error(`lyn desktop: ${path} must contain a package manifest`)
  return {
...
function packageManifestPath(projectDir: string, packageName: string): string {
  ...
  if (!existsSync(path)) throw new Error(`lyn desktop: installed package ${JSON.stringify(packageName)} has no manifest`)
  return path
}
...
function desktopPatches(projectDir: string, allowLinkedPackages: boolean): PatchOptions[] {
  const lynRoot = dirname(packageManifestPath(projectDir, '@lyness/lyn'))
  const profile = loadProfileDirectory('lyn desktop', projectDir, join(lynRoot, 'package.json'))
  for (const layer of profile.layers) {
    if (!allowLinkedPackages && !isProjectPath(projectDir, layer.packageDir)) {
      throw new Error(`lyn desktop: profile bundle ${JSON.stringify(layer.packageName)} resolved outside the desktop profile`)
    }
  }
  const layers = [
    ...profile.layers.map(layer => layer.patches),
    profile.patches,
    loadOverlayPatches('lyn desktop', DESKTOP_PATCH),
  ]
  ...
  if (agentPresets !== undefined) {
    layers.push([{
      ...
      config: {
        ...
        roots: [{ path: join(lynRoot, 'config', 'agent-presets'), trust: 'system' }],
      },
    }])
  }
  return layers.flat()
...
function lynVersion(projectDir: string): string {
  const manifest = readManifest(packageManifestPath(projectDir, '@lyness/lyn'))
  if (typeof manifest.version !== 'string') throw new Error('lyn desktop: installed lyn manifest has no version')
...
export async function runDesktopHost(
  ...
  const environment = loadLayeredEnv('lyn desktop')
  let current: Context | undefined
  const ctx = await boot('lyn desktop', rootConfig, structuredClone(desktopPatches(
    absoluteProject,
    options.allowLinkedPackages === true,
  )), (hostCtx) => {
    current = hostCtx
    hostCtx.provide(LYNESS_LAUNCH_ENVIRONMENT_KEY, environment)
  ...
  if (connection === undefined || clientModules === undefined || gateway === undefined) {
    ...
    throw new Error('lyn desktop: composition did not provide connection, typertGateway, and clientModules')
  }
  ...
  return {
    lynVersion: lynVersion(absoluteProject),
    cancel(streamId) {
      requests.get(streamId)?.abort()
    },
    async fetch(command, body) {
      if (disposing !== undefined) throw new Error('lyn desktop: host is disposing')
...
async function main(): Promise<void> {
  ...
  if (projectDir === undefined || process.send === undefined) {
    throw new Error('lyn desktop: expected project directory, byte pipes, and a Node IPC channel')
  }
  const option = process.argv[3]
  if (option !== undefined && option !== '--allow-linked-profile') {
    throw new Error(`lyn desktop: unsupported internal option ${JSON.stringify(option)}`)
  }
  ...
  const writeResponse = (frame: Buffer): Promise<void> => {
    const write = responseWriteTail.then(async () => {
      if (responsePipe.destroyed) throw new Error('lyn desktop: Electron response pipe is unavailable')
  ...
  send({
    ...
    lynVersion: controller.lynVersion,
  })
  ...
  const stop = (exitCode = 0): Promise<void> => {
    ...
    stopping ??= (async () => {
      ...
      const stopped = new Error('lyn desktop: Host is stopping')
  ...
  const beginRequest = (frame: Extract<DesktopHostRequestFrame, { type: 'start' }>): void => {
    if (frame.streamId <= lastStreamId) {
      throw new Error(`lyn desktop: Electron reused or reordered request stream ${String(frame.streamId)}`)
    }
    ...
    void run.catch(failTransport).finally(() => {
      ...
      openBody.error(new Error('lyn desktop: response completed before the request body ended'))
      requestBodies.delete(frame.streamId)
      blockedRequests.delete(frame.streamId)
    ...
    switch (frame.type) {
      ...
      case 'data': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`lyn desktop: Electron sent body data for inactive stream ${String(frame.streamId)}`)
        }
      ...
      case 'end': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`lyn desktop: Electron ended inactive body stream ${String(frame.streamId)}`)
        }
        body.close()
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
      ...
      case 'cancel': {
        if (frame.streamId > lastStreamId) {
          throw new Error(`lyn desktop: Electron canceled unknown stream ${String(frame.streamId)}`)
        }
        const body = requestBodies.get(frame.streamId)
        body?.error(new Error('lyn desktop: Electron canceled the request'))
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
  ...
  requestPipe.once('end', () => {
    ...
    try {
      decoder.finish()
      failTransport(new Error('lyn desktop: Electron request pipe ended'))
    } catch (error) {
      failTransport(error)
    }
  })
  ...
  process.on('message', (message: unknown) => {
    if (!isDesktopHostCommand(message)) {
      send({ type: 'fatal', message: 'lyn desktop: invalid Electron IPC command' })
      void stop(1)
      return
    }
    void stop()
...
if (import.meta.main) {
  main().catch((error: unknown) => {
    ...
    else process.stderr.write(`lyn desktop: ${message}\n`)
    process.exitCode = 1
  })
}
