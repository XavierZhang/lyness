...
import { loadLayeredEnv, loadProfileDirectory, reportSkippedBundles } from '@lyness/lyn-app-boot'
import { runProfile } from '@lyness/lyn/profile-boot'
import type {} from '@lyness/lyn-client-connection'
import type {} from '@lyness/lyn-host-webserver'
import type {} from '@lyness/lyn-deepseek-account'
...
async function main(): Promise<void> {
  ...
  const installAnchor = join(runtimeDir, 'node_modules', '@lyness', 'lyn', 'package.json')
  const profile = loadProfileDirectory('lyn', projectDir, installAnchor)
  reportSkippedBundles('lyn', profile)
    ...
    ...(process.argv[5] === undefined ? {} : {
      packageManager: {
        ...
        env: {
          ...
          LYNESS_DESKTOP_NODE_EXECUTABLE: process.execPath,
  ...
  await ctx.plugin(desktopOffice, {
    ...
    root: join(resolveLynHome(), 'lyn-runtimes', 'lyn-primary-runtime'),
  })
