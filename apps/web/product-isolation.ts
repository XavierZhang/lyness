...
const DEPENDENCY_ANALYSIS_PLUGIN = 'lyn-browser-dependency-analysis'
...
export function productWebBundleIsolation(repository: string, webRoot: string): Plugin[] {
  ...
  return [{
    name: 'lyn-product-web-chunk-inputs',
    apply: 'build',
    generateBundle: {
      order: 'pre',
    ...
    name: 'lyn-product-web-bundle-isolation',
    apply: 'build',
    enforce: 'post',
    config(config) {
