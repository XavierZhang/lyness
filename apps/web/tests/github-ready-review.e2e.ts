...
describe.skipIf(MODE === 'record')('web e2e: GitHub ready-for-review', () => {
  ...
  beforeAll(async () => {
    previousPort = process.env.LYNESS_GITHUB_WEBHOOK_PORT
    previousSecret = process.env.LYNESS_GITHUB_WEBHOOK_SECRET
    const port = await freePort()
    process.env.LYNESS_GITHUB_WEBHOOK_PORT = String(port)
    process.env.LYNESS_GITHUB_WEBHOOK_SECRET = SECRET
  ...
  afterAll(async () => {
    ...
    if (previousPort === undefined) Reflect.deleteProperty(process.env, 'LYNESS_GITHUB_WEBHOOK_PORT')
    else process.env.LYNESS_GITHUB_WEBHOOK_PORT = previousPort
    if (previousSecret === undefined) Reflect.deleteProperty(process.env, 'LYNESS_GITHUB_WEBHOOK_SECRET')
    else process.env.LYNESS_GITHUB_WEBHOOK_SECRET = previousSecret
  })
