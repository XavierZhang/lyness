...
的运行机制——模式、fixture、golden，以及与 `lyn web` 之间刻意保留的组合差异——记录在
...
Client face，而该 face 必须等 Host tsdown 生成 `@lyness/goal/remote` 之后才能编译，
...
`@lyness/client-web` import `AppWebEntry`、从
`@lyness/client-modules/client` import boot manifest 类型：启动真实 shell 正是该
