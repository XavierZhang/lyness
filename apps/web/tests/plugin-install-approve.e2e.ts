...
it('offers approval for blocked install scripts and installs once they are allowed', async () => {
  const scratch = await mkdtemp(join(tmpdir(), 'lyn-install-approve-'))
  ...
  try {
    ...
    try {
      ...
      await writeFile(join(profile, 'view'), 'console.log(JSON.stringify({ name: "native-package", version: "1.0.0", lyn: { bundle: { patch: "./cordis.patch.yml" } } }))\n')
      await writeFile(join(profile, 'add'), `
        ...
        fs.writeFileSync('node_modules/native-package/package.json', JSON.stringify({ name: 'native-package', version: '1.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }));
