/** Built lyn web + the `pnpm run dev:web --no-serve` watchers → browser HMR, with no page reload. */
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  try {
    ...
    // Watchers only: the built `lyn web` below is the server under test, and the
