...
it.each(['https://registry.npmjs.org/', MIRROR])('selects the fastest responding registry %s, remembers it, and falls back after a registry failure', async (winner) => {
  const scratch = await mkdtemp(join(tmpdir(), 'lyn-install-registry-'))
    ...
    try {
      ...
      await writeFile(join(profile, 'view'), `
        ...
        if (!process.argv.includes('--registry=${MIRROR}')) {
          ...
          console.log(JSON.stringify({ name: 'mirrored-package', version: '2.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }));
        }
      `)
      await writeFile(join(profile, 'add'), `
        ...
        fs.writeFileSync('node_modules/mirrored-package/package.json', JSON.stringify({ name: 'mirrored-package', version: '2.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }));
      ...
      // LYN peers at the registry that attempt would use: the mirror, then pnpm's own.
