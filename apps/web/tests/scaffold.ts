...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // `lyn-agent-presets` and prepended by it. Pin only the machine-local
    // root away: a developer's own `~/.lyn/.agent-presets` must not be able
    // to change a golden.
    {
      id: 'agent-presets',
      config: {
    ...
    { id: 'settings', config: { lynHome: harnessHome } },
  ...
  try {
    ...
    // `@lyness/cordis-plugin-group` by name.
