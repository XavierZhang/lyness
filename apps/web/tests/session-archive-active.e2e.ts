...
import { ToolCallId, type StreamChunk } from '@lyness/lyn-llm'
import type { ReplayEntry } from '@lyness/lyn-llm-replay'
...
describe.skipIf(MODE === 'record')('web e2e: archiving a running Session stops it first', () => {
  ...
  beforeAll(async () => {
    replayDir = await mkdtemp(join(tmpdir(), 'lyn-session-archive-active-'))
