...
import type {} from '@lyness/lyn-api-terminal-controller'
import {
...
const PROMPTS = [
  ...
  'Does the LYN file sandbox currently restrict file operations? Answer directly in one sentence. Do not call tools.',
...
describe('web e2e: current sandbox policy reaches the model before tools', () => {
  ...
  beforeAll(async () => {
    ...
    outsideWorkspace = await mkdtemp(join(tmpdir(), 'lyn-user-terminal-'))
  ...
  it.skipIf(MODE === 'record')('records cache-safe current policy before the corresponding model behavior', async () => {
    ...
    expect(systems[0]).not.toContain('Current LYN file policy:')
    ...
    expect(contexts[0]).toContain('Current LYN file policy: read-only. Any available operation enforced by the LYN file sandbox cannot modify files in the standing mode.')
    ...
    expect(contexts[1]).toContain('Current LYN file policy: danger-full-access. The LYN file sandbox does not restrict file modifications by available operations.')
    expect(contexts[1]).toContain('Approval prompts are disabled in this session')

    if (sessionWorkspace === undefined) throw new Error('permission-policy scenario observed no session workspace')
    expect(contexts[2]).toContain(`Current LYN file policy: workspace-write. Any available operation enforced by the LYN file sandbox may modify files under the session workspace: ${JSON.stringify(canonicalPath(sessionWorkspace))}. Some platform temporary areas may also be writable.`)
    ...
    expect(contexts[3]).toContain('Current LYN file policy: read-only.')
