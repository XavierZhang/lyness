/** Persisted Session recovery across two real lyn server processes on the same port. */
  ...
  async start(port: number, holdStartup = false): Promise<string> {
    ...
    const child = spawn(process.execPath, [
      ...
      env: {
        ...
        LYNESS_HOME: join(this.world, 'home'), LYNESS_AGENTS_HOME: join(this.world, 'agents'),
        LYNESS_TELEMETRY_DISABLED: '1', DEEPSEEK_API_KEY: 'keyless-server-restart-fixture',
        DEEPSEEK_BASE_URL: this.modelUrl,
        LYNESS_WEB_RESTART_HOLD_STARTUP: holdStartup ? '1' : '0',
      },
    ...
    const receive = (data: Buffer): void => {
      ...
      const url = /lyn web: (http:\/\/[^\s]+)/u.exec(this.output)?.[1]
...
it.each([false, true])('keeps the same revision, Session and page across a server restart (delayed startup: %s)', async (holdStartup) => {
  const world = await mkdtemp(join(tmpdir(), 'lyn-server-restart-'))
