...
describe('web e2e: plan review takeover round trip', () => {
  ...
  it.skipIf(MODE === 'record')('previews an unlogged review automatically and reopens it without deciding', async () => {
    ...
    try {
      const card = page.locator('[data-plan-review-key]')
      const preview = page.locator('[data-plan-preview^="lyn-resource://plan-review/"]')
      await preview.waitFor({ state: 'visible' })
      expect(await preview.getByText('Ask before implementation.').isVisible()).toBe(true)
      await compareOrRefreshGolden(TEMPORARY_EXPECTED, await captureStableAria(page, '[data-plan-preview^="lyn-resource://plan-review/"]', scaffold.workspaceCwd), MODE)
      ...
      const saved = await page.evaluate(() => Object.keys(localStorage)
        .filter(key => key.startsWith('lyn.sidebar-right.v1.')).map(key => localStorage.getItem(key)).join('\n'))
      expect(saved).toContain('lyn-resource://plan-review/')
