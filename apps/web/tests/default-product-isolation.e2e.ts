...
import type { Context, Plugin, RegistryService } from '@lyness/cordis'
import type { ClientModuleLoader, ClientModuleLoaderTarget } from '@lyness/lyn-client-modules/client'
...
async function readClientRoster() {
  const observation = Reflect.get(globalThis, '__lynIsolationObservation') as ClientObservation
...
function experimentalClientReferences(roster: Awaited<ReturnType<typeof readClientRoster>>): string[] {
  return [
    ...
    ...roster.modules,
  ].filter(name => name.startsWith('@lyness/lyn-experimental-'))
}
  ...
  await withDefaultWeb(test, async ({ url, request }) => {
    ...
    try {
      ...
      await page.addInitScript(() => {
        ...
        Reflect.set(globalThis, '__lynIsolationObservation', observation)
        Object.defineProperty(globalThis, '__ModuleLoader__', {
          ...
          set(target: ClientModuleLoaderTarget) {
            ...
            target.create = function (options) {
              const cordis = options.staticModules['@lyness/cordis'] as { RegistryService: typeof RegistryService }
      ...
      expect(roster.plugins.some(plugin => plugin.modules.includes('@lyness/lyn-client-ui-layout'))).toBe(true)
