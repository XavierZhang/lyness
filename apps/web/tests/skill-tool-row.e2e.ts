...
describe.skipIf(MODE === 'record')('web e2e: dedicated Skill tool row', () => {
  ...
  it('expands the loaded skill to its exact recorded instructions', async () => {
    ...
    expect(await output.textContent()).toContain('Agent presets are ordinary `@lyness/lyn-agent-preset` declarations carried by bundle patches.')
  ...
  it('keeps the collapsed Skill line on the global ToolCall font-size axis', async () => {
    ...
    const previous = await page.evaluate(() => ({
      fontSize: {
        value: document.body.style.getPropertyValue('--lyn-content-font-size-secondary'),
        priority: document.body.style.getPropertyPriority('--lyn-content-font-size-secondary'),
      },
      delta: {
        value: document.body.style.getPropertyValue('--lyn-content-font-delta'),
        priority: document.body.style.getPropertyPriority('--lyn-content-font-delta'),
      },
    }))
    await page.evaluate(() => {
      document.body.style.setProperty('--lyn-content-font-size-secondary', '15px')
      document.body.style.setProperty('--lyn-content-font-delta', '2px')
    })
    try {
      ...
      await page.evaluate((saved) => {
        ...
        restore('--lyn-content-font-size-secondary', saved.fontSize)
        restore('--lyn-content-font-delta', saved.delta)
      }, previous)
    }
  })
