...
it('guides a newly enabled voice plugin to installation and lets the user postpone it', async () => {
  ...
  const card = page.locator('[data-plugin-package="@lyness/lyn-experimental-voice-input-bundle"]')
  ...
  expect(await dialog.count()).toBe(0)
  expect(await page.getByText('Local models will be downloaded to the machine running LYN. No Python or compiler is required.', { exact: true }).count()).toBe(1)
  expect(await page.getByText('Audio is recognized on the machine running LYN. If models need downloading, that machine must be able to reach the selected source and its file services. Configure a proxy on that machine if needed.', { exact: true }).count()).toBe(1)
