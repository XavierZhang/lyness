...
import { OPTIONAL_BUNDLES } from '@lyness/lyn-app-boot'
...
const SCAFFOLD_BUNDLES = ['@lyness/lyn-base', '@lyness/lyn-web-app', SCAFFOLD_DEFAULTS_BUNDLE]
...
describe('web e2e: plugin manager', () => {
  ...
  it('starts with an unavailable selected bundle and lets the user clear its error', async () => {
    ...
    }).lyn.profile.bundles, { timeout: 10_000 }).toEqual(SCAFFOLD_BUNDLES)
  ...
  it('lists the installed bundles with their switches and leaves the installation\'s own to Settings', async () => {
    ...
    expect(await panel.getByText('实验性', { exact: true }).count())
      .toBe(OPTIONAL_BUNDLES.filter(name => name.startsWith('@lyness/lyn-experimental-')).length)
    expect(await panel.locator('[data-plugin-package="@lyness/lyn-experimental-inspector"]').count()).toBe(0)
  ...
  it('decodes manifest icons for disabled bundles and independent plugin rows', async () => {
    ...
    const team = panel.locator('[data-plugin-package="@lyness/lyn-experimental-agent-team-profile"]')
    expect(await team.getByRole('switch').getAttribute('aria-checked')).toBe('false')
    await checkImage('[data-plugin-package="@lyness/lyn-experimental-agent-team-profile"]', teamIcon, 'Disabled Agent Teams card')
    try {
  ...
  it('updates built-in names and descriptions when the UI language changes', async () => {
    ...
    expect(await packageName.textContent()).toBe('@lyness/lyn-experimental-agent-team-profile')
  ...
  it('enables the Team tools and browser plugin with one bundle switch', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(manifest.lyn.profile.bundles).toEqual([...SCAFFOLD_BUNDLES, '@lyness/lyn-experimental-agent-team-profile'])
        await panel.getByRole('button', { name: '查看 智能体团队', exact: true }).click()
  ...
  it('saves a bundle selection that waits for the next start and keeps its rows read-only', async () => {
    ...
    try {
      ...
      const bundles = async () => {
        ...
        return (JSON.parse(text) as { lyn: { profile: { bundles: string[] } } }).lyn.profile.bundles
      }
      expect(mounted()?.fiber?.state).toBeUndefined()
      await toggle.click()
