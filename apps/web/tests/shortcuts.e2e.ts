...
describe('web e2e: shortcut reference', () => {
  ...
  it.each([
    ...
    try {
      ...
      expect(await page.getByRole('button', { name: view, exact: true }).getAttribute('data-lyn-automatic-focus')).toBe('')
      ...
      expect(await trigger.getAttribute('data-lyn-automatic-focus')).toBe('')
      ...
      expect(await viewButton.getAttribute('data-lyn-automatic-focus')).toBe('')
      ...
      await cancel.click()
      expect(await resetAll.evaluate(element => element === document.activeElement)).toBe(true)
      expect(await resetAll.getAttribute('data-lyn-automatic-focus')).toBe('')
  ...
  it.each([
    ...
    try {
      ...
      expect(await page.getByRole('button', { name: 'Collapse sidebar', exact: true }).getAttribute('aria-keyshortcuts'))
        .toBe(defaultBinding)
      const stored = await page.evaluate((): unknown => JSON.parse(localStorage.getItem('lyn.keybindings.v1')!))
