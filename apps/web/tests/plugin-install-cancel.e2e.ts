...
it('cancels installation through the UI, restores files, and offers the spec again', async () => {
  const scratch = await mkdtemp(join(tmpdir(), 'lyn-install-cancel-'))
  ...
  try {
    ...
    try {
      ...
      await writeFile(join(profile, 'view'), 'console.log(JSON.stringify({ name: process.argv[2], version: "1.0.0", lyn: { bundle: { patch: "./cordis.patch.yml" } } }))\n')
      ...
      await writeFile(join(profile, 'add'), `
        ...
        fs.writeFileSync('node_modules/' + name + '/package.json', JSON.stringify({ name, version: '1.0.0', lyn: { bundle: { patch: './cordis.patch.yml' } } }));
