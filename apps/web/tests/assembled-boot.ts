...
import type { RemoteMock } from '@lyness/lyn-remote-mock'
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares lyn.client without a ./client export`)
  }
...
/** Derive the assembled browser graph from the same bundle patches and package declarations as `lyn web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  const entries = appBoot.composeEntries(BUNDLE_LAYERS.map((layer) => {
    const declared = (JSON.parse(readFileSync(layer.manifest, 'utf8')) as { lyn: { bundle: { patch: string | string[] } } }).lyn.bundle
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.lyn?.client
...
interface FixtureWindow extends Window {
  __LYNESS_BOOT__?: WebBootGraph
  __ModuleLoader__?: ClientModuleLoaderTarget
  __LYNESS_TRANSPORT__?: { readonly rpc: RemoteMock['rpc'] }
}
...
export function installAssembledBootEnv(): void {
  ...
  afterEach(async () => {
    ...
    cleanup()
    delete win.__LYNESS_BOOT__
    delete win.__ModuleLoader__
    delete win.__LYNESS_TRANSPORT__
...
export function mountAssembledApp(options: AssembledBootOptions = {}): AssembledRemote {
  ...
  win.__LYNESS_TRANSPORT__ = { rpc: remote.mock.rpc }
  history.replaceState(null, '', '/')
  ...
  win.__LYNESS_BOOT__ = graph
  const [facadeRow] = bootInjections(win.__LYNESS_BOOT__)
 ...
 * the snapshot gate's `LYNESS_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.LYNESS_SNAPSHOT === 'record' || process.env.LYNESS_SNAPSHOT === 'refresh'
