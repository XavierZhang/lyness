...
describe('web e2e: workspace recency', () => {
  ...
  beforeAll(async () => {
    ...
    await page.addInitScript(({ account, ids }) => {
      if (localStorage.getItem('lyn.workspace.view.v5') !== null) return
      localStorage.setItem('lyn.sessions.current', JSON.stringify({ sessionId: ids[0] }))
      localStorage.setItem('lyn.workspace.view.v5', JSON.stringify({
  ...
  it('pauses current recency in Manual, preserves drags on reload, and resets on Last updated', async () => {
    ...
    await expect.poll(() => page.evaluate(() => {
      const { sessionId } = JSON.parse(localStorage.getItem('lyn.sessions.current')!) as { sessionId: string }
      const { sessionOrderByAccount } = JSON.parse(localStorage.getItem('lyn.workspace.view.v5')!) as {
