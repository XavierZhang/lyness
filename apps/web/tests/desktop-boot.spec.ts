...
it('forwards client initialization failures to native recovery', async () => {
  ...
  vi.stubGlobal('lynDesktopBoot', { ready: async () => ({ injections: [], streamBaseUrl: 'http://127.0.0.1:3080' }), failed })
  vi.stubGlobal('__LYNESS_TRANSPORT__', undefined)
...
it('rejects the shared boot wait when the Desktop Host cannot start', async () => {
  ...
  vi.stubGlobal('__LYNESS_BOOT_READY__', gate)
  vi.stubGlobal('lynDesktopBoot', { ready: async () => { throw failure }, failed: vi.fn() })
  await import('../src/main.ts')
  await rejected
...
it('leaves browser failure presentation with the Web boot kernel', async () => {
  document.body.innerHTML = '<div id="root"></div>'
  vi.stubGlobal('lynDesktopBoot', undefined)
  await import('../src/main.ts')
...
it.each(['root', 'readiness'])('reports missing %s before client plugin startup', async (missing) => {
  ...
  const failed = vi.fn(async () => {})
  vi.stubGlobal('lynDesktopBoot', { ready: vi.fn(), failed })
  vi.stubGlobal('__LYNESS_BOOT_READY__', undefined)
  await import('../src/main.ts')
