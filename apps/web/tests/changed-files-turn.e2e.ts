...
import type {} from '@lyness/lyn-workspace-changes'
import type { ChangesSummary } from '@lyness/lyn-client-ui-deliverables/src/changes.ts'
...
describe('web e2e: a git workspace turn ends with its changed files', () => {
  ...
  beforeAll(async () => {
    ...
    if (MODE !== 'record') {
      replayRoot = await mkdtemp(join(tmpdir(), 'lyn-changed-files-turn-replay-'))
