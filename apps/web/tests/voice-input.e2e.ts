...
import type { SpeechInput, SpeechPreparationState, SpeechProviderId } from '@lyness/lyn-experimental-speech-to-text/types'
import type {} from '@lyness/lyn-experimental-speech-to-text'
import {
...
it.skipIf(webSnapshotMode() === 'record')('guides voice setup, records from standby and submits only the reviewed transcript through Session replay', async () => {
  const scratch = await mkdtemp(join(tmpdir(), 'lyn-voice-browser-'))
  ...
  await page.locator('[data-plugin-detail="@lyness/lyn-experimental-voice-input-bundle"]')
  ...
  await page.locator('[data-plugin-package="@lyness/lyn-experimental-voice-input-bundle"]').waitFor()
