...
// in the temp workspace) all run; the model adapter is lyn-llm-replay (keyless)
...
// Record: LYNESS_SNAPSHOT=record writes session.v3.jsonl, then a keyless
// LYNESS_SNAPSHOT=refresh regenerates ui.expected.md.
...
import type { SessionListValue } from '@lyness/lyn-api-session-controller/types'
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it('drives the recorded prompt to a settled turn (all modes)', async () => {
    ...
    const beforeReload = await page.evaluate(() => localStorage.getItem('lyn.sessions.current'))
    ...
    await expect.poll(() => page.evaluate(() => localStorage.getItem('lyn.sessions.current'))).toBe(beforeReload)
    if (MODE !== 'record') {
    ...
    const sessionId = await page.evaluate(() => {
      const selected = JSON.parse(localStorage.getItem('lyn.sessions.current')!) as { sessionId: string }
