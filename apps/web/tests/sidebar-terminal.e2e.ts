...
import type { SubprocessTerminalHandle } from '@lyness/lyn-subprocess'
import { createProcessInspector, type ProcessIdentity } from '@lyness/lyn-subprocess-local/src/process-inspector.ts'
...
describe.skipIf(process.platform === 'win32')('Web sidebar terminal', () => {
  ...
  it('preserves program palettes and keeps ANSI text and cursors readable across LYN themes', async () => {
  ...
  it('follows light, dark and system themes while preserving the running shell and its output', async () => {
    ...
    const terminal = page.locator('[data-sidebar-terminal]')
    const screen = page.locator('.xterm-rows:visible')
    await command(page, "LYNESS_THEME_PROBE=retained; PS1=''; printf '\\033cTHEME_CONTENT_RETAINED\\n'")
  ...
  it('completes commands, preserves the process through collapse and reload, resizes, and kills on tab close', async () => {
    ...
    await command(page, "printf 'LYNESS_PID:%s\\n' \"$$\"")
    await expect.poll(async () => await screen.innerText()).toMatch(/LYNESS_PID:\d+/u)
    const pid = Number((await screen.innerText()).match(/LYNESS_PID:(\d+)/u)?.[1])
    const firstProcess = processIdentity(0)
    expect(alive(firstProcess)).toBe(true)
    await command(page, 'lyn_terminal_completion_probe(){ printf "completed_from_shell\\n"; }')
    await page.keyboard.press('Control+l')
    await page.keyboard.insertText('lyn_terminal_completion_pro')
  ...
  it('chooses a shell from the guide menu, opens it directly, and remembers it after reload', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('lyn.terminal.shell'))).toBe('/bin/sh')
  ...
  it('keeps new terminals independent when two same-origin windows mint the same tab id', async () => {
    ...
    const bindings = () => page.evaluate(() => Object.keys(localStorage)
      .filter(key => key.startsWith('lyn.terminal.binding.v1.')).map(key => localStorage.getItem(key)))
