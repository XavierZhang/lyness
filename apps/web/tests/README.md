...
the deliberate composition divergences from `lyn web` — are documented in
...
`@lyness/goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@lyness/client-web` and the boot-manifest
type from `@lyness/client-modules/client`: booting the real shell is what
