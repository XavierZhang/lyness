...
vi.mock('../scripts/desktop-package-environment.mjs', () => ({
  loadDesktopPackageEnvironment: () => ({ LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'credential-sentinel' }),
...
it.each(['win32', 'darwin'] as const)('records and prints redacted parent failures on %s', async (platform) => {
  ...
  const savedDirectory = process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR
  try {
    ...
    expect(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR).toBe(savedDirectory)
  } finally {
    Object.defineProperty(process, 'platform', savedPlatform)
    Object.defineProperty(process, 'arch', savedArch)
    process.argv = savedArgv
    process.exitCode = savedExit
    if (savedDirectory === undefined) delete process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR
    else process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR = savedDirectory
    stderr.mockRestore()
