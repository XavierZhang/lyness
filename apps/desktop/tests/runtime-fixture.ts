 ...
 * @param version - Shell and lyn version.
...
export function runtimeFixture(root: string, version = '1.0.0', nodeVersion = '24.17.0'): DesktopRuntimeDescriptor {
  const names = ['@lyness/lyn', '@lyness/lyn-desktop-host', '@lyness/lyn-base', '@lyness/lyn-web-app', '@lyness/cordis']
  for (const name of names) {
    const bundle = name === '@lyness/lyn-base' || name === '@lyness/lyn-web-app'
    const path = writePackage(join(root, 'node_modules'), name, {
      version,
      ...(name === '@lyness/lyn' ? { dependencies: Object.fromEntries(names.slice(1).map(dependency => [dependency, version])) } : {}),
      ...(bundle ? { lyn: { bundle: { patch: './bundle.yml' } } } : {}),
    })
    if (bundle) writeFileSync(join(path, 'bundle.yml'), '[]\n')
  }
  for (const file of DESKTOP_HOST_RUNTIME_FILES) {
    const path = join(root, 'node_modules', '@lyness/lyn-desktop-host', file)
