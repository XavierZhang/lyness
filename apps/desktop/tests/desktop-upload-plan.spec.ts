...
async function fixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'lyn-desktop-upload-'))
  ...
  const base = `lyness-${version}-${os}-${arch}`
  ...
  await writeFile(join(artifactsRoot, `${target}-release.json`), `${JSON.stringify({
    ...
    version,
    environment,
    publicUrl: `${origin}/lyn-desk/${environment === 'test' ? `${RELEASE_ID}/` : ''}feeds/${target}/`,
  })}\n`)
...
describe('desktop upload plan', () => {
  it('publishes fixed feeds referencing versioned binaries without overriding CDN cache policy', async () => {
    ...
    expect(plan.artifacts.map(artifact => artifact.key)).toEqual([
      'lyn-desk/bin/win-x64/lyness-1.2.3-win-x64.exe',
      'lyn-desk/bin/win-x64/lyness-1.2.3-win-x64.exe.blockmap',
      'lyn-desk/feeds/win-x64/nightly.yml',
      'lyn-desk/feeds/win-x64/latest.yml',
    ])
    expect(load(plan.artifacts[2]!.contents!)).toMatchObject({
      version: '1.2.3',
      files: [{
        url: 'https://download.deepseek.com/lyn-desk/bin/win-x64/lyness-1.2.3-win-x64.exe',
  ...
  it('validates macOS artifacts and puts channel metadata last', async () => {
    ...
    expect(plan).toMatchObject({
      environment: 'test',
      version: '1.2.3',
      publicUrl: `https://desktop-updates.example.com/lyn-desk/${RELEASE_ID}/feeds/mac-arm64/`,
      bucket: TEST_BUCKET,
    })
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'lyness-1.2.3-mac-arm64.dmg',
      'lyness-1.2.3-mac-arm64.zip',
      'lyness-1.2.3-mac-arm64.zip.blockmap',
      'nightly-mac.yml',
      'latest-mac.yml',
    ])
  ...
  it.each(['mac-arm64', 'mac-x64', 'win-x64'] as const)('publishes every %s object and YAML reference inside the test release directory', async (target) => {
    ...
    const prefix = `lyn-desk/${RELEASE_ID}`
  ...
  it('uploads the prerelease channel metadata emitted by electron-builder', async () => {
    ...
    const plan = await createDesktopUploadPlan('mac-arm64', paths)
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'lyness-1.2.3-alpha.4-mac-arm64.dmg',
      'lyness-1.2.3-alpha.4-mac-arm64.zip',
      'lyness-1.2.3-alpha.4-mac-arm64.zip.blockmap',
      'nightly-mac.yml',
    ])
  })
  ...
  it('validates the Windows installer with the emitted external blockmap and production destination', async () => {
    ...
    const plan = await createDesktopUploadPlan('win-x64', paths)
    expect(plan.artifacts.map(artifact => artifact.filename)).toEqual([
      'lyness-2.0.0-win-x64.exe',
      'lyness-2.0.0-win-x64.exe.blockmap',
      'nightly.yml',
      'latest.yml',
    ])
    expect(plan).toMatchObject({
      publicUrl: 'https://download.deepseek.com/lyn-desk/feeds/win-x64/',
  ...
  it.each(['missing', 'empty'])('rejects a %s Windows blockmap before publishing its feed', async (condition) => {
    ...
    const path = join(paths.artifactsRoot, 'lyness-1.2.3-win-x64.exe.blockmap')
  ...
  it('rejects a completed build from another lyn version or deployment', async () => {
  ...
  it('rejects stale architecture metadata and modified updater bytes', async () => {
    ...
    const zipPath = join(paths.artifactsRoot, 'lyness-1.2.3-mac-arm64.zip')
