...
it('refuses a build that carries no client version instead of guessing one', () => {
  vi.stubEnv('LYNESS_CLIENT_VERSION', undefined)
  expect(() => desktopClientVersion()).toThrow(/LYNESS_CLIENT_VERSION/)
  vi.stubEnv('LYNESS_CLIENT_VERSION', '')
  expect(() => desktopClientMetadata('en')).toThrow(/LYNESS_CLIENT_VERSION/)
})
