...
import { readProfilePlugins } from '@lyness/lyn-app-boot'
import { runtimeFixture } from './runtime-fixture.ts'

const roots: string[] = []
function temporaryRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'lyn-desktop-test-'))
  roots.push(root)
  return root
}
function seedPlugin(manager: DesktopProjectManager): void {
  ...
  manifest.lyn.profile.bundles.push('plugin')
  writeFileSync(path, JSON.stringify(manifest))
  const directory = join(manager.paths.profile, 'node_modules/plugin')
  mkdirSync(directory, { recursive: true })
  writeFileSync(join(directory, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', lyn: { bundle: { patch: 'bundle.yml' } } }))
  writeFileSync(join(directory, 'bundle.yml'), '[]\n')
}
function plugins(manager: DesktopProjectManager) {
  return readProfilePlugins({ binName: 'lyn', profileDir: manager.paths.profile,
    installAnchor: join(manager.runtime.lyn, 'node_modules/@lyness/lyn/package.json') }).dependencies
    .map(({ name, version, enabled }) => ({ name, version, enabled }))
}
function setup(): { root: string; manager: DesktopProjectManager } {
  const root = temporaryRoot()
  const lyn = join(root, 'resources', 'lyn')
  runtimeFixture(lyn)
  return { root, manager: new DesktopProjectManager(resolveDesktopPaths(join(root, '.lyn')), { lyn }) }
}
afterEach(() => {
...
describe('desktop external plugin profile', () => {
  ...
  it('disables every third-party bundle without reading a broken plugin patch declaration', async () => {
    ...
    expect((JSON.parse(readFileSync(join(manager.paths.profile, 'package.json'), 'utf8')) as {
      lyn: { profile: { bundles: string[] } }
    }).lyn.profile.bundles).not.toContain('plugin')
  ...
  it('disables plugins before runtime initialization and backs up the patch while preserving package files', async () => {
    ...
    const uninitialized = new DesktopProjectManager(manager.paths, { ...manager.runtime, lyn: 'missing-runtime' })
    ...
    expect(manifest.lyn.profile.bundles).not.toContain('plugin')
    expect(manifest.lyn.profile.bundles).toContain('@lyness/lyn-web-app')
    await manager.applyRelease()
  ...
  it('reports damaged application metadata as a reinstall failure', async () => {
    const { manager } = setup()
    writeFileSync(join(manager.runtime.lyn, 'desktop-runtime.json'), '{broken')
  ...
  it.skipIf(process.platform !== 'win32')('reuses the profile when the launch path changes only Windows letter casing', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    const relaunched = new DesktopProjectManager(manager.paths, { ...manager.runtime, lyn: manager.runtime.lyn.toUpperCase() })
  ...
  it.each(['changed', 'same-size', 'extra', 'missing'])('starts and reuses a profile without checking %s runtime bytes', async (operation) => {
    const { manager } = setup()
    if (operation === 'changed') writeFileSync(join(manager.runtime.lyn, 'package.json'), '{}')
    if (operation === 'same-size') writeFileSync(join(manager.runtime.lyn, 'package.json'), '{"type":"Module"}\n')
    if (operation === 'extra') writeFileSync(join(manager.runtime.lyn, 'extra'), '')
    if (operation === 'missing') unlinkSync(join(manager.runtime.lyn, 'package.json'))
    await expect(manager.applyRelease()).resolves.toBeUndefined()
  ...
  it('keeps plugin files and patches through a compatible release and application relocation', async () => {
    ...
    const nextRoot = join(root, 'relocated', 'lyn')
    runtimeFixture(nextRoot, '1.1.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, lyn: nextRoot })
  ...
  it('preserves plugin files without running pnpm when bundled Node changes', async () => {
    ...
    await manager.applyRelease()
    seedPlugin(manager)
    const lyn = join(root, 'new-node')
    runtimeFixture(lyn, '1.1.0', '24.18.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, lyn })
  ...
  it('allows peer version mismatches to reach Host startup and remain available for recovery', async () => {
    ...
    await manager.applyRelease()
    seedPlugin(manager)
    const lyn = join(root, 'next-major')
    runtimeFixture(lyn, '2.0.0')
    const next = new DesktopProjectManager(manager.paths, { ...manager.runtime, lyn })
    await expect(next.applyRelease()).resolves.toBeUndefined()
...
describe('desktop link-backend projections', () => {
  it('removes .lyn-module-fallback projections when preparing a launch', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    const profile = manager.paths.profile
    ...
    const owned = join(profile, '.lyn-module-fallback', 'node_modules', 'bridge')
