...
it('defaults to four workers and keeps download and Apple proxies independent', () => {
  ...
  expect(resolveMacOSPackageSettings({ LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY: '2',
    LYNESS_DESKTOP_MACOS_DOWNLOAD_PROXY: 'https://download.example:8080',
  ...
  expect(resolveMacOSPackageSettings({ LYNESS_DESKTOP_MACOS_DOWNLOAD_PROXY: ' ', LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: '' })).toEqual({ packConcurrency: 4 })
})

it.each(['', '0', '-1', '1.5', '1e2', 'Infinity', '9007199254740992'])('rejects invalid concurrency %j', (value) => {
  expect(() => resolveMacOSPackageSettings({ LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY: value })).toThrow('PACK_CONCURRENCY')
})
...
('rejects unsupported proxy input without echoing it: %s', (value) => {
  expect(() => resolveMacOSPackageSettings({ LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: value })).toThrow('NOTARIZATION_PROXY')
  expect(() => resolveMacOSPackageSettings({ LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: value })).not.toThrow(value)
})
