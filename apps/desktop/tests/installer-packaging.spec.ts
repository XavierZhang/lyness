...
describe('installer preparation preserves application dependencies', () => {
  it.each(['win32', 'darwin'] as const)('rejects a missing production policy before signing on %s', async (platform) => {
    ...
    expect(() => createElectronBuilderConfig({ LYNESS_DESKTOP_APP_ID: 'com.example.installer',
      LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'production',
      LYNESS_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://test.example.com',
    }, platform, 'x64')).toThrow('LYNESS_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN')
  })
  it.each(['win32', 'darwin'] as const)('keeps electron-builder responsible for node_modules on %s', async (platform) => {
    execute.mockClear()
    const env = {
      LYNESS_DESKTOP_APP_ID: 'com.example.installer',
      ...
      LYNESS_DESKTOP_TARGET_PLATFORM: platform,
      LYNESS_DESKTOP_TARGET_ARCH: 'x64',
      LYNESS_DESKTOP_UNSIGNED: platform === 'win32' ? '1' : '0',
  ...
  it('names unsigned Windows artifacts so they cannot pass for release builds', async () => {
    ...
    expect(config.artifactName).toBe('lyness-${version}-${os}-${arch}-unsigned.${ext}')
  })
  ...
  it('packages every preload entry point the shell loads', async () => {
    ...
    const config = createElectronBuilderConfig({
      LYNESS_DESKTOP_APP_ID: 'com.example.installer',
      LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'production',
      LYNESS_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://harness-test.deepseek.com',
