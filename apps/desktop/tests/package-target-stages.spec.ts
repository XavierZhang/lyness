...
const environment = { LYNESS_DESKTOP_APP_ID: 'com.example.test', LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'test',
  DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com', DOWNLOAD_TEST_RELEASE_ID: '0123456789abcdef0123456789abcdef',
  LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin', LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: '2' }
...
it('requires one signing preflight before building, then records only the complete release', async () => {
  ...
  expect(run.run.mock.calls[0]![3]).toMatchObject({ env: { LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin' }, timeoutMs: 60_000 })
  expect(run.run.mock.calls[1]![3].env).not.toHaveProperty('LYNESS_DESKTOP_WINDOWS_TOKEN_PIN')
  expect(stages.indexOf('run sign:primary-runtime --lyn')).toBeGreaterThan(stages.indexOf('run prepare:lyn --defer-runtime-smoke'))
  ...
  for (const call of run.run.mock.calls) {
    if (call[0].startsWith('run prepare:') || call[0].includes('smoke-packaged-runtime')) {
      expect(call[3].env).not.toHaveProperty('LYNESS_DESKTOP_WINDOWS_TOKEN_PIN')
      expect(call[3].env).not.toHaveProperty('LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY')
    }
    if (call[0].startsWith('run sign:primary-runtime')) {
      expect(call[3].env).toHaveProperty('LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY', '2')
    }
  }
  expect(writeFileSync).toHaveBeenCalledOnce()
  const record = JSON.parse(vi.mocked(writeFileSync).mock.calls[0]![1] as string) as { publicUrl: string }
  expect(record.publicUrl).toBe('https://updates.example.com/lyn-desk/0123456789abcdef0123456789abcdef/feeds/win-x64/')
})
...
it.each(['preflight:windows-signing', 'run build:official', 'run sign:primary-runtime', 'run prepare:lyn --defer-runtime-smoke', 'run sign:primary-runtime --lyn',
...
it.each(['--unsigned', '--prepare-only'])('keeps %s hardware-free and creates no release record', async (mode) => {
  ...
  expect(stages).not.toContain('run sign:primary-runtime --lyn')
  expect(withWindowsSigningStage).not.toHaveBeenCalled()
  for (const call of run.run.mock.calls) expect(call[3].env).not.toHaveProperty('LYNESS_DESKTOP_WINDOWS_TOKEN_PIN')
  expect(writeFileSync).not.toHaveBeenCalled()
...
it.each([undefined, '2'])('passes macOS pack concurrency %s only to workspace packing and download routing only to download stages', async (concurrency) => {
  ...
  await packageTarget(parseDesktopPackageInvocation(['mac-arm64', '--prepare-only'], 'darwin', 'arm64'), {
    ...environment, LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY: concurrency,
    LYNESS_DESKTOP_MACOS_DOWNLOAD_PROXY: 'http://downloads.example:8080',
    LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: 'http://apple.example:8081',
  }, run)
  ...
  for (const call of calls) {
    expect(call[3].env.HTTP_PROXY).toBe(/^run prepare:(?:runtime|lyn)$/u.test(call[0]) ? 'http://downloads.example:8080' : undefined)
  }
  expect(withMacOSNotarizationProxy).not.toHaveBeenCalled()
})
...
it.each([false, true])('scopes the Apple proxy around notarization (directory=%s)', async (directory) => {
  ...
  await packageTarget(parseDesktopPackageInvocation(['mac-arm64', ...(directory ? ['--dir'] : [])], 'darwin', 'arm64'), {
    ...environment, APPLE_KEYCHAIN_PROFILE: 'fixture', LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: 'http://apple.example:8081',
  }, run)
