...
async function fixture<T>(body: (directory: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'lyn-update-bootstrap-'))
...
console.log(JSON.stringify({ paths: app.paths, home: process.env.LYNESS_HOME, journals: process.env.LYNESS_DESKTOP_UPDATE_JOURNAL_DIR }))
`)
      const results = []
      for (const version of versions) {
        await writeFile(join(bootstrap, 'package.json'), JSON.stringify({ type: 'module', version, lynDesktopAppId: run.appId }))
        const { stdout, stderr } = await promisify(execFile)(process.execPath, [join(bootstrap, 'qualification-bootstrap.mjs')], {
          windowsHide: true, env: { SystemRoot: process.env.SystemRoot, PATH: process.env.PATH,
            LYNESS_HOME: 'must-not-be-used', LYNESS_DESKTOP_UPDATE_JOURNAL_DIR: 'must-not-be-used' },
        })
        expect(stderr).toBe('')
        results.push(JSON.parse(stdout))
      }
      const root = join(directory, 'application-data/lyn-update-qualification', run.id)
      expect(results).toEqual([0, 1].map(() => ({ paths: { userData: join(root, 'user-data'), sessionData: join(root, 'user-data') },
        home: join(root, 'lyn-home'), journals: join(root, 'journals') })))
      expect((await readdir(root)).sort()).toEqual(['lyn-home', 'journals', 'user-data'])
  ...
  it('refuses foreign metadata before creating any data directory or changing the environment', async () => {
    await fixture(async (directory) => {
      const env = { LYNESS_HOME: 'existing' }
      ...
      expect(() => configureInstalledUpdateIdentity(app, { id: 'a'.repeat(24), versions },
        { version: versions[0], lynDesktopAppId: 'com.deepseek.lyn' }, env)).toThrow('identity')
      expect(await readdir(directory)).toEqual([])
      expect(env).toEqual({ LYNESS_HOME: 'existing' })
    })
  })
