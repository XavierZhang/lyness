...
vi.mock('electron', () => ({
  ...
  BrowserWindow: class {
    ...
    webContents = { mainFrame: { url: 'lyn-app://app/' }, setWindowOpenHandler: vi.fn(),
...
it.each([false, true])('starts welcome onboarding without carrying update focus into login or skip (Windows update=%s)', async (updated) => {
  ...
  vi.useFakeTimers()
  vi.stubEnv('LYNESS_DESKTOP_DEV_PROJECT_DIR', '/development-profile')
  vi.stubEnv('LYNESS_DESKTOP_NODE_BINARY', '/runtime/node')
  vi.stubEnv('LYNESS_DESKTOP_PNPM_ENTRY', '/runtime/pnpm')
  vi.stubEnv('LYNESS_DESKTOP_LYNESS_DIR', '/runtime/lyn')
  vi.stubEnv('LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR', '/runtime/primary-runtime')
  vi.stubEnv('LYNESS_DESKTOP_HOST_INSPECT_PORT', undefined)
  vi.stubEnv('LYNESS_DESKTOP_OPEN_DEVTOOLS', '0')
  ...
  const activate = () => {
    ...
    state.appListeners.get('open-url')!({ preventDefault: vi.fn() }, 'lyn://open')
  }
  ...
  expect(state.loadWorkspace).toHaveBeenCalledExactlyOnceWith('lyn-app://app/')
  expect(state.showWorkspace).not.toHaveBeenCalled()
  ...
  vi.stubEnv('LYNESS_DESKTOP_OPEN_DEVTOOLS', '1')
