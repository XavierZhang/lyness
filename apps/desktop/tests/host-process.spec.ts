...
process.send({ type: 'ready', protocolVersion: 3, lynVersion: process.env.NODE_OPTIONS ?? 'clean' })
...
`)
    ...
    try {
      await expect(host.start()).resolves.toMatchObject({ lynVersion: 'clean' })
      const response = await host.fetch(new Request('lyn-app://app/example', { method: 'POST', body: 'request' }))
  ...
  it('streams a large binary response in bounded raw frames', async () => {
    ...
    const project = projectWithHost(`
process.send({ type: 'ready', protocolVersion: 3, lynVersion: 'large-response' })
function onRequestFrame(frame) {
...
`)
    const host = new DesktopHostProcess(process.execPath, project)
    try {
      const response = await host.fetch(new Request('lyn-app://app/large'))
  ...
  it('stops an unfinished upload when the Host completes its response early', async () => {
    const project = projectWithHost(`
process.send({ type: 'ready', protocolVersion: 3, lynVersion: 'early-response' })
function onRequestFrame(frame) {
...
`)
    ...
    const host = new DesktopHostProcess(process.execPath, project)
    try {
      const request = new Request('lyn-app://app/early', {
        method: 'POST',
        body,
        duplex: 'half',
  ...
  it('ignores a response end that arrives after the renderer cancels its stream', async () => {
    const project = projectWithHost(`
process.send({ type: 'ready', protocolVersion: 3, lynVersion: 'cancel-race' })
const urls = new Map()
function onRequestFrame(frame) {
  if (frame.type === 1) {
    const request = JSON.parse(frame.payload)
...
`)
    const host = new DesktopHostProcess(process.execPath, project)
    try {
      const canceled = await host.fetch(new Request('lyn-app://app/cancel'))
      await canceled.body?.cancel()
      await new Promise(resolve => setTimeout(resolve, 25))
      const after = await host.fetch(new Request('lyn-app://app/after'))
...
process.send({ type: 'ready', protocolVersion: 3, lynVersion: 'invalid-frame' })
function onRequestFrame(frame) {
  if (frame.type === 1) responsePipe.write(Buffer.alloc(13))
}
`))
    await invalid.start()
    await expect(invalid.fetch(new Request('lyn-app://app/invalid'))).rejects.toThrow(/invalid Host response frame marker/u)
