...
async function fixture(body: (context: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'lyn-package-content-'))
  ...
  try {
    ...
    await writeFile(join(source, 'package.json'), JSON.stringify({ name: `lyn-update-test-${run.id}`, version,
      lynDesktopAppId: run.appId, main: 'qualification-bootstrap.mjs', type: 'module',
      lynMandatoryUpdatePolicy: { origin: 'https://policy.example.com', allowedPageOrigins: ['https://policy.example.com'],
    ...
    const lyn = join(run.root, version, 'lyn')
    const descriptor = runtimeFixture(lyn, version)
    await writeFile(join(lyn, 'tool.exe'), 'inert executable fixture')
    ...
    reseal(lyn)
    await cp(lyn, join(source, 'lyn'), { recursive: true })
    ...
    await writeFile(join(payload, 'resources/app-update.yml'), JSON.stringify({ provider: 'generic', channel: 'nightly',
      ...
      updaterCacheDirName: `lyn-update-test-${run.id}-updater` }))
    const seal = async () => {
...
describe('installed update archive contents', () => {
  it.each(versions)('reads real ASAR and runtime inventories for %s without claiming installation', async (version) => {
    await fixture(async ({ manifest, payload }) => {
      expect(await verifyInstalledUpdatePackageContent(manifest, version, payload, publisher)).toMatchObject({
        ...
        resignedExecutables: [join(payload, 'resources/app.asar.unpacked/lyn/tool.exe')],
      })
    }, version)
  })

  it.each(['name', 'version', 'lynDesktopAppId', 'main', 'type', 'lynMandatoryUpdatePolicy'])(
    'rejects mismatched packaged %s', async (field) => {
      await fixture(async ({ manifest, source, payload, version, seal }) => {
        ...
        await expect(verifyInstalledUpdatePackageContent(manifest, version, payload, publisher))
          .rejects.toThrow(field === 'lynMandatoryUpdatePolicy' ? 'desktop policy' : 'identity')
      })
    })
  ...
  it('accepts only the builder transformation of runtime dependency manifests', async () => {
    await fixture(async ({ manifest, source, payload, version, seal }) => {
      const preparedRoot = join(dirname(manifest), version, 'lyn')
      const packagePath = 'node_modules/@lyness/lyn/package.json'
      ...
      await cp(join(preparedRoot, 'desktop-runtime.json'), join(source, 'lyn/desktop-runtime.json'))
      const packagedPath = join(source, 'lyn', packagePath)
      const packaged = { ...data }
  ...
  it.each(['unsealed', 'resealed-change', 'resealed-addition'])(
    'rejects %s runtime content even when metadata is regenerated', async (failure) => {
      await fixture(async ({ manifest, source, payload, version, seal, resealRuntime }) => {
        await writeFile(join(source, 'lyn', failure === 'resealed-addition' ? 'extra.js' : 'package.json'), '{}')
  ...
  it('identifies changed executable resources for separate signature verification', async () => {
    await fixture(async ({ manifest, source, payload, version, resealRuntime }) => {
      const executable = join(payload, 'resources/app.asar.unpacked/lyn/tool.exe')
      await writeFile(join(source, 'lyn/tool.exe'), 'inert changed executable, not a signature')
      await resealRuntime()
  ...
  it.each(['success', 'signature', 'unsafe-path', 'extraction', 'content', 'changed-input'])(
    'retains %s evidence and stops later stages after failure', async (failure) => {
      await fixture(async ({ manifest, payload, version }) => {
        ...
        await mkdir(output)
        const name = `lyness-${version}-win-x64.exe`
