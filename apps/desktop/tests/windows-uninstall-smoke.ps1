...
try {
    foreach ($mode in $expected.cases) {
        ...
        if ($mode -eq 'home-inside') { $env:LYNESS_HOME = Join-Path $userDataPath 'home' } else { Remove-Item Env:LYNESS_HOME -ErrorAction SilentlyContinue }
        ...
        # A LYNESS_HOME inside the Electron directory protects that directory (and its scope) but not the other roots.
    ...
    Remove-Item Env:LYNESS_HOME -ErrorAction SilentlyContinue
