...
import { DESKTOP_IPC, type LynDesktopProductApi } from '../src/ipc.ts'
...
it('reads only native login API-key presence through the onboarding bridge', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/'))
  electron.ipcRenderer.invoke.mockResolvedValueOnce(true)
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === 'lynOnboarding')?.[1] as { hasApiKey(): Promise<boolean> }
...
it('exposes onboarding size activation to the application document', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === 'lynOnboarding')?.[1] as { setActive(active: boolean): void }
  api.setActive(true)
  api.setActive(false)
...
it('limits product documents to update status and a native confirmation action', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/index.html'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === 'lynDesktop')?.[1] as LynDesktopProductApi
...
it.each(['lyn-app://shell/plugin-manager.html', 'lyn-app://other/index.html', 'https://shell/startup.html', 'http://example.com/'])('exposes only the carrier marker to %s', async (url) => {
  vi.stubGlobal('location', new URL(url))
  await import('../src/preload-app.ts')
  expect(electron.contextBridge.exposeInMainWorld).toHaveBeenCalledWith('lynDesktop', { protocolVersion: 1 })
...
it('exposes asynchronous boot only to the local application document', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === 'lynDesktopBoot')?.[1] as { ready(): Promise<unknown>; failed(message: string): Promise<void> }
  await api.ready()
  ...
  await import('../src/preload-app.ts')
  expect(electron.contextBridge.exposeInMainWorld.mock.calls.some(([name]) => name === 'lynDesktopBoot')).toBe(false)
})

it('exposes a directory picker only to the local application document', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === '__LYNESS_DIRECTORY_PICKER__')?.[1] as { pick(): Promise<string | null> }
  ...
  for (const url of ['lyn-app://shell/startup.html', 'https://example.com/']) {
    vi.resetModules()
    electron.contextBridge.exposeInMainWorld.mockClear()
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(electron.contextBridge.exposeInMainWorld.mock.calls.some(([name]) => name === '__LYNESS_DIRECTORY_PICKER__')).toBe(false)
  }
})
...
it('reports host paths of picked files only to the local application document', async () => {
  vi.stubGlobal('location', new URL('lyn-app://app/'))
  await import('../src/preload-app.ts')
  const api = electron.contextBridge.exposeInMainWorld.mock.calls.find(([name]) => name === '__LYNESS_HOST_PATHS__')?.[1] as { pathFor(file: File): string }
  ...
  for (const url of ['lyn-app://shell/startup.html', 'https://example.com/']) {
    ...
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(electron.contextBridge.exposeInMainWorld.mock.calls.some(([name]) => name === '__LYNESS_HOST_PATHS__')).toBe(false)
  }
})

it.each(['lyn-app://app/', 'lyn-app://shell/plugin-manager.html', 'https://example.com/'])(
  'installs Windows appearance only for the application document (%s)', async (url) => {
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(syncWindowsAppearance).toHaveBeenCalledTimes(url === 'lyn-app://app/' ? 1 : 0)
  },
)
...
it.each(['win32', 'darwin'] as const)('installs the embedded mandatory UI only in the Windows app document (%s)', async (platform) => {
  ...
  for (const url of ['lyn-app://app/', 'lyn-app://shell/mandatory-update.html', 'https://example.com/']) {
    vi.resetModules()
    vi.mocked(installMandatoryUpdateOverlay).mockClear()
    vi.stubGlobal('location', new URL(url))
    await import('../src/preload-app.ts')
    expect(installMandatoryUpdateOverlay).toHaveBeenCalledTimes(platform === 'win32' && url === 'lyn-app://app/' ? 1 : 0)
  }
})
