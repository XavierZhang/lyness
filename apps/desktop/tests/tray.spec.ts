...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('lyness')
  expect(labels(native.menus[0]!)).toEqual(['Open lyness', 'separator', 'Quit lyness'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 lyness', 'separator', '退出 lyness'])
  f.tray.dispose()
  f.tray.dispose()
