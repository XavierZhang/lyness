...
async function fixture(arch: 'arm64' | 'x64' = 'arm64') {
  ...
  const appPath = join(artifactsRoot, arch === 'arm64' ? 'mac-arm64' : 'mac', 'lyness.app')
  ...
  await writeMacOSAppUpdateConfig(join(appPath, 'Contents', 'Resources'), {
    publicUrl: `https://desktop-updates.example.com/lyn-desk/0123456789abcdef0123456789abcdef/feeds/mac-${arch}/`,
  }, 'lyness-updater')
  const version = '1.2.3-alpha.1'
  const base = `lyness-${version}-mac-${arch}`
...
describe('parallel macOS artifacts', () => {
  it.each(['arm64', 'x64'] as const)('overlaps notarization on isolated %s copies and promotes only completed payloads', async (arch) => {
    ...
    try {
      ...
      expect(await readFile(join(f.appPath, 'Contents', 'Resources', 'app-update.yml'), 'utf8'))
        .toContain(`/lyn-desk/0123456789abcdef0123456789abcdef/feeds/mac-${arch}/`)
  ...
  it('passes the actual App and isolated output directory to each single-target builder', () => {
    ...
    for (const format of ['zip', 'dmg'] as const) {
      const appPath = join('private build', format, 'lyness.app')
