...
function setup() {
  dom = new JSDOM('<html><body><button>Product action</button><div data-windows-menu></div></body></html>', { url: 'lyn-app://app/' })
  ...
  expect(post).toHaveBeenCalledWith({ type: 'lyn-mandatory-connect' }, 'lyn-app://shell', [channel.port2])
...
it('uses an in-page frame below the caption and reuses it on updates', () => {
  ...
  expect(f.frame.src).toBe('lyn-app://shell/mandatory-update.html')
...
it('rejects forged window events and accepts actions only through the transferred private port', async () => {
  const f = setup()
  const data = { type: 'lyn-mandatory-action', id: 1, action: 'download', version: '2.0.0', revision: undefined }
  dom.window.dispatchEvent(new dom.window.MessageEvent('message', { source: f.frame.contentWindow, origin: 'lyn-app://shell', data }))
  expect(ipc.invoke).not.toHaveBeenCalled()
  const result = new Promise<unknown>(resolve => channel.port2.on('message', (message: unknown) => {
    if (typeof message === 'object' && message !== null && 'type' in message && message.type === 'lyn-mandatory-result') resolve(message)
  }))
  channel.port2.postMessage(data)
  await expect(result).resolves.toEqual({ type: 'lyn-mandatory-result', id: 1, ok: true })
