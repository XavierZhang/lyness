...
describe('ordinary update polling configuration', () => {
  it('resolves defaults and supports an explicit interval longer than the default backoff cap', () => {
    ...
    expect(resolveDesktopUpdateScheduleConfig({ LYNESS_DESKTOP_UPDATE_CHECK_INTERVAL_MS: '7200000' }))
  ...
  it.each([
    ...
    expect(() => resolveDesktopUpdateScheduleConfig({ [`LYNESS_DESKTOP_UPDATE_CHECK_${suffix}`]: value })).toThrow()
  })
})
