...
it('serves the Web entry and assets without starting or contacting a Host', async () => {
  ...
  const response = await serveWebDocument(new Request('lyn-app://app/'), root)
  const html = await response.text()
  expect(html.indexOf('Promise.withResolvers()')).toBeLessThan(html.indexOf('assets/entry.js'))
  expect(await (await serveWebDocument(new Request('lyn-app://app/assets/entry.js'), root)).text()).toContain('entryLoaded')
  expect(fetch).not.toHaveBeenCalled()
  expect((await serveWebDocument(new Request('lyn-app://app/%2e%2e%2fprivate'), root)).status).toBe(403)
  expect((await serveWebDocument(new Request('lyn-app://app/missing.js'), root)).status).toBe(404)
})
...
it('forwards upload bytes and cancellation with Host credentials while keeping the response streaming', async () => {
  ...
  vi.stubGlobal('fetch', fetch)
  const request = new Request('lyn-app://app/api/upload?name=file', {
    method: 'POST', body: 'upload bytes', headers: { origin: 'lyn-app://app', cookie: 'untrusted' },
  })
...
it('drops connection-level headers the Host wrote for its own transport', async () => {
  ...
  vi.stubGlobal('fetch', fetch)
  const response = await forwardWebRequest(new Request('lyn-app://app/api/read'), 'http://127.0.0.1:1234/', 'session=owned')
...
it('replaces the immutable cache header of plugin bundles with no-store and leaves other routes alone', async () => {
  ...
  vi.stubGlobal('fetch', fetch)
  const bundle = await forwardWebRequest(new Request('lyn-app://app/plugins/??a/client.js&rev=1'), 'http://127.0.0.1:1234/', 'c')
  expect(bundle.headers.get('cache-control')).toBe('no-store')
  const chunk = await forwardWebRequest(new Request('lyn-app://app/plugins/a/client.x.js?rev=1'), 'http://127.0.0.1:1234/', 'c')
  expect(chunk.headers.get('cache-control')).toBe('no-store')
  const asset = await forwardWebRequest(new Request('lyn-app://app/api/plugins/list'), 'http://127.0.0.1:1234/', 'c')
...
it('refuses another page origin without forwarding its request', async () => {
  const fetch = vi.fn()
  vi.stubGlobal('fetch', fetch)
  const response = await forwardWebRequest(new Request('lyn-app://app/api/read', { headers: { origin: 'https://other.example' } }), 'http://127.0.0.1:1234/', 'session=owned')
