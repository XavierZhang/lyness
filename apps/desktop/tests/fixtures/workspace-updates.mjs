...
const root = process.env.LYNESS_WORKSPACE_UPDATE_ROOT
assert.ok(root)
...
process.env.LYNESS_DESKTOP_APP_ID = 'com.deepseek.qualification'
process.env.LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG = JSON.stringify({ origin: new URL(server.url).origin,
...
async function windowAt(url) {
  if (process.platform === 'win32' && url === 'lyn-app://shell/mandatory-update.html') {
    return mandatoryFrameDriver(await windowAt('lyn-app://app/'))
  }
...
async function control(action) {
  ...
  const response = await fetch(`${url}/${action}`, { method: 'POST', headers: { 'x-qualification-token': process.env.LYNESS_WORKSPACE_UPDATE_TOKEN } })
...
async function dialogWith(message) {
  ...
  await waitFor(async () => {
    for (const window of BrowserWindow.getAllWindows()) {
      if (window.webContents.getURL() !== 'lyn-app://shell/update-dialog.html') continue
      try {
...
async function qualify() {
  ...
  try {
    ...
    mainWindow = await windowAt('lyn-app://app/')
    await documentReady(mainWindow, `document.querySelector('[class*="frame"]') && window.lynDesktop?.updates`)
    ...
    assert.equal(BrowserWindow.getAllWindows().some(window => window.webContents.getURL() === 'lyn-app://shell/update-dialog.html'), false)
    ...
    const mandatory = await windowAt('lyn-app://shell/mandatory-update.html')
    ...
    checkMenu.click()
    await waitFor(async () => (await mandatory.webContents.executeJavaScript('window.lynMandatoryUpdate.status()')).policy.error === 'unavailable', 'retained policy failure')
    ...
    server.policy('force')
    checkMenu.click()
    let forcedRecovery = await windowAt('lyn-app://shell/mandatory-update.html')
