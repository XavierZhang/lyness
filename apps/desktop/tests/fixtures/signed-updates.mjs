...
const root = process.env.LYNESS_SIGNED_UPDATE_TEST_ROOT
...
async function main() {
  await app.whenReady()
  const publisher = resolveWindowsUpdatePublisher(process.env.LYNESS_SIGNED_UPDATE_CERTIFICATE)
  const server = await createArtifactUpdateServer({
    signed: process.env.LYNESS_SIGNED_UPDATE_SIGNED,
    unsigned: process.env.LYNESS_SIGNED_UPDATE_UNSIGNED,
    ...(process.env.LYNESS_SIGNED_UPDATE_OLD ? {
      old: process.env.LYNESS_SIGNED_UPDATE_OLD,
      oldBlockmap: `${process.env.LYNESS_SIGNED_UPDATE_OLD}.blockmap`,
      signedBlockmap: `${process.env.LYNESS_SIGNED_UPDATE_SIGNED}.blockmap`,
    } : {}),
  })
