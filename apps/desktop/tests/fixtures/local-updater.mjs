...
const root = process.env.LYNESS_LOCAL_UPDATE_TEST_ROOT
...
protocol.registerSchemesAsPrivileged([{ scheme: 'lyn-app', privileges: { standard: true, secure: true, supportFetchAPI: true } }])
const cases = []
const presentations = []
...
async function main() {
  ...
  try {
    ...
    await scenario('mandatory-real-window-policy-download-retry-and-clear', async () => {
      protocol.handle('lyn-app', async request => {
      ...
      await parent.loadURL('lyn-app://app/')
      const f = await fixture('hold-download')
      ...
      let modal
      const policy = new DesktopMandatoryUpdatePolicy(config, { platform: 'win32', arch: 'x64', bundledLynVersion: '1.0.0' },
      ...
      try {
        ...
        async function until(expression) {
          try { return await contents.executeJavaScript(`new Promise((resolve, reject) => {
            ...
            console.error('Modal condition:', expression, await contents.executeJavaScript('({ url: location.href, text: document.body?.innerText, bridge: typeof window.lynMandatoryUpdate })'))
            throw error
          }
        }
