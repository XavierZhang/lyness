...
assert.ok(runtime, 'Pass the filtered resources/lyn directory')
...
const scratch = mkdtempSync(join(tmpdir(), 'lyn-runtime-payload-'))
...
`)
  ...
  const output = execFileSync(process.execPath, ['--expose-internals', pnpm, 'run', 'check'], {
    ...
    env: { ...environment, pnpm_config_verify_deps_before_run: 'false',
      ELECTRON_RUN_AS_NODE: '1', LYNESS_DESKTOP_NODE_EXECUTABLE: process.execPath,
...
async function checkPty() {
  ...
  env.LYNESS_DESKTOP_NODE_EXECUTABLE = process.execPath
...
async function checkSearch() {
  const { resolveRgPath } = await import(pathToFileURL(requireRuntime.resolve('@lyness/lyn-tool-fs-search')).href)
