...
child.once('message', () => {
  writeFileSync(join(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'descendant.json'), JSON.stringify({ pid: child.pid }))
  failPackagingRun(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'simulated-signing-failure')
})
setInterval(() => {}, 1000)
