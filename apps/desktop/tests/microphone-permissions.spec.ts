...
it('allows only audio checks from the owned primary application frame', () => {
  ...
  expect(f.check(f.primary, 'media', 'lyn-app://app', details)).toBe(true)
  expect(f.check(null, 'media', 'lyn-app://app', details)).toBe(false)
  expect(f.check({} as WebContents, 'media', 'lyn-app://app', details)).toBe(false)
  ...
  expect(f.check(f.primary, 'media', 'lyn-app://app', { ...details, isMainFrame: false })).toBe(false)
  expect(f.check(f.primary, 'media', 'lyn-app://app', { ...details, mediaType: 'video' })).toBe(false)
  expect(f.check(f.primary, 'clipboard-sanitized-write', 'lyn-app://app', details)).toBe(true)
})
it.each(['not-determined', 'granted', 'denied', 'restricted', 'unknown'] as const)(
  'reports existing macOS microphone authorization for %s', (status) => {
    ...
    expect(f.check(f.primary, 'media', 'lyn-app://app', { isMainFrame: true, mediaType: 'audio' })).toBe(status === 'granted')
...
it.each([true, false])('waits for the first macOS authorization decision: %s', async (allowed) => {
  ...
  try {
    f.request(f.primary, 'media', done, { isMainFrame: true, requestingUrl: 'lyn-app://app/', mediaTypes: ['audio'] })
...
it('requests macOS microphone access and reports operating-system rejection', async () => {
  const f = fixture(), done = vi.fn(), details = { isMainFrame: true, requestingUrl: 'lyn-app://app/', mediaTypes: ['audio'] }
  ...
  for (const denied of [{ ...details, mediaTypes: ['audio', 'video'] }, { ...details, mediaTypes: [] },
    { ...details, isMainFrame: false }, { isMainFrame: true, requestingUrl: 'lyn-app://app/' }]) {
...
it.each(['win32', 'linux'] as const)('leaves system authorization to Chromium on %s', (platform) => {
  ...
  expect(f.check(f.primary, 'media', 'lyn-app://app', { isMainFrame: true, mediaType: 'audio' })).toBe(true)
  f.request(f.primary, 'media', done, { isMainFrame: true, requestingUrl: 'lyn-app://app/', mediaTypes: ['audio'] })
