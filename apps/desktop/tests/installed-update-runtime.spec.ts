...
async function fixture<T>(body: (directory: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'lyn-update-materials-'))
  ...
  it('retags only release-owned packages and dependencies while preserving source and third-party versions', async () => {
    await fixture(async (directory) => {
      ...
      for (const version of versions) {
        const runtime = join(run.root, version, 'lyn')
        const descriptor = await verifyDesktopRuntime(runtime, version)
        expect(descriptor.sharedPackages.find(entry => entry.name === '@lyness/cordis')?.version).toBe(source.version)
        expect(descriptor.sharedPackages.find(entry => entry.name === '@lyness/lyn')?.version).toBe(version)
        const metadata = JSON.parse(await readFile(join(runtime, 'node_modules/@lyness/lyn/package.json'), 'utf8')) as {
          dependencies: Record<string, string>
        }
        expect(metadata.dependencies['@lyness/lyn-desktop-host']).toBe(version)
        expect(metadata.dependencies['@lyness/cordis']).toBe(source.version)
      }
