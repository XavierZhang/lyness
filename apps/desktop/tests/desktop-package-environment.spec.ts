...
const POLICY = { LYNESS_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://policy.example.com',
  LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG: JSON.stringify({ allowedAuthOrigins: ['https://login.example.com'] }) }
const RELEASE = { ...POLICY, LYNESS_DESKTOP_APP_ID: 'com.example.desktop', DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com',
  DOWNLOAD_TEST_RELEASE_ID: '0123456789abcdef0123456789abcdef' }
const MAC_IDENTITY = { LYNESS_DESKTOP_MACOS_SIGNING_IDENTITY: 'Example Company (TEAMID1234)', LYNESS_DESKTOP_MACOS_TEAM_ID: 'TEAMID1234' }
...
describe('Desktop local packaging configuration', () => {
  it('takes cache concurrency from the Windows file and defaults to four without ambient overrides', async () => {
    await withDirectory(async (directory) => {
      const parent = { LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: '8' }
      ...
      await writeFile(join(directory, '.env.windows'), 'LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY=2\n')
  ...
  it.each(['1', '2', '4', '8'])('accepts cache concurrency %s', (value) => {
    const settings = resolveWindowsPackageSettings({ LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: value })
  ...
  it.each(['', '0', '-1', '9', '1.5', 'Infinity', '01'])('rejects invalid cache concurrency %s', (value) => {
    expect(() => resolveWindowsPackageSettings({ LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: value })).toThrow('integer from 1 to 8')
    for (const options of [{ unsigned: true }, { prepareOnly: true }]) {
      expect(() => {
        validateDesktopPackageEnvironment({ ...RELEASE, LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: value }, WINDOWS, options)
  ...
  it('selects the platform file, preserves literal secrets, and excludes stale ambient release settings', async () => {
    await withDirectory(async (directory) => {
      await writeFile(join(directory, '.env.windows'), '\uFEFFLYNESS_DESKTOP_APP_ID=com.example.windows\r\nLYNESS_DESKTOP_WINDOWS_TOKEN_PIN=" #!$%&literal "\r\nLYNESS_DESKTOP_WINDOWS_CER_FILE="keys/public certificate.cer"\r\n')
      await writeFile(join(directory, '.env.macos'), 'LYNESS_DESKTOP_APP_ID=com.example.mac\nAPPLE_KEYCHAIN_PROFILE=release\nCSC_LINK=keys/signing.p12\nCSC_KEY_PASSWORD=" # literal "\n')
      const parent = {
      ...
      expect(loadDesktopPackageEnvironment('win32', parent, directory)).toEqual({
        ...
        LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: ' #!$%&literal ',
        LYNESS_DESKTOP_WINDOWS_CER_FILE: join(directory, 'keys', 'public certificate.cer'),
      })
      expect(loadDesktopPackageEnvironment('darwin', parent, directory)).toEqual({
        PATH: 'build-tools', LYNESS_DESKTOP_APP_ID: 'com.example.mac', APPLE_KEYCHAIN_PROFILE: 'release',
  ...
  it('loads mandatory update origins and options only from the platform file without changing its parent', async () => {
    await withDirectory(async (directory) => {
      ...
      const origins = { LYNESS_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://test.example.invalid',
        LYNESS_DESKTOP_MANDATORY_UPDATE_PROD_ORIGIN: 'https://prod.example.invalid' }
      const lines = Object.entries(origins).map(([key, value]) => `${key}=${value}\n`).join('')
      await writeFile(join(directory, '.env.windows'), `${lines}LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG='${windowsPolicy}'\n`)
      await writeFile(join(directory, '.env.macos'), `${lines}LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG='${macPolicy}'\n`)
      const parent = { LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG: 'stale-policy' }
      expect(loadDesktopPackageEnvironment('win32', parent, directory)).toEqual({ ...origins, LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG: windowsPolicy })
      expect(loadDesktopPackageEnvironment('darwin', parent, directory)).toEqual({ ...origins, LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG: macPolicy })
      expect(parent).toEqual({ LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG: 'stale-policy' })
    })
  })
  ...
  it('checks application and update configuration before Windows credentials while preserving unsigned and preparation modes', () => {
    ...
    }).toThrow(/LYNESS_DESKTOP_APP_ID/u)
    expect(() => {
      validateDesktopPackageEnvironment({ LYNESS_DESKTOP_APP_ID: 'invalid' }, WINDOWS)
    }).toThrow(/reverse-DNS/u)
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, LYNESS_DESKTOP_APP_ID: RELEASE.LYNESS_DESKTOP_APP_ID }, WINDOWS)
    }).toThrow(/DOWNLOAD_TEST_ORIGIN/u)
    expect(() => {
      validateDesktopPackageEnvironment(RELEASE, WINDOWS)
    }).toThrow(/LYNESS_DESKTOP_WINDOWS_CER_FILE/u)
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, LYNESS_DESKTOP_APP_ID: RELEASE.LYNESS_DESKTOP_APP_ID }, WINDOWS, { unsigned: true })
    }).not.toThrow()
    expect(() => {
      validateDesktopPackageEnvironment({ ...POLICY, LYNESS_DESKTOP_APP_ID: RELEASE.LYNESS_DESKTOP_APP_ID }, WINDOWS, { prepareOnly: true })
    }).not.toThrow()
  })

  it('accepts one local npm registry mirror and rejects other registry forms', () => {
    const release = { ...POLICY, LYNESS_DESKTOP_APP_ID: RELEASE.LYNESS_DESKTOP_APP_ID }
    expect(() => {
      validateDesktopPackageEnvironment({ ...release, LYNESS_DESKTOP_NPM_REGISTRY: 'https://registry.npmmirror.com/' }, WINDOWS, { unsigned: true })
    }).not.toThrow()
    for (const value of ['http://registry.example.com/', 'https://registry.example.com/path', 'https://user:secret@registry.example.com/', 'not-a-url']) {
      expect(() => {
        validateDesktopPackageEnvironment({ ...release, LYNESS_DESKTOP_NPM_REGISTRY: value }, WINDOWS, { unsigned: true })
      }).toThrow(/LYNESS_DESKTOP_NPM_REGISTRY/u)
    }
  })
  ...
  it('rejects incomplete macOS identity and credentials and checks referenced files without contacting Apple', async () => {
    ...
    }).toThrow(/LYNESS_DESKTOP_MACOS_SIGNING_IDENTITY/u)
    expect(() => {
...
it('owns macOS tuning in the local file and validates it before signing credentials', async () => {
  await withDirectory(async (directory) => {
    await writeFile(join(directory, '.env.macos'), 'LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY=2\nLYNESS_DESKTOP_MACOS_DOWNLOAD_PROXY=http://proxy.example:8080\nLYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY=\n')
    const env = loadDesktopPackageEnvironment('darwin', { LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY: '99' }, directory)
    expect(env.LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY).toBe('2')
    expect(env.LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY).toBe('')
    expect(() =>{  validateDesktopPackageEnvironment({ ...RELEASE, LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY: '' }, MACOS) }).toThrow('PACK_CONCURRENCY')
    expect(() =>{  validateDesktopPackageEnvironment({ ...RELEASE, LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY: 'socks5://localhost:8080' }, MACOS) }).toThrow('NOTARIZATION_PROXY')
  })
})
