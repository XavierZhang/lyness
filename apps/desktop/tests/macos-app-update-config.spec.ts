...
const roots: string[] = []
const update = { publicUrl: 'https://desktop-updates.example.com/lyn-desk/feeds/mac-arm64/' }
...
async function fixture(): Promise<{ appPath: string; resourcesDir: string }> {
  ...
  roots.push(root)
  const appPath = join(root, 'lyness.app')
...
describe('macOS packaged updater configuration', () => {
  ...
  it('writes and verifies the fixed release feed before signing', async () => {
    ...
    expect(createMacOSAppUpdateConfig(update, 'lyness-updater')).toEqual({
      provider: 'generic',
      url: update.publicUrl,
      channel: 'nightly',
      updaterCacheDirName: 'lyness-updater',
    })
    await writeMacOSAppUpdateConfig(paths.resourcesDir, update, 'lyness-updater')
    await expect(verifyMacOSAppUpdateConfig(paths.appPath, update, 'lyness-updater')).resolves.toBeUndefined()
  })
  ...
  it('rejects another updater cache directory when the signed value is known', async () => {
    ...
    await expect(verifyMacOSAppUpdateConfig(paths.appPath, update, 'lyness-updater'))
      .rejects.toThrow(/expected lyness-updater/u)
  })
})
