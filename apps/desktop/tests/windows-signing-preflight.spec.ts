...
async function fixture() {
  ...
  const environment = { SystemRoot: root,
    LYNESS_DESKTOP_WINDOWS_CER_FILE: join(root, 'public.cer'),
    LYNESS_DESKTOP_WINDOWS_SIGNTOOL: join(root, 'signtool.exe'),
    LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'fixture-container', LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin' }
  await writeFile(environment.LYNESS_DESKTOP_WINDOWS_CER_FILE, 'inert certificate')
  await writeFile(environment.LYNESS_DESKTOP_WINDOWS_SIGNTOOL, 'inert signer')
...
('rejects static %s failure without compiling or signing', async (failure) => {
  ...
  else if (failure === 'certificate') await rm(options.environment.LYNESS_DESKTOP_WINDOWS_CER_FILE)
  else if (failure === 'signTool') await rm(options.environment.LYNESS_DESKTOP_WINDOWS_SIGNTOOL)
  else if (failure === 'pin') options.environment.LYNESS_DESKTOP_WINDOWS_TOKEN_PIN = ''
  else {
