import type { AccountView } from '@lyness/lyn-deepseek-account/types'
  ...
  class FakeWindow extends EventEmitter {
    ...
    readonly webContents = Object.assign(new EventEmitter(), {
      ...
      send: vi.fn((channel: string, state: { policy?: { blocking: boolean } }) => {
        if (channel === 'lyn-desktop:mandatory-state' && state.policy?.blocking) policyBlocked.resolve()
      }),
    })
    ...
    async loadURL(url: string) {
      ...
      if (url === 'lyn-app://app/') navigated.resolve()
    }
...
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return { ...original, readFile: vi.fn((path: Parameters<typeof original.readFile>[0], encoding?: 'utf8') => {
    if (path === join('desktop-test-app', 'package.json')) {
      return Promise.resolve(JSON.stringify({ lynDesktopAppId: 'com.deepseek.lyn', lynMandatoryUpdatePolicy: harness.embeddedPolicy }))
    }
...
function invoke(channel: string, origin = channel === DESKTOP_IPC.boot ? 'app' : 'shell', ...args: unknown[]): unknown {
  ...
  return handler({ senderFrame: { url: `lyn-app://${origin}/index.html` } }, ...args)
}
...
beforeEach(() => {
  ...
  vi.useFakeTimers()
  harness.reset()
  const userData = mkdtempSync(join(tmpdir(), 'lyn-main-user-data-'))
  ...
  vi.stubEnv('LYNESS_DESKTOP_PNPM_ENTRY', 'test-pnpm')
  vi.stubEnv('LYNESS_DESKTOP_LYNESS_DIR', 'test-runtime')
  vi.stubEnv('LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR', 'test-primary-runtime')
  vi.stubGlobal('process', { ...process, platform: 'win32', arch: 'x64', resourcesPath: 'desktop-test-resources' })
  vi.stubEnv('LYNESS_DESKTOP_HOST_INSPECT_PORT', undefined)
  vi.stubEnv('LYNESS_DESKTOP_DEV_PROJECT_DIR', undefined)
  vi.stubEnv('LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG', undefined)
  vi.stubEnv('LYNESS_DESKTOP_UPDATE_JOURNAL_DIR', undefined)
  vi.stubEnv('LYNESS_CLIENT_VERSION', '1.2.3')
})
...
describe('desktop main startup', () => {
  it('routes shell update documents and assets through the registered main protocol handler', async () => {
    ...
    try {
      ...
      for (const [name, mime] of [
        ...
        const response = await handler(new Request(`lyn-app://shell/${name}`))
      ...
      const head = await handler(new Request('lyn-app://shell/update-dialog.html', { method: 'HEAD' }))
      expect(head.status).toBe(200)
      expect(await head.text()).toBe('')
      expect((await handler(new Request('lyn-app://shell/update-dialog.html', { method: 'POST' }))).status).toBe(405)
      expect((await handler(new Request('lyn-app://shell/%'))).status).toBe(400)
      expect((await handler(new Request('lyn-app://shell/%2e%2e%2fpackage.json'))).status).toBe(403)
      expect((await handler(new Request('lyn-app://shell/missing.html'))).status).toBe(404)
      expect((await handler(new Request('lyn-app://other/update-dialog.html'))).status).toBe(404)
    } finally {
  ...
  it('serves shell dialogs and their assets without forwarding them to the Host', async () => {
    ...
    for (const file of ['update-dialog.html', 'update-dialog.js', 'update-dialog.css', 'update-close.svg', 'mandatory-update.html']) {
      const request = new Request(`lyn-app://shell/${file}`)
    ...
    expect((await handler(new Request('lyn-app://unknown/update-dialog.html'))).status).toBe(404)
  })
  ...
  it.each([
    ...
    expect(harness.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      type: 'info', title: zh ? '关于 lyness' : 'About lyness', message: 'lyness',
  ...
  it('persists opt-in update evidence from the real main entry without private diagnostics', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lyn-main-update-journal-'))
    try {
      vi.stubEnv('LYNESS_DESKTOP_UPDATE_JOURNAL_DIR', directory)
      await readyForUpdate()
  ...
  it.each(['darwin', 'win32', 'linux'] as const)('limits native titlebar styling to macOS on %s', async (platform) => {
    ...
    expect(window.urls).toEqual(['lyn-app://app/'])
    if (platform === 'darwin') {
  ...
  it('serves packaged shell documents instead of rejecting the shell origin', async () => {
    ...
    await harness.preparing.promise
    const handler = harness.protocolHandle.mock.calls[0]![1]
    const request = new Request('lyn-app://shell/update-dialog.html')
    ...
    expect((await handler(new Request('lyn-app://foreign/index.html'))).status).toBe(404)
  })
  ...
  it('follows the Windows primary document language and palette without trusting other frames', async () => {
    ...
    listener({ ...event, senderFrame: { url: 'lyn-app://app/' } }, 'zh-CN', '#ffffff', '#000000')
    ...
    window.setTitleBarOverlay.mockClear()
    window.webContents.mainFrame.url = 'lyn-app://unowned/index.html'
  ...
  it('maps Windows caption menus to localized native commands and rejects foreign popup requests', async () => {
    ...
    expect(harness.menu.buildFromTemplate.mock.lastCall![0].map(item => item.label ?? item.type)).toEqual([
      '关于 lyness', 'separator', '检查更新…', 'separator', '退出',
    ])
  ...
  it.each(['en-US', 'zh-CN'])('localizes macOS visibility and quit commands without changing the application name (%s)', async (locale) => {
    ...
    harness.app.name = '@lyness/lyn-desktop'
    try {
      await import('../src/main.ts')
      await harness.preparing.promise
      ...
      expect(harness.app.name).toBe('@lyness/lyn-desktop')
  ...
  it('attaches Host socket credentials only to the owned application origin and window', async () => {
    ...
    const details = { url: 'ws://127.0.0.1:3080/api/remote.mux', webContentsId: 42, requestHeaders: { Origin: 'lyn-app://app' } }
  ...
  it('holds boot injections until the Host is ready and rejects foreign boot callers', async () => {
    ...
    let settled = false
    const boot = Promise.resolve(handler({ senderFrame: { url: 'lyn-app://app/' } })).then((value) => { settled = true; return value })
    await Promise.resolve()
    expect(settled).toBe(false)
    expect(harness.windows[0]!.urls).toEqual(['lyn-app://app/'])
    harness.prepared.resolve()
    await harness.hostStarted.promise
    expect(settled).toBe(false)
    harness.hosts[0]!.ready.resolve()
  ...
  it('opens message links externally while retaining same-origin application navigation', async () => {
    ...
    window.webContents.emit('will-navigate', internal, 'lyn-app://app/session/task-1')
  ...
  it('shares the failed-check deadline across focus and resume, while explicit checks reset polling', async () => {
    vi.stubEnv('LYNESS_DESKTOP_UPDATE_CHECK_INTERVAL_MS', '1000')
    vi.stubEnv('LYNESS_DESKTOP_UPDATE_CHECK_MAX_BACKOFF_MS', '4000')
    vi.stubEnv('LYNESS_DESKTOP_UPDATE_CHECK_JITTER', '0')
  ...
  it('blocks subsequent product operations without stopping the Host and clears only on a fresh no-force policy', async () => {
    harness.embeddedPolicy = { origin: 'https://policy.example.com',
      allowedPageOrigins: ['https://downloads.example.com'], intervalMs: 10_000, jitter: 0 }
    vi.stubEnv('LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG', '{invalid environment override}')
    ...
    const unowned = [
      { ...owned, sender: {} },
      { ...owned, senderFrame: { url: 'lyn-app://app/index.html' } },
    ...
    expect(request.mock.calls[0]![1]!.headers).toMatchObject({
      ...
      'x-client-arch': 'x64', 'x-client-update-channel': 'nightly', 'x-client-bundled-lyn-version': '1.0.0',
  ...
  it.each([false, true])('restores a cleanly stopped Host after installer failure and retains mandatory blocking: %s', async (mandatory) => {
    ...
    await vi.waitFor(() => { expect(harness.windows[0]!.urls).toEqual(['lyn-app://app/', 'lyn-app://app/']) })
  ...
  it('ignores subresource failures and navigation cancellation but reports a failed main document', async () => {
    ...
    await harness.preparing.promise
    const window = harness.windows[0]!
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'lyn-app://app/image.png', false)
    window.webContents.emit('did-fail-load', {}, -3, 'aborted', 'lyn-app://app/', true)
    expect(harness.dialog.showMessageBox).not.toHaveBeenCalled()
    window.webContents.emit('did-fail-load', {}, -2, 'failed', 'lyn-app://app/', true)
    await harness.dialogShown.promise
  ...
  it('attaches the primary window\'s error-level console output to a running-phase Host crash report', async () => {
    ...
    await Promise.resolve(invoke(DESKTOP_IPC.boot))
    const window = harness.windows[0]!
    window.webContents.emit('console-message', { level: 'info', message: 'boot ok', sourceId: 'lyn-app://app/assets/entry.js', lineNumber: 1 })
    window.webContents.emit('console-message', {
      ...
      sourceId: 'lyn-app://app/assets/entry.js', lineNumber: 12,
    })
    host.onFailure!(new Error('lyn desktop host exited with 1: fatal uncaught exception: ENOENT'))
    await host.stopping.promise
    ...
    expect(report.rendererConsole).toEqual(['lyn-app://app/assets/entry.js:12 client-modules: bundle script plugins/??a/client.js&rev=1 failed to load'])
  ...
  it('prepares recovery offscreen and starts one Host before choosing the first visible window', async () => {
    ...
    expect(harness.hosts[0]).toMatchObject({
      ...
      runtime: join(harness.app.getAppPath(), 'lyn'),
  ...
  it('prepares an independent plugin profile for the unpackaged Host', async () => {
    harness.app.isPackaged = false
    vi.stubEnv('LYNESS_DESKTOP_LYNESS_DIR', undefined)
    await import('../src/main.ts')
    await harness.preparing.promise
    harness.prepared.resolve()
    await harness.hostStarted.promise
  ...
  it('fails an unpackaged launch that receives no primary runtime directory', async () => {
    harness.app.isPackaged = false
    vi.stubEnv('LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR', undefined)
    await import('../src/main.ts')
    await harness.dialogShown.promise
    const options = harness.dialog.showMessageBox.mock.calls[0]![0] as MessageBoxOptions
    expect(options.detail).toContain('LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR is required for an unpackaged launch')
    expect(harness.hosts).toHaveLength(0)
  })
  ...
  it('keeps startup errors in the existing window without a retry handler', async () => {
    ...
    expect(harness.windows[0]!.urls).toEqual(['lyn-app://app/'])
    expect(harness.dialog.showMessageBox).toHaveBeenCalledOnce()
    expect(harness.handlers.has('lyn-desktop:backend-retry')).toBe(false)
    expect(harness.hosts).toHaveLength(1)
  })
...
it.each(['failed', 'expired'] as const)('focuses LYN once when browser authorization becomes %s', async (phase) => {
  await import('../src/main.ts')
  await harness.preparing.promise
  harness.prepared.resolve()
  await harness.hostStarted.promise
...
it.each([['light', false], ['dark', true]] as const)('opens Platform authorization in the effective %s palette', async (theme, shouldUseDarkColors) => {
  ...
  const state: AccountView = {
    ...
    attempt: { id: 'test-theme-attempt' as NonNullable<AccountView['attempt']>['id'], phase: 'waiting-browser',
      authorizeUrl: 'https://platform.deepseek.com/lyn/authorize?state=state-1' },
  }
  harness.publishAccount(state)
  harness.publishAccount(state)
  expect(harness.openExternal).toHaveBeenCalledExactlyOnceWith(`https://platform.deepseek.com/lyn/authorize?state=state-1&theme=${theme}`)
})
