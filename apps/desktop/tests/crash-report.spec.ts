...
const input = (overrides: Partial<CrashReportInput> = {}): CrashReportInput => ({
  source: 'host',
  phase: 'running',
  error: Object.assign(new Error("ENOENT: no such file or directory, open 'C:\\\\Temp\\\\lyn-subprocess-x\\\\out.log'"), {
    errno: -4058, code: 'ENOENT', syscall: 'open', path: 'C:\\Temp\\lyn-subprocess-x\\out.log',
  }),
  rendererConsole: [],
  app: { name: 'lyness', version: '0.1.7', platform: 'win32', arch: 'x64', electron: '44.0.0', node: '24.18.1', locale: 'zh_CN' },
...
it('renders the facts header, the inspected error with its properties and cause, and the renderer console', () => {
  const text = renderCrashReport(input({
    ...
    rendererConsole: ['lyn-app://app/assets/entry.js:12 client-modules: bundle script plugins/??a/client.js&rev=1 failed to load'],
  }))
  expect(text).toContain('time: 2026-09-22T10:30:00.123Z\nsource: host\nphase: running\napp: lyness 0.1.7\nplatform: win32 x64\nelectron: 44.0.0\nnode: 24.18.1\nlocale: zh_CN\n')
  expect(text).toContain('--- error ---\nError: wrapper')
  expect(text).toContain("syscall: 'open'")
  expect(text).toContain("path: 'C:\\\\Temp\\\\lyn-subprocess-x\\\\out.log'")
  expect(text).toContain('[cause]: Error: ENOENT')
  expect(text).toContain('--- renderer console (error level, oldest first) ---\nlyn-app://app/assets/entry.js:12 client-modules: bundle script')
...
it('returns undefined and logs when the report cannot be written', async () => {
  ...
  expect(error).toHaveBeenCalledWith('lyn desktop: crash report could not be written', expect.stringContaining('crash-'), expect.anything())
})
