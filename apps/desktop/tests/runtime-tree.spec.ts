...
it('verifies a runtime after relocation without depending on build paths', async () => {
  const root = fixture()
  const before = await verifyDesktopRuntime(join(root, 'lyn'), '1.0.0')
  cpSync(join(root, 'lyn'), join(root, 'moved'), { recursive: true })
...
it.each(['changed', 'same-size', 'extra', 'missing'])('checks %s runtime bytes only during build verification', async (operation) => {
  const lyn = join(fixture(), 'lyn')
  const before = readDesktopRuntime(lyn)
  if (operation === 'changed') writeFileSync(join(lyn, 'package.json'), '{}')
  if (operation === 'same-size') writeFileSync(join(lyn, 'package.json'), '{"type":"Module"}\n')
  if (operation === 'extra') writeFileSync(join(lyn, 'extra'), '')
  if (operation === 'missing') rmSync(join(lyn, 'package.json'))
  expect(readDesktopRuntime(lyn)).toEqual(before)
  await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow(/integrity/u)
})
it('rejects filesystem links and incompatible targets', async () => {
  const lyn = join(fixture(), 'lyn')
  await expect(verifyDesktopRuntime(lyn, '1.0.0', { platform: process.platform, arch: 'wrong' })).rejects.toThrow(/incompatible/u)
  symlinkSync(join(lyn, 'node_modules'), join(lyn, 'outside'), process.platform === 'win32' ? 'junction' : 'dir')
  expect(readDesktopRuntime(lyn).release.version).toBe('1.0.0')
  await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow(/unsupported filesystem/u)
})
it('reads file inventory records unchanged during startup', () => {
  const lyn = join(fixture(), 'lyn')
  const path = join(lyn, DESKTOP_RUNTIME_FILE)
  ...
  expect(readDesktopRuntime(lyn).files).toEqual(descriptor.files)
})
it.each(['missing', 'directory'])('checks a %s Host entry only during build verification', async (operation) => {
  const lyn = join(fixture(), 'lyn')
  const path = join(lyn, 'node_modules', DESKTOP_HOST_PACKAGE, DESKTOP_HOST_RUNTIME_FILES[0])
  rmSync(path)
...
it('checks the bundled version only during build verification', async () => {
  const lyn = join(fixture(), 'lyn')
  expect(readDesktopRuntime(lyn).release.version).toBe('1.0.0')
  await expect(verifyDesktopRuntime(lyn, '2.0.0')).rejects.toThrow(/bundled 1\.0\.0 is not the expected 2\.0\.0/u)
})
it.each([
  { schemaVersion: 2 },
  { platform: 'other' },
  ...
  const lyn = join(fixture(), 'lyn')
  const path = join(lyn, DESKTOP_RUNTIME_FILE)
  const original = readDesktopRuntime(lyn)
  const descriptor = { ...original, ...patch, release: { ...original.release, ...patch.release } }
  writeFileSync(path, JSON.stringify(descriptor))
  expect(readDesktopRuntime(lyn)).toEqual(descriptor)
  await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow(/invalid|incompatible/u)
})
it.each(['missing', 'invalid-json', 'mismatched'])('checks %s shared manifests only during build verification', async (operation) => {
  const lyn = join(fixture(), 'lyn')
  const before = readDesktopRuntime(lyn)
  const path = join(lyn, 'node_modules', DESKTOP_HOST_PACKAGE, 'package.json')
  if (operation === 'missing') rmSync(path)
  else writeFileSync(path, operation === 'invalid-json' ? '{' : '{}')
  expect(readDesktopRuntime(lyn)).toEqual(before)
  await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow()
})
it('rejects a descriptor that maps a shared package outside node_modules', async () => {
  ...
  writeFileSync(path, JSON.stringify(descriptor))
  await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow(/shared package record/u)
})
it('verifies recorded executable permissions only on Unix', async () => {
  ...
  writeFileSync(path, JSON.stringify(descriptor))
  if (process.platform === 'win32') await expect(verifyDesktopRuntime(lyn, '1.0.0')).resolves.toMatchObject(descriptor)
  else await expect(verifyDesktopRuntime(lyn, '1.0.0')).rejects.toThrow(/integrity/u)
})
