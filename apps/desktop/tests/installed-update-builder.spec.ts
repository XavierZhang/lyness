...
const environment = { LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'test', DOWNLOAD_TEST_ORIGIN: 'https://download-test.deepseek.com', DOWNLOAD_TEST_RELEASE_ID: '0123456789abcdef0123456789abcdef',
...
async function fixture<T>(body: (manifest: string, source: string) => Promise<T>): Promise<T> {
  const root = await mkdtemp(join(tmpdir(), 'lyn-update-builder-'))
  try {
    ...
    for (const version of versions) {
      ...
      const runtime = runtimeFixture(lyn, version)
      writeDesktopRuntime(lyn, runtime.release, runtime.sharedPackages.map(entry => entry.name), { platform: 'win32', arch: 'x64' })
    }
  ...
  it('keeps signed NSIS hooks and one isolated identity/feed while selecting each private runtime and output', async () => {
    await fixture(async (manifest, source) => {
      ...
      for (const [index, config] of [first!, second!].entries()) {
        ...
        const lyn = join(manifest, '..', versions[index]!, 'lyn')
        expect(config.files[2]).toMatchObject({ from: lyn, to: 'lyn' })
        expect(config.files[3]).toMatchObject({ from: join(lyn, 'node_modules'), to: 'lyn/node_modules' })
        ...
        expect(config.publish[0]!.url)
          .toMatch(/^https:\/\/download-test\.deepseek\.com\/lyn-desk\/feeds\/qualification\/[a-f0-9]{24}\/win-x64\/$/u)
      }
    })
  })
