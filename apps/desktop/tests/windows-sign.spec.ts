...
describe('Windows token signing', () => {
  it.each(['invalid-primary', 'timestamp-exhausted', 'timestamp-recovered', 'normalization-corrupt'] as const)(
    'keeps hardware protection separate from %s', async (mode) => {
      const root = await mkdtemp(join(tmpdir(), 'lyn-sign-completion-'))
      ...
      vi.mocked(execFile).mockImplementation((...args: unknown[]) => {
        ...
        if (argv[0] === '/d') {
          ...
          writeFileSync(options.env.LYNESS_DESKTOP_WINDOWS_SIGN_TARGET!, 'signed:input')
          callback(null, '', '')
        } else {
          expect(options.env.LYNESS_DESKTOP_WINDOWS_TOKEN_PIN).toBeUndefined()
          expect(options.env.LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER).toBeUndefined()
          if (command === 'powershell.exe') {
            const timestamped = readFileSync(options.env.LYNESS_RUNTIME_VERIFY_FILE!, 'utf8').endsWith(':timestamp')
  ...
  it('preserves verified copies without hardware and rejects the entire queue after preservation failure', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'lyn-windows-copy-signature-'))
    try {
  ...
  it('stops concurrent and subsequent signing tasks after a PIN failure', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'lyn-windows-pin-failure-'))
    try {
      const certificateFile = join(directory, 'server.cer')
      const signTool = join(directory, 'signtool.exe')
      ...
      validateDesktopPackageEnvironment({
        LYNESS_DESKTOP_APP_ID: 'com.example.desktop', DOWNLOAD_TEST_ORIGIN: 'https://updates.example.com', DOWNLOAD_TEST_RELEASE_ID: '0123456789abcdef0123456789abcdef',
        ...
        LYNESS_DESKTOP_WINDOWS_CER_FILE: certificateFile, LYNESS_DESKTOP_WINDOWS_SIGNTOOL: signTool,
        LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin', LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'fixture-container',
        LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_DIR: 'C:\\fixture\\signature-cache',
  ...
  it('passes only the validated BAT fields to the signing command interpreter', () => {
    expect(buildWindowsSigningEnvironment({
      ...
      LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'inherited-token-secret',
      DEEPSEEK_API_KEY: 'api-secret',
      BUILD_PASSWORD: 'build-secret',
    }, {
      certificateFile: CERTIFICATE_FILE,
      signTool: 'C:\\tools\\signtool.exe',
      path: 'C:\\release\\lyness.exe',
      isNest: false,
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toEqual({
      SystemRoot: 'C:\\Windows',
      LYNESS_DESKTOP_WINDOWS_SIGNTOOL: 'C:\\tools\\signtool.exe',
      LYNESS_DESKTOP_WINDOWS_CER_FILE: CERTIFICATE_FILE,
      LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'token-secret!',
      LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'te-container',
      LYNESS_DESKTOP_WINDOWS_SIGN_TARGET: 'C:\\release\\lyness.exe',
      LYNESS_DESKTOP_WINDOWS_SIGN_APPEND: '',
    })
  })
  ...
  it('requests an appended signature only for an electron-builder nested task', () => {
    expect(buildWindowsSigningEnvironment({}, {
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    }).LYNESS_DESKTOP_WINDOWS_SIGN_APPEND).toBe('1')
  })
  ...
  it('keeps the verified SafeNet command in an ASCII CRLF CMD file', async () => {
    ...
    expect(text).toContain('set "LYNESS_DESKTOP_WINDOWS_CER_FILE="\r\n')
    expect(text).toContain('set "LYNESS_DESKTOP_WINDOWS_TOKEN_PIN="\r\n')
  ...
  it('rejects incomplete signing identities and non-SHA-256 signing tasks', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'lyn-windows-sign-tool-'))
    const certificateFile = join(directory, 'server.cer')
    const signTool = join(directory, 'signtool.exe')
    await writeFile(certificateFile, 'code-signing-certificate-fixture')
    await writeFile(signTool, 'fixture')
    ...
    expect(() => createWindowsTokenSigner({
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toThrow(/LYNESS_DESKTOP_WINDOWS_SIGNTOOL/u)
    ...
    try {
      expect(() => createWindowsTokenSigner({
        ...
        signTool,
        tokenPin: 'token-secret!',
      })).toThrow(/LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: '',
        keyContainer: 'te-container',
      })).toThrow(/LYNESS_DESKTOP_WINDOWS_TOKEN_PIN/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: 'token]secret',
  ...
  it('removes inherited credentials and redacts SignTool process failures', () => {
    expect(scrubWindowsSigningEnvironment({
      SystemRoot: 'C:\\Windows',
      LYNESS_DESKTOP_WINDOWS_CER_FILE: 'C:\\release\\server.cer',
      LYNESS_DESKTOP_WINDOWS_SIGNTOOL: 'C:\\tools\\signtool.exe',
      LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'token-secret',
      DEEPSEEK_API_KEY: 'api-secret',
      BUILD_PASSWORD: 'build-secret',
  ...
  it('clears a certificate table inherited beyond the generated uninstaller', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'lyn-windows-sign-'))
