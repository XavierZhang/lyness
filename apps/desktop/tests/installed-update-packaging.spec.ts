...
async function fixture(body: (manifest: string, root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'lyn-packaging-entry-'))
  ...
  try {
    ...
    await mkdir(join(run.root, versions[0], 'lyn'), { recursive: true })
    await writeFile(join(run.root, versions[0], 'lyn/desktop-runtime.json'), '{}')
    ...
    LYNESS_DESKTOP_WINDOWS_CER_FILE: certificate, LYNESS_DESKTOP_WINDOWS_SIGNTOOL: tool,
    LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-secret-pin', LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'fixture-container' }
...
describe('operator-driven packaging entry', () => {
  it('refuses the interlock before loading credentials, confirmation, or child allocation and preserves it byte-for-byte', async () => {
    await fixture(async (manifest, root) => {
      const lock = join(root, '.lyn-desktop-signing/attempt.json')
      await mkdir(join(root, '.lyn-desktop-signing'))
      ...
      expect(await readdir(join(manifest, '..', versions[0]))).toEqual(['lyn'])
  ...
  it('strips unrelated secrets and preload overrides while retaining signing inputs and the Windows archive filter', () => {
    expect(installedUpdatePackagingEnvironment({ LYNESS_DESKTOP_TARGET_PLATFORM: 'win32', PATH: 'tool-path',
      LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'pin', LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'container',
      DEEPSEEK_API_KEY: 'llm', DOWNLOAD_TEST_COS_SECRET_KEY: 'cos', NODE_OPTIONS: 'preload', NODE_PATH: 'injected' }))
      .toEqual({ LYNESS_DESKTOP_TARGET_PLATFORM: 'win32', PATH: 'tool-path', LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: 'pin',
        LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: 'container', LYNESS_DESKTOP_UNSIGNED: '0', ELECTRON_BUILDER_7Z_FILTER: 'BCJ' })
  })
  ...
  it.runIf(process.platform === 'win32' && process.arch === 'x64').each([true, false])(
    'supervises an inert child with failure=%s, retains redaction and refuses reuse', async (failure) => {
      await fixture(async (manifest) => {
        state.settings.LYNESS_TEST_PACKAGING_FAIL = failure ? '1' : '0'
  ...
  it.runIf(process.platform === 'win32' && process.arch === 'x64')('does not allocate a build after declined confirmation or an intervening interlock', async () => {
    await fixture(async (manifest, root) => {
      ...
      await expect(packageInstalledUpdate(manifest, versions[0], { execute: true, confirm: async () => {
        await mkdir(join(root, '.lyn-desktop-signing'))
        await writeFile(join(root, '.lyn-desktop-signing/attempt.json'), 'another operation acquired the token')
        return true
