...
import type { DesktopShortcutInput, ShortcutBinding, ShortcutCommandId, ShortcutConfigSnapshot,
  ShortcutDefinition, ShortcutSaveResult } from '@lyness/lyn-client-shortcuts/protocol'
import { ShortcutRegistry } from '@lyness/lyn-client-shortcuts/src/client/registry.ts'
import { installKeyboard } from '@lyness/lyn-client-shortcuts/src/client/dom.ts'
import { installNativeKeyboard } from '@lyness/lyn-client-shortcuts/src/client/native.ts'
import type { DesktopBrowserLeaseId, DesktopBrowserReservation } from '@lyness/lyn-client-ui-sidebar-browser/types'
...
async function fixture(platform: 'macos' | 'windows' | 'linux' = 'macos') {
  const root = await mkdtemp(join(tmpdir(), 'lyn-keyboard-'))
  onTestFinished(async () => { await rm(root, { recursive: true, force: true }) })
  const frame: FrameFixture = { url: 'lyn-app://app/', name: '', parent: null }
...
it('mirrors only successful bindings, suppresses recording menus, and invalidates pre-navigation drafts', async () => {
  ...
  f.contents.emit('did-start-navigation', { isMainFrame: true, isSameDocument: false }, 'lyn-app://app/', false, true)
...
it('rejects other windows, subframes, remote/shell pages, and malformed edits', async () => {
  ...
  for (const url of ['lyn-app://shell/', 'https://example.com/', 'http://localhost/']) {
    f.frame.url = url
    await expect(get(f.event, f.definitions)).rejects.toThrow('unowned renderer')
  }
  f.frame.url = 'lyn-app://app/'
