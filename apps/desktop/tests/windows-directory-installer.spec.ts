...
it.each(['allowOnlyOneInstallerInstance.nsh', 'installUtil.nsh'])('cleans staged files before %s exits', (helper) => {
  ...
  expect(adapted.match(/Call lynCleanupDirectories/g)).toHaveLength(exits.length)
...
it('stages before stopping the application and promotes before registering the installation', () => {
  ...
  expect(result.indexOf('!insertmacro lynStageApplication')).toBeLessThan(result.indexOf('!insertmacro CHECK_APP_RUNNING'))
  expect(result.indexOf('Call lynPromoteDirectories')).toBeLessThan(result.indexOf('!insertmacro registryAddInstallInfo'))
