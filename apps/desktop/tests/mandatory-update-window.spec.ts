...
function fakeWindow() {
  const contents = Object.assign(new EventEmitter(), { mainFrame: { url: 'lyn-app://shell/mandatory-update.html' },
...
function setup(platform: NodeJS.Platform = 'darwin') {
  ...
  window = fakeWindow()
  if (platform === 'win32') window.webContents.mainFrame.url = 'lyn-app://app/'
...
it('rebinds policy updates and reloads to a replacement Windows main window', () => {
  ...
  window = fakeWindow()
  window.webContents.mainFrame.url = 'lyn-app://app/?recovery=1#home'
  ui!.sync()
