...
async function fixture<T>(body: (manifest: string) => Promise<T>): Promise<T> {
  const directory = await mkdtemp(join(tmpdir(), 'lyn-update-distribution-'))
  try {
    ...
    for (const version of versions) {
      ...
      const filename = `lyness-${version}-win-x64.exe`
...
describe('qualification distribution file planning', () => {
  it('keeps two binary sets separate from successive bodies at one fixed feed URL without publication authority', async () => {
    await fixture(async (manifest) => {
      ...
      expect(next!.binaries.every(file => file.key.startsWith('lyn-desk/bin/qualification/'))).toBe(true)
      expect(next!.feed.key).toMatch(/^lyn-desk\/feeds\/qualification\/[a-f0-9]{24}\/win-x64\/nightly.yml$/u)
