...
const REPORT_PATH = 'C:\\Users\\someone\\AppData\\Roaming\\lyness\\logs\\crash-2026-09-22T10-30-00-000Z-host.log'
...
it.each(['en', 'zh-CN'])('offers only exit and restart for a listener conflict in %s', async (locale) => {
  ...
  const pending = recovery.report(new AggregateError([
    new Error('webserver (@lyness/lyn-host-webserver): Error: listen EADDRINUSE: address already in use 127.0.0.1:19387'),
