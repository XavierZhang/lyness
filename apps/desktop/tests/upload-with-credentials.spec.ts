...
async function check(mode: 'valid' | 'plaintext' | 'blank' | 'missing' | 'upload' | 'upload-failure', deployment = 'production') {
  ...
  if (mode === 'upload' || mode === 'upload-failure') {
    ...
    await writeFile(join(scripts, 'upload-target.ts'), `
      ...
      assert.equal(process.env.LYNESS_DESKTOP_WINDOWS_TOKEN_PIN, undefined)
  ...
  const script = `
    ...
    $env:LYNESS_DESKTOP_WINDOWS_TOKEN_PIN = 'unrelated-signing-pin'
  ...
  const env = Object.fromEntries(Object.entries(process.env).filter(([name]) =>
    !/KEY|SECRET|TOKEN|PASSWORD|^NODE_OPTIONS$|^LYNESS_DESKTOP_WINDOWS_|^APPLE_|^CSC_/iu.test(name),
  ))
...
describe('credential launcher destination', () => {
  const fileEnvironment = {
    LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'test', DOWNLOAD_TEST_ORIGIN: 'https://download-test.example.com', DOWNLOAD_TEST_RELEASE_ID: '0123456789abcdef0123456789abcdef',
  ...
  const injectedEnvironment = {
    LYNESS_DESKTOP_AUTO_UPDATE_ENV: 'test', DOWNLOAD_TEST_COS_BUCKET: 'test-bucket',
