...
The desktop application is an Electron shell around the lyn Web UI. It opens no listening port: a bundled upstream Node.js child boots the installed lyn project, versioned framed byte pipes carry Fetch requests and streaming responses without an outer Base64 envelope, Node IPC carries lifecycle control, and `lyn-app://` serves the matching client assets.

## Key technical decisions

| Decision | Why | Direct consequence |
|---|---|---|
| Release identity | The shell API, Web client, backend, and plugin graph are qualified as one combination; independent versions would create untested combinations and ambiguous update availability. | Electron and `@lyness/lyn` always have the same exact version. A lyn upgrade is a Desktop release, even when the shell code is unchanged. |
| Runtime | Electron's Node.js carries Electron patches, fuses, ABI, and lifecycle constraints, while system runtimes and package-manager state are uncontrolled. | lyn runs under the bundled upstream Node.js and every package operation uses the bundled pnpm. Electron's Node.js, system Node.js, system pnpm, and user package-manager configuration are outside the execution path. |
| Package sources | The exact lyn source build must be packageable before npm publication and install offline; plugins must remain ordinary user-selected npm packages. | The signed application carries locally packed first-party lyn packages and an offline seed store. Desktop plugins remain ordinary npm dependencies resolved from the fixed Desktop registry. |
...
| State ownership | Sharing executable dependency graphs would let CLI and Desktop change each other's lyn, Cordis, plugin, or native-module versions, while two desktop processes could race on the same profile. | Electron acquires its process-lifetime single-instance lock before any profile access and exclusively owns `$LYNESS_HOME/profiles/desktop` plus its package-manager state. CLI and Desktop share supported product data under `$LYNESS_HOME`, but never executable packages, plugin activation, lockfiles, or `node_modules`. |
| Transport | A listening Web service adds port ownership, authentication, CORS, and exposure concerns; Electron and upstream Node.js also need an explicit cross-process protocol. | The application opens no Web port. `lyn-app://` carries Web assets and Fetch traffic; framed byte pipes carry bounded request and response chunks with backpressure, while Node IPC carries only child lifecycle control. |
...
| Updates | Independent shell and lyn updates would recreate version splits, while unchanged shell blocks should not require a complete transfer. | The Electron shell, matching lyn seed, Node.js, and pnpm form one signed update unit. Platform update artifacts may reuse unchanged blocks, but runtime version selection never splits from the Desktop release. |
...
Electron owns the reserved profile at `$LYNESS_HOME/profiles/desktop`. Its manifest lists the built-in and installed plugin bundles in `lyn.profile.bundles`, while its `node_modules` contains the exact `@lyness/lyn` release, its matching private `@lyness/lyn-desktop-host`, and every desktop plugin. Keeping the Electron-only process entry and overlay in a private app package prevents Desktop implementation from becoming part of the public CLI package. The CLI cannot boot or mutate this profile. Electron always invokes its bundled Node.js and pnpm with the store at `$LYNESS_HOME/desktop/pnpm/store`; it never uses system pnpm or the caller's npm/pnpm configuration.

The main lyn renderer receives only the desktop protocol marker. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.
...
|---|---|
| `integrity.json` and `desktop-packages.json` | Verify every inventoried seed file, local tarball hash, and the bound lyn and Desktop Host versions before package state changes. |
| `store-archives.json` and `store-archives/*.tar` | Validate the deterministic uncompressed shards, extract them into a unique Desktop staging directory, replace matching immutable store files, and transactionally merge pnpm's versioned SQLite package index into `$LYNESS_HOME/desktop/pnpm/store` without removing packages already downloaded for Desktop plugins. |
| Project metadata and `desktop-packages/` | Copy into a unique `$LYNESS_HOME/desktop/staging/<transaction-id>/profile` project. |
...
2. If the active profile already contains that release plus the matching lyn and Desktop Host versions, verify its local package set and reuse it without reinstalling.
...
5. Stop the active backend, boot and stop the complete staged backend as a health check, then restart the active backend before activation. This serialization prevents two desktop backends from sharing `$LYNESS_HOME`; installation or plugin incompatibility before activation deletes staging and leaves the active profile unchanged.
6. Persist each next activation phase before its directory move, move the active profile to `$LYNESS_HOME/desktop/rollback/profile`, and move staging into `$LYNESS_HOME/profiles/desktop`. Recovery combines the journal with the actual profile, rollback, and staging directories, so interruption in either write-to-move gap restores or retains a complete profile.
...
The normal packaging path is one complete command. It performs release preparation before creating the host platform's installers and update metadata. Every target requires a reverse-DNS `LYNESS_DESKTOP_APP_ID`. macOS targets additionally require the electron-builder certificate qualifier in `LYNESS_DESKTOP_MACOS_SIGNING_IDENTITY`, its 10-character Apple Team ID in `LYNESS_DESKTOP_MACOS_TEAM_ID`, and one complete notarytool credential strategy. The App Store Connect API-key strategy uses these variables:

```sh
export LYNESS_DESKTOP_APP_ID='<reverse-DNS application ID>'
export LYNESS_DESKTOP_MACOS_SIGNING_IDENTITY='<certificate name without the Developer ID Application prefix>'
export LYNESS_DESKTOP_MACOS_TEAM_ID='<10-character Apple Team ID>'
...
Windows release packaging requires `LYNESS_DESKTOP_WINDOWS_CER_FILE` to identify the public GlobalSign EV leaf certificate, `LYNESS_DESKTOP_WINDOWS_SIGNTOOL` to identify the SafeNet-compatible SignTool executable, `LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER` to identify the matching private-key container, and `LYNESS_DESKTOP_WINDOWS_TOKEN_PIN` to contain the SafeNet Token Password. The certificate file remains outside source control, and the matching private key stays on the USB token. Set the four inputs before running the fixed Windows target:

```powershell
$env:LYNESS_DESKTOP_WINDOWS_CER_FILE = 'C:\path\to\server.cer'
$env:LYNESS_DESKTOP_WINDOWS_SIGNTOOL = 'C:\path\to\the\validated\signtool.exe'
$env:LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER = '<SafeNet private-key container name>'
$env:LYNESS_DESKTOP_WINDOWS_TOKEN_PIN = '<SafeNet Token Password>'
pnpm run package:desktop:win:x64
```
...
The PIN cannot contain `]`, a quote, or a line break because those characters delimit the SafeNet `/kc` value or its CMD argument. The CMD disables delayed expansion so a PIN containing `!` reaches SafeNet unchanged. Packaging withholds every `LYNESS_DESKTOP_WINDOWS_*` field from build and seed-preparation subprocesses, gives electron-builder only the four configured inputs, gives the signing CMD only the validated signing fields in an otherwise scrubbed environment, clears those fields before SignTool starts, and redacts SignTool diagnostics. SafeNet still requires the PIN in the SignTool process command line. Inject it as an ephemeral secret only on a controlled self-hosted Windows runner with the physical token attached; never commit it, put it in `.env`, or persist it as a Windows user or system environment variable.
...
- The desktop shell shares sessions, settings, credentials, workspaces, and storage under `$LYNESS_HOME` with CLI lyn, while executable packages, plugin activation, lockfiles, and package-manager state remain separate.
