...
const clientEnvironment = resolveClientBuildEnvironment(process.env.LYNESS_CLIENT_VERSION === undefined
  ? repositoryClientBuildEnvironment(REPOSITORY_ROOT, process.env)
  : process.env)
const clientVersion = clientEnvironment.LYNESS_CLIENT_VERSION
if (clientVersion === undefined) throw new Error('desktop build: the client environment carries no LYNESS_CLIENT_VERSION')

/** Inline the one public build value the Node entry reads; every other variable stays a runtime lookup. */
const clientVersionDefine = { 'process.env.LYNESS_CLIENT_VERSION': JSON.stringify(clientVersion) }
