...
Function un.CleanData
  ...
  ${IfNot} ${Errors}
    Return
  ${EndIf}
  ; Only a LYNESS_HOME published as a Windows environment variable is visible here.
  ReadEnvStr $UnHome LYNESS_HOME
  ClearErrors
  ...
  !endif
  StrCpy $UnTarget "$LOCALAPPDATA\${LYNESS_UPDATER_CACHE_NAME}"
  Call un.RemoveData
FunctionEnd
