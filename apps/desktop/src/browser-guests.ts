...
import type { DesktopBrowserLeaseId, DesktopBrowserOpenRequest, DesktopBrowserReservation } from '@lyness/lyn-client-ui-sidebar-browser/types'
...
export class DesktopBrowserGuests {
  ...
  /** @param hostUrl - current authenticated LYN Host, which guests cannot request. */
  ...
  acquire(owner: WebContents, workspace: unknown): DesktopBrowserReservation {
    ...
    if (partition === undefined) {
      partition = `lyn-sidebar-browser-${randomUUID()}`
