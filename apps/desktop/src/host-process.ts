...
function isDesktopHostEvent(message: unknown): message is DesktopHostEvent {
  ...
  switch (candidate.type) {
    case 'ready':
      return candidate.protocolVersion === DESKTOP_HOST_PROTOCOL_VERSION && typeof candidate.lynVersion === 'string'
    case 'fatal':
...
export class DesktopHostProcess {
  ...
  async start(): Promise<DesktopHostReady> {
    ...
    const entry = join(this.projectDir, 'node_modules', '@lyness', 'lyn-desktop-host', 'lib', 'index.js')
    const child = spawn(this.node, [
      ...
      env: Object.fromEntries(Object.entries(process.env).filter(([name]) => (
        name !== 'NODE_OPTIONS' && !/^LYNESS_DESKTOP_/u.test(name) && !/^(?:npm|pnpm|corepack)_/iu.test(name)
      ))),
    ...
    if (!(requestPipe instanceof Writable) || !(responsePipe instanceof Readable)) {
      child.kill('SIGTERM')
      throw new Error('lyn desktop host did not expose the required byte pipes and IPC channel')
    }
    this.child = child
    ...
    responsePipe.once('end', () => {
      try {
        ...
        this.fail(new Error('lyn desktop host response pipe ended'))
      } catch (error) {
        this.fail(errorOf(error, 'lyn desktop host response pipe failed'))
      }
    })
    ...
    child.on('message', (message: unknown) => {
      if (!isDesktopHostEvent(message)) {
        this.fail(new Error('lyn desktop host sent an invalid IPC event'))
        child.kill('SIGTERM')
        return
      }
    ...
    this.exitPromise = new Promise<void>((resolve) => {
      child.once('exit', (code) => {
        ...
        if (code !== 0 && code !== null) this.fail(new Error(`lyn desktop host exited with ${String(code)}${suffix}`))
        else this.fail(new Error(`lyn desktop host stopped${suffix}`))
        resolve()
      })
    })
  ...
  /** Forward one `lyn-app://app` request to the child without buffering its body. */
  async fetch(request: Request): Promise<Response> {
    await this.start()
    const child = this.child
    if (child === undefined || !child.connected || this.requestPipe === undefined) {
      throw new Error('lyn desktop host is unavailable')
    }
    if (this.nextStreamId > 0xffff_ffff) throw new Error('lyn desktop host exhausted its request stream ids')
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      const abort = (): void => {
        ...
        this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((pipeError: unknown) => {
          this.fail(errorOf(pipeError, 'lyn desktop request pipe failed'))
        })
      ...
      this.pumpRequest(streamId, request, hasBody).catch((error: unknown) => {
        this.failPending(streamId, errorOf(error, 'lyn desktop request upload failed'))
      })
    })
  }
  ...
  async stop(): Promise<void> {
    ...
    if (!await exitsWithin(exited, 5_000)) {
      child.kill('SIGKILL')
      if (!await exitsWithin(exited, 5_000)) {
        throw new Error('lyn desktop host did not exit after SIGKILL')
      }
    }
  ...
  private async pumpRequest(streamId: number, request: Request, hasBody: boolean): Promise<void> {
    ...
    if (body === null) throw new Error('lyn desktop request body disappeared before upload')
  ...
  private enqueueRequestFrame(frame: Buffer): Promise<void> {
    const write = this.requestWriteTail.then(async () => {
      ...
      if (pipe === undefined || pipe.destroyed) throw new Error('lyn desktop host request pipe is unavailable')
  ...
  private send(message: DesktopHostCommand): void {
    const child = this.child
    if (child === undefined || !child.connected) throw new Error('lyn desktop host IPC is unavailable')
    child.send(message)
  }
  ...
  private handleResponseFrame(frame: DesktopHostResponseFrame): void {
    ...
    if (pending === undefined) {
      if (frame.streamId >= this.nextStreamId) {
        throw new Error(`lyn desktop host responded for unknown stream ${String(frame.streamId)}`)
      }
      return
    }
    switch (frame.type) {
      case 'start': {
        if (pending.responseStarted) throw new Error(`lyn desktop host started stream ${String(frame.streamId)} twice`)
      ...
      case 'data': {
        ...
        if (!pending.responseStarted || controller === undefined) {
          throw new Error(`lyn desktop host sent body data before a body start for stream ${String(frame.streamId)}`)
        }
      ...
      case 'end':
        if (!pending.responseStarted) {
          throw new Error(`lyn desktop host ended stream ${String(frame.streamId)} before its response start`)
        }
  ...
  private cancelResponse(streamId: number, reason: unknown): void {
    ...
    this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((error: unknown) => {
      this.fail(errorOf(error, 'lyn desktop request pipe failed'))
    })
    this.finishPending(streamId, false)
  }
