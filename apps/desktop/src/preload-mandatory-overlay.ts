...
export function installMandatoryUpdateOverlay(): void {
  ...
  const publish = (): void => { port?.postMessage({ type: 'lyn-mandatory-state', state }) }
  ...
  const render = (): void => {
    ...
    if (frame === undefined) {
      ...
      frame.src = 'lyn-app://shell/mandatory-update.html'
      shadow.append(frame)
      ...
      frame.addEventListener('load', () => {
        ...
        frame?.contentWindow?.postMessage({ type: 'lyn-mandatory-connect' }, 'lyn-app://shell', [channel.port2])
        publish()
        frame?.focus()
      })
    }
  ...
  const message = (event: MessageEvent<unknown>): void => {
    ...
    if (value.type !== 'lyn-mandatory-action' || !('id' in value) || !Number.isSafeInteger(value.id)
    ...
    void ipcRenderer.invoke(MANDATORY_IPC.action, value.action, value.version, value.revision).then(
      () => { target.postMessage({ type: 'lyn-mandatory-result', id: value.id, ok: true }) },
      () => { target.postMessage({ type: 'lyn-mandatory-result', id: value.id, ok: false }) },
    )
  }
