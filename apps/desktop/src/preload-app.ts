...
import type { DesktopShortcutInput, ShortcutConfigSnapshot, ShortcutSaveResult } from '@lyness/lyn-client-shortcuts/protocol'
import { contextBridge, ipcRenderer, webUtils } from 'electron'
import { DESKTOP_IPC, SCHEME, type LynDesktopProductApi, type DesktopUpdatePresentation } from './ipc.ts'
...
function createProductApi(): LynDesktopProductApi {
  return {
    protocolVersion: 1,
...
if (location.protocol === `${SCHEME}:` && location.hostname === 'app') {
  contextBridge.exposeInMainWorld('lynOnboarding', {
  ...
  contextBridge.exposeInMainWorld('__LYNESS_DIRECTORY_PICKER__', {
  ...
  contextBridge.exposeInMainWorld('__LYNESS_HOST_PATHS__', {
    pathFor: (file: File) => webUtils.getPathForFile(file),
  })
  contextBridge.exposeInMainWorld('lynDesktopBoot', {
  ...
  contextBridge.exposeInMainWorld('lynPlatform', {
...
contextBridge.exposeInMainWorld('lynDesktop', location.protocol === `${SCHEME}:` && location.hostname === 'app' && process.isMainFrame ? createProductApi() : { protocolVersion: 1 })

if (location.protocol === `${SCHEME}:` && location.hostname === 'app') {
  contextBridge.exposeInMainWorld('__LYNESS_LOCALE__', {
