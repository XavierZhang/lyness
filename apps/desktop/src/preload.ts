...
import { DESKTOP_IPC, type LynDesktopApi, type DesktopUpdateState } from './ipc.ts'

const api: LynDesktopApi = {
  protocolVersion: 1,
  locale: () => ipcRenderer.invoke(DESKTOP_IPC.localeGet) as Promise<ReturnType<LynDesktopApi['locale']> extends Promise<infer T> ? T : never>,
  plugins: {
    list: () => ipcRenderer.invoke(DESKTOP_IPC.pluginsList) as Promise<ReturnType<LynDesktopApi['plugins']['list']> extends Promise<infer T> ? T : never>,
...
contextBridge.exposeInMainWorld('lynDesktop', api)
