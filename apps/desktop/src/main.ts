...
function runtimeResources(): RuntimeResources {
  ...
  const pnpm = (development ? process.env.LYNESS_DESKTOP_PNPM_ENTRY : undefined)
    ?? (development ? join(app.getAppPath(), 'node_modules', 'pnpm', 'bin', 'pnpm.mjs')
      : join(process.resourcesPath, 'runtime', 'pnpm', 'bin', 'pnpm.mjs'))
  const lyn = (development ? process.env.LYNESS_DESKTOP_LYNESS_DIR : undefined)
    ?? (development ? join(app.getAppPath(), '.desktop-build', 'development', 'project') : join(app.getAppPath(), 'lyn'))
  return { node, nodeBin, pnpm, lyn }
}

function developmentPrimaryRuntime(): string {
  const directory = process.env.LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR
  if (directory === undefined || directory === '') {
    throw new Error('lyn desktop: LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR is required for an unpackaged launch')
  }
  return directory
}

function developmentHostInspectPort(enabled: boolean): number | undefined {
  const configured = process.env.LYNESS_DESKTOP_HOST_INSPECT_PORT
  if (!enabled || configured === undefined || configured === '') return undefined
  const port = Number(configured)
  if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) {
    throw new Error('lyn desktop: LYNESS_DESKTOP_HOST_INSPECT_PORT must be an integer from 1 through 65535')
  }
  return port
}
...
async function main(): Promise<void> {
  ...
  const journalDirectory = process.env.LYNESS_DESKTOP_UPDATE_JOURNAL_DIR
  ...
  const backend = new DesktopBackendController((onFailure) => {
    ...
    const host = new DesktopHostProcess(resources.node, resources.lyn, activeProject,
  ...
  protocol.handle(SCHEME, (request) => {
    ...
    if (url.hostname === 'app') {
      if (url.pathname === '/' || url.pathname === '/index.html' || url.pathname.startsWith('/assets/')
        ...
        return serveWebDocument(request, join(resources.lyn, 'node_modules', '@lyness', 'lyn-web-frontend', 'dist'))
      }
  ...
  ipcMain.handle(DESKTOP_IPC.bootFailed, (event, message: unknown) => {
    ...
    if (event.sender !== mainWindow?.webContents || event.senderFrame !== event.sender.mainFrame) {
      throw new Error('lyn desktop: rejected startup failure from a non-primary frame')
    }
    if (typeof message !== 'string') throw new Error('lyn desktop: startup failure must be text')
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders({ urls: ['ws://127.0.0.1/*'] }, (details, callback) => {
    ...
    if (headers.origin !== 'lyn-app://app') { callback({ cancel: true }); return }
  ...
  const assertMainApplication = (event: IpcMainInvokeEvent): BrowserWindow => {
    ...
    if (owner === undefined || event.sender !== owner.webContents || event.senderFrame !== owner.webContents.mainFrame
      || !event.senderFrame.url.startsWith('lyn-app://app/')) throw new Error('Rejected Platform command')
    return owner
  }
  ...
  const enterWorkspace = async ({ activate = true }: { activate?: boolean } = {}): Promise<void> => {
    ...
    if (activate && development && process.env.LYNESS_DESKTOP_OPEN_DEVTOOLS !== '0') {
  ...
  if (app.isPackaged || process.env.LYNESS_DESKTOP_DEV_APP === '1') app.setAsDefaultProtocolClient('lyn')
  app.on('open-url', (event, url) => {
    event.preventDefault()
    if (url === 'lyn://open' || url === 'lyn://open/') focusPrimaryWindow()
  })
  ...
  const developmentPolicy = app.isPackaged ? undefined : process.env.LYNESS_DESKTOP_MANDATORY_UPDATE_CONFIG
  const policyInput: unknown = app.isPackaged
    ? ('lynMandatoryUpdatePolicy' in manifest ? manifest.lynMandatoryUpdatePolicy : undefined)
  ...
  if (policyConfig !== undefined) {
    ...
    mandatoryPolicy = new DesktopMandatoryUpdatePolicy(policyConfig, {
      ...
      bundledLynVersion: app.isPackaged ? readDesktopRuntime(resources.lyn).release.version : app.getVersion(),
    }, (state) => {
  ...
  if (window !== undefined && development && process.env.LYNESS_DESKTOP_OPEN_DEVTOOLS !== '0') {
    window.webContents.openDevTools({ mode: 'detach' })
  }
...
if (ownsDesktopInstance) void app.whenReady().then(main).catch(async (error: unknown) => {
  ...
  console.error(error)
  const diagnosticFile = process.env.LYNESS_DESKTOP_DIAGNOSTIC_FILE
