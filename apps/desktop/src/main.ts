...
function runtimeResources(): RuntimeResources {
  ...
  const node = (development ? process.env.LYNESS_DESKTOP_NODE_BINARY : undefined)
  ...
  const seed = (development ? process.env.LYNESS_DESKTOP_SEED_DIR : undefined) ?? join(process.resourcesPath, 'seed')
  return { node, pnpm, seed }
}

function developmentProject(): string | undefined {
  const configured = process.env.LYNESS_DESKTOP_DEV_PROJECT_DIR
  if (configured === undefined || configured === '') return undefined
  if (app.isPackaged) throw new Error('lyn desktop: development project override is unavailable in packaged applications')
...
async function main(): Promise<void> {
  ...
  const mutate = async (event: IpcMainInvokeEvent, mutation: Parameters<DesktopProjectManager['mutate']>[0]): Promise<void> => {
    ...
    if (development !== undefined) {
      throw new Error('lyn desktop: plugin package changes require a packaged application')
    }
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsAdd, (event, spec: unknown) => {
    if (typeof spec !== 'string') throw new Error('lyn desktop: plugin spec must be a string')
    return mutate(event, { type: 'plugin-add', spec })
  })
  ipcMain.handle(DESKTOP_IPC.pluginsRemove, (event, name: unknown) => {
    if (typeof name !== 'string') throw new Error('lyn desktop: plugin name must be a string')
  ...
  ipcMain.handle(DESKTOP_IPC.pluginsUpdate, (event, name: unknown, version: unknown) => {
    if (typeof name !== 'string' || typeof version !== 'string') {
      throw new Error('lyn desktop: plugin name and version must be strings')
    }
  ...
  if (development !== undefined && process.env.LYNESS_DESKTOP_OPEN_DEVTOOLS !== '0') {
