...
export async function writeCrashReport(directory: string, input: CrashReportInput): Promise<string | undefined> {
  ...
  try {
    ...
    console.error('lyn desktop: crash report could not be written', path, error)
    return undefined
  }
}
...
export async function pruneCrashReports(directory: string, retained = CRASH_REPORTS_RETAINED): Promise<void> {
  ...
  try {
    ...
    console.error('lyn desktop: crash report directory could not be listed', directory, error)
    return
  }
  ...
  for (const name of excess) {
    try {
      ...
      console.error('lyn desktop: stale crash report could not be removed', join(directory, name), error)
    }
  }
}
