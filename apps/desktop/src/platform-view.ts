...
import { mergePlatformCookies, platformClientHeaders, type PlatformSession } from '@lyness/lyn-deepseek-account'
...
export class DesktopPlatformView {
  ...
  async open(owner: PlatformOwner, page: 'usage' | 'top-up', bounds: PlatformBounds): Promise<void> {
    ...
    const partition = account.userId === null ? `lyn-platform-${randomUUID()}`
      : `persist:lyn-platform-${createHash('sha256').update(JSON.stringify([account.origin, account.userId])).digest('hex')}`
    ...
    const view = new WebContentsView({ webPreferences: {
      ...
      additionalArguments: [`--lyn-platform-origin=${account.origin}`],
