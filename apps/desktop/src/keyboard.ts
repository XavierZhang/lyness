...
import { bindingKey, effectiveShortcuts, presentBinding, parseShortcutDefinitions, parseShortcutEdit } from '@lyness/lyn-client-shortcuts/protocol'
import type { NormalizedBinding, ShortcutConfigSnapshot, ShortcutDefinition, ShortcutPlatform, ShortcutRevision } from '@lyness/lyn-client-shortcuts/protocol'
import type { DesktopBrowserLeaseId } from '@lyness/lyn-client-ui-sidebar-browser/types'
...
export function installDesktopShortcuts(
  ...
  const publish = (snapshot: ShortcutConfigSnapshot): void => {
    ...
    const window = getWindow()
    if (window !== undefined && !window.isDestroyed() && window.webContents.mainFrame.url.startsWith('lyn-app://app/')) {
  ...
  return {
    ...
    fileMenu: (labels) => {
      ...
      return { label: labels.fileMenu, submenu: [{
        id: 'lyn-page-close', label: labels.closePage, enabled: revision !== undefined,
