...
export function resolveDesktopUpdateScheduleConfig(env: NodeJS.ProcessEnv): DesktopUpdateScheduleConfig {
  ...
  const intervalMs = duration('LYNESS_DESKTOP_UPDATE_CHECK_INTERVAL_MS', 600_000)
  const maxBackoffMs = duration('LYNESS_DESKTOP_UPDATE_CHECK_MAX_BACKOFF_MS', Math.max(intervalMs, 3_600_000))
  const jitter = Number(env.LYNESS_DESKTOP_UPDATE_CHECK_JITTER ?? 0.2)
