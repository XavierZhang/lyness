/** Signed local npm package set that supplies the Desktop-owned lyn runtime and private Host. */
...
/** Complete union of the first-party package closures rooted at lyn and its private Desktop Host. */
...
const RELEASE_PACKAGES = [LYNESS_PACKAGE, DESKTOP_HOST_PACKAGE] as const
 ...
 * @param expectedReleaseVersion - Required lyn and Desktop Host version when validating one release.
...
/** Return the local direct dependency spec for the lyn package. */
export function desktopLynPackageSpec(packageSet: DesktopCorePackageSet): string {
  const record = packageSet.packages.find(entry => entry.name === LYNESS_PACKAGE)
  if (record === undefined) throw new Error(`desktop package set: missing ${LYNESS_PACKAGE}`)
 ...
 * @param expectedReleaseVersion - Exact lyn and Desktop Host version bound to Electron.
