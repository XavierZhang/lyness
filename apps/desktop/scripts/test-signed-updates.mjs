...
let timedOut = false
const timeoutMs = Number(process.env.LYNESS_SIGNED_UPDATE_TEST_TIMEOUT_MS ?? 180_000)
assert.ok(Number.isSafeInteger(timeoutMs) && timeoutMs >= 1000 && timeoutMs <= 180_000,
  'LYNESS_SIGNED_UPDATE_TEST_TIMEOUT_MS must be an integer from 1000 to 180000')
let termination = Promise.resolve()
let terminationError
let launchError
const startedAt = new Date().toISOString()
const child = spawn(electron, [fileURLToPath(new URL('../tests/fixtures/signed-updates.mjs', import.meta.url))], {
  env: { ...environment, LYNESS_SIGNED_UPDATE_TEST_ROOT: root,
    LYNESS_SIGNED_UPDATE_CERTIFICATE: paths[0], LYNESS_SIGNED_UPDATE_SIGNED: paths[1], LYNESS_SIGNED_UPDATE_UNSIGNED: paths[2],
    LYNESS_SIGNED_UPDATE_OLD: paths[3] ?? '' },
