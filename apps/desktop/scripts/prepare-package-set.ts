/** Select and copy the local npm tarball closures that supply Desktop lyn and its private Host. */
...
const ROOT_PACKAGES = [LYNESS_PACKAGE, DESKTOP_HOST_PACKAGE] as const
...
/**
 * Select the complete available first-party dependency closures rooted at lyn and its private Host.
  ...
  const visit = (name: string): void => {
    ...
    for (const section of REQUIRED_DEPENDENCY_SECTIONS) {
      for (const dependency of dependencyNames(packed.manifest, section)) {
        ...
        else if (dependency.startsWith('@lyness/')) {
