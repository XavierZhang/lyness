...
export async function verifyInstalledUpdatePackageContent(manifest: string, version: string, payload: string, publisher: string) {
  ...
  const policy = resolveDesktopPolicyConfig(metadata.lynMandatoryUpdatePolicy)
  if (metadata.name !== `lyn-update-test-${run.id}` || metadata.version !== version
    || metadata.lynDesktopAppId !== run.appId || metadata.main !== 'qualification-bootstrap.mjs'
  ...
  const inspect = (node: AsarNode, path = ''): void => {
    ...
    if (node.link !== undefined || (node.unpacked === true && !path.startsWith('lyn/'))) {
    ...
    } else if (!path.startsWith('lyn/') && !expectedPaths.has(path)) {
      throw new Error('installed update: application archive contains additional files')
    }
  }
  inspect(archive.header)
  if (archive.header.files?.lyn?.files === undefined) throw new Error('installed update: application archive lacks the lyn runtime')
  ...
  if (update.provider !== 'generic' || update.url !== url || update.channel !== 'nightly'
    || update.updaterCacheDirName !== `lyn-update-test-${run.id}-updater`
  ...
  const prepared = await verifyDesktopRuntime(join(run.root, version, 'lyn'), version, { platform: 'win32', arch: 'x64' })
  const runtime = readDesktopRuntime(join(extracted, 'lyn'))
  ...
  const actualFiles = inventoryDesktopRuntime(join(extracted, 'lyn'))
  ...
  for (const [index, file] of prepared.files.entries()) {
    ...
    const transformed = file.path.startsWith('node_modules/') && file.path.endsWith('/package.json')
      ? await transform(runtimePath(join(run.root, version, 'lyn'), file.path)) : null
  ...
  return { version, appId: run.appId, applicationFiles: inventory.files.length, dependencies, dependenciesFrozen: false,
    ...
    resignedExecutables: runtime.files.filter(file => file.path.endsWith('.exe')).map((file) => {
      if (archive.getFile(join('lyn', file.path), false).unpacked !== true) {
        throw new Error('installed update: executable runtime file must be outside ASAR')
      }
      return join(payload, 'resources/app.asar.unpacked/lyn', file.path)
    }) }
}
