...
const NSIS_BOOTSTRAP_PATCH = Symbol.for('@lyness/lyn-desktop/nsis-bootstrap-signing')
...
const WINDOWS_SIGNING_ENVIRONMENT_PREFIX = 'LYNESS_DESKTOP_WINDOWS_'
...
function resolveTokenIdentity(input) {
  ...
  if (!keyContainer) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER must contain the SafeNet private-key container name')
  }
  if (/["\r\n]/u.test(keyContainer)) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER cannot contain quotes or line breaks')
  }
  const tokenPin = input.tokenPin
  if (tokenPin === undefined || tokenPin.length === 0) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_TOKEN_PIN must contain the SafeNet Token Password')
  }
  if (/[\]"\r\n]/u.test(tokenPin)) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_TOKEN_PIN cannot contain "]", quotes, or line breaks because the SafeNet key-container syntax uses them as delimiters')
  }
...
function resolveCertificateFile(value) {
  ...
  if (!candidate) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_CER_FILE must identify the public X.509 leaf certificate file')
  }
  let path
  let certificate
  try {
...
function resolveSignTool(value) {
  const candidate = value?.trim()
  if (!candidate) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_SIGNTOOL must identify the SafeNet-compatible SignTool executable')
  }
  let path
  try {
    path = realpathSync(candidate)
    ...
    throw new Error(`LYNESS_DESKTOP_WINDOWS_SIGNTOOL is missing or is not an executable file: ${candidate}`)
  }
  return path
}
...
export function buildWindowsSigningEnvironment(environment, input) {
  return {
    ...
    LYNESS_DESKTOP_WINDOWS_SIGNTOOL: input.signTool,
    LYNESS_DESKTOP_WINDOWS_CER_FILE: input.certificateFile,
    LYNESS_DESKTOP_WINDOWS_TOKEN_PIN: input.tokenPin,
    LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER: input.keyContainer,
    LYNESS_DESKTOP_WINDOWS_SIGN_TARGET: input.path,
    LYNESS_DESKTOP_WINDOWS_SIGN_APPEND: input.isNest ? '1' : '',
  }
}
...
export function createWindowsTokenSigner(options) {
  ...
  return (configuration) => {
    pending = pending.then(async () => {
      ...
      const runDirectory = options.runDirectory ?? process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR
