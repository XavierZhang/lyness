...
export async function createInstalledUpdateBuilderConfig(manifest: string, version: string, environment: NodeJS.ProcessEnv) {
  ...
  if (environment.LYNESS_DESKTOP_AUTO_UPDATE_ENV !== 'test' || environment.LYNESS_DESKTOP_UNSIGNED === '1'
  ...
  await verifyDesktopRuntime(lyn, version, { platform: 'win32', arch: 'x64' })
  const config = createElectronBuilderConfig({ ...environment, LYNESS_DESKTOP_APP_ID: run.appId,
    LYNESS_DESKTOP_TARGET_PLATFORM: 'win32', LYNESS_DESKTOP_TARGET_ARCH: 'x64', LYNESS_DESKTOP_UNSIGNED: '0' }, 'win32', 'x64', lyn, version)
  return { ...config,
    productName: run.productName,
    directories: { ...config.directories, output: join(run.root, version, 'installer') },
    extraMetadata: { ...config.extraMetadata, name: `lyn-update-test-${run.id}`, version, main: 'qualification-bootstrap.mjs' },
    files: [
      { from: application, to: '.', filter: ['lib/*.js', 'lib/*.cjs', 'renderer/**/*', 'qualification-bootstrap.mjs', 'installed-update-identity.mjs'] },
      'package.json',
      { from: lyn, to: 'lyn', filter: ['**/*'] },
      { from: join(lyn, 'node_modules'), to: 'lyn/node_modules', filter: ['**/*'] },
    ],
