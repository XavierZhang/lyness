...
export function resolveWindowsSignatureCacheDirectory(environment) {
  const path = environment.LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_DIR
    ?? win32.join(homedir(), '.lyn-desktop-signing', 'signature-cache', 'v1')
  if (!/^[a-z]:[\\/]/iu.test(path) || path.slice(2).includes(':') || /[\0\r\n]/u.test(path)) {
    throw new Error('LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_DIR must be an absolute local drive directory')
  }
...
export async function prepareWindowsSignatureCacheDirectory(path, existingSource = false) {
  ...
  if ((await realpath(path)).toLowerCase() !== resolve(path).toLowerCase()) {
    throw new Error('Signature cache directory is redirected; configure LYNESS_DESKTOP_WINDOWS_SIGNATURE_CACHE_DIR outside redirected storage')
  }
}
