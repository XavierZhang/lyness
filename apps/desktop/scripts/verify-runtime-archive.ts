...
/**
 * Compare the complete archived lyn tree with the sealed preparation inventory.
...
export async function verifyRuntimeArchive(archivePath: string, expected: DesktopRuntimeDescriptor): Promise<void> {
  ...
  const descriptor = await archive.readFile(join('lyn', 'desktop-runtime.json'))
  ...
  async function visit(node: Node, path: string): Promise<void> {
    ...
    const name = join('lyn', ...path.split('/'))
  ...
  await visit(archive.getFile('lyn', false), '')
  const entries = await readdir(join(`${archivePath}.unpacked`, 'lyn'), { recursive: true, withFileTypes: true })
