...
try {
    ...
    foreach ($name in @($startInfo.EnvironmentVariables.Keys)) {
        ...
        if ($name -match 'KEY|SECRET|TOKEN|PASSWORD|^NODE_OPTIONS$|^LYNESS_DESKTOP_WINDOWS_|^APPLE_|^CSC_') {
    ...
    $startInfo.EnvironmentVariables['LYNESS_DESKTOP_AUTO_UPDATE_ENV'] = $Environment
    if ($Upload) {
