...
$scratch = [System.IO.Directory]::CreateTempSubdirectory('lyn-directory-smoke-').FullName
...
try {
  ...
  foreach ($mode in @('first', 'upgrade', 'locked', 'missing-stage', 'broken', 'cancelled')) {
    ...
    $compileArgs = @('/V2', "/DOUTPUT_FILE=$probe", "/DPAYLOAD_FILE=$caseArchive", "/DTARGET_DIR=$target", "/DLYNESS_SEVENZIP_PATH=$SevenZip")
