...
export function resolveDesktopBuildTarget(
  ...
  const platform = env.LYNESS_DESKTOP_TARGET_PLATFORM ?? env.npm_config_platform ?? hostPlatform
  const arch = env.LYNESS_DESKTOP_TARGET_ARCH ?? env.npm_config_arch
 ...
 * @returns {{ root: string, artifacts: string, unsignedArtifacts: string, runtime: string, packageSet: string, lyn: string, lynPnpm: string, electron: string, packedLyn: string, packedVendor: string, packedLandlock: string, downloads: string }} Target paths plus the shared immutable download cache.
 */
export function desktopTargetBuildPaths(target) {
  ...
  return {
    ...
    lyn: join(root, 'lyn'),
    lynPnpm: join(root, 'lyn-pnpm'),
    electron: join(root, 'electron'),
    packedLyn: join(packed, 'lyn'),
