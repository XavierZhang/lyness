...
type FlockEntry = typeof import('@lyness/node-addon-system/flock')
...
export interface FlockEntryLoaderSteps {
  /** Import `@lyness/node-addon-system/flock`; rejects with `ERR_MODULE_NOT_FOUND` while its `lib/` is unbuilt. */
...
function hostAddonBuilt(): boolean {
  ...
  const entryManifest = createRequire(import.meta.url).resolve('@lyness/node-addon-system/package.json')
  let prebuilds: string
  try {
    prebuilds = createRequire(entryManifest).resolve(`@lyness/node-addon-system-${platform}-${arch}/prebuilds.json`)
  } catch (error) {
...
/**
 * Load `@lyness/node-addon-system/flock`, building the host addon and the entry's JavaScript through pnpm when either is missing.
...
export const loadFlockEntry = createFlockEntryLoader({
  importEntry: () => import('@lyness/node-addon-system/flock'),
  hostAddonBuilt,
