 ...
 * @param {{ version?: string, lynDesktopAppId?: string }} metadata Installed package metadata.
...
export function configureInstalledUpdateIdentity(app, run, metadata, environment) {
  if (!/^[a-f0-9]{24}$/u.test(run.id) || run.versions.length !== 2
    || metadata.lynDesktopAppId !== `com.deepseek.lyn.qualification.q${run.id}`
  ...
  const root = join(app.getPath('appData'), 'lyn-update-qualification', run.id)
  const paths = { root, userData: join(root, 'user-data'), harnessHome: join(root, 'lyn-home'), journals: join(root, 'journals') }
  ...
  environment.LYNESS_HOME = paths.harnessHome
  environment.LYNESS_DESKTOP_UPDATE_JOURNAL_DIR = paths.journals
  return paths
}
