...
!macro customInstallerExtract Archive
  ...
  System::Store /NOUNLOAD "S"
  System::Call /NOUNLOAD '$PLUGINSDIR\window-frame.dll::InstallerExtract(p $HWNDPARENT, w "$PLUGINSDIR\lyn-7za.exe", w "${Archive}", w "$INSTDIR", w "$PLUGINSDIR\extract.log") i.s ?c'
  System::Store "L"
  Pop $R0
...
; The directory smoke fixture predefines LYNESS_INSTALLER_LOG_DIR to keep reports inside its scratch tree.
!ifndef LYNESS_INSTALLER_LOG_DIR
  !define LYNESS_INSTALLER_LOG_DIR "$LOCALAPPDATA\${LYNESS_UPDATER_CACHE_NAME}\installer-logs"
!endif
