...
export function resolveMacOSPackageSettings(environment) {
  const concurrency = environment.LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY ?? '4'
  if (!/^[1-9]\d*$/u.test(concurrency) || !Number.isSafeInteger(Number(concurrency))) {
    throw new Error('desktop package: LYNESS_DESKTOP_MACOS_PACK_CONCURRENCY must be a positive integer')
  }
  return {
    packConcurrency: Number(concurrency),
    downloadProxy: proxy(environment, 'LYNESS_DESKTOP_MACOS_DOWNLOAD_PROXY', ['http:', 'https:']),
    notarizationProxy: proxy(environment, 'LYNESS_DESKTOP_MACOS_NOTARIZATION_PROXY', ['http:']),
  }
}
