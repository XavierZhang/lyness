...
export function createInstalledUpdateCos(): InstalledUpdatePublicationStore {
  ...
  if (environment.LYNESS_DESKTOP_AUTO_UPDATE_ENV !== 'test' || environment.DOWNLOAD_TEST_ORIGIN !== ORIGIN
  ...
  const keyAllowed = (key: string): void => {
    if (!/^lyn-desk\/(?:bin|feeds)\/qualification\/[a-f0-9]{24}\/win-x64\/[A-Za-z0-9][A-Za-z0-9._-]*$/u.test(key)) {
