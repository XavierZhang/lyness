...
async function main(): Promise<void> {
  ...
  const runDirectory = process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR
  ...
  const certificateFile = process.env.LYNESS_DESKTOP_WINDOWS_CER_FILE
  ...
  try {
    await withWindowsSigningStage({ stage: process.argv.includes('--lyn') ? 'lyn-runtime' : 'primary-runtime', record }, async () => {
      const paths = resolveDesktopTargetBuildPaths()
      const sign = createWindowsTokenSigner({ certificateFile, signTool: process.env.LYNESS_DESKTOP_WINDOWS_SIGNTOOL,
        keyContainer: process.env.LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER, tokenPin: process.env.LYNESS_DESKTOP_WINDOWS_TOKEN_PIN })
      const identity = await signatureCacheIdentity([
        await realpath(certificateFile), await realpath(process.env.LYNESS_DESKTOP_WINDOWS_SIGNTOOL!),
      ...
      try {
        if (process.argv.includes('--lyn')) {
          const version = JSON.parse(await readFile(join(paths.lyn, 'package.json'), 'utf8')).version as string
          await signWindowsDesktopRuntime(paths.lyn, version, { ...options,
            smoke: descriptor => smokePreparedRuntime(paths.lyn, join(paths.electron, 'electron.exe'), paths.runtime, descriptor),
          })
        } else {
