 ...
 * @param {string} sourceRoot Verified prepared lyn directory; its files remain unchanged.
 ...
 * @param {string} sourceRoot Original signed and sealed lyn directory.
...
export async function verifyWindowsAsarUnpack(sourceRoot, resourcesDir, files) {
  ...
  for (const file of files) {
    const entry = archive.getFile(join('lyn', file), false)
    if (entry.unpacked !== true || entry.link !== undefined) {
      throw new Error(`Windows ASAR: PE must be unpacked: ${file}`)
    }
    const copied = join(resourcesDir, 'app.asar.unpacked', 'lyn', file)
