...
try {
  ...
  await writeFile(join(project, 'desktop-runtime.json'), JSON.stringify({
    ...
    sharedPackages: ['@lyness/lyn', '@lyness/lyn-desktop-host']
  ...
  const child = spawn(electron, [fileURLToPath(new URL('../tests/fixtures/workspace-updates.mjs', import.meta.url)), '--lang=zh-CN',
    ...
    cwd: root, env: { ...environment, LYNESS_HOME: join(root, 'home'), USERPROFILE: root, HOME: root,
      TEMP: root, TMP: root, TMPDIR: root, LYNESS_WORKSPACE_UPDATE_ROOT: root, LYNESS_WORKSPACE_UPDATE_TOKEN: randomUUID(),
      LYNESS_DESKTOP_PRIMARY_RUNTIME_DIR: developmentRuntimeDirectory(), LYNESS_DESKTOP_OPEN_DEVTOOLS: '0' },
