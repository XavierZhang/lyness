...
export async function withMacOSSigningKeychain(environment, action, run = execute) {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'lyn-macos-signing-'))
  ...
  try {
    ...
    run('/usr/bin/codesign', ['--force', '--sign', `Developer ID Application: ${environment.LYNESS_DESKTOP_MACOS_SIGNING_IDENTITY}`, '--keychain', keychain, '--timestamp', '--options', 'runtime', probe])
    run('/usr/bin/codesign', ['--verify', '--strict', probe])
    const childEnvironment = { ...environment, CSC_KEYCHAIN: keychain, LYNESS_DESKTOP_MACOS_SIGNING_PROBE: probe }
