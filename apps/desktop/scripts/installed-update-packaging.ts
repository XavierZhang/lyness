...
const SIGNING_FIELDS = new Set(['LYNESS_DESKTOP_WINDOWS_CER_FILE', 'LYNESS_DESKTOP_WINDOWS_SIGNTOOL',
  'LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER', 'LYNESS_DESKTOP_WINDOWS_TOKEN_PIN'])
 ...
 */
export async function assertInstalledUpdateSigningClear(stateFile = join(homedir(), '.lyn-desktop-signing', 'attempt.json')): Promise<void> {
...
async function inputHashes(manifest: string, version: string, environment: NodeJS.ProcessEnv): Promise<object> {
  ...
  const paths = [manifest, join(run.root, 'application/result.json'), join(run.root, version, 'lyn/desktop-runtime.json'),
    ...
    environment.LYNESS_DESKTOP_WINDOWS_CER_FILE!, environment.LYNESS_DESKTOP_WINDOWS_SIGNTOOL!]
  const files = []
