...
export async function createInstalledUpdateRun(
  ...
  const run: InstalledUpdateRun = {
    ...
    appId: `com.deepseek.lyn.qualification.q${id}`, productName: `LYN Update Test ${id}`,
    environment: 'test', origin: 'https://download-test.deepseek.com', bucket: 'bj-toc-download-test-1320056602',
    feedKey: `lyn-desk/feeds/qualification/${id}/win-x64/nightly.yml`,
    binPrefix: `lyn-desk/bin/qualification/${id}/win-x64`,
  }
...
export async function readInstalledUpdateRun(path: string): Promise<InstalledUpdateRun> {
  ...
  if (row.schemaVersion !== 1 || typeof row.id !== 'string' || !/^[a-f0-9]{24}$/u.test(row.id)
    ...
    || row.appId !== `com.deepseek.lyn.qualification.q${row.id}` || row.productName !== `LYN Update Test ${row.id}`
    || row.feedKey !== `lyn-desk/feeds/qualification/${row.id}/win-x64/nightly.yml`
    || row.binPrefix !== `lyn-desk/bin/qualification/${row.id}/win-x64`
...
export async function collectInstalledUpdateJournals(manifest: string, directory: string): Promise<string> {
  ...
  if (!resolve(directory).replaceAll('\\', '/').endsWith(`/lyn-update-qualification/${run.id}/journals`)) {
