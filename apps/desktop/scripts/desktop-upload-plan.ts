...
/**
 * Validate the completed package record, lyn version, update metadata, hashes, and target files.
...
export async function createDesktopUploadPlan(
  ...
  const lynVersion = await manifestVersion(join(repositoryRoot, 'package.json'), 'lyn package')
  const desktopVersion = await manifestVersion(join(appRoot, 'package.json'), 'desktop package')
  if (lynVersion !== desktopVersion) {
    throw new Error(`desktop upload: desktop version ${desktopVersion} does not match current lyn version ${lynVersion}`)
  }
  ...
  if (buildRecord.schemaVersion !== 1
    ...
    || buildRecord.version !== lynVersion
    || buildRecord.environment !== update.environment
    || buildRecord.publicUrl !== update.publicUrl) {
    throw new Error(`desktop upload: ${targetName} package completion record does not match lyn ${lynVersion} and ${update.environment} update destination`)
  }

  const metadataFilename = desktopUpdateMetadataFilename(lynVersion, target.platform)
  ...
  if (metadataVersion !== lynVersion) {
    throw new Error(`desktop upload: ${metadataFilename} version ${metadataVersion} does not match current lyn version ${lynVersion}`)
  }
  ...
  const base = `lyness-${lynVersion}-${target.os}-${target.arch}`
