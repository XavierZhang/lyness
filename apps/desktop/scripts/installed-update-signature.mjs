...
export async function verifyInstalledUpdateSignature(file, publisher, directory) {
  ...
  const { stdout, stderr } = await promisify(execFile)('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command',
    '$ErrorActionPreference="Stop"; $s=Get-AuthenticodeSignature -LiteralPath $env:LYNESS_VERIFY_FILE; [pscustomobject]@{valid=($s.Status -eq "Valid");timestamped=($null -ne $s.TimeStamperCertificate);signer=$s.SignerCertificate.Thumbprint;timestamp=$s.TimeStamperCertificate.Thumbprint}|ConvertTo-Json -Compress'],
  { ...options, env: { ...environment, LYNESS_VERIFY_FILE: file } })
