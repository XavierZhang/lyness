...
export async function preflightWindowsSigning(options: SigningPreflightOptions): Promise<void> {
  ...
  const state = options.stateDirectory ?? join(homedir(), '.lyn-desktop-signing')
  ...
  const certificate = await readFile(environment.LYNESS_DESKTOP_WINDOWS_CER_FILE!)
  ...
  record({ type: 'signing-preflight-static', compiler,
    ...
    signToolSha256: createHash('sha256').update(await readFile(environment.LYNESS_DESKTOP_WINDOWS_SIGNTOOL!)).digest('hex'),
