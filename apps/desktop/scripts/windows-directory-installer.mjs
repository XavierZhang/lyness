...
const patched = Symbol.for('@lyness/lyn-desktop/directory-installer')
...
!macro extractUsing7za FILE
  !insertmacro lynExtractPayload "\${FILE}"
!macroend
...
!insertmacro lynStageApplication`)
  result = replaceOnce(result, '!insertmacro installApplicationFiles', 'Call lynPromoteDirectories\nIfErrors 0 +4\n  SetErrorLevel 2\n  MessageBox MB_OK|MB_ICONEXCLAMATION "$(appCannotBeClosed)" /SD IDOK\n  Quit')
...
export function installWindowsDirectoryInstaller() {
  ...
  prototype.computeFinalScript = async function (source, ...args) {
    ...
    return `!define LYNESS_UPDATER_CACHE_NAME "${this.packager.appInfo.updaterCacheDirName}"\n!define LYNESS_SEVENZIP_PATH "${tool}"\n!define LYNESS_SEVENZIP_LICENSE_DIR "${dirname(dirname(sourceTool))}"\n${await compute.call(this, adapted, ...args)}`
  }
}
