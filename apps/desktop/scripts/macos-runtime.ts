...
export async function signMacOSRuntime(
  ...
  const policy = cacheDirectory === undefined ? undefined : macOSCachePolicy(process.env.LYNESS_DESKTOP_MACOS_SIGNING_PROBE ?? '')
  let hits = 0
  let misses = 0
  let next = 0
  const workers = Array.from({ length: Math.min(4, files.length) }, async () => {
    for (;;) {
      ...
      const needsJit = path === 'dependencies/node/bin/node'
        || /^node_modules\/@lyness\/libreoffice-kit-darwin-(?:arm64|x64)\/bin\/libreoffice-kit$/u.test(path)
