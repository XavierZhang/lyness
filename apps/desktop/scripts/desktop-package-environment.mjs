...
const SHARED_SETTING = /^(?:LYNESS_DESKTOP_(?:APP_ID|AUTO_UPDATE_ENV|NPM_REGISTRY|MANDATORY_UPDATE_(?:CONFIG|(?:TEST|PROD)_ORIGIN))|DOWNLOAD_TEST_RELEASE_ID|DOWNLOAD_(?:TEST|PROD)_(?:ORIGIN|COS_BUCKET|COS_SECRET_ID|COS_SECRET_KEY))$/u
const WINDOWS_SETTING = /^LYNESS_DESKTOP_WINDOWS_(?:CER_FILE|SIGNTOOL|KEY_CONTAINER|TOKEN_PIN|SIGNATURE_CACHE_DIR|SIGNATURE_CACHE_CONCURRENCY)$/u
const MACOS_SETTING = /^(?:LYNESS_DESKTOP_MACOS_(?:SIGNING_IDENTITY|TEAM_ID|PACK_CONCURRENCY|DOWNLOAD_PROXY|NOTARIZATION_PROXY)|APPLE_(?:API_KEY|API_KEY_ID|API_ISSUER|ID|APP_SPECIFIC_PASSWORD|TEAM_ID|KEYCHAIN|KEYCHAIN_PROFILE)|CSC_(?:LINK|KEY_PASSWORD))$/u
const AMBIENT_RELEASE_SETTING = /^(?:LYNESS_DESKTOP_(?:APP_ID|AUTO_UPDATE_ENV|MANDATORY_UPDATE_.*|WINDOWS_.*|MACOS_.*)|APPLE_.*|(?:WIN_)?CSC_.*|DOWNLOAD_(?:TEST|PROD)_.*)$/iu
const FILE_SETTINGS = ['LYNESS_DESKTOP_WINDOWS_CER_FILE', 'LYNESS_DESKTOP_WINDOWS_SIGNTOOL', 'APPLE_API_KEY', 'APPLE_KEYCHAIN', 'CSC_LINK']
...
export function validateDesktopPackageEnvironment(environment, target, options = {}) {
  ...
  if (target.platform === 'win32') {
    if (!options.prepareOnly) createWindowsTokenSigner({
      certificateFile: environment.LYNESS_DESKTOP_WINDOWS_CER_FILE,
      signTool: environment.LYNESS_DESKTOP_WINDOWS_SIGNTOOL,
      tokenPin: environment.LYNESS_DESKTOP_WINDOWS_TOKEN_PIN,
      keyContainer: environment.LYNESS_DESKTOP_WINDOWS_KEY_CONTAINER,
    })
