...
const LYNESS_OUTPUT_ROOT = BUILD_PATHS.lyn
const BUILD_ROOT = mkdtempSync(join(tmpdir(), 'lyn-desktop-runtime-'))
const STORE_ROOT = join(BUILD_ROOT, 'store')
const RUNTIME_ROOT = BUILD_PATHS.runtime
const PNPM_BUILD_STATE = BUILD_PATHS.lynPnpm
...
function desktopRelease(): DesktopRelease {
  ...
  const lynVersion = manifestVersion(resolve(APP_ROOT, '..', '..', 'package.json'), 'root lyn package')
  if (version !== lynVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @lyness/lyn, found ${lynVersion}`)
  }
    ...
    const child = spawn(NODE, [
      ...
      env: {
        ...Object.fromEntries(Object.entries(process.env).filter(([name]) => (
          name !== 'NODE_OPTIONS' && name !== 'NODE_PATH' && !/^LYNESS_DESKTOP_/u.test(name) && !/^(?:npm|pnpm|corepack)_/iu.test(name)
        ))),
...
async function main(): Promise<void> {
  try {
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:reset', async () => {
      rmSync(LYNESS_OUTPUT_ROOT, { recursive: true, force: true })
      rmSync(PNPM_BUILD_STATE, { recursive: true, force: true })
      mkdirSync(STORE_ROOT, { recursive: true })
    })
    const release = desktopRelease()
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:stage-packages', async () => {
    ...
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:lockfile', () => runPnpm(['install', '--lockfile-only']))
    verifyDesktopCoreLockfile(
      readFileSync(join(BUILD_ROOT, 'pnpm-lock.yaml'), 'utf8'),
      readDesktopCorePackageSet(BUILD_ROOT, release.version),
    )
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:install', () => runPnpm(['install', '--prod', '--frozen-lockfile', '--trust-lockfile']))
    ...
    mkdirSync(LYNESS_OUTPUT_ROOT, { recursive: true })
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:materialize-modules', async () => {
      cpSync(modules, join(LYNESS_OUTPUT_ROOT, 'node_modules'), {
        recursive: true, dereference: true,
        filter: source => desktopRuntimeFileExclusion(relative(modules, source), target, officeEngine) === undefined,
      })
    })
    writeFileSync(join(LYNESS_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@lyness/lyn-desktop-runtime', private: true, version: release.version, type: 'module',
    ...
    for (const file of DESKTOP_HOST_RUNTIME_FILES) {
      if (!existsSync(join(LYNESS_OUTPUT_ROOT, 'node_modules', DESKTOP_HOST_PACKAGE, file))) {
        throw new Error(`desktop runtime: missing private Host file ${file}`)
      }
    }
    if (!existsSync(join(LYNESS_OUTPUT_ROOT, 'node_modules', '@lyness', `libreoffice-kit-${officeEngine}`, 'prebuilds.json'))) {
      throw new Error(`desktop runtime: missing required LibreOffice engine ${officeEngine}`)
    }
    if (process.platform === 'darwin') {
      await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'sign:lyn-native', () => signMacOSRuntime(LYNESS_OUTPUT_ROOT, resolveDesktopAppId(process.env), resolveMacOSSigningEnvironment(process.env), join(BUILD_PATHS.root, 'signature-cache')))
      await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'sign:primary-native', () => signMacOSRuntime(join(RUNTIME_ROOT, 'primary-runtime'), resolveDesktopAppId(process.env), resolveMacOSSigningEnvironment(process.env), join(BUILD_PATHS.root, 'signature-cache')))
    }
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:manifests', () => prepareRuntimeManifests(LYNESS_OUTPUT_ROOT))
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:primary-smoke', async () => smokePrimaryRuntime(join(RUNTIME_ROOT, 'primary-runtime')))
    await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:write-descriptor', async () => writeDesktopRuntime(LYNESS_OUTPUT_ROOT, release, packageSet.packages.map(entry => entry.name), target))
    const descriptor = await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:verify-before-smoke', () => verifyDesktopRuntime(LYNESS_OUTPUT_ROOT, release.version, target))
    if (!process.argv.includes('--defer-runtime-smoke')) {
      await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:smoke', () => smokePreparedRuntime(LYNESS_OUTPUT_ROOT, NODE, RUNTIME_ROOT, descriptor))
      await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:verify-after-smoke', () => verifyDesktopRuntime(LYNESS_OUTPUT_ROOT, release.version, target))
    }
  } catch (error) {
    rmSync(LYNESS_OUTPUT_ROOT, { recursive: true, force: true })
    throw error
    ...
    try { await packagingStep(process.env.LYNESS_DESKTOP_PACKAGING_RUN_DIR, 'runtime:cleanup', async () => cleanup()) }
