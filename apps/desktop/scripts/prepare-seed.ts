...
const SEED_ROOT = mkdtempSync(join(tmpdir(), 'lyn-desktop-seed-'))
...
function desktopRelease(): DesktopRelease {
  ...
  if (version !== lynVersion) {
    throw new Error(`desktop seed: Electron ${version} must bind the same version of @lyness/lyn, found ${lynVersion}`)
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const targetPlatform = process.env.LYNESS_DESKTOP_TARGET_PLATFORM ?? process.platform
    ...
    if (macOSSigning !== undefined && signedMachOFiles !== undefined) {
      const extractedStore = mkdtempSync(join(tmpdir(), 'lyn-desktop-seed-verification-'))
      try {
