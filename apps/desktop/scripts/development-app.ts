...
export function prepareDevelopmentApp(options: DevelopmentAppOptions): string {
  ...
  const stamp = join(bundle, 'Contents', 'Resources', 'lyn-development.json')
  ...
  if (!existsSync(stamp) || readFileSync(stamp, 'utf8') !== identity) {
    ...
    const values = {
      ...
      CFBundleURLTypes: [{ CFBundleURLName: 'lyness', CFBundleURLSchemes: ['lyn'], CFBundleTypeRole: 'Viewer' }],
    }
...
export function developmentLauncher(options: DevelopmentAppOptions, bundle: string): string {
  const environment = {
    LYNESS_HOME: options.home,
    LYNESS_DESKTOP_DEV_APP: '1',
    LYNESS_DESKTOP_HOST_INSPECT_PORT: String(options.hostPort),
    LYNESS_DESKTOP_OPEN_DEVTOOLS: options.openDevtools,
