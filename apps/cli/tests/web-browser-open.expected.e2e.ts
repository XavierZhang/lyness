/** Assembled keyless snapshot for the default `lyn web` browser handoff. */
...
describe.skipIf(!builtArtifactsExist)('lyn web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-web-browser-open-snapshot-'))
    tempRoots.push(root)
    ...
    const readyUrl = /lyn web: (http:\/\/[^\s]+)/u.exec(result.stdout)?.[1]
    const openLine = result.stdout.split('\n').find(line => line.startsWith('lyn browser-open: '))
    const opening = result.stdout.includes(openingMessage)
    if (readyUrl === undefined || openLine === undefined || !opening) {
      throw new Error(`lyn web browser-open evidence missing\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    const opened = JSON.parse(openLine.slice('lyn browser-open: '.length)) as BrowserOpenRecord
    ...
    expect({
      ...
      lynHomePresent: opened.lynHomePresent,
      stderr: result.stderr,
    }).toMatchInlineSnapshot(`
      {
        "apiKeyPresent": false,
        "bootManifest": true,
        "lynHomePresent": false,
        "exitCode": 0,
  ...
  it('prints the launcher reason and manual URL after the Web app is ready', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-web-browser-open-failure-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
      ...
      env: {
        ...
        DEEPSEEK_API_KEY: 'keyless-browser-open-no-call',
        LYNESS_AGENTS_HOME: join(root, '.agents'),
        LYNESS_BROWSER_OPEN_TEST_EXIT_ON_FAILURE: '1',
        LYNESS_HOME: join(root, '.lyn'),
        LYNESS_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
        SSH_CONNECTION: '',
    ...
    expect({
      diagnostic,
      exitCode: result.exitCode,
      opened: result.stdout.includes('lyn browser-open: '),
      opening: result.stdout.includes(openingMessage),
      readyUrl: readyUrl === undefined ? undefined : normalizeLocalUrl(readyUrl),
    }).toMatchInlineSnapshot(`
      {
        "diagnostic": "web-app: could not open the default browser because fixture desktop unavailable; use the lyn web URL printed at startup",
        "exitCode": 0,
        "opened": false,
        "opening": true,
        "readyUrl": "http://127.0.0.1:{{port}}/?token={{token}}",
  ...
  it('prints the host URL without launching a browser in a VS Code Remote SSH session', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-web-browser-open-ssh-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
  ...
  it('rejects a project browser command before starting the Web app', async () => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-web-browser-open-env-snapshot-'))
    tempRoots.push(root)
    ...
    const diagnostic = result.stderr.split(/\r?\n/u)
      .find(line => line.startsWith('Error: lyn: '))
      ?.replace(/^Error: lyn: .*[/\\]\.env/u, 'lyn: {{root}}/.env')
    ...
    expect({
      ...
      opening: result.stdout.includes(openingMessage),
      opened: result.stdout.includes('lyn browser-open: '),
      ready: result.stdout.includes('lyn web: '),
    }).toMatchInlineSnapshot(`
      {
        "diagnostic": "lyn: {{root}}/.env sets "BROWSER", which only the launching environment may set (it decides how this process starts, where its code and instructions load from, or how it reaches the network); export BROWSER instead of putting it in a .env file",
        "exitCode": 1,
        "opened": false,
        "opening": false,
        "ready": false,
