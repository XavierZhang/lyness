/** Real `lyn web` authentication against a temporary Harness home. */
...
const LYNESS_SOURCE_BIN = join(REPO_ROOT, 'apps/cli/src/bin.ts')
...
function cleanEnvironment(root: string, lynHome: string): NodeJS.ProcessEnv {
...
async function startWeb(root: string, lynHome: string, port: number): Promise<RunningWeb> {
  const child = spawn(process.execPath, [
    ...
    cwd: root,
    env: cleanEnvironment(root, lynHome),
  ...
  const launchUrl = await new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`lyn web did not become ready:\n${redact(output)}`))
    }, 90_000)
    ...
    child.once('exit', (code) => {
      fail(new Error(`lyn web exited before readiness (${String(code)}):\n${redact(output)}`))
    })
  })
...
describe('lyn web authentication through the real CLI', () => {
  it('rejects a forged loopback Host and preserves the browser cookie across restart', { timeout: 180_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-web-auth-real-cli-'))
    const lynHome = join(root, '.lyn')
    const port = await freePort()
    let first: RunningWeb | undefined
    let second: RunningWeb | undefined
    try {
      first = await startWeb(root, lynHome, port)
      ...
      await stopWeb(first)
      first = undefined
      second = await startWeb(root, lynHome, port)
      ...
      const credentialMode = (await stat(join(lynHome, '.credentials.yaml'))).mode & 0o777
