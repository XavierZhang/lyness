...
import { StartupError } from '@lyness/lyn-app-boot'
import { reportStartupFailure } from '../src/startup-diagnostics.ts'

async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'lyn-startup-diagnostics-'))
  onTestFinished(() => rm(dir, { recursive: true, force: true }))
  return dir
}

function startupError(reason: unknown): StartupError {
  const error = new StartupError('lyn: startup failed: 1 required plugin did not activate', [
...
describe('startup diagnostic files', () => {
  it('prints the summary and saved path to stderr by default', async () => {
    ...
    expect(write).toHaveBeenCalledWith(expect.stringContaining('lyn: startup failed:'), expect.any(Function))
  ...
  it('retains original error properties, causes, aggregate members, cycles, and long values', async () => {
    ...
    for (const text of [
      'lynVersion: \'1.2.3\'', "profile: 'web'", process.version, 'configurationPath:', '/example/cordis.yml',
  ...
  it('prints the complete report when the logs directory cannot be created', async () => {
    ...
    expect(output).toContain('lyn: startup failed:')
    expect(output).toContain('lyn: warning: could not write startup diagnostics:')
