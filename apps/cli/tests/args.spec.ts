...
import { parseLynArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseLynArgs(argv, '1.2.3')
...
describe('parseLynArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@lyness/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@lyness/cordis'] })
