...
describe.skipIf(!existsSync(lynBin))('lyn BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { LYNESS_HOME: home })
      expect(result.code).toBe(1)
  ...
  it('creates a custom profile from the shipped web template before booting it', async () => {
    ...
    try {
      ...
      const manifest = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8')) as {
        ...
        lyn: { profile: { bundles: string[]; patchReload: string } }
      }
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: lyn exited 13 with no diagnostic instead of settling
  ...
  it('activates a dependency that gained lyn.bundle in a later update', async () => {
    ...
    // version declares lyn.bundle. Simulated without a registry: hand-place
  ...
  describe('config dump', () => {
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      ...
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { LYNESS_HOME: home })
