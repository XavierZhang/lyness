...
export default async function open(url) {
  ...
  console.log(`lyn browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__LYNESS_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    lynHomePresent: process.env.LYNESS_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.lyn-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
