...
markers = [b"lyn-test: never-dispose ready", b"lyn-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'lyn-headless-shutdown-'))
  try {
    const home = join(cwd, '.lyn')
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        LYNESS_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`lyn headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`lyn headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('lyn: observing at ')
    expect(output).toContain('lyn-test: never-dispose ready')
    expect(output).toContain('lyn-test: never-dispose started')
