...
import type { ConfigSchemaDump } from '@lyness/lyn-app-boot'
...
if (process.env.LYNESS_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
  throw new Error('lyn config-schema acceptance requires built CLI artifacts in lib mode; run pnpm run build first')
}
const packageName = 'lyn-schema-acceptance-fixture'
const profileName = 'schema-acceptance'
const processTimeoutMs = 90_000
const schemaImport = "import Schema from '@lyness/schemastery'"
const forbiddenApply = `
import { writeFileSync } from 'node:fs'
import { join } from 'node:path'
export function apply() {
  writeFileSync(join(process.env.LYNESS_HOME, 'apply-ran'), 'unexpected')
...
async function createFixture(modules: Record<string, string>, patches: string): Promise<Fixture> {
  const root = realpathSync.native(mkdtempSync(join(tmpdir(), 'lyn-schema-acceptance-')))
  ...
  await writeFile(join(moduleDir, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@lyness/schemastery': '*' },
  ...
  await writeFile(join(profile, 'package.json'), JSON.stringify({
    name: 'lyn-profile-schema-acceptance',
    private: true,
    dependencies: { [packageName]: '1.0.0' },
    lyn: { profile: { bundles: [packageName] } },
  }))
...
async function installNativeInclude(fixture: Fixture): Promise<void> {
  const name = '@lyness/cordis-plugin-include'
  ...
  await writeFile(join(directory, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@lyness/cordis': '*', '@lyness/cordis-plugin-loader': '*' },
...
async function dump(
  ...
  const env = Object.fromEntries(Object.entries(process.env).filter(
    (entry): entry is [string, string] => entry[1] !== undefined
      ...
      && !/^(LYNESS_|NODE_OPTIONS$|NODE_PATH$)/i.test(entry[0]),
  ))
  const result = await execa(process.execPath, [
    builtBin, '--profile', profileName, format, ...args,
  ], {
    cwd: fixture.root,
    env: { ...env, LYNESS_HOME: fixture.home, LYNESS_TELEMETRY_DISABLED: '1' },
    extendEnv: false,
    input: '',
...
describe.skipIf(!builtArtifactsExist)('lyn --dump-config-schema assembled output', () => {
...
`)
  ...
  it.each([false, true])('expands canonical Include and re-export aliases with profile-local copy: %s', async (localCopy) => {
    const fixture = await createFixture({
      alias: "export { default } from '@lyness/cordis-plugin-include'",
    }, `- insert:
    - id: canonical
...
`)
    ...
    expect(result.stderr.split('\n')).toEqual([
      'lyn: error: [/0] Config is not a native Schemastery schema',
      'lyn: error: [/1] fixture lazy schema failed',
      'lyn: error: [/2] fixture module import failed',
    ])
  })
  ...
  it('marks a skipped selected bundle incomplete and exits 1', async () => {
    ...
    - id: schema-retained
      name: ${packageName}/absent
`)
    const missingBundle = 'lyn-schema-acceptance-missing-bundle'
    await writeFile(join(fixture.profile, 'package.json'), JSON.stringify({
      private: true,
      dependencies: { [packageName]: '1.0.0' },
      lyn: { profile: { bundles: [packageName, missingBundle] } },
    }))
    const result = await dump(fixture)
    expect(result.exitCode, result.stderr).toBe(1)
  ...
  it('keeps schema warnings plain and provides source-labelled warnings through the YAML dump', async () => {
    ...
    expect(schema.stderr).toBe('lyn: warning: patch: entry "missing" not found')
