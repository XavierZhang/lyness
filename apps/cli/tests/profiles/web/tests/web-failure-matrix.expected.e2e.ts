...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), 'lyn-web-failure-matrix-'))
...
function start(f: ReturnType<typeof fixture>, extra: string[] = []) {
  const child = execa(process.execPath, [bin, '--profile', 'web', '--patch', f.watcher, ...extra, '--no-open', '--port', '0'], {
    cwd: f.root,
    env: { ...process.env, LYNESS_HOME: f.home, LYNESS_AGENTS_HOME: join(f.root, '.agents'), LYNESS_TELEMETRY_DISABLED: '1', DEEPSEEK_API_KEY: 'keyless-matrix-no-call', NODE_NO_WARNINGS: '1' },
  ...
  async function serves(currentServer = false) {
    await wait(() => /lyn web: http:\/\//u.test(stdout))
    const url = currentServer ? readFileSync(f.serverUrl, 'utf8') : /lyn web: (http:\/\/[^\s]+)/u.exec(stdout)?.[1]
