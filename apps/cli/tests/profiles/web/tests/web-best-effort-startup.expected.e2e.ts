...
const builtArtifactsExist = existsSync(lynBin) && existsSync(frontendIndex)
...
function createFixture(): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'lyn-web-best-effort-'))
...
async function waitForStartup(
  ...
  const finish = (): void => {
    ...
    if (!stderrText.includes('lyn: warning: 6 entries did not activate')) return
  ...
  stdout.on('data', (chunk: string) => {
    stdoutText += chunk
    url ??= /lyn web: (http:\/\/[^\s]+)/u.exec(stdoutText)?.[1]
    finish()
  })
...
describe.skipIf(!builtArtifactsExist)('lyn Web profile best-effort startup', () => {
  it('serves the full Web app while unrelated entries fail to start', async () => {
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        LYNESS_AGENTS_HOME: join(fixture.root, '.agents'),
  ...
  it.each([
    ...
    try {
      ...
      expect(result.stdout).not.toContain('lyn web: http://')
  ...
  it.each([false, true])('fails the full Web profile when its required HTTP server cannot bind (logs blocked: %s)', async (logsBlocked) => {
    const root = mkdtempSync(join(tmpdir(), 'lyn-web-required-bind-'))
    const home = join(root, 'home')
    mkdirSync(home)
    ...
    try {
      ...
      expect(result.stdout).not.toContain('lyn web: http://')
      expect(result.stderr).toContain('startup failed:')
      expect(result.stderr).toContain('lyn: startup failed: 2 required plugins did not activate')
      ...
      const summary = result.stderr.split(/\n\n(?:Full diagnostics:|lyn: warning:)/u)[0]!
      expect(summary.match(/EADDRINUSE/gu)).toHaveLength(1)
      expect(summary).not.toMatch(/lyn: warning:|\[cause\]|at boot \(|at runCli \(|Node\.js v/u)
      let report: string
      if (logsBlocked) {
        expect(result.stderr).toContain('lyn: warning: could not write startup diagnostics:')
      ...
      expect(report).toContain('lynVersion:')
      ...
      expect(report).toContain("module: '@lyness/lyn-client-connection'")
