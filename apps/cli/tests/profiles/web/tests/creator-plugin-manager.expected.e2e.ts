...
import { readProfileManifest } from '@lyness/lyn-app-boot'
...
it('configures MCP on a live profile, restores it on restart, and removes its tools', async (test) => {
  ...
  await writeFile(join(bundle, 'cordis.patch.yml'), JSON.stringify([{ insert: [{ id: 'demo',
    name: '@lyness/lyn-mcp-client', config: { serverName: 'demo', transport: 'streamable-http',
  ...
  const start = async () => {
    const child = spawn(process.execPath, [join(repo, 'apps/cli/lib/bin.js'), '--profile', 'web', '--patch', patch,
      ...
      env: { ...process.env, LYNESS_HOME: join(root, 'home'), LYNESS_AGENTS_HOME: join(root, 'agents'),
        LYNESS_TELEMETRY_DISABLED: '1', DEEPSEEK_API_KEY: 'keyless-no-model-calls' },
    ...
    await expect.poll(() => {
      ...
      return output.includes('lyn web: http://')
  ...
  const saved = readProfileManifest('lyn', join(root, 'home/profiles/web'))
  expect(saved.lyn?.profile?.bundles).toContain('@test/creator-mcp')
