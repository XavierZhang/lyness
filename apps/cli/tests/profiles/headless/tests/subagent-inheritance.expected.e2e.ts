...
import { createMessage, createUserMessage, ReasoningEffortId } from '@lyness/lyn-llm'
import { SessionSeq, SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@lyness/lyn-session'
...
async function seedReadOnlyParent(root: string, cwd: string): Promise<void> {
  ...
  const events: SessionEvent[] = [
    ...
    { type: 'step/start', seq: SessionSeq(1), time: 11, data: { turn: 1, step: 1 } },
    {
      ...
      data: { turn: 1, step: 1, message: createMessage({ role: 'system', content: [], source: { kind: 'plugin', plugin: '@lyness/lyn-system-prompt' } }) },
      surfaceOp: 'append',
    },
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-subagent-inherit-',
      binScript,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@lyness/lyn-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the LYN file sandbox cannot modify files in the standing mode.')
