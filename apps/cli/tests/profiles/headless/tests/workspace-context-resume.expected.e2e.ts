...
import { renderWorkspaceContext } from '@lyness/lyn-agent-instructions'
import { resolveConfig, workspaceBaselineIdentity } from '@lyness/lyn-agent-instructions/src/config.ts'
...
describe('agent-instructions resume snapshot', () => {
  it('appends an offline replacement without duplicating the visible baseline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-workspace-context-resume-',
      binScript,
      ...
      tsconfigPath,
      env: {
        LYNESS_SNAPSHOT_FILE: replayFixture,
  ...
  it('supersedes an incompatible baseline when precedence changed offline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-workspace-context-precedence-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Acknowledge the current workspace instruction.'],
