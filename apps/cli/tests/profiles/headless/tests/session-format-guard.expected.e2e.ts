...
const fixtureEnv = {
  ...
  LYNESS_LOADER_SMOKE_REQUIRED_ENTRY_ID: 'resumed-agent',
}
...
describe('session format guard through the assembled app', () => {
  it('migrates v0 before resume, preserves its source, and appends only to the current generation', async () => {
    ...
    await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-format-migrate-v0-',
      binScript,
  ...
  it('refuses to resume a newer-format log, naming the upgrade direction and the raw log path', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-format-guard-version-',
      binScript,
      libBinScript: binScript,
      configPath,
  ...
  it('refuses unaudited V2 queued content before resume without publishing a successor', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-format-guard-content-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Try to resume.'],
  ...
  it('refuses to resume a log with an unknown required event type', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lyn-format-guard-event-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Try to resume.'],
