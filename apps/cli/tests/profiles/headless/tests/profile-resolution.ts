...
import { resolveExampleLaunch, type ExampleMode } from '@lyness/lyn-loader-smoke'
...
const marker = 'LYNESS_PROFILE_RESOLUTION '
...
export function testProfileResolution(mode: ExampleMode): void {
  it.each(['installed', 'npm-link'] as const)(`resolves a %s profile dependency graph in ${mode} mode`, {
    ...
    // The child reports loaded module paths in the form LYNESS_HOME was given; hand it the native realpath so
    // Windows 8.3 tmpdir names and macOS /var symlinks match the expectations computed below.
    const root = realpathSync.native(await mkdtemp(join(tmpdir(), 'lyn-profile-resolution-')))
    const links: string[] = []
    try {
      ...
      // developer's devDependency lyn-tools copy (stale, never read) and the declared dependency's install.
      ...
      await writePackage(pluginDir, {
        ...
        peerDependencies: { '@lyness/lyn-tools': '*' }, devDependencies: { '@lyness/lyn-tools': '*' },
      }, {
        'index.mjs': [
      ...
      const staleToolsLink = join(sharedModules, '@lyness', 'lyn-tools')
      await writePackage(staleTools, { name: '@lyness/lyn-tools', version: '0.0.0', exports }, {
        'index.mjs': "export default class Tools {}\nexport const TOOL_RUNTIME_SCHEDULER = Symbol()\nthrow new Error('STALE_LYNESS_TOOLS')",
        'index.cjs': "throw new Error('STALE_LYNESS_TOOLS')",
      })
      const sourceDir = join(sourcePackageDir, 'src')
      const toolsCjsExpression = mode === 'lib' ? "require.resolve('@lyness/lyn-tools')" : 'null'
      ...
      await writePackage(sourcePackageDir, {
        name: 'source-package', version: '1.0.0', peerDependencies: { '@lyness/lyn-tools': '*', [leafName]: '*' },
      }, {
        'src/query.mjs': [
      ...
      for (const [target, link] of [
        ...
        [staleTools, join(sourcePackageDir, 'node_modules', '@lyness', 'lyn-tools')],
        ...layout === 'npm-link' ? [
          [externalDir, sharedExternal],
          [pluginDir, installedPlugin],
          [staleTools, join(pluginDir, 'node_modules', '@lyness', 'lyn-tools')],
      ...
      await writePackage(profileDir, {
        ...
        lyn: { profile: { bundles: ['@lyness/lyn-base', '@lyness/lyn-headless', bundleName] } },
      }, {
        'probe.mjs': [
          "import { createRequire } from 'node:module'",
          "import { getEnvironmentData } from 'node:worker_threads'",
          "import Tools, { TOOL_RUNTIME_SCHEDULER } from '@lyness/lyn-tools'",
          ...
          "    const entries = getEnvironmentData('@lyness/lyn-app-boot/profile-resolution').resolution.entries",
          '    const evidence = {',
      ...
      const launch = resolveExampleLaunch({
        ...
        env: {
          LYNESS_HOME: home, LYNESS_AGENTS_HOME: join(root, 'agents'), LYNESS_TELEMETRY_DISABLED: '1',
