...
describe.skipIf(!existsSync(join(repoRoot, 'apps/cli/lib/bin.js')))('lyn SOURCE tools with prepared runtime artifacts', () => {
  it('dispatches a source-profile shell call to a result, including sandbox-unavailable hosts, without mixing tools src/lib', async () => {
    const root = await mkdtemp(join(tmpdir(), 'lyn-source-tool-'))
    try {
      ...
      const result = await execa(process.execPath, [
        '--import', 'tsx/esm', lynSourceBin, '--profile', 'headless', '--patch', patch,
      ], {
        cwd: repoRoot,
        env: {
          ...clearedProxyEnv(),
          LYNESS_HOME: join(root, 'home'),
          LYNESS_AGENTS_HOME: join(root, 'agents'),
          LYNESS_TELEMETRY_DISABLED: '1',
          LYNESS_TOOLS_MODE: 'native',
          LYNESS_CLI_MOCK_FAILURE: '0',
        },
        input: '',
      ...
      const records = result.stdout.split('\n').filter(line => line.startsWith('LYNESS_SOURCE_TOOL_RESULT '))
      expect(records, diagnostic).toHaveLength(1)
      const evidence = JSON.parse(records[0]!.slice('LYNESS_SOURCE_TOOL_RESULT '.length)) as SourceToolEvidence
