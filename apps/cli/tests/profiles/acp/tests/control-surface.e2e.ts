/** Generic keyless ACP v1 automation-control conformance over the real lyn profile. */
...
describe('standard ACP v1 control surface', () => {
  it('selects, mounts MCP, closes, restarts, resumes, and cancels through the SDK only', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'lyn-acp-control-'))
    const persistenceRoot = join(cwd, '.sessions')
    const env = { LYNESS_CONFORMANCE_PERSISTENCE_ROOT: persistenceRoot, LYNESS_TELEMETRY_DISABLED: '1' }
