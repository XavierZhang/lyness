...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$LYNESS_HOME/settings.yaml`. Left alone it
  ...
  let bundlePatches: PatchOptions[] = [
    ...
    ...loadOverlayPatches('lyn-test', WEB_PATCH),
  ]
  if (profileBundles !== undefined) {
    ...
    const profile = loadProfile('lyn-test', 'spec', INSTALL_ANCHOR, home, { userLayer: false })
    bundlePatches = profile.layers.flatMap(layer => layer.patches)
  }
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('lyn-test', rootConfig, [...bundlePatches, ...overrides], (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'lyn-web-presets-')), 'settings.yaml')
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<lynHome>/.agent-presets` itself. `$LYNESS_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'lyn-preset-derived-'))
    previousHome = process.env.LYNESS_HOME
    process.env.LYNESS_HOME = home
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'lyn-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'lyn-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('a composition that configures its own preset roots', () => {
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'lyn-preset-roots-'))
