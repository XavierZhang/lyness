...
it('refuses incompatible installation and startup until an exact risk exemption is granted', async () => {
  const home = mkdtempSync(join(tmpdir(), 'lyn-compatibility-cli-'))
  ...
  const initial = { name: 'compatibility-profile', private: true, dependencies: {}, lyn: { profile: { bundles: [] } } }
  ...
  writeFileSync(join(source, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@lyness/lyn': '999.0.0' }, lyn: { bundle: { patch: './cordis.patch.yml' } },
  }))
  ...
  const run = async (args: string[]) => {
    const result = await execa(process.execPath, [bin, ...args], {
      cwd: home, env: { ...process.env, LYNESS_HOME: home }, extendEnv: false, reject: false, timeout: 60_000,
    })
  ...
  // A package installed outside LYN still goes through the independent startup check.
  cpSync(source, join(profile, 'node_modules', 'compatibility-fixture'), { recursive: true })
  writeFileSync(profileFile, JSON.stringify({ ...initial,
    dependencies: { 'compatibility-fixture': `file:${source}` }, lyn: { profile: { bundles: ['compatibility-fixture'] } },
  }))
  ...
  const missingConsent = await plugin(['allow-version', 'compatibility-fixture@1.0.0', '--lyn-version', version])
  expect(missingConsent.exitCode).toBe(1)
  const granted = await plugin(['allow-version', 'compatibility-fixture@1.0.0', '--lyn-version', version, '--accept-risk'])
  ...
  expect(JSON.parse(readFileSync(profileFile, 'utf8'))).not.toHaveProperty('lyn.profile.versionExemptions')
  ...
  expect((await plugin(['revoke-version', 'compatibility-fixture@1.0.0', '--lyn-version', version])).exitCode).toBe(0)
