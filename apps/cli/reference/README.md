...
The launcher's flags come first and end at the first token it does not recognize; everything from there on is handed to the booted profile verbatim through `ctx.cmdlineArgs`, where any injected app plugin may parse it ([`lyn-cmdline`](../../../packages/boot/cmdline/README.md)). `lyn --profile web --port 8080` therefore reaches the web app's `--port`, `lyn --profile web --help` prints that app's help and boots nothing, and `lyn --help` (no profile to hand it to) prints the launcher's own. `-V`/`--version` prints the launcher's version when it appears before the app-argument boundary.
