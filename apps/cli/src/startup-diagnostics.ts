...
import type { StartupError } from '@lyness/lyn-app-boot'
...
/**
 * Print the startup summary and save a private, uniquely named report under LYNESS_HOME/logs.
...
export async function reportStartupFailure(
  ...
  try {
    ...
    await write(`\nlyn: warning: could not write startup diagnostics: ${String(writeError)}\nFull diagnostics:\n${report}`)
    return
  }
