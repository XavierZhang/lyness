...
export interface ResolvedProfileRuntime {
  ...
  profile: Profile
  /** Absolute package.json path of the application's lyn installation. */
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const dispose = (): Promise<void> => disposal ??= (async () => {
    ...
    if (failures.length > 1) throw new AggregateError(failures, 'lyn: profile cleanup failed')
  })()
  try {
    ...
    const profileContext: ProfileContext = {
      ...
      cwd: process.cwd(), home: resolveLynHome(),
      overlays: composed.overlays, telemetryDisabledEnv: process.env.LYNESS_TELEMETRY_DISABLED,
    }
    ...
    try { await dispose() } catch (cleanupError) {
      throw new AggregateError([error, cleanupError], 'lyn: profile startup and cleanup failed')
    }
    throw error
  }
}
