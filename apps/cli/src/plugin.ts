/**
 * `lyn plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `lyn.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `lyn.bundle` joins the layer stack; a
 ...
 * gained its `lyn.bundle` declaration in a newer version.
 * @module @lyness/lyn/plugin
 */
 ...
 * @returns true when the package manifest declares `lyn.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.lyn?.bundle?.patch !== undefined
}

/**
 * Reconcile `lyn.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `lyn.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.lyn?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no lyn.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (lyn-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.lyn = { ...after.lyn, profile: { ...after.lyn?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `lyn` was invoked from.
...
/**
 * Run one `lyn plugin` invocation: init if needed, forward to pnpm, reconcile.
