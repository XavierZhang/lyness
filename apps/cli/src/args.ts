/**
 * Commander adapter for the `lyn` command line.
 *
 ...
 *
 * `lyn <name>` abbreviates `lyn --profile <name>`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @lyness/lyn/args
 */
...
/** The resolved `lyn` invocation. Help, version, and errors exit inside {@link parseLynArgs}. */
export type LynInvocation = ProfileInvocation | DumpConfigInvocation | DumpConfigSchemaInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  lyn web                                   boot the web profile (same as: lyn --profile web)
  lyn rescue --from-default-profile web
                                            create rescue from the shipped web template, then boot it
  lyn headless "run the tests"              answer one task, print the result, and exit
  lyn tui --patch ./extra.yml               boot a custom profile with one extra overlay
  lyn tui --resume <session>                arguments after the launcher flags reach the app
  lyn web --help                            the web app's own flags and help
  lyn plugin --profile tui add <package>    install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): LynInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseLynArgs(argv: readonly string[], version: string): LynInvocation {
  const first = argv[0]
  let resolved: LynInvocation | undefined
  ...
  program
    ...
    .usage('[--profile] <name> [options] [app-args...]\n       lyn plugin --profile <name> <pnpm-args...>')
    .description('lyn: boot a lyness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `lyn -h` with no profile still prints this help, below.
    .helpOption(false)
    .helpCommand(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: lyn --profile <name> --help)')
    .option('--profile <name>', 'the profile under $LYNESS_HOME/profiles to boot', selectProfile)
    ...
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      ...
      // `lyn -h` (no profile to hand it to) must print.
  ...
  if (resolved === undefined) throw new Error('lyn: no invocation resolved')
  return resolved
}
