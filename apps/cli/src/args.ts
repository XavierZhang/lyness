...
export type LynInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  lyn --profile web                          boot the web profile (same as: lyn web)
  lyn --profile rescue --from-default-profile web
                                             create rescue from the shipped web template, then boot it
  lyn --profile headless "run the tests"     answer one task, print the result, and exit
  lyn --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  lyn --profile tui --resume <session>       arguments after the launcher flags reach the app
  lyn --profile web --help                   the web app's own flags and help
...
export function parseLynArgs(argv: readonly string[], version: string): LynInvocation {
  ...
  program
    ...
    .option('--profile <name>', 'the profile under $LYNESS_HOME/profiles to boot')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: lyn web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
